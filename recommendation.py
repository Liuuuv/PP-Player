...
class RecommendationSystem:
    ...
    def apply_ai_recommendation_selection(self, recommendations):
        """Applique la sélection IA aux recommandations"""
        try:
            if not recommendations:
                return recommendations
            
            # Obtenir le système d'IA
            ai_system = None
            if (hasattr(self.main_app, 'ai_menu_system') and 
                self.main_app.ai_menu_system and
                hasattr(self.main_app.ai_menu_system, 'ai_system')):
                ai_system = self.main_app.ai_menu_system.ai_system
            
            if not ai_system:
                print("🤖 Système IA non disponible pour la sélection")
                return recommendations
            
            # Créer des scores IA pour chaque recommandation
            scored_recommendations = []
            
            for rec in recommendations:
                try:
                    # Créer un objet de métadonnées simulé pour l'IA
                    fake_metadata = {
                        'title': rec.get('title', ''),
                        'artist': ', '.join([artist.get('name', '') for artist in rec.get('artists', [])]),
                        'duration': rec.get('duration_seconds', 0),
                        'videoId': rec.get('videoId', ''),
                        'thumbnails': rec.get('thumbnails', [])
                    }
                    
                    # Calculer un score IA basé sur les métadonnées
                    # Pour l'instant, utiliser un score basique basé sur les patterns appris
                    score = self.calculate_ai_score_for_recommendation(fake_metadata, ai_system)
                    
                    scored_recommendations.append((rec, score))
                    
                except Exception as e:
                    print(f"⚠️ Erreur calcul score IA pour {rec.get('title', 'Unknown')}: {e}")
                    # En cas d'erreur, donner un score neutre
                    scored_recommendations.append((rec, 0.5))
            
            # Trier par score décroissant
            scored_recommendations.sort(key=lambda x: x[1], reverse=True)
            
            # Retourner les recommandations triées par score IA
            ai_sorted_recommendations = [rec for rec, score in scored_recommendations]
            
            # Afficher les scores pour debug
            for i, (rec, score) in enumerate(scored_recommendations[:3]):
                print(f"🤖 Rec {i+1}: {rec.get('title', 'Unknown')[:30]}... - Score IA: {score:.3f}")
            
            return ai_sorted_recommendations
            
        except Exception as e:
            print(f"⚠️ Erreur sélection IA recommandations: {e}")
            return recommendations
    
    def calculate_ai_score_for_recommendation(self, metadata, ai_system):
        """Calcule un score IA pour une recommandation basé sur les métadonnées"""
        try:
            score = 0.5  # Score de base
            
            # Facteurs basés sur les préférences apprises
            title = metadata.get('title', '').lower()
            artist = metadata.get('artist', '').lower()
            duration = metadata.get('duration', 0)
            
            # Bonus basé sur l'historique des likes (mots-clés dans les titres)
            if hasattr(ai_system, 'user_behavior_data'):
                behavior_data = ai_system.user_behavior_data
                
                # Analyser les patterns de likes
                liked_titles = []
                for like_data in behavior_data.get('like_patterns', []):
                    if 'song_path' in like_data:
                        song_title = os.path.basename(like_data['song_path']).lower()
                        liked_titles.append(song_title)
                
                # Bonus si des mots-clés des titres likés apparaissent
                for liked_title in liked_titles:
                    words = liked_title.split()
                    for word in words:
                        if len(word) > 3 and word in title:
                            score += 0.1
                            break
                
                # Bonus basé sur la durée préférée
                listening_sessions = behavior_data.get('listening_sessions', [])
                if listening_sessions:
                    # Calculer la durée moyenne des chansons écoutées complètement
                    completed_durations = []
                    for session in listening_sessions[-10:]:  # 10 dernières sessions
                        for song_data in session.get('songs_played', []):
                            if song_data.get('listening_ratio', 0) > 0.8:  # Écoutée à plus de 80%
                                completed_durations.append(song_data.get('duration', 0))
                    
                    if completed_durations:
                        avg_preferred_duration = sum(completed_durations) / len(completed_durations)
                        # Bonus si la durée est proche de la préférence
                        if duration > 0:
                            duration_diff = abs(duration - avg_preferred_duration) / avg_preferred_duration
                            if duration_diff < 0.3:  # Moins de 30% de différence
                                score += 0.15
                
                # Bonus basé sur l'heure actuelle (patterns temporels)
                current_hour = time.localtime().tm_hour
                time_patterns = behavior_data.get('time_patterns', {})
                hour_preferences = time_patterns.get('hourly_preferences', {})
                
                if str(current_hour) in hour_preferences:
                    hour_score = hour_preferences[str(current_hour)]
                    score += hour_score * 0.1  # Bonus basé sur les préférences horaires
            ...
            print(f"⚠️ Erreur calcul score IA: {e}")
