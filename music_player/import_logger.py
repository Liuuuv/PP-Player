...
class ImportLogger:
    def __init__(self, base_dir):
        ...
        self.cancellation_flags = {}  # Dict pour stocker les flags d'annulation par session_id
    ...
    def start_session(self, session_type, source_info):
        ...
        with self.lock:
            ...
            # Initialiser le flag d'annulation pour cette session
            self.cancellation_flags[session_id] = False
            
            # Ajouter les logs initiaux directement pour éviter la récursion
            start_log = {
                'timestamp': datetime.now().isoformat(),
                'level': 'INFO',
                'message': f"Démarrage de la session {session_type}"
            }
            source_log = {
                'timestamp': datetime.now().isoformat(),
                'level': 'INFO',
                'message': f"Source: {source_info}"
            }
            
            self.current_session['logs'].append(start_log)
            self.current_session['logs'].append(source_log)
            
            print(f"[INFO] Démarrage de la session {session_type}")
            print(f"[INFO] Source: {source_info}")
    ...
    def set_total_links(self, total):
        ...
        if self.current_session:
            with self.lock:
                ...
                # Ajouter le log directement pour éviter le deadlock
                log_entry = {
                    'timestamp': datetime.now().isoformat(),
                    'level': 'INFO',
                    'message': f"Total de liens à traiter: {total}"
                }
                self.current_session['logs'].append(log_entry)
                print(f"[INFO] Total de liens à traiter: {total}")
    ...
    def log_url_processed(self, url, title, status, reason=None):
        ...
        if self.current_session:
            with self.lock:
                ...
                # Ajouter les logs directement pour éviter le deadlock
                if status == 'success':
                    ...
                    log_entry = {
                        'timestamp': datetime.now().isoformat(),
                        'level': 'SUCCESS',
                        'message': f"✅ Téléchargé: {title}"
                    }
                    self.current_session['logs'].append(log_entry)
                    print(f"[SUCCESS] ✅ Téléchargé: {title}")
                elif status == 'failed':
                    ...
                    log_entry = {
                        'timestamp': datetime.now().isoformat(),
                        'level': 'ERROR',
                        'message': f"❌ Échec: {title} - {reason}"
                    }
                    self.current_session['logs'].append(log_entry)
                    print(f"[ERROR] ❌ Échec: {title} - {reason}")
                elif status == 'skipped':
                    self.current_session['skipped_links'] += 1
                    log_entry = {
                        'timestamp': datetime.now().isoformat(),
                        'level': 'WARNING',
                        'message': f"⚠️ Ignoré: {title} - {reason}"
                    }
                    self.current_session['logs'].append(log_entry)
                    print(f"[WARNING] ⚠️ Ignoré: {title} - {reason}")
    ...
    def delete_session(self, session_id):
        """Supprime une session et son fichier de logs"""
        try:
            session_file = os.path.join(self.logs_dir, f"{session_id}.json")
            if os.path.exists(session_file):
        ...
        if session:
            with self.lock:
                ...
                try:
                    ...
                    print(f"Session {session_id} annulée")
                except Exception as e:
                    print(f"Erreur sauvegarde annulation: {e}")
                    raise
    ...
    def cleanup_session(self, session_id):
        """Nettoie les ressources d'une session terminée"""
