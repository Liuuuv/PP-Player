...
def install_dependencies(venv_path):
    ...
    for dep in dependencies:
        try:
            ...
            print(
                f"⚠️  Erreur lors de l'installation de {dep}: {e.stderr.decode() if e.stderr else 'Unknown error'}"
            )
    ...
    for dep in dependencies:
        try:
            ...
            result = subprocess.run(
                [pip_path, "show", dep], capture_output=True, text=True, check=False
            )

            if result.returncode != 0:
                print(f"📦 Installation de {dep}...")
                subprocess.run(
                    [pip_path, "install", dep], check=True, capture_output=True
                )
                print(f"✅ Installé: {dep}")
            else:
            ...
            print(
                f"❌ Erreur avec {dep}: {e.stderr.decode() if e.stderr else 'Unknown error'}"
            )
...
def build_with_pyinstaller(venv_path):
    ...
    if os.path.isdir("ffmpeg"):
        ...
        build_command += ["--add-data", f"ffmpeg:ffmpeg{data_sep}"]
    else:
