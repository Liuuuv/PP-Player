...
def show_settings_menu(self):
    ...
    try:
        ...
        # Créer le menu déroulant
        settings_menu = tk.Menu(self.root, tearoff=0, bg='white', fg='black', 
                            activebackground='#4a8fe7', activeforeground='white',
                            relief='flat', bd=1)
        
        # Ajouter un titre
        settings_menu.add_command(label="Settings", state='disabled')
        ...
        settings_menu.add_command(
            label=f"Check for updates",
            command=self.AutoUpdater.show_window
        )
        
        # update_menu = tk.Menu(settings_menu, tearoff=0)
        # update_menu.add_command(label="Vérifier les mises à jour", command=lambda : manual_update_check(self))
        # update_menu.add_command(label="Installer la mise à jour", command=lambda : run_update(self))
        # settings_menu.add_cascade(label="Mise à jour", menu=update_menu)
        ...
        try:
            ...
            # Si erreur de positionnement, afficher au curseur
            settings_menu.tk_popup(self.root.winfo_pointerx(), self.root.winfo_pointery())
            
    except Exception as e:
        messagebox.showerror("Erreur", f"Impossible d'afficher les paramètres:\n{str(e)}")
...
#         self.AutoUpdater.run_update_check()
...
def _download_youtube_video(self, video, add_to_main_playlist=False, callback=None):
    ...
    if url:
        ...
        threading.Thread(
            ...
            args=(url, add_to_main_playlist, callback),  # url et add_to_playlist
            daemon=True
        ).start()
...
def _open_on_youtube_from_result(self, video):
    ...
    if url:
        ...
        self.status_bar.config(text="Aucun lien trouvé :/")
...
def select_downloads_folder(self):
    ...
    try:
        ...
        new_folder = filedialog.askdirectory(
            title="Choisir le nouveau dossier de téléchargements, actuel : " + self.downloads_folder,
        ...
        current_downloads_folder = self.downloads_folder
        ...
        if files_to_move > 0:
            result = messagebox.askyesno(
                ...
                "Il faudra redémarrer l'application pour que tous les changements prennent effet.",
                icon='question'
            )
        else:
            result = messagebox.askyesno(
        ...
        # Gestion spéciale pour le fichier youtube_urls.json
        old_youtube_metadata_file = os.path.join(current_downloads_folder, "youtube_urls.json")
        new_youtube_metadata_file = os.path.join(new_downloads_folder, "youtube_urls.json")
        
        if os.path.exists(old_youtube_metadata_file):
            try:
                import shutil
                shutil.copy2(old_youtube_metadata_file, new_youtube_metadata_file)
                print(f"Fichier youtube_urls.json copié vers le nouveau dossier")
                
                # Marquer le fichier pour suppression différée si nécessaire
                if not hasattr(self, 'pending_file_deletions'):
                    self.pending_file_deletions = []
                self.pending_file_deletions.append(old_youtube_metadata_file)
                
            except PermissionError:
                # Le fichier est en cours d'utilisation, programmer le déplacement pour la fermeture
                print(f"Fichier youtube_urls.json en cours d'utilisation, déplacement programmé à la fermeture")
                if not hasattr(self, 'pending_file_moves'):
                    self.pending_file_moves = []
                self.pending_file_moves.append((old_youtube_metadata_file, new_youtube_metadata_file))
            except Exception as e:
                print(f"Erreur lors de la copie du fichier youtube_urls.json: {e}")
        ...
        messagebox.showinfo(
            ...
            "⚠️ Redémarrez l'application pour que tous les changements prennent effet."
        )
...
def _handle_pending_file_moves(self):
    """Gère les déplacements de fichiers différés lors de la fermeture"""
    import shutil
    
    for old_path, new_path in self.pending_file_moves:
        try:
            if os.path.exists(old_path):
                ...
                print(f"Fichier déplacé avec succès: {os.path.basename(old_path)}")
                
                # Marquer pour suppression
                if not hasattr(self, 'pending_file_deletions'):
                    self.pending_file_deletions = []
                self.pending_file_deletions.append(old_path)
                
        except Exception as e:
            print(f"Erreur lors du déplacement différé de {os.path.basename(old_path)}: {e}")

def _handle_pending_file_deletions(self):
    """Gère les suppressions de fichiers différées lors de la fermeture"""
    for file_path in self.pending_file_deletions:
        try:
            if os.path.exists(file_path):
                os.remove(file_path)
                print(f"Ancien fichier supprimé: {os.path.basename(file_path)}")
        except Exception as e:
            print(f"Erreur lors de la suppression différée de {os.path.basename(file_path)}: {e}")
