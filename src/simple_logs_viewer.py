...
class SimpleLogsViewer:
    def __init__(self, parent, logger, music_player=None):
        self.parent = parent
        self.logger = logger
        self.music_player = music_player  # Référence directe au music player
        self.window = None
    ...
    def pause_session(self):
        ...
        try:
            self.logger.pause_session()
            messagebox.showinfo("Succès", "Session mise en pause")
            self.load_sessions()
        except Exception as e:
            messagebox.showerror("Erreur", f"Erreur: {e}")
    ...
    def resume_session(self):
        ...
        try:
            # Charger la session et reprendre le traitement
            session_id = self.current_session.get('id')
            session = self.logger.load_session(session_id)
            
            if session:
                # Créer une instance d'ImportDialog pour reprendre le traitement
                from inputs import ImportDialog
                if self.music_player:
                    import_dialog = ImportDialog(self.music_player.root, self.music_player)
                    import_dialog.resume_session_processing(session)
                else:
                    messagebox.showerror("Erreur", "Référence au music player manquante")
                
                messagebox.showinfo("Succès", "Session reprise")
                self.load_sessions()
            else:
                messagebox.showerror("Erreur", "Session non trouvée")
