...
class LogsViewer:
    ...
    def auto_refresh(self):
        ...
        current_selection = self.sessions_tree.selection()
            ...
            for i, session in enumerate(sessions):
                if session.get('session_id') == current_session_id:
                    ...
                    if i < len(items):
                        ...
                        # Recharger la session depuis le fichier pour avoir les derniers logs
                        updated_session = self.import_logger.load_session_state(current_session_id)
                        if updated_session:
