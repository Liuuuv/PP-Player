    ...
    if len(self.main_playlist) > 0 and self.current_index < len(self.main_playlist):
        ...
        try:
            ...
            if self.thumbnail_image.winfo_exists():
                ...
                print("MAIS NON CA NEXISTE PLUS")
            ...
            self.thumbnail_image.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=0, pady=0)
            ...
            current_song_rel = os.path.relpath(current_song, self.downloads_folder)
            thumbnail_offset = self.thumbnail_offsets.get(current_song_rel, 0)
            self._load_large_thumbnail(current_song, self.thumbnail_image, thumbnail_offset)
        ...
        try:
            ...
            self.thumbnail_image.config(text="♪", image=None)
            # Pack à gauche sans padding pour coller au bord
            self.thumbnail_image.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=0, pady=0)
...
def _on_result_right_click(self, event, frame):
        ...
        if hasattr(frame, 'video_data'):
            ...
            self._show_result_context_menu(video, event)
