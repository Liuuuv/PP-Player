...
class MusicPlayer:
    def __init__(self, root):
        ...
        # Variables pour le positionnement intelligent du scroll lors de la recherche
        self.last_downloads_scroll_position = 0.0  # Position du scroll avant la recherche
        self.search_result_frame = None  # Frame du résultat unique pour le positionnement
    ...
    def _on_library_search_change(self, event):
        ...
        # Sauvegarder la position de scroll actuelle avant la recherche
        if hasattr(self, 'downloads_canvas') and current_query:
            try:
                scroll_top, _ = self.downloads_canvas.yview()
                self.last_downloads_scroll_position = scroll_top
            except:
                self.last_downloads_scroll_position = 0.0
    ...
    def _smart_scroll_for_search_results(self):
        """Ajuste intelligemment le scroll pour s'assurer que les résultats de recherche sont visibles"""
        if not hasattr(self, 'downloads_canvas') or not hasattr(self, 'downloads_container'):
            return
        ...
        try:
            ...
            container_height = self.downloads_container.winfo_reqheight()
            
            if container_height <= canvas_height:
                # Tout tient dans le canvas, pas besoin d'ajustement
                return
            
            # Obtenir la position de scroll actuelle
            current_scroll_top, current_scroll_bottom = self.downloads_canvas.yview()
            
            # Vérifier s'il y a des résultats affichés
            children = self.downloads_container.winfo_children()
            if not children:
                return  # Pas de résultats à ajuster
            
            # Obtenir la position du premier résultat
            first_result = children[0]
            first_result_y = first_result.winfo_y()
            first_result_height = first_result.winfo_height()
            
            # Calculer la position visible du premier résultat
            visible_top = current_scroll_top * container_height
            visible_bottom = current_scroll_bottom * container_height
            
            # Vérifier si le premier résultat est visible
            result_bottom = first_result_y + first_result_height
            
            if first_result_y >= visible_top and result_bottom <= visible_bottom:
                # Le premier résultat est déjà visible, pas besoin d'ajustement
                return
            
            # Le premier résultat n'est pas visible, calculer une nouvelle position
            
            # Position cible : afficher le premier résultat avec un peu de marge
            margin = 20  # pixels de marge en haut
            target_y = max(0, first_result_y - margin)
            target_scroll_top = target_y / container_height
            
            # Si le scroll était très bas et qu'on a plusieurs résultats,
            # essayer de trouver un compromis pour montrer plus de résultats
            if self.last_downloads_scroll_position > 0.7 and len(children) > 1:
                # Calculer une position qui montre plusieurs résultats
                visible_height = canvas_height
                results_that_fit = min(len(children), max(2, int(visible_height / 80)))  # Environ 80px par résultat
                
                if results_that_fit < len(children):
                    # Ajuster pour montrer plus de résultats vers le milieu/bas
                    middle_result_index = min(results_that_fit // 2, len(children) - 1)
                    middle_result = children[middle_result_index]
                    middle_y = middle_result.winfo_y()
                    
                    # Position pour centrer ce résultat
                    canvas_center = visible_height / 2
                    target_y = max(0, middle_y - canvas_center)
                    target_scroll_top = target_y / container_height
            
            # S'assurer que la position est dans les limites valides
            max_scroll = 1.0 - (canvas_height / container_height) if container_height > canvas_height else 0
            target_scroll_top = max(0.0, min(max_scroll, target_scroll_top))
            
            # Appliquer le scroll avec animation seulement si c'est un ajustement significatif
            scroll_diff = abs(current_scroll_top - target_scroll_top)
            if scroll_diff > 0.1:  # Seulement si l'ajustement est significatif
                self._animate_downloads_scroll(target_scroll_top)
            
        except Exception as e:
            print(f"Erreur ajustement scroll pour résultats de recherche: {e}")
    
    def _animate_downloads_scroll(self, target_position, steps=8):
        """Anime le scroll vers la position cible"""
        if not hasattr(self, 'downloads_canvas'):
            return
            
        try:
            current_top, _ = self.downloads_canvas.yview()
            
            if abs(current_top - target_position) < 0.01:
                return  # Déjà à la bonne position
            
            # Calculer l'incrément pour l'animation
            step_size = (target_position - current_top) / steps
            
            def animate_step(current_step):
                if current_step >= steps:
                    # Animation terminée, mettre la position exacte
                    self.downloads_canvas.yview_moveto(target_position)
                    return
                
                # Calculer la nouvelle position
                new_position = current_top + (step_size * (current_step + 1))
                self.downloads_canvas.yview_moveto(new_position)
                
                # Programmer l'étape suivante
                self.root.after(20, lambda: animate_step(current_step + 1))
            
            # Démarrer l'animation
            animate_step(0)
            
        except Exception as e:
            print(f"Erreur animation scroll téléchargements: {e}")
    ...
    def _perform_library_search(self):
        ...
        if not search_term:
            ...
            # Si un seul résultat et le scroll était bas, ajuster intelligemment
            if len(filtered_files) == 1:
                # Programmer l'ajustement après que l'affichage soit terminé
                self.root.after(100, self._smart_scroll_to_single_result)
