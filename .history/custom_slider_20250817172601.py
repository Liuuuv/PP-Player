...
class CustomProgressSlider(tk.Frame):
    ...
    def draw_slider(self, force_redraw=False):
        ...
        try:
            ...
            # Pendant le drag, TOUJOURS redessiner pour que la barre bleue continue d'avancer
            if not force_redraw and not self.dragging:
                # Hors drag, redessiner seulement si la valeur change
                if self.last_drawn_value == self.current_value:
                    return
            # Pendant le drag, on redessine toujours pour maintenir la progression de la barre bleue
