...
class MusicPlayer:
    def __init__(self, root):
        ...
        # Variables pour le choix de plateforme de recherche
        self.youtube_selected = tk.BooleanVar(value=True)  # YouTube sélectionné par défaut
        self.youtube_music_selected = tk.BooleanVar(value=False)  # YouTube Music non sélectionné
        
        # Variables pour les filtres de type de contenu
        self.filter_track = tk.BooleanVar(value=True)  # Track activé par défaut
        self.filter_artist = tk.BooleanVar(value=True)  # Artist activé par défaut
        self.filter_album = tk.BooleanVar(value=True)  # Album activé par défaut
        self.filter_playlist = tk.BooleanVar(value=True)  # Playlist activé par défaut
    ...
    def show_playlists_content(self):
        ...
        library_tab__playlists.show_playlists_content(self)
    
    def _display_playlists(self):
        """Affiche toutes les playlists en grille 3x3"""
        library_tab__playlists._display_playlists(self)
    
    def _add_playlist_card(self, parent_frame, playlist_name, songs, column):
        """Ajoute une carte de playlist avec miniatures"""
        library_tab__playlists._add_playlist_card(self, parent_frame, playlist_name, songs, column)
    ...
    def _add_playlist_song_item(self, container, filepath, playlist_name):
        ...
        try:
            ...
            is_artist = self._is_artist_file(filepath)
            self._load_download_thumbnail(filepath, thumbnail_label, is_artist)
            ...
            def on_playlist_song_click(event):
            ...
            def on_playlist_song_double_click(event):
                ...
                if event.state & 0x1 or self.selected_items:  # Shift est enfoncé ou mode sélection
                    ...
                    self._play_from_playlist(filepath, playlist_name)
            
            def on_playlist_song_right_click(event):
                # Si on a des éléments sélectionnés, ouvrir le menu de sélection
                if self.selected_items:
                    self.show_selection_menu(event)
                else:
                    # Comportement normal : jouer après la chanson en cours
            ...
            widgets_to_bind = [item_frame, thumbnail_label, title_label, duration_label]
            for widget in widgets_to_bind:
                widget.bind("<Button-1>", on_playlist_song_click)
                widget.bind("<Double-1>", on_playlist_song_double_click)
                widget.bind("<Button-3>", on_playlist_song_right_click)
    ...
    def _load_download_thumbnail(self, filepath, label, is_artist=False):
        ...
        for ext in image_extensions:
            thumbnail_path = base_name + ext
            if os.path.exists(thumbnail_path):
                self._load_image_thumbnail(thumbnail_path, label, is_artist)
                thumbnail_found = True
                break
        
        if not thumbnail_found:
            # Utiliser la miniature MP3 ou une image par défaut
            self._load_mp3_thumbnail(filepath, label, is_artist)
    ...
    def _create_circular_image(self, img, size):
        """Crée une image circulaire à partir d'une image carrée"""
        ...
        # Créer une image de sortie avec transparence
    ...
    def _load_image_thumbnail(self, image_path, label, is_artist=False):
        """Charge une image normale comme thumbnail"""
        try:
            img = Image.open(image_path)
            
            if is_artist:
                # Pour les artistes, créer une miniature ronde
                circular_img = self._create_circular_image(img, 80)
                photo = ImageTk.PhotoImage(circular_img)
            else:
                # Pour les autres, miniature rectangulaire normale
            ...
            if is_artist:
                default_icon = Image.new('RGBA', (80, 80), color=(61, 61, 61, 255))
                circular_default = self._create_circular_image(default_icon, 80)
                photo = ImageTk.PhotoImage(circular_default)
            else:
    ...
    def _load_mp3_thumbnail(self, filepath, label, is_artist=False):
    ...
    def on_youtube_checkbox_change(self):
        """Gère le changement de la case YouTube"""
        if self.youtube_selected.get():
            # Si YouTube est coché, décocher YouTube Music
            self.youtube_music_selected.set(False)
        else:
            # Si YouTube est décoché, cocher YouTube Music
            self.youtube_music_selected.set(True)
    
    def on_youtube_music_checkbox_change(self):
        """Gère le changement de la case YouTube Music"""
        if self.youtube_music_selected.get():
            # Si YouTube Music est coché, décocher YouTube
            self.youtube_selected.set(False)
        else:
            # Si YouTube Music est décoché, cocher YouTube
            self.youtube_selected.set(True)
    
    def on_content_filter_change(self):
        """Gère le changement des filtres de contenu"""
        # S'assurer qu'au moins un filtre est activé
        if not any([self.filter_track.get(), self.filter_artist.get(), 
                   self.filter_album.get(), self.filter_playlist.get()]):
            # Si tous sont décochés, réactiver track par défaut
            self.filter_track.set(True)
        
        # Relancer la recherche si on a des résultats
        if hasattr(self, 'all_search_results') and self.all_search_results:
            self._apply_content_filters()
    
    def _apply_content_filters(self):
        """Applique les filtres de contenu aux résultats existants"""
        if not hasattr(self, 'all_search_results') or not self.all_search_results:
            return
        
        # Sauvegarder les résultats originaux si ce n'est pas déjà fait
        if not hasattr(self, 'original_search_results'):
            self.original_search_results = self.all_search_results.copy()
        
        # Appliquer les filtres sur les résultats originaux
        filtered_results = self._filter_results_by_content_type(self.original_search_results)
        ...
        if filtered_results:
            ...
            self.status_bar.config(text=f"{len(filtered_results)} résultats trouvés")
        else:
            self.status_bar.config(text="Aucun résultat ne correspond aux filtres")
            self._show_current_song_thumbnail()
    
    def _apply_content_filters_to_search_results(self):
        """Applique les filtres de contenu aux résultats de recherche (version pour thread)"""
        if not hasattr(self, 'all_search_results') or not self.all_search_results:
            return
        
        # Si tous les filtres sont activés, ne pas filtrer
        if all([self.filter_track.get(), self.filter_artist.get(), 
               self.filter_album.get(), self.filter_playlist.get()]):
            return
        
        # Filtrer les résultats selon les cases cochées
        filtered_results = []
        
        for entry in self.all_search_results:
            ...
            # Vérifier le filtre Track
            if self.filter_track.get():
                track_keywords = ['song', 'track', 'music', 'audio', 'single', 'hit', 'cover', 'remix', 'version', 'ft.', 'feat.']
                if any(keyword in title for keyword in track_keywords) or 'music' in uploader:
                    should_include = True
            
            # Vérifier le filtre Artist
            if self.filter_artist.get():
                artist_keywords = ['official', 'vevo', 'records', 'music', 'artist', 'singer', 'band']
                if any(keyword in uploader for keyword in artist_keywords):
                    should_include = True
            
            # Vérifier le filtre Album
            if self.filter_album.get():
                album_keywords = ['album', 'full album', 'complete album', 'lp', 'ep', 'compilation']
                if any(keyword in title for keyword in album_keywords):
                    should_include = True
            
            # Vérifier le filtre Playlist
            if self.filter_playlist.get():
                playlist_keywords = ['playlist', 'mix', 'compilation', 'collection', 'best of', 'greatest hits', 'top']
                if any(keyword in title for keyword in playlist_keywords):
                    should_include = True
    ...
    def _filter_results_by_content_type(self, results):
        """Filtre les résultats selon les types de contenu sélectionnés"""
        if not results:
            return results
        
        # Si tous les filtres sont activés, retourner tous les résultats
        if all([self.filter_track.get(), self.filter_artist.get(), 
               self.filter_album.get(), self.filter_playlist.get()]):
            return results
        ...
        for entry in results:
            ...
            # Vérifier le filtre Track (chansons individuelles)
            if self.filter_track.get():
                track_indicators = [
                    'official music video', 'official video', 'music video',
                    'ft.', 'feat.', 'featuring', 'remix', 'cover', 'version',
                    'single', 'new song', 'latest song'
                ]
                # Exclure les albums et playlists
                exclude_indicators = ['full album', 'complete album', 'playlist', 'mix', 'compilation']
                
                is_track = any(indicator in title for indicator in track_indicators)
                is_not_album_or_playlist = not any(indicator in title for indicator in exclude_indicators)
                
                if is_track or (is_not_album_or_playlist and any(term in uploader for term in ['vevo', 'official', 'records'])):
                    should_include = True
            
            # Vérifier le filtre Artist (chaînes d'artistes)
            if self.filter_artist.get():
                artist_indicators = ['vevo', 'official', 'records', 'music']
                if any(indicator in uploader for indicator in artist_indicators):
                    should_include = True
            
            # Vérifier le filtre Album
            if self.filter_album.get():
                album_indicators = ['full album', 'complete album', 'album', 'lp', 'ep']
                if any(indicator in title for indicator in album_indicators):
                    should_include = True
            
            # Vérifier le filtre Playlist
            if self.filter_playlist.get():
                playlist_indicators = ['playlist', 'mix', 'compilation', 'best of', 'greatest hits', 'top songs', 'collection']
                if any(indicator in title for indicator in playlist_indicators):
                    should_include = True
    ...
    def _is_artist_content(self, title, uploader):
        """Détermine si le contenu correspond à un artiste (pour miniature ronde)"""
        title_lower = title.lower()
        uploader_lower = uploader.lower()
        
        # Indicateurs d'artistes
        artist_indicators = [
            'vevo', 'official', 'records', 'music', 'artist', 'singer', 'band'
        ]
        
        # Vérifier si l'uploader contient des indicateurs d'artiste
        is_artist_channel = any(indicator in uploader_lower for indicator in artist_indicators)
        
        # Exclure les playlists et albums même si c'est un artiste
        exclude_indicators = ['playlist', 'mix', 'compilation', 'full album', 'complete album']
        is_excluded = any(indicator in title_lower for indicator in exclude_indicators)
        
        return is_artist_channel and not is_excluded
    
    def _is_artist_file(self, filepath):
        """Détermine si un fichier téléchargé correspond à un artiste (pour miniature ronde)"""
        filename = os.path.basename(filepath)
        filename_lower = filename.lower()
        
        # Indicateurs d'artistes dans le nom de fichier
        artist_indicators = [
            'official', 'vevo', 'records', 'music video', 'official video',
            'official music video'
        ]
        
        # Exclure les playlists et albums
        exclude_indicators = ['playlist', 'mix', 'compilation', 'full album', 'complete album']
        
        is_artist_file = any(indicator in filename_lower for indicator in artist_indicators)
        is_excluded = any(indicator in filename_lower for indicator in exclude_indicators)
        
        return is_artist_file and not is_excluded
    ...
    def _perform_complete_search(self, query):
        ...
        try:
            ...
            with YoutubeDL(search_opts) as ydl:
                # Choisir la plateforme selon la case cochée
                if self.youtube_music_selected.get():
                    # Recherche orientée musique avec des filtres spécifiques
                    # Ajouter des mots-clés pour privilégier le contenu musical
                    music_keywords = ["music", "audio", "song", "track", "album", "artist"]
                    # Ne pas ajouter de mots-clés si la requête en contient déjà
                    if not any(keyword in query.lower() for keyword in music_keywords):
                        enhanced_query = f"{query} music"
                    else:
                        enhanced_query = query
                    search_query = f"ytsearch50:{enhanced_query}"
                else:
                    # Recherche sur YouTube (par défaut)
                ...
                # Si YouTube Music est sélectionné, privilégier les contenus musicaux
                if self.youtube_music_selected.get():
                    music_keywords = ['music', 'song', 'audio', 'track', 'album', 'artist', 'band', 'singer', 'vocal', 'instrumental']
                    
                    # Séparer les résultats musicaux des autres
                    music_results = []
                    other_results = []
                    ...
                    for entry in video_results:
                        ...
                        # Vérifier si le titre ou l'uploader contient des mots-clés musicaux
                        is_music = any(keyword in title or keyword in uploader for keyword in music_keywords)
                        
                        # Privilégier aussi les chaînes officielles d'artistes
                        is_official = any(term in uploader for term in ['official', 'records', 'music', 'vevo'])
                        
                        if is_music or is_official:
                            music_results.append(entry)
                        else:
                            other_results.append(entry)
                    
                    # Combiner les résultats en privilégiant la musique
                    video_results = music_results + other_results
                
                # Stocker tous les résultats (maximum 50)
                original_results = video_results[:self.max_search_results]
                self.original_search_results = original_results.copy()
                
                # Appliquer les filtres de contenu si nécessaire
                filtered_results = self._filter_results_by_content_type(original_results)
                self.all_search_results = filtered_results
    ...
    def _add_search_result(self, video, index):
        ...
        try:
            ...
            if video.get('thumbnails'):
                # Déterminer si c'est un contenu d'artiste pour une miniature ronde
                is_artist = self._is_artist_content(title, video.get('uploader', ''))
                
                thumbnail_url = video['thumbnails'][1]['url'] if len(video['thumbnails']) > 1 else video['thumbnails'][0]['url']
                threading.Thread(
                    target=self._load_thumbnail,
                    args=(thumbnail_label, thumbnail_url, is_artist),
                    daemon=True
                ).start()
    ...
    def _load_thumbnail(self, label, url, is_artist=False):
