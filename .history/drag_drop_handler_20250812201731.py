...
class DragDropHandler:
    ...
    def _add_dragged_item_to_playlist(self, frame):
        ...
        try:
            if frame.drag_item_type == "file":
                # Fichier local
                self.music_player.add_to_main_playlist(frame.drag_file_path, allow_duplicates=True)
    ...
    def _add_file_to_playlist(self, file_path):
        """Ajoute un fichier local à la liste de lecture principale"""
        if file_path:
            # Vérifier si la playlist était vide avant l'ajout
            was_empty = len(self.music_player.main_playlist) == 0
            
            # Utiliser la fonction centralisée pour ajouter à la main playlist
            # Cela va automatiquement ajouter l'élément visuel et mettre à jour l'affichage
            self.music_player.add_to_main_playlist(file_path, allow_duplicates=True)
            
            # Si la playlist était vide avant, ne pas afficher l'élément dans la playlist
            # (l'utilisateur voulait juste ajouter à la playlist, pas la voir)
            if was_empty:
                # Supprimer l'élément visuel qui vient d'être ajouté
                if hasattr(self.music_player, 'playlist_container'):
                    # Trouver et supprimer le dernier élément ajouté
                    children = self.music_player.playlist_container.winfo_children()
                    if children:
                        last_child = children[-1]
                        if hasattr(last_child, 'filepath') and last_child.filepath == file_path:
                            last_child.destroy()
    ...
    def _add_youtube_to_playlist(self, video_data, frame):
        ...
        try:
            # Télécharger ET ajouter à la liste de lecture principale
            self.music_player.download_selected_youtube(None, add_to_playlist=False)
            title = video_data.get('title', 'Titre inconnu')
            self.music_player.status_bar.config(text=f"Ajout à la liste de lecture: {title[:30]}...")
            
            # Marquer cette vidéo pour être ajoutée à la main playlist après téléchargement
            url = video_data.get('webpage_url') or f"https://www.youtube.com/watch?v={video_data.get('id')}"
            if url not in self.music_player.pending_playlist_additions:
                self.music_player.pending_playlist_additions[url] = []
            if "Main Playlist" not in self.music_player.pending_playlist_additions[url]:
    ...
    def _add_playlist_item_to_main(self, file_path):
        """Ajoute un élément de playlist à la liste de lecture principale"""
        if file_path:
            # Utiliser la fonction centralisée pour ajouter à la main playlist
            # Cela va automatiquement ajouter l'élément visuel et mettre à jour l'affichage
            self.music_player.add_to_main_playlist(file_path, allow_duplicates=True)
