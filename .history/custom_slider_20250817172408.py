...
class CustomProgressSlider(tk.Frame):
    ...
    def draw_slider(self, force_redraw=False):
        ...
        try:
            ...
            # Pendant le drag, toujours redessiner si current_value change (pour la barre bleue)
            # Sinon, redessiner seulement si la valeur ou la position de drag change
            if not force_redraw:
                if self.dragging:
                    # Pendant le drag, redessiner si la valeur OU la position de drag change
                    if (self.last_drawn_value == self.current_value and 
