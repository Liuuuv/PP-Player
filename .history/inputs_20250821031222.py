    ...
    def _process_html_file(self, html_file_path, max_duration):
        ...
        def process_in_thread():
            try:
                ...
                session_id = None
                try:
                    # Petit délai avant l'initialisation du logger
                    time.sleep(0.1)
                    # Initialisation simple et directe du logger
                    session_id = self.import_logger.start_session('HTML', html_file_path)
                    print(f"✅ Session créée: {session_id}")
                    # Petit délai après l'initialisation
                    time.sleep(0.1)
                except Exception as e:
                    print(f"⚠️ Erreur logger: {e}, on continue sans logs")
                    session_id = None
        ...
        def process_wave_thread():
            try:
                ...
                if session_id:
                    ...
                    self.import_logger.log_info(f"📊 Configuration: vagues de {batch_size}, durée max: {max_duration}s")
                ...
                # Terminer la session avec résumé
                if session_id:
                    self.import_logger.log_info(f"🏁 Traitement terminé: {total_downloaded}/{total_links} vidéos téléchargées")
                    self.import_logger.log_info(f"📊 Résumé: {len(all_skipped_videos)} vidéos ignorées")
