...
class WindowsMediaControls:
    ...
    async def _async_update_media_info(self, title, artist, album):
        ...
        try:
            ...
            # Définir les informations de base de manière sécurisée
            if hasattr(updater, 'music_properties'):
                music_props = updater.music_properties
                if title:
                    music_props.title = str(title)
                if artist:
                    music_props.artist = str(artist)
                if album:
                    music_props.album_title = str(album)
