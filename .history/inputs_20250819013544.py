...
def _on_mousewheel(self, event, canvas):
        ...
        # DEBUG: Afficher d'où vient l'événement
        widget_class = event.widget.__class__.__name__
        widget_path = str(event.widget).split('.')[-1]
        print(f"DEBUG: _on_mousewheel appelé depuis {widget_class} ({widget_path})")
        ...
        delta_info = f"delta={event.delta}" if hasattr(event, 'delta') else f"num={getattr(event, 'num', 'N/A')}"
        print(f"DEBUG: Scroll {delta_info} sur canvas {canvas}")
        ...
        if hasattr(self, 'playlist_canvas') and canvas == self.playlist_canvas:
            if hasattr(self, '_check_infinite_scroll'):
                ...
                print("DEBUG: Programmation de _check_infinite_scroll")
