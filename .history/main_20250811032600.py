...
class MusicPlayer:
    def __init__(self, root):
        ...
        self.max_search_batchs = (self.max_search_results + self.results_per_page - 1) // self.results_per_page  # Calcul correct du nombre de lots
        ...
        self.max_thumbnail_threads = 5  # Limite de threads simultanés pour les miniatures
    ...
    def _should_load_more_results(self):
        ...
        # Calculer combien de résultats ont déjà été affichés
        already_displayed = self.current_search_batch * self.results_per_page
        
        should_load = not (self.is_loading_more or 
            self.is_searching or
            not self.current_search_query or 
            already_displayed >= len(self.all_search_results) or
            self.current_search_batch >= self.max_search_batchs)
        
        if should_load:
            print(f"_should_load_more_results: OUI - Lot {self.current_search_batch}, Affichés: {already_displayed}/{len(self.all_search_results)}")
    ...
    def _start_new_search(self):
        ...
        # Lancer une recherche complète au début avec priorité réduite
        self.current_search_thread = threading.Thread(target=self._perform_complete_search_with_priority, args=(query,), daemon=True)
        self.current_search_thread.start()

    def _perform_complete_search_with_priority(self, query):
        """Wrapper pour réduire la priorité du thread de recherche"""
        try:
            import os
            # Réduire la priorité du thread sur Windows
            if os.name == 'nt':
                import psutil
                p = psutil.Process()
                p.nice(psutil.BELOW_NORMAL_PRIORITY_CLASS)
        except:
            pass  # Si psutil n'est pas disponible, continuer sans réduction de priorité
        
        # Appeler la fonction de recherche normale
        self._perform_complete_search(query)
    ...
    def _perform_complete_search(self, query):
        ...
        try:
            ...
            search_opts = {
                ...
                'quiet': True,
                'no_warnings': True,
                'max_downloads': 20,  # Réduire à 20 résultats pour moins de charge
                'ignoreerrors': True,
            ...
            with YoutubeDL(search_opts) as ydl:
                ...
                # Recherche de 20 résultats avec pause pour laisser respirer le système
                import time
                time.sleep(0.1)  # Petite pause avant la recherche
                results = ydl.extract_info(f"ytsearch20:{query}", download=False)
                time.sleep(0.1)  # Petite pause après la recherche
                ...
                # Stocker tous les résultats (maximum 20)
                self.all_search_results = filtered_results[:self.max_search_results]
                
                # Recalculer le nombre de lots en fonction du nombre réel de résultats
                if self.all_search_results:
                    self.max_search_batchs = (len(self.all_search_results) + self.results_per_page - 1) // self.results_per_page
    ...
    def _display_batch_results(self, batch_number):
        ...
        # Afficher les résultats de ce lot avec un délai entre chaque pour éviter le lag
    ...
    def _display_results_progressively(self, start_index, end_index, batch_number, current_index=None):
        ...
        if self.search_cancelled or current_index >= end_index:
            if not self.search_cancelled and current_index >= end_index:
                ...
                # Mettre à jour le compteur avec le nombre réel de résultats affichés
                self.search_results_count = min(end_index, len(self.all_search_results))
        ...
        if next_index < end_index and not self.search_cancelled:
            # Délai de 100ms entre chaque résultat pour laisser respirer l'UI
            self.root.after(100, lambda: self._display_results_progressively(start_index, end_index, batch_number, next_index))
    ...
    def _load_more_search_results(self):
        ...
        # Calculer combien de résultats ont déjà été affichés
        already_displayed = self.current_search_batch * self.results_per_page
        print(f"_load_more_search_results appelée - Lot actuel: {self.current_search_batch}, Déjà affichés: {already_displayed}/{len(self.all_search_results)}")
        ...
        if (self.is_loading_more or 
            ...
            already_displayed >= len(self.all_search_results) or
            self.current_search_batch >= self.max_search_batchs):
            print(f"Arrêt du chargement: loading={self.is_loading_more}, searching={self.is_searching}, cancelled={self.search_cancelled}, displayed={already_displayed}, total={len(self.all_search_results)}, batch={self.current_search_batch}/{self.max_search_batchs}")
            return
    ...
    def _clear_results(self):
        ...
        try:
            # Nettoyer la queue des miniatures
        ...
        try:
            ...
            if video.get('thumbnails'):
                ...
                if is_channel:
                    ...
                    self._queue_thumbnail_load(thumbnail_label, thumbnail_url, is_circular=True)
                else:
                    # Pour les vidéos, miniature normale
                    self._queue_thumbnail_load(thumbnail_label, thumbnail_url, is_circular=False)
