...
class MusicPlayer:
    ...
    def _show_artist_content(self, artist_name, video_data):
        ...
        try:
            ...
            # Lancer les recherches en arrière-plan
            self._search_artist_videos()
            self._search_artist_releases()
            self._search_artist_playlists()
    ...
    def _create_artist_tabs(self):
        ...
        # Créer un frame header fixe pour les onglets et la croix
        header_frame = tk.Frame(main_container, bg='#3d3d3d', height=32)
        header_frame.pack(fill=tk.X, side=tk.TOP)
        header_frame.pack_propagate(False)  # Maintenir la hauteur fixe
        
        # Ajouter le bouton croix dans le coin supérieur droit du header
        ...
        # Positionner la croix à droite dans le header fixe
        close_btn.pack(side=tk.RIGHT, padx=(5, 5), pady=2)
        
        # Créer le notebook sous le header
        self.artist_notebook = ttk.Notebook(main_container, takefocus=0)
        self.artist_notebook.pack(fill=tk.BOTH, expand=True, side=tk.TOP)
        
        # Ajouter un tooltip au bouton croix
        try:
            from tooltip import create_tooltip
            create_tooltip(close_btn, "Retourner à la recherche\nQuitte l'affichage de l'artiste et retourne aux résultats de recherche\n(Raccourci: Échap)")
        except:
    ...
    def _search_artist_videos(self):
        """Recherche les vidéos de l'artiste depuis l'onglet Vidéos de sa chaîne par ordre de sortie (les plus récentes d'abord)"""
        def search_videos():
            try:
                ...
                # Options pour extraire les vidéos de la chaîne
                search_opts = {
                    'extract_flat': True,
                    'quiet': True,
                    'no_warnings': True,
                    'ignoreerrors': True,
                    'playlistend': 50  # Limiter à 50 vidéos pour éviter les timeouts
                }
                ...
                with YoutubeDL(search_opts) as ydl:
                    ...
                    # D'abord, essayer de trouver l'ID réel de la chaîne
                    channel_id = None
                    if hasattr(self, 'current_artist_channel_url') and self.current_artist_channel_url:
                        # Extraire l'ID de la chaîne depuis l'URL si elle en contient un
                        import re
                        channel_match = re.search(r'/channel/([^/?]+)', self.current_artist_channel_url)
                        if channel_match:
                            channel_id = channel_match.group(1)
                            print(f"ID de chaîne extrait: {channel_id}")
                    
                    # Si pas d'ID direct, faire une recherche pour trouver la chaîne officielle
                    if not channel_id:
                        try:
                            ...
                            print(f"Erreur recherche chaîne: {e}")
                    
                    # Sauvegarder l'ID pour le réutiliser dans les autres fonctions
                    self.current_artist_channel_id = channel_id
                    
                    # Construire les URLs avec l'ID de la chaîne si trouvé
                    channel_urls_to_try = []
                    if channel_id:
                        # Utiliser le format officiel avec l'ID
                        base_channel_url = f"https://www.youtube.com/channel/{channel_id}"
                        channel_urls_to_try.append(base_channel_url + '/videos')
                        print(f"URL chaîne construite: {base_channel_url}")
                    else:
                        # Fallback avec l'URL originale si disponible
                        if hasattr(self, 'current_artist_channel_url') and self.current_artist_channel_url:
                            # Nettoyer et décoder l'URL si nécessaire
                            import urllib.parse
                            base_url = self.current_artist_channel_url.rstrip('/')
                            
                            # Si l'URL contient des caractères sur-encodés, essayer de les décoder
                            try:
                                if '%' in base_url:
                                    # Décoder l'URL
                                    decoded_url = urllib.parse.unquote(base_url, encoding='utf-8')
                                    print(f"URL décodée (fallback): {base_url} -> {decoded_url}")
                                    # Utiliser l'URL décodée comme base
                                    base_url = decoded_url
                            except Exception as e:
                                print(f"Erreur décodage URL (fallback): {e}")
                                # Garder l'URL originale si le décodage échoue
                            
                            # 1. URL directe avec l'onglet vidéos
                            if '/channel/' in base_url or '/@' in base_url:
                                channel_urls_to_try.append(base_url + '/videos')
                            else:
                                channel_urls_to_try.append(base_url)
                        
                        # 2. Essayer différentes variantes du nom d'artiste avec encodage approprié
                        artist_variants = [
                            self.current_artist_name,
                            self.current_artist_name.replace(' ', ''),
                            self.current_artist_name.replace('　', ''),  # Espace japonais
                            self.current_artist_name.replace(' ', '').replace('　', ''),
                            # Variante avec espaces normaux seulement
                            self.current_artist_name.replace('　', ' '),
                            # Variante sans espaces du tout
                            ''.join(self.current_artist_name.split())
                        ]
                        
                        for variant in artist_variants:
                            if variant and variant != self.current_artist_name:
                                import urllib.parse
                                # Encodage plus approprié pour YouTube qui préserve les caractères japonais
                                try:
                                    # Essayer d'abord sans encodage (YouTube gère souvent bien l'UTF-8)
                                    channel_urls_to_try.append(f"https://www.youtube.com/@{variant}/videos")
                                    # Puis avec encodage URL si nécessaire
                                    encoded_variant = urllib.parse.quote(variant.encode('utf-8'), safe='')
                                    if encoded_variant != variant:  # Seulement si l'encodage change quelque chose
                                        channel_urls_to_try.append(f"https://www.youtube.com/@{encoded_variant}/videos")
                                except:
                                    # Fallback avec encodage standard
                                    encoded_variant = urllib.parse.quote(variant, safe='')
                                    channel_urls_to_try.append(f"https://www.youtube.com/@{encoded_variant}/videos")
                        
                        # Trier les URLs pour essayer d'abord les plus simples (sans encodage excessif)
                        def url_priority(url):
                            # Compter le nombre de caractères encodés
                            encoded_chars = url.count('%')
                            # Privilégier les URLs avec moins d'encodage
                            return encoded_chars
                        
                        channel_urls_to_try.sort(key=url_priority)
                        
                        # Essayer chaque URL jusqu'à ce qu'une fonctionne
                        consecutive_404_count = 0
                        max_404_attempts = 3  # Limiter les tentatives après des erreurs 404 consécutives
                        
                        for url in channel_urls_to_try:
                            if self.artist_search_cancelled:
                                return
                            # Si on a eu trop d'erreurs 404 consécutives, arrêter les tentatives de chaîne
                            if consecutive_404_count >= max_404_attempts:
                                print(f"Arrêt des tentatives de chaîne après {consecutive_404_count} erreurs 404 consécutives")
                                break
                            # Ignorer les URLs avec trop de caractères encodés (probablement problématiques)
                            if url.count('%') > 10:
                                print(f"URL ignorée car trop d'encodage: {url}")
                                continue
                            try:
                                print(f"Tentative d'extraction de: {url}")
                                ...
                                if channel_info and 'entries' in channel_info:
                                    ...
                                    if videos:  # Si on a trouvé des vidéos, arrêter d'essayer
                                        all_videos.extend(videos[:30])  # Prendre les 30 premières pour un meilleur tri
                                        print(f"Trouvé {len(videos)} vidéos depuis la chaîne")
                                        break
                                # Réinitialiser le compteur d'erreurs si une tentative réussit
                                consecutive_404_count = 0
                            except Exception as e:
                                error_msg = str(e)
                                if "404" in error_msg or "Not Found" in error_msg:
                                    consecutive_404_count += 1
                                    print(f"Chaîne introuvable ({consecutive_404_count}/{max_404_attempts}): {url}")
                                elif "Unable to download API page" in error_msg:
                                    consecutive_404_count += 1  # Traiter comme une 404
                                    print(f"API indisponible ({consecutive_404_count}/{max_404_attempts}): {url}")
                                else:
                                    print(f"Erreur extraction chaîne {url}: {e}")
                                    # Les autres erreurs ne comptent pas vers la limite 404
                                if self.artist_search_cancelled:
                                    return
                                continue
                    
                    # Si pas de vidéos trouvées, faire une recherche classique
                    if not all_videos and not self.artist_search_cancelled:
                        search_queries = [
                            f"ytsearch20:{self.current_artist_name}",
                            f"ytsearch10:{self.current_artist_name} official"
                        ]
                        ...
                        for query in search_queries:
                            ...
                            try:
                                ...
                                if results and 'entries' in results:
                                    videos = self._filter_search_results(results['entries'])
                                    # Filtrer pour garder seulement les vidéos, pas les chaînes
                                    videos = [v for v in videos if v and 'youtube.com/watch' in v.get('webpage_url', v.get('url', ''))]
                                    all_videos.extend(videos)
                                    if len(all_videos) >= 15 or self.artist_search_cancelled:  # Arrêter si on a assez de vidéos
                                        break
                            except Exception as e:
                    ...
                    for video in all_videos:
                        ...
                        if video_id and video_id not in unique_videos:
                            ...
                            if not video.get('url'):
                                video['url'] = video.get('webpage_url', f"https://www.youtube.com/watch?v={video_id}")
                    ...
                    # Trier par date de publication (plus récentes en premier) avec une meilleure logique
                    def get_upload_date(video):
                        # Essayer plusieurs champs de date
                        for date_field in ['upload_date', 'published', 'release_date']:
                            upload_date = video.get(date_field)
                            if upload_date:
                                if isinstance(upload_date, str) and upload_date:
                                    try:
                                        # Nettoyer la date et convertir
                                        clean_date = upload_date.replace('-', '').replace('/', '').replace(' ', '')[:8]
                                        return int(clean_date) if clean_date.isdigit() else 19700101
                                    except:
                                        continue
                        # Date par défaut très ancienne si aucune date trouvée
                        return 19700101
        ...
        self.artist_videos_thread = threading.Thread(target=search_videos, daemon=True)
        self.artist_videos_thread.start()
    
    def _search_artist_releases(self):
        """Recherche les albums et singles de l'artiste (spécialement pour les chaînes sans onglets spécialisés)"""
        def search_releases():
            try:
                # Vérifier si la recherche a été annulée avant de commencer
                if self.artist_search_cancelled:
                ...
                with YoutubeDL(search_opts) as ydl:
                    ...
                    # Utiliser l'ID de chaîne trouvé précédemment s'il est disponible
                    urls_to_try = []
                    if hasattr(self, 'current_artist_channel_id') and self.current_artist_channel_id:
                        ...
                        urls_to_try.append(base_channel_url + '/releases')  # Essayer releases en premier
                        urls_to_try.append(base_channel_url + '/playlists')  # Puis playlists si pas de releases
                        print(f"URLs releases construites avec ID chaîne: {base_channel_url}")
                    elif hasattr(self, 'current_artist_channel_url') and self.current_artist_channel_url:
                        # Fallback avec l'URL originale
                        try:
                            # Nettoyer et décoder l'URL si nécessaire
                            import urllib.parse
                            base_url = self.current_artist_channel_url.rstrip('/')
                            ...
                            try:
                                if '%' in base_url:
                                    # Décoder l'URL
                                    decoded_url = urllib.parse.unquote(base_url, encoding='utf-8')
                                    print(f"URL décodée pour releases (fallback): {base_url} -> {decoded_url}")
                                    # Utiliser l'URL décodée comme base
                                    base_url = decoded_url
                            except Exception as e:
                                print(f"Erreur décodage URL releases (fallback): {e}")
                                # Garder l'URL originale si le décodage échoue
                            
                            # Construire l'URL de l'onglet releases puis playlists
                            if '/channel/' in base_url or '/@' in base_url:
                                urls_to_try.append(base_url + '/releases')  # Essayer releases en premier
                                urls_to_try.append(base_url + '/playlists')  # Puis playlists si pas de releases
                            else:
                                urls_to_try.append(base_url)
                        except Exception as e:
                            print(f"Erreur construction URLs releases: {e}")
                    
                    if urls_to_try:
                        channel_info = None
                        consecutive_404_count = 0
                        max_404_attempts = 2  # Moins de tentatives pour releases/playlists
                        ...
                        for url in urls_to_try:
                            ...
                            if consecutive_404_count >= max_404_attempts:
                                print(f"Arrêt des tentatives releases après {consecutive_404_count} erreurs consécutives")
                                break
                            try:
                                print(f"Extraction depuis: {url}")
                                channel_info = ydl.extract_info(url, download=False)
                                if channel_info and 'entries' in channel_info:
                                    print(f"Trouvé contenu sur {url}")
                                    break
                                consecutive_404_count = 0  # Réinitialiser si succès
                            except Exception as e:
                                error_msg = str(e)
                                if "404" in error_msg or "Not Found" in error_msg:
                                    consecutive_404_count += 1
                                    print(f"URL releases/playlists introuvable ({consecutive_404_count}/{max_404_attempts}): {url}")
                                elif "Unable to download API page" in error_msg:
                                    consecutive_404_count += 1
                                    print(f"API indisponible ({consecutive_404_count}/{max_404_attempts}): {url}")
                                else:
                            ...
                            if channel_info and 'entries' in channel_info:
                                ...
                                print(f"Nombre de playlists trouvées: {len(playlists)}")
                            ...
                            error_msg = str(e)
                            if "404" in error_msg or "Not Found" in error_msg:
                                print(f"URL releases/playlists introuvable pour la chaîne, recherche alternative...")
                            elif "Unable to download API page" in error_msg:
                                print(f"API YouTube indisponible pour les releases, recherche alternative...")
                            else:
                                print(f"Erreur extraction playlists chaîne: {e}")
                    
                    # Si pas de playlists trouvées, essayer une recherche spécifique aux albums/singles
                    if not all_playlists and not self.artist_search_cancelled:
                        print("Aucune release trouvée sur la chaîne, recherche alternative...")
                        
                        # Essayer de trouver des albums et singles via une recherche ciblée
                        search_queries = [
                            f"ytsearch15:{self.current_artist_name} full album",
                            f"ytsearch15:{self.current_artist_name} album",
                            f"ytsearch10:{self.current_artist_name} EP",
                            f"ytsearch10:{self.current_artist_name} mini album",
                            f"ytsearch10:{self.current_artist_name} single",
                            f"ytsearch10:{self.current_artist_name} discography"
                        ]
                        ...
                        for query in search_queries:
                            try:
                                ...
                                results = ydl.extract_info(query, download=False)
                                if results and 'entries' in results:
                                    items = self._filter_search_results(results['entries'])
                                    # Filtrer pour garder seulement les playlists
                                    playlists_only = []
                                    for item in items:
                                        if (item.get('_type') == 'playlist' or 
                                            'playlist' in item.get('url', '') or 
                                            'list=' in item.get('url', '') or
                                            item.get('playlist_count', 0) > 0 or
                                            any(keyword in item.get('title', '').lower() 
                                                for keyword in ['album', 'ep', 'single', 'full album', 'mini album', 'discography', 'collection', 'release'])):
                                            playlists_only.append(item)
                                            print(f"Playlist trouvée: {item.get('title', 'Sans titre')}")
                                    
                                    all_playlists.extend(playlists_only)
                                    if len(all_playlists) >= 10:  # Arrêter si on a assez de playlists
                                        break
                            except Exception as e:
                                print(f"Erreur recherche {query}: {e}")
                                continue
    ...
    def _search_artist_playlists(self):
        """Recherche toutes les playlists de la chaîne"""
                ...
                with YoutubeDL(search_opts) as ydl:
                    ...
                    if hasattr(self, 'current_artist_channel_id') and self.current_artist_channel_id:
                        ...
                        try:
                            ...
                            error_msg = str(e)
                            if "404" in error_msg or "Not Found" in error_msg:
                                print(f"URL playlists introuvable avec ID chaîne, recherche alternative...")
                            elif "Unable to download API page" in error_msg:
                                print(f"API YouTube indisponible pour les playlists, recherche alternative...")
                            else:
                                print(f"Erreur extraction playlists avec ID chaîne: {e}")
                    elif hasattr(self, 'current_artist_channel_url') and self.current_artist_channel_url:
                        # Fallback avec l'URL originale
                        try:
                            # Nettoyer et décoder l'URL si nécessaire
                            ...
                            try:
                                if '%' in base_url:
                                    # Décoder l'URL
                                    decoded_url = urllib.parse.unquote(base_url, encoding='utf-8')
                                    print(f"URL décodée pour playlists (fallback): {base_url} -> {decoded_url}")
                                    # Utiliser l'URL décodée comme base
                                    base_url = decoded_url
                            except Exception as e:
                                print(f"Erreur décodage URL playlists (fallback): {e}")
                                # Garder l'URL originale si le décodage échoue
                            
                            # Construire l'URL de l'onglet playlists
                            if '/channel/' in base_url or '/@' in base_url:
                                playlists_url = base_url + '/playlists'
                            else:
                                playlists_url = base_url
                            
                            print(f"Extraction des playlists depuis (fallback): {playlists_url}")
                            ...
                            error_msg = str(e)
                            if "404" in error_msg or "Not Found" in error_msg:
                                print(f"URL playlists introuvable pour la chaîne, recherche alternative...")
                            elif "Unable to download API page" in error_msg:
                                print(f"API YouTube indisponible pour les playlists, recherche alternative...")
                            else:
                                print(f"Erreur extraction directe playlists: {e}")
                    
                    # Recherche alternative si pas de résultats
                    if not all_playlists and not self.artist_search_cancelled:
                        search_queries = [
                            f"ytsearch10:{self.current_artist_name} playlist",
                            f"ytsearch10:{self.current_artist_name} official playlist"
                        ]
                        ...
                        for query in search_queries:
                            if self.artist_search_cancelled:
                                return
                            try:
                                print(f"Recherche playlists: {query}")
                                results = ydl.extract_info(query, download=False)
                                if self.artist_search_cancelled:
                                    return
                                if results and 'entries' in results:
                                    ...
                                    for item in items:
                                        ...
                                        if (item.get('_type') == 'playlist' or 
                                            'playlist' in item.get('url', '') or 
                                            'list=' in item.get('url', '') or
                                            item.get('playlist_count', 0) > 0):
                                            all_playlists.append(item)
                                            print(f"Playlist trouvée: {item.get('title', 'Sans titre')}")
                                    
                                    if len(all_playlists) >= 20 or self.artist_search_cancelled:  # Plus de playlists que pour les sorties
                                        break
                            except Exception as e:
                                print(f"Erreur recherche {query}: {e}")
                                if self.artist_search_cancelled:
                    ...
                    # Limiter à 20 playlists max
                    final_playlists = final_playlists[:20]
    ...
    def _display_artist_releases(self, releases):
        ...
        # Afficher les playlists dans le frame scrollable
        for i, playlist in enumerate(releases):
            self._add_artist_playlist_result(playlist, i, scrollable_frame)
    ...
    def _add_artist_result(self, video, index, container):
        ...
        try:
            ...
            # Configuration de la grille (plus compact)
            ...
            result_frame.rowconfigure(0, minsize=35, weight=0)     # Hauteur réduite
            ...
            title_label.grid(row=0, column=0, sticky='ew', pady=1)
            ...
            duration_label = tk.Label(
                ...
                bg='#4a4a4a',
                fg='#cccccc',
                font=('TkDefaultFont', 7),  # Police plus petite
                anchor='center'
            )
            duration_label.grid(row=0, column=2, sticky='ns', padx=(0, 5), pady=3)
            ...
            # Événements de clic (double-clic pour télécharger)
            def on_artist_video_double_click(event, frame=result_frame):
                self._on_result_click(frame, add_to_playlist=False)
            
            # Bindings
            result_frame.bind("<Double-1>", on_artist_video_double_click)
            text_frame.bind("<Double-1>", on_artist_video_double_click)
            title_label.bind("<Double-1>", on_artist_video_double_click)
            thumbnail_label.bind("<Double-1>", on_artist_video_double_click)
            duration_label.bind("<Double-1>", on_artist_video_double_click)
        ...
        def load_thumbnail():
            try:
                ...
                if response.status_code == 200:
                    ...
                    # Redimensionner l'image (60x45 pour un affichage plus compact)
    ...
    def _add_artist_playlist_result(self, playlist, index, container):
        """Ajoute une playlist dans l'onglet Sorties avec double-clic pour voir le contenu"""
        try:
            title = playlist.get('title', 'Sans titre')
            playlist_count = playlist.get('playlist_count', 0)
            ...
            result_frame.columnconfigure(0, minsize=60, weight=0)  # Miniature plus petite
            result_frame.columnconfigure(1, weight=1)              # Titre
            result_frame.columnconfigure(2, minsize=50, weight=0)  # Nombre de vidéos
            result_frame.rowconfigure(0, minsize=35, weight=0)     # Hauteur réduite
            
            # Miniature (plus petite) avec icône playlist
            thumbnail_label = tk.Label(
                result_frame,
                bg='#4a4a4a',
                width=8,
            ...
            count_text = f"{playlist_count} vidéos" if playlist_count > 0 else "Playlist"
            ...
            count_label.grid(row=0, column=2, sticky='ns', padx=(0, 5), pady=3)
            ...
            def on_playlist_double_click(event, frame=result_frame):
                self._show_playlist_content(frame.playlist_data)
            
            # Bindings pour le double-clic
            ...
            for widget in [result_frame, title_label, thumbnail_label, count_label]:
    ...
    def _show_playlist_content(self, playlist_data):
        ...
        def load_playlist_content():
            try:
                ...
                if 'watch?v=' in playlist_url and 'list=' not in playlist_url:
                    ...
                    self.root.after(0, lambda: self._display_playlist_content([video_data], playlist_title))
                    return
                ...
                with YoutubeDL(playlist_opts) as ydl:
                    ...
                    if playlist_info and 'entries' in playlist_info:
                        ...
                        self.root.after(0, lambda: self._display_playlist_content(videos, playlist_title))
                    elif playlist_info:
        ...
        self._show_playlist_loading(playlist_data.get('title', 'Contenu'))
        
        # Lancer en arrière-plan
        threading.Thread(target=load_playlist_content, daemon=True).start()
    
    def _show_playlist_loading(self, playlist_title):
        """Affiche un message de chargement pour la playlist"""
        # Vider l'onglet sorties et afficher le chargement
    ...
    def _display_playlist_content(self, videos, playlist_title):
        ...
        header_frame = tk.Frame(self.sorties_frame, bg='#3d3d3d')
        ...
        back_button = tk.Button(
            ...
            font=('TkDefaultFont', 8),
            relief='flat',
            command=self._return_to_playlists
        )
    ...
    def _return_to_search(self):
        ...
        try:
            ...
            # Supprimer seulement le notebook artiste
