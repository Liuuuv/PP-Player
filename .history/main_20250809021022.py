...
class MusicPlayer:
    ...
    def _add_playlist_item(self, filepath, thumbnail_path=None):
        ...
        try:
            ...
            # Frame principal
            item_frame = ttk.Frame(self.playlist_container, height=50)
            ...
            text_id = canvas.create_text(
                ...
                text=filename,
                fill='white',
                width=300,  # Largeur max avant retour à la ligne
                font=('Arial', 10)
            )
            
            # Fonction d'élision dynamique
            def elide_text():
                text = canvas.itemcget(text_id, 'text')
                bbox = canvas.bbox(text_id)
                while bbox and bbox[2] > 380 and len(text) > 3:  # 380 = position bouton
                    text = text[:-4] + '...'
                    canvas.itemconfig(text_id, text=text)
                    bbox = canvas.bbox(text_id)
            
            canvas.after(100, elide_text)  # Exécuter après le rendu
