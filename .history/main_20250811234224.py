...
class MusicPlayer:
    ...
    def _format_artist_album_info(self, artist, album):
        """Formate les informations d'artiste et d'album pour l'affichage"""
        if not artist and not album:
            return ""
        
        # Afficher le séparateur même si l'un des deux est vide
        if artist or album:
            artist_part = artist if artist else ""
            album_part = album if album else ""
    ...
    def play_track(self):
        try:
            ...
            # Obtenir les métadonnées et créer le texte complet
            artist, album = self._get_audio_metadata(song)
            metadata_text = self._format_artist_album_info(artist, album)
            
            # Créer le texte complet avec titre et métadonnées
            if metadata_text:
                full_display_text = f"{song_name}\n{metadata_text}"
            else:
                full_display_text = song_name
            
            # Démarrer l'animation du titre (ou afficher normalement si pas tronqué)
            self._start_title_animation(full_display_text)
    ...
    def _add_search_result(self, video, index):
        ...
        try:
            ...
            if not is_channel:
                ...
                # Si l'album n'est pas disponible dans les métadonnées YouTube (cas fréquent avec extract_flat=True),
                # essayer d'extraire l'artiste et l'album depuis le titre
                if not album and title:
                    extracted_artist, extracted_album = self._extract_artist_album_from_title(title)
