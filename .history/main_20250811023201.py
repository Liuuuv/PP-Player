...
class MusicPlayer:
    ...
    def show_output_menu(self):
        ...
        try:
            ...
            # Ajouter chaque périphérique comme option du menu avec checkbutton
            for device in devices:
                device_name = device.decode('utf-8') if isinstance(device, bytes) else device
                
                # Utiliser checkbutton pour marquer le périphérique actuel
                is_current = (self.current_audio_device == device_name)
                
                output_menu.add_checkbutton(
                    label=device_name,
                    variable=tk.BooleanVar(value=is_current),
