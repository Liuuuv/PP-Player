    ...
    def download_recommendations(self, recommendations):
        ...
        def download_thread():
            try:
                ...
                for i, rec in enumerate(recommendations):
                    try:
                        ...
                        with YoutubeDL(ydl_opts) as ydl:
                            ...
                            # Ajouter selon le mode de recommandation
                            ...
                            if recommendation_mode == "sparse":
                                ...
                                self.main_app.root.after(0, lambda path=final_path:
                                    self.main_app._safe_add_to_queue(path))
                            else:
                                # Mode add: ajouter à la playlist principale
                                self.main_app.root.after(0, lambda path=final_path:
                                    self.main_app.add_to_main_playlist(path, thumbnail_path=None, song_index=None, show_status=False))
                ...
                recommendation_mode = getattr(self.main_app, 'recommendation_mode', 'sparse')
                if recommendation_mode == "sparse":
                    message = f"{downloaded_count} recommandation éparse ajoutée à la playlist"
                else:
                    message = f"{downloaded_count} recommandation(s) ajoutée(s) à la playlist (mode add)"
