...
class ImportDialog:
    ...
    def setup_drag_drop(self):
        ...
        try:
            ...
            # Si tkinterdnd2 est disponible, l'utiliser
            self.dialog.drop_target_register(DND_FILES)
            self.dialog.dnd_bind('<<Drop>>', self.on_drop)
        except ImportError:
            # Fallback: utiliser les événements de base de tkinter
            self.dialog.bind('<Button-1>', self.on_click)
            self.dialog.bind('<B1-Motion>', self.on_drag)
            self.dialog.bind('<ButtonRelease-1>', self.on_drop_fallback)
    ...
    def on_drop_fallback(self, event):
        """Fallback pour le drag & drop - ouvre un dialog de sélection de fichier"""
        if event.x > 0 and event.y > 0:  # Vérifier que c'est un vrai drop
