...
def _add_song_item(self, filepath_or_video, container, playlist_name=None, song_index=None, item_type="downloads", video_index=None):
    ...
    try:
        ...
        artist_label.pause_counter = 0  # Compteur pour la pause entre les cycles
        artist_label.max_width = 30  # Largeur maximale du titre
        ...
        album_label.grid(row=0, column=1, sticky='w', padx=(5, 0))
        ...
        album_label.pause_counter = 0  # Compteur pour la pause entre les cycles
        album_label.max_width = 30  # Largeur maximale du titre
        album_label.animation_active = False  # Animation en cours
        album_label.full_text = album_label.cget('text')  # Texte complet du titre
        
        album_label.grid(row=0, column=2, sticky='w', padx=(5, 0))
        ...
        date_label.grid(row=0, column=3, sticky='w', padx=(5, 0))
        ...
        widgets_to_bind = [item_frame, number_label, thumbnail_label, text_frame, title_label, duration_label, metadata_frame, album_label, date_label]
        ...
        def force_motion_bindings():
            widgets_to_fix = [item_frame, number_label, thumbnail_label, text_frame, title_label, duration_label, metadata_frame, album_label, date_label]
                ...
                for label in all_labels:
                    if label.cget('text') == "⏵":  # C'est le label de miniature
                        ...
                        #         label.config(text="• " + other_metadata_text)
                            ...
                            other_parts = []
                            if album:
                                self._truncate_text_for_display(album, max_width_pixels=label.max_width, font_size=SUBTITLE_FONT_SIZE)
                                other_parts.append(album)
                            ...
                            other_metadata_text = " • ".join(other_parts) if other_parts else ""
                            print("DEBUG: LOAD_NEXT_THUMBNAIL OTHER METADATAS", other_metadata_text)
                            if other_metadata_text:
                                # Ajouter le séparateur • au début si on a des données
                                label.config(text="• " + other_metadata_text)
                break
