...
class MusicPlayer:
    def __init__(self, root):
        ...
        # Initialiser les contrôles multimédia Windows
        self.media_controls = media_controls.initialize_media_controls(self)
    ...
    def play_track(self):
        try:
            ...
            # Mettre à jour les contrôles multimédia Windows
            if self.media_controls:
                from winsdk.windows.media import MediaPlaybackStatus
                self.media_controls.update_playback_status(MediaPlaybackStatus.PLAYING)
                
                # Extraire les informations du fichier
                song_title = os.path.basename(song)[:-4] if os.path.basename(song).lower().endswith('.mp3') else os.path.basename(song)
                
                # Chercher une miniature correspondante
                thumbnail_path = None
                ...
                self.media_controls.update_media_info(
                    title=song_title,
                    artist="Pipi Player",
                    album="",
    ...
    def on_closing(self):
        ...
        try:
            # Nettoyer les contrôles multimédia Windows
            if hasattr(self, 'media_controls') and self.media_controls:
                self.media_controls.cleanup()
