...
class MusicPlayer:
    ...
    def _show_artist_content(self, artist_name, video_data):
        ...
        try:
            ...
            # Créer les onglets au-dessus des résultats existants
    ...
    def _save_current_search_state(self):
        ...
        self.saved_search_state = {
            'results_visible': hasattr(self, 'youtube_canvas') and self.youtube_canvas.winfo_viewable(),
        ...
        if hasattr(self, 'youtube_canvas') and self.youtube_canvas.winfo_exists():
            try:
                ...
                self.saved_search_state['canvas_visible'] = self.youtube_canvas.winfo_viewable()
            except:
                pass
        
        # Sauvegarder l'état de la scrollbar
        if hasattr(self, 'scrollbar') and self.scrollbar.winfo_exists():
            self.saved_search_state['scrollbar_visible'] = self.scrollbar.winfo_viewable()
        
        # Sauvegarder les widgets enfants du results_container
        if hasattr(self, 'results_container') and self.results_container.winfo_exists():
            self.saved_search_state['results_children'] = list(self.results_container.winfo_children())
    
    def _create_artist_tabs(self):
        """Crée les onglets Musiques et Sorties dans la zone YouTube"""
        # Cacher complètement le canvas et la scrollbar des résultats
        if hasattr(self, 'youtube_canvas') and self.youtube_canvas.winfo_exists():
        ...
        # Cacher tous les autres widgets du results_container
        if hasattr(self, 'results_container') and self.results_container.winfo_exists():
            for child in self.results_container.winfo_children():
                child.pack_forget()
        
        # Créer le notebook qui remplace complètement les résultats
        self.artist_notebook = ttk.Notebook(self.results_container, takefocus=0)
        ...
        def search_videos():
            try:
                ...
                search_opts = {
                    ...
                    'quiet': True,
                    'no_warnings': True,
                    'max_downloads': 10,  # Limiter à 10 vidéos
                    'ignoreerrors': True
                }
                ...
                with YoutubeDL(search_opts) as ydl:
                    ...
                    search_queries = [
                        ...
                        f"ytsearch10:{self.current_artist_name} music video"
                    ]
            ...
            try:
                ...
                with YoutubeDL(search_opts) as ydl:
                    ...
                    search_queries = [
                        ...
                        f"ytsearch10:{self.current_artist_name} official audio"
                    ]
                    ...
                    # Limiter à 10 sorties max
                    final_releases = final_releases[:10]
    ...
    def _display_artist_releases(self, releases):
        ...
        # Afficher les sorties dans le frame scrollable
        for i, release in enumerate(releases):
            self._add_artist_result(release, i, scrollable_frame)
        ...
        def load_thumbnail():
            try:
                ...
                if response.status_code == 200:
                    ...
                    # Redimensionner l'image (80x60 pour s'adapter au label)
    ...
    def _return_to_search(self):
        ...
        try:
            ...
            if hasattr(self, 'saved_search_state'):
                # Restaurer la scrollbar si elle était visible
                if (self.saved_search_state.get('scrollbar_visible', False) and 
                    hasattr(self, 'scrollbar') and self.scrollbar.winfo_exists()):
                    self.scrollbar.pack(side="right", fill="y")
                
                # Restaurer le canvas si il était visible
                if (self.saved_search_state.get('canvas_visible', False) and 
                    hasattr(self, 'youtube_canvas') and self.youtube_canvas.winfo_exists()):
                    self.youtube_canvas.pack(side="left", fill="both", expand=True)
                
                # Restaurer les autres widgets du results_container
                for child in self.saved_search_state.get('results_children', []):
                    if child.winfo_exists() and child not in [getattr(self, 'youtube_canvas', None), getattr(self, 'scrollbar', None)]:
