...
class ImportDialog:
    ...
    def create_widgets(self):
        ...
        # Ajouter le menu contextuel au bouton d'import
    ...
    def setup_import_context_menu(self):
        ...
        self.context_menu = tk.Menu(self.dialog, tearoff=0, bg='#3d3d3d', fg='white')
        ...
        self.download_btn.bind("<Button-3>", self.show_context_menu)
    ...
    def show_import_logs(self):
        ...
        logs_viewer = LogsViewer(self.dialog, self.import_logger)
    ...
    def _process_html_file(self, html_file_path, max_duration):
        ...
        def process_in_thread():
            try:
                ...
                session_id = self.import_logger.start_session('HTML', html_file_path)
    ...
    def _process_videos_in_waves_sequential(self, youtube_links, max_duration, batch_size, html_file_path, session_id):
        """Traite les vidéos séquentiellement par vagues avec logs et sauvegarde d'état"""
        def process_sequential_thread():
            try:
                self.import_logger.log_info(f"Démarrage du traitement séquentiel de {len(youtube_links)} liens")
                
                # Traiter chaque lien un par un
                    ...
                    try:
                        ...
                        self.music_player.root.after(0, lambda i=i+1, total=len(youtube_links): 
                            self.music_player.status_bar.config(
                                text=f"🔍 Vérification {i}/{total}: {url[:50]}..."))
                        
                        self.import_logger.log_info(f"Vérification de l'URL {i+1}/{len(youtube_links)}: {url}")
                        ...
                        with YoutubeDL(ydl_opts) as ydl:
                            ...
                            if existing_file:
                                self.import_logger.log_url_processed(url, title, 'skipped', 'Fichier déjà téléchargé')
                                continue
                            
                            # Télécharger la vidéo
                            self.music_player.root.after(0, lambda t=title: 
                                self.music_player.status_bar.config(
                                    text=f"⬇️ Téléchargement: {t[:50]}..."))
                            
                            self.import_logger.log_info(f"Début du téléchargement: {title}")
                            
                            # Téléchargement synchrone
                            success = self._download_video_sync(url, title, info)
                        ...
                        error_msg = str(e)
                        self.import_logger.log_error(f"Erreur lors du traitement de {url}: {error_msg}")
                ...
                # Message final
                session = self.import_logger.current_session or self.import_logger.load_session_state(session_id)
                if session:
                    success_count = session.get('successful_downloads', 0)
                    total_count = session.get('total_links', 0)
                    self.music_player.root.after(0, lambda s=success_count, t=total_count: 
                        self.music_player.status_bar.config(
                            text=f"✅ Terminé ! {s}/{t} vidéos téléchargées avec succès"))
        ...
        # Lancer le traitement séquentiel dans un thread
        threading.Thread(target=process_sequential_thread, daemon=True).start()
