...
def update_progress_overlay(self, download_item):
    ...
    try:
        ...
        if download_item.completed:
            progress_overlay.configure(bg='#4CAF50')  # Vert pour terminé
        elif download_item.error:
            progress_overlay.configure(bg='#ff9800')  # Orange pour erreur
        else:
            progress_overlay.configure(bg='#4CAF50')  # Vert pour en cours
