...
class MusicPlayer:
    ...
    def setup_media_keys(self):
        ...
        try:
            # Première tentative : avec suppression complète des événements multimédia
            self.media_key_listener = keyboard.Listener(
                on_press=on_media_key_press,
                suppress=True  # Supprimer complètement les événements pour empêcher leur propagation
            )
            self.media_key_listener.daemon = True  # Thread daemon pour qu'il se ferme avec l'application
            self.media_key_listener.start()
            print("Touches multimédia activées avec suppression complète (priorité maximale)")
        except Exception as e:
