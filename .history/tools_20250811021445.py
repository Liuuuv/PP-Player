...
def _apply_volume(self):
    ...
    if self.volume == 0:
        ...
        # Pour des volumes > 100%, on utilise toujours 1.0 pour pygame
        # mais on stocke la valeur réelle pour d'éventuelles amplifications futures
        pygame_volume = min(1.0, final_volume)
        final_volume = max(0, final_volume)
    
    # Appliquer le volume à pygame (limité à 1.0)
    pygame.mixer.music.set_volume(pygame_volume if self.volume > 0 else 0)
    
    # Si le volume demandé est > 100%, on peut utiliser le volume système
    if final_volume > 1.0:
        try:
            # Essayer d'augmenter le volume système (Windows)
            import subprocess
            import platform
            if platform.system() == "Windows":
                # Calculer le pourcentage pour le volume système (max 100%)
                system_volume = min(100, int(final_volume * 50))  # Amplification modérée
                subprocess.run([
                    "powershell", "-Command", 
                    f"(New-Object -comObject WScript.Shell).SendKeys([char]175)"
                ], capture_output=True, check=False)
        except:
            pass  # Si ça échoue, on continue avec le volume pygame normal
