...
def load_icons(self):
    ...
    for key, filename in icon_names.items():
        try:
            ...
            # Créer une version plus petite pour les icônes output et stats
            if key in ["output", "stats"]:
                image_small = Image.open(path).resize((16, 16), Image.Resampling.LANCZOS)
                self.icons[key + "_small"] = ImageTk.PhotoImage(image_small)
            
            # Traitement spécial pour l'icône delete - enlever le fond bleu
            if key == "delete":
                try:
                    delete_image = Image.open(path).resize((32, 32), Image.Resampling.LANCZOS)
                    # Convertir en RGBA pour gérer la transparence
                    delete_image = delete_image.convert("RGBA")
                    
                    # Créer une version sans fond bleu
                    data = delete_image.getdata()
                    new_data = []
                    
                    for item in data:
                        # Si le pixel est bleu (ou proche du bleu), le rendre transparent
                        # Détecter les nuances de bleu (#5a9fd8 et similaires)
                        r, g, b, a = item
                        if (b > r + 30 and b > g + 30) or (r < 150 and g < 150 and b > 150):
                            # Pixel bleu détecté, le rendre transparent
                            new_data.append((r, g, b, 0))
                        else:
                            # Garder le pixel tel quel
                            new_data.append(item)
                    
                    delete_image.putdata(new_data)
                    self.icons["delete_clean"] = ImageTk.PhotoImage(delete_image)
                except Exception as e:
                    print(f"Erreur traitement icône delete: {e}")
                    # En cas d'erreur, utiliser l'icône cross comme fallback
                    self.icons["delete_clean"] = self.icons.get("cross", self.icons[key])
        except Exception as e:
