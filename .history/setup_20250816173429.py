...
def load_icons(self):
    ...
    for key, filename in icon_names.items():
        try:
            ...
            # Créer une version plus petite pour les icônes output, stats, import, clear_cache et select_downloads_folder
            if key in ["output", "stats", "import", "clear_cache", "select_downloads_folder"]:
                image_small = Image.open(path).resize((16, 16), Image.Resampling.LANCZOS)
                self.icons[key + "_small"] = ImageTk.PhotoImage(image_small)
        except Exception as e:
    ...
    try:
        ...
        if os.path.exists(playlists_file):
            ...
            for name, songs in loaded_playlists.items():
                ...
                # Vérifier que les fichiers existent encore et normaliser les chemins
                for song in songs:
                    print("DEBUG SONG:", song)
                    
                    # Si le chemin est relatif (commence par "downloads"), le convertir en absolu
                    if song.startswith("downloads"):
                        # Enlever le préfixe "downloads" et créer le chemin absolu
                        relative_path = song[len("downloads"):].lstrip(os.sep).lstrip("/").lstrip("\\")
                        full_song_path = os.path.join(self.downloads_folder, relative_path)
                    elif not os.path.isabs(song):
                        # Si c'est un autre chemin relatif, le joindre au dossier downloads
                    ...
                    if os.path.exists(full_song_path):
                        existing_songs.append(full_song_path)  # Stocker le chemin absolu
