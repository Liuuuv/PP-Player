...
def _safe_add_to_queue_first_from_result(self, video):
    ...
    try:
        def callback(filepath):
            print(f"_safe_add_to_queue_first_from_result {video['title']} {filepath}")
            if hasattr(self, 'root') and self.root.winfo_exists() and hasattr(self, 'drag_drop_handler'):
                self.drag_drop_handler._add_to_queue_first(filepath)
        
        self._download_youtube_video(video, callback)
    except tk.TclError:
        pass  # Widget détruit, ignorer silencieusement
...
def _safe_add_to_queue(self, filepath):
    ...
    try:
        if hasattr(self, 'root') a/nd self.root.winfo_exists() and hasattr(self, 'drag_drop_handler'):
            self.drag_drop_handler._add_to_queue(filepath)
    except tk.TclError:
        pass  # Widget détruit, ignorer silencieusement
...
def _safe_add_to_queue_from_result(self, item):
    ...
    try:
        filepath = self._download_youtube_video(item)
        if hasattr(self, 'root') and self.root.winfo_exists() and hasattr(self, 'drag_drop_handler'):
            self.drag_drop_handler._add_to_queue(filepath)
    except tk.TclError:
        pass  # Widget détruit, ignorer silencieusement
