...
class MusicPlayer:
    ...
    def _add_playlist_song_item(self, container, filepath, playlist_name):
        ...
        try:
            ...
            def on_playlist_song_click(event):
                ...
                # Si on a une sélection active, ne pas l'effacer sur un clic simple
    ...
    def toggle_item_selection(self, filepath, frame):
        ...
        # Mettre à jour la barre de statut avec le nombre d'éléments sélectionnés
    ...
    def _update_selection_status(self):
        """Met à jour la barre de statut avec le nombre d'éléments sélectionnés"""
        if self.selected_items:
            count = len(self.selected_items)
            if count == 1:
                self.status_bar.config(text=f"1 élément sélectionné")
            else:
                self.status_bar.config(text=f"{count} éléments sélectionnés")
        else:
    ...
    def show_selection_menu(self, event):
        ...
        context_menu.add_command(
            label=f"{len(self.selected_items)} élément(s) sélectionné(s)",
            state="disabled"
        )
