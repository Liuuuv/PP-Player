    ...
    def _apply_content_filters(self):
        ...
        for entry in self.all_search_results:
            ...
            if self.filter_track.get():
                track_keywords = ['song', 'track', 'music', 'audio', 'single', 'hit', 'cover', 'remix', 'version']
                if any(keyword in title for keyword in track_keywords):
                    should_include = True
            
            # Vérifier le filtre Artist
            if self.filter_artist.get():
                artist_keywords = ['artist', 'singer', 'band', 'musician', 'performer', 'official', 'vevo']
            ...
            if self.filter_playlist.get():
                playlist_keywords = ['playlist', 'mix', 'compilation', 'collection', 'best of', 'greatest hits']
