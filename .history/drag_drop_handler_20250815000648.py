...
class DragDropHandler:
    ...
    def _update_drag_visual(self, frame, dx, dy):
        ...
        if dx > 50:
            self._set_frame_colors(frame, COLORDRAG_RIGHT_HALF)  # Vert clair pour indiquer l'activation partielle
        elif dx > 100:  # Drag vers la droite - ajouter à la playlist
            self._set_frame_colors(frame, COLORDRAG_RIGHT)  # Vert pour indiquer l'activation
        elif dx < -50:
            self._set_frame_colors(frame, COLORDRAG_LEFT_HALF)  # Orange clair pour indiquer l'activation partielle
        elif dx < -100:  # Drag vers la gauche - placer en premier dans la queue
            self._set_frame_colors(frame, COLORDRAG_LEFT)  # Jaune-orange pour indiquer "premier dans la queue"
        else:
