...
class MusicPlayer:
    def __init__(self, root):
        ...
        self.queue_items = set()  # Set des fichiers qui font partie de la queue
        ...
        self.numbered_songs_count = 0  # Nombre de musiques originales numérotées
    ...
    def _show_playlist_content_in_tab(self, playlist_name):
        ...
        # Bouton pour jouer toute la playlist dans l'ordre
        play_all_btn = tk.Button(
            ...
            padx=8,
            pady=8,
            command=lambda: self._play_entire_playlist_ordered(playlist_name),
            takefocus=0
        )
        play_all_btn.pack(side=tk.LEFT, padx=(0, 10))
        create_tooltip(play_all_btn, "Jouer toute la playlist\nLit toute la playlist dans l'ordre avec numérotation")
        
        # Bouton pour jouer toute la playlist en mode aléatoire
        shuffle_all_btn = tk.Button(
            ...
            padx=8,
            pady=8,
            command=lambda: self._play_entire_playlist_shuffle(playlist_name),
            takefocus=0
        )
        shuffle_all_btn.pack(side=tk.LEFT)
        create_tooltip(shuffle_all_btn, "Jouer en mode aléatoire\nLit toute la playlist dans un ordre aléatoire avec numérotation")
    ...
    def _play_playlist_from_song(self, playlist_name, song_index):
        ...
        # Définir le nombre de musiques numérotées (toutes les musiques de la playlist)
        self.numbered_songs_count = len(self.playlists[playlist_name])
    ...
    def _play_entire_playlist_ordered(self, playlist_name):
        """Joue toute la playlist dans l'ordre avec numérotation"""
        ...
        # Marquer que la main playlist provient d'une playlist (pour afficher les numéros)
        self.main_playlist_from_playlist = True
        # Définir le nombre de musiques numérotées (toutes les musiques de la playlist)
        self.numbered_songs_count = len(self.playlists[playlist_name])
        ...
        # Message de statut
        self.status_bar.config(text=f"Lecture de la playlist '{playlist_name}' ({len(self.playlists[playlist_name])} titres)")
    
    def _play_entire_playlist_shuffle(self, playlist_name):
        """Joue toute la playlist en mode aléatoire avec numérotation"""
        if playlist_name not in self.playlists or not self.playlists[playlist_name]:
            return
        ...
        # Message de statut
        self.status_bar.config(text=f"Lecture aléatoire de la playlist '{playlist_name}' ({len(self.playlists[playlist_name])} titres)")
    ...
    def play_all_downloads_ordered(self):
        ...
        # Marquer que la main playlist provient d'une playlist (pour afficher les numéros)
        self.main_playlist_from_playlist = True
        # Définir le nombre de musiques numérotées (toutes les musiques téléchargées)
        self.numbered_songs_count = len(self.all_downloaded_files)
    ...
    def _add_main_playlist_item(self, filepath, thumbnail_path=None, song_index=None):
        ...
        try:
            ...
            # Déterminer si on affiche les numéros (seulement si provient d'une playlist ET dans la plage numérotée)
            ...
            # Afficher les numéros seulement pour les musiques originales (dans la plage numérotée)
            show_numbers = (self.main_playlist_from_playlist and 
                          current_song_index < self.numbered_songs_count)
