...
class MusicPlayer:
    ...
    def _add_artist_playlist_result(self, playlist, index, container, target_tab="sorties"):
        ...
        try:
            ...
            # Debug pour voir les champs disponibles
            print(f"Playlist '{title}': playlist_count={playlist.get('playlist_count')}, video_count={playlist.get('video_count')}, entry_count={playlist.get('entry_count')}, entries_len={len(playlist.get('entries', []))}")
            print(f"Champs disponibles: {list(playlist.keys())}")
            ...
            # Nombre de musiques sous le titre
            count_text = f"{playlist_count} musiques" if playlist_count > 0 else "Playlist"
    ...
    def _display_playlist_content(self, videos, playlist_title, target_tab="sorties"):
        ...
        # Positionner le bouton à gauche de la croix (croix à x=-5, back à x=-30)
        self.playlist_back_btn.place(in_=main_container, relx=1.0, rely=0.0, anchor="ne", x=-30, y=5)
