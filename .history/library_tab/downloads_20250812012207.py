...
def _show_no_results_message(self):
    ...
    # Créer un frame principal pour le message
    no_results_frame = tk.Frame(
        self.downloads_container,
        bg='#2d2d2d',
        height=400
    )
    no_results_frame.pack(fill="both", expand=True, padx=20, pady=50)
    no_results_frame.pack_propagate(False)  # Empêcher le frame de se redimensionner
    
    # Créer un frame central
    center_frame = tk.Frame(no_results_frame, bg='#2d2d2d')
    center_frame.place(relx=0.5, rely=0.5, anchor="center")
    
    # Debug : vérifier si l'icône existe
    icon_found = hasattr(self, 'icons') and 'none' in self.icons
    print(f"Debug: Icône 'none' trouvée: {icon_found}")
    if hasattr(self, 'icons'):
        print(f"Debug: Icônes disponibles: {list(self.icons.keys())}")
    
    # Afficher l'image none.png si elle existe
    if icon_found:
        icon_label = tk.Label(
            center_frame,
        ...
        print("Debug: Image none.png affichée")
    else:
        # Fallback : afficher un placeholder si l'image n'est pas trouvée
        ...
        print("Debug: Placeholder affiché à la place de l'image")
    ...
    text_label.pack()
    print("Debug: Texte 'Aucun résultat' affiché")
