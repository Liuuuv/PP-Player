...
def _display_playlists(self):
    ...
    # Organiser les playlists en lignes de 8 (exclure la main playlist)
    playlist_items = [(name, songs) for name, songs in self.playlists.items() if name != "Main Playlist"]
    
    for row in range(0, len(playlist_items), 8):
        ...
        # Configurer les colonnes pour qu'elles soient égales (8 colonnes maintenant)
        for col in range(8):
            row_frame.columnconfigure(col, weight=1, uniform="playlist_col")
        
        # Ajouter jusqu'à 8 playlists dans cette ligne
        for col in range(8):
...
def _add_playlist_card(self, parent_frame, playlist_name, songs, column):
    ...
    try:
        ...
        # 1. Zone des miniatures (2x2 grid) - taille fixe pour uniformité (réduite pour 8 par ligne)
        thumbnails_frame = tk.Frame(card_frame, bg='#4a4a4a', width=100, height=100)
