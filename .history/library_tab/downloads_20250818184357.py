...
def play_all_downloads_ordered(self):
    ...
    # Rafraîchir d'abord l'affichage de la playlist
    self._refresh_main_playlist_display_async()
    
    # Démarrer la lecture après un court délai
    self.root.after(100, self.play_track)
    
    # Réactiver les boutons et mettre à jour le statut final
    self.root.after(200, self._enable_play_buttons)
    self.root.after(250, lambda: self.status_bar.config(text=f"Lecture démarrée - {len(self.all_downloaded_files)} musiques chargées"))
...
def play_all_downloads_shuffle(self):
    ...
    # Rafraîchir d'abord l'affichage de la playlist pour s'assurer que l'interface est prête
    self._refresh_main_playlist_display_async()
    
    # Démarrer la lecture après un court délai pour s'assurer que l'interface est mise à jour
    self.root.after(100, lambda: self.play_track())
    
    # Réactiver les boutons et mettre à jour le statut final
    self.root.after(250, lambda: self._enable_play_buttons())
    self.root.after(300, lambda: self.status_bar.config(text=f"Lecture démarrée - {len(self.all_downloaded_files)} musiques chargées"))
