...
def _show_no_results_message(self):
    ...
    # Créer un frame principal avec beaucoup d'espace vertical (comme les résultats normaux)
    no_results_frame = tk.Frame(
        self.downloads_container,
        bg='#3d3d3d',
        height=600
    )
    no_results_frame.pack(fill="both", expand=True, pady=100)
    no_results_frame.pack_propagate(False)
    
    # Frame centré avec contraste pour debug
    center_frame = tk.Frame(
        no_results_frame, 
        bg='#4d4d4d',  # Couleur légèrement différente pour voir le frame
        relief='raised',
        bd=2
    )
    ...
    if hasattr(self, 'icons') and 'none' in self.icons:
        print("Debug: Affichage de l'icône 'none'")
        # Afficher l'image none.png
        icon_label = tk.Label(
            center_frame,
        ...
        icon_label.pack(pady=20, padx=20)
    else:
        print("Debug: Icône 'none' non trouvée - affichage du placeholder")
        # Fallback : grand emoji
        ...
        placeholder_label.pack(pady=20, padx=20)
    ...
    print("Debug: Frame de message créé et affiché")
