...
def _show_no_results_message(self):
    ...
    print(f"Debug: Icône 'none' disponible: {has_icon}")
    ...
    if has_icon:
        try:
            ...
            print("Debug: Image 'none' agrandie affichée")
        except Exception as e:
            print(f"Debug: Erreur avec l'image: {e}")
            has_icon = False
    ...
    if not has_icon:
        ...
        emoji_label.pack()
        print("Debug: Emoji de fallback affiché")
    ...
    print("Debug: Message 'Aucun résultat' affiché")
...
def _update_downloads_queue_visual(self):
    ...
    print('Mise à jour de la visualisation de la queue des téléchargements, _update_downloads_queue_visual')
    try:
        ...
        for widget in self.downloads_container.winfo_children():
            if hasattr(widget, 'filepath'):  # C'est un frame d'élément de téléchargement
                ...
                # # Chercher s'il y a déjà un indicateur de queue (colonne 0)
                # has_queue_indicator = False
                # for child in widget.winfo_children():
                #     grid_info = child.grid_info()
                #     if grid_info.get('column') == 0 and hasattr(child, 'cget'):
                #         try:
                #             if child.cget('bg') == 'black':  # C'est l'indicateur de queue
                #                 has_queue_indicator = True
                #                 if not is_in_queue:
                #                     # Supprimer l'indicateur car plus dans la queue
                #                     child.destroy()
                #                 break
                #         except:
                #             pass
                ...
                # if is_in_queue and not has_queue_indicator:
                #     queue_indicator = tk.Frame(
                #         widget,
                #         bg='black',  # Trait noir
                #         width=3
                ...
                print(f"Mis à jour l'indicateur de queue pour {widget.queue_indicator}")
                if hasattr(widget, 'queue_indicator') and hasattr(widget, 'is_in_queue'):
                    if widget.is_in_queue:
                        self.show_queue_indicator(widget)
                    else:
                        self.hide_queue_indicator(widget)
