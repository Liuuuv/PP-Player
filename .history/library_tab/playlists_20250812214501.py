...
def _add_playlist_card(self, parent_frame, playlist_name, songs, column):
    ...
    try:
        ...
        # Bouton supprimer - icône complète nettoyée
        delete_icon = self.icons.get("delete_clean", self.icons["cross"])
        delete_btn = tk.Button(
            ...
            relief="flat",
            bd=0,
            width=delete_icon.width(),
            height=delete_icon.height(),
            takefocus=0
        )
...
def _add_playlist_song_item(self, filepath, playlist_name, song_index):
    ...
    try:
        ...
        # 6. Bouton "Supprimer de la playlist" (colonne 5) avec icône delete nettoyée
        remove_btn = tk.Button(
            item_frame,
            image=self.icons.get("delete_clean", self.icons["cross"]),  # Utiliser delete_clean ou cross en fallback
            bg="#3d3d3d",
            activebackground='#ff6666',
            relief='flat',
            bd=0,
