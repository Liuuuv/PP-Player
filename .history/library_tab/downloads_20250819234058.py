...
class DownloadsProgressiveLoader:
    ...
    def initialize(self, files_list):
        ...
        # Obtenir la configuration de load_more_count
        try:
            self.load_count = get_config('load_more_count')
        except:
            self.load_count = 20
...
def _display_filtered_downloads(self, files_to_display_, preserve_scroll=False):
    ...
    global files_to_display, _last_window_start, _last_window_end
    
    print("_display_filtered_downloads appelée")
    files_to_display = files_to_display_
    ...
    if not files_to_display:
        ...
        # Réinitialiser les variables de fenêtrage
        _last_window_start, _last_window_end = None, None
        # Marquer la fin du refresh
        self._refreshing_downloads = False
        return
    
    # Initialiser le chargement progressif
    try:
        load_count = get_config('load_more_count')
    except:
        load_count = 20  # Valeur par défaut
    
    # Réinitialiser les variables de fenêtrage
    _last_window_start = 0
    _last_window_end = min(load_count, len(files_to_display))
    ...
    visible_files = files_to_display[_last_window_start:_last_window_end]
    self._start_thumbnail_loading(visible_files, self.downloads_container)
...
def _load_more_songs_below(self, unload=True):
        """Charge plus de musiques en-dessous de la fenêtre actuelle pour les téléchargements"""
        global _last_window_start, _last_window_end, files_to_display
        ...
        try:
            ...
            if _last_window_end is None:
                return
            
            current_end = _last_window_end
            if current_end >= len(files_to_display):
            ...
            new_end = min(len(files_to_display), current_end + load_count)
            
            print(f"Chargement de {load_count} musiques en-dessous (index {current_end} à {new_end})")
            
            # Étendre la fenêtre vers le bas
            _extend_window_down(self, new_end)
            
            print("_load_more_songs_below terminé", current_end, load_count, new_end)
            ...
            # Délai pour éviter les chargements répétés
            try:
                timer_delay = MAIN_PLAYLIST_REST_TIMER_AFTER_LOADING
            except:
                timer_delay = 500  # 500ms par défaut
                
            self.root.after(timer_delay, reset_loading_down_flag)
...
def _extend_window_down(self, new_end):
        """Étend la fenêtre d'affichage vers le bas pour les téléchargements"""
        global _last_window_start, _last_window_end, files_to_display
        
        try:
            print("_extend_window_down appelée")
            if _last_window_start is None or _last_window_end is None:
            ...
            for i in range(current_end, new_end):
                if i < len(files_to_display):
                    filepath = files_to_display[i]
                    self._add_download_item_fast(filepath)
            
            # Mettre à jour les paramètres de fenêtre
            _last_window_end = new_end
            
            # Mettre à jour la région de scroll
            self._update_scrollbar()
            
            print(f"Fenêtre étendue vers le bas: {current_end} -> {new_end}")
