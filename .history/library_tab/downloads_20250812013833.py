...
def _show_no_results_message(self):
    ...
    # Frame principal avec la bonne couleur de fond
    ...
    content_frame = tk.Frame(no_results_frame, bg='#3d3d3d')
    ...
    if has_icon:
        try:
            ...
            print("Debug: Image 'none' affichée avec succès")
        except Exception as e:
            print(f"Debug: Erreur affichage image 'none': {e}")
            has_icon = False
    
    if not has_icon:
        # Fallback: Emoji ou texte alternatif
        placeholder_label = tk.Label(
            content_frame,
            text="🔍",  # Loupe pour "pas de résultats"
            bg='#3d3d3d',
            fg='#888888',
            font=('Arial', 48),
            bd=0
        )
        placeholder_label.pack(pady=(40, 20))
