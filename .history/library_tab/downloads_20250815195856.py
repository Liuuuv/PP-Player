...
def _disable_play_buttons(self):
    ...
    try:
        # Trouver et désactiver les boutons de lecture
        for widget in self.downloads_frame.winfo_children():
            if isinstance(widget, tk.Frame):
                for child in widget.winfo_children():
                    if isinstance(child, tk.Button):
                        if ("Jouer toutes" in child.cget('text') or 
                            hasattr(child, 'command') and 
                            (child.cget('command') == self.play_all_downloads_ordered or
                             child.cget('command') == self.play_all_downloads_shuffle)):
...
def _enable_play_buttons(self):
    ...
    try:
        # Trouver et réactiver les boutons de lecture
        for widget in self.downloads_frame.winfo_children():
            if isinstance(widget, tk.Frame):
                for child in widget.winfo_children():
                    if isinstance(child, tk.Button):
