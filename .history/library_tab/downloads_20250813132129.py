...
def _on_library_search_change(self, event):
    ...
    if hasattr(self, '_app_destroyed') and self._app_destroyed:
        print("DEBUG: Application détruite, ignorer l'événement de recherche")
        return
    ...
    if hasattr(self, '_last_search_query') and self._last_search_query == current_query:
        print(f"DEBUG: Requête identique à la précédente ('{current_query}'), ignorer")
        return
    
    print(f"DEBUG: Nouvelle recherche: '{current_query}'")
