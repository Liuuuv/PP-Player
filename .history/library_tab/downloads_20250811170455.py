...
def _add_download_item(self, filepath):
    ...
    try:
        ...
        # Configuration de la grille en 4 colonnes : miniature, texte, durée, bouton supprimer
        ...
        item_frame.columnconfigure(3, minsize=40, weight=0)  # Bouton supprimer
        ...
        # 4. Bouton "Lire après la chanson actuelle" (colonne 3)
        add_btn = tk.Button(
            item_frame,
            image=self.icons["add_next"],
            bg="#4a8fe7",
            fg="white",
        ...
        add_btn.grid(row=0, column=3, sticky='ns', padx=(0, 5), pady=8)
        
        # Configurer la commande pour lire après la chanson actuelle
        add_btn.config(command=lambda f=filepath: self._play_after_current(f))
        create_tooltip(add_btn, "Lire après la chanson actuelle\nPlace cette chanson juste après celle en cours et la lit")
