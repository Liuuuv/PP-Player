...
def _display_filtered_downloads(self, files_to_display):
    ...
    if len(files_to_display) > 20:  # Seuil abaissé pour utiliser le batch plus souvent
        # Afficher un indicateur de progression pour les grandes collections
        if len(files_to_display) > 100:
            self._show_loading_progress(len(files_to_display))
        self._display_files_batch_optimized(files_to_display, 0, len(files_to_display))
    else:
        # Afficher directement si peu de fichiers, mais avec chargement différé des miniatures
