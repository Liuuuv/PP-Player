...
def _display_playlist_songs(self, playlist_name):
        ...
        # Forcer la réinitialisation de tous les boutons après un court délai
        self.root.after(100, self._reset_all_playlist_buttons)
...
def _add_playlist_song_item(self, filepath, playlist_name, song_index):
    ...
    try:
        ...
        def on_remove_button_click(event):
            ...
            remove_btn.config(bg="#3d3d3d", state="normal")
        ...
        def on_remove_button_enter(event):
            # S'assurer que le bouton a la bonne couleur au survol
            if remove_btn.cget("state") == "normal":
                remove_btn.config(bg="#3d3d3d")
                
        def on_remove_button_release(event):
            # Réinitialiser après relâchement du clic
            remove_btn.config(bg="#3d3d3d", state="normal")
            
        remove_btn.bind("<Double-1>", on_remove_button_click)
        remove_btn.bind("<Leave>", on_remove_button_leave)
        remove_btn.bind("<Enter>", on_remove_button_enter)
        remove_btn.bind("<ButtonRelease-1>", on_remove_button_release)
