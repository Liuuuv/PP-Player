...
def _display_filtered_downloads(self, files_to_display):
    ...
    # Réactiver la scrollbar s'il y a des résultats
    if hasattr(self, 'downloads_scrollbar') and hasattr(self, 'downloads_canvas'):
        self.downloads_canvas.configure(yscrollcommand=self.downloads_scrollbar.set)
...
def _show_no_results_message(self):
    ...
    # Désactiver le scroll en détachant la scrollbar du canvas
    if hasattr(self, 'downloads_canvas'):
        self.downloads_canvas.configure(yscrollcommand='')
    
    # Créer un frame principal pour le message qui prend toute la hauteur
    no_results_frame = tk.Frame(
        self.downloads_container,
        bg='#2d2d2d',
        height=600  # Hauteur plus importante
    )
    ...
    # Afficher l'image none.png (plus grande)
    ...
    # Afficher le texte "Aucun résultat" plus grand
    text_label = tk.Label(
        center_frame,
        text="Aucun résultat",
        bg='#2d2d2d',
