...
def _show_no_results_message(self):
    ...
    print("Debug: Début de _show_no_results_message")
    
    # TEST ULTRA-SIMPLE : un gros widget très visible
    test_frame = tk.Frame(
        self.downloads_container,
        bg='red',  # Rouge vif pour être sûr de le voir
        width=400,
        height=200,
        relief='solid',
        bd=5
    ...
    # Texte de test ultra-visible
    test_label = tk.Label(
        test_frame,
        text="TEST - AUCUN RÉSULTAT",
        bg='red',
        fg='yellow',
        font=('Arial', 24, 'bold'),
        bd=0
    ...
    print("Debug: Widget rouge créé")
    
    # Vérifier les enfants du container
    children = self.downloads_container.winfo_children()
    print(f"Debug: Nombre d'enfants dans downloads_container: {len(children)}")
    ...
    # Reconfigurer la scrollregion
    self.downloads_canvas.configure(scrollregion=self.downloads_canvas.bbox("all"))
    
    # Information de debug sur le canvas
    bbox = self.downloads_canvas.bbox("all")
    print(f"Debug: bbox du canvas: {bbox}")
    
    # Vérifier la position de la fenêtre dans le canvas
    window_coords = self.downloads_canvas.coords(self.downloads_canvas.find_all()[0] if self.downloads_canvas.find_all() else None)
    print(f"Debug: Coordonnées de la fenêtre dans le canvas: {window_coords}")
    
    print("Debug: Fin de _show_no_results_message")
