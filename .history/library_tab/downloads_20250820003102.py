...
def _display_filtered_downloads(self, files_to_display_, preserve_scroll=False):
    ...
    if getattr(self, '_display_in_progress', False):
        print("⚠️ _display_filtered_downloads déjà en cours, éviter la récursion")
        return
    ...
    self._start_thumbnail_loading(initial_files, self.downloads_container)
        ...
        # Libérer le verrou de récursion
...
# Ancienne fonction _refresh_downloads_library supprimée - remplacée par la version indépendante
...
def _refresh_downloads_library(self, preserve_scroll=False):
    ...
    try:
        ...
        # Recharger seulement les fichiers sans refaire tout l'affichage
        if hasattr(self, 'downloads_manager'):
            self.all_downloaded_files = self.downloads_manager.get_all_files()
            self.num_downloaded_files = len(self.all_downloaded_files)
            
            # Mettre à jour l'affichage si on est dans l'onglet téléchargements
            if (hasattr(self, 'current_library_tab') and 
                self.current_library_tab == "téléchargées" and
                hasattr(self, 'downloads_container')):
                self._display_filtered_downloads(self.all_downloaded_files, preserve_scroll)
