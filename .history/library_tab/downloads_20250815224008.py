...
def _display_filtered_downloads(self, files_to_display, preserve_scroll=False):
    ...
    # Afficher un indicateur de chargement pour les grandes collections
    if total_files > 30:
        self._show_loading_indicator(total_files)
    
    # Choisir la stratégie d'affichage selon la taille de la collection (seuils optimisés)
    if total_files > 80:  # Seuil réduit pour activer les optimisations avancées plus tôt
        # Très grande collection : optimisations avancées
        self._display_downloads_large_collection(files_to_display)
    elif total_files > 15:  # Seuil réduit pour l'affichage par batch
