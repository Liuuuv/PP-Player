...
def _add_playlist_card(self, parent_frame, playlist_name, songs, column):
    ...
    try:
        ...
        title_label.pause_counter = DOWNLOADS_LABEL_ANIMATION_STARTUP  # Compteur pour la pause entre les cycles
        title_label.max_width = DOWNLOADS_ARTIST_MAX_WIDTH  # Largeur maximale du titre
        title_label.animation_active = False  # Animation en cours
        title_label.full_text = title_label.cget('text')  # Texte complet du titre
        title_label.pause_cycles = DOWNLOADS_LABEL_ANIMATION_PAUSE
    ...
    try:
        ...
        for ext in image_extensions:
            ...
            if os.path.exists(thumbnail_path):
                ...
                # Redimensionner directement à une taille carrée (resize au lieu de crop)
                # Cela va déformer légèrement l'image mais garder tout le contenu
                # img_resized = image.resize((65, 65), Image.Resampling.LANCZOS)
                
                
                w, h = image.size

                # Déterminer la taille du carré à partir du côté le plus petit
                min_side = min(w, h)
                ...
                # Croper puis redimensionner
                image_cropped = image.crop((left, top, right, bottom))
                img_resized = image_cropped.resize((65, 65), Image.Resampling.LANCZOS)
...
def save_playlists(self):
    """Sauvegarde les playlists dans un fichier JSON avec des chemins relatifs"""
    try:
        import json
        ...
        # Convertir les chemins absolus en chemins relatifs pour la sauvegarde
        playlists_to_save = {}
        downloads_folder_normalized = os.path.normpath(self.downloads_folder)
        
        for name, songs in self.playlists.items():
            if name != "Main Playlist":
                relative_songs = []
                for song_path in songs:
                    song_path_normalized = os.path.normpath(song_path)
                    
                    # Si le fichier est dans le dossier downloads, stocker le chemin relatif
                    if song_path_normalized.startswith(downloads_folder_normalized):
                        relative_path = os.path.relpath(song_path_normalized, downloads_folder_normalized)
                        relative_songs.append(relative_path)
                    else:
