...
def _display_playlist_content(self, videos, playlist_title, target_tab="sorties"):
    ...
    if target_tab == "playlists":
        ...
        self.current_playlist_content_tab = "Playlists"
    else:
        self.playlist_content_active_sorties = True
        self.current_playlist_content_tab = "Sorties"
    ...
    # Déterminer quel bouton utiliser selon l'onglet
    button_attr = 'playlist_back_btn_playlists' if target_tab == "playlists" else 'playlist_back_btn_sorties'
    
    # Créer le bouton retour seulement si nécessaire
    if hasattr(self, button_attr) and getattr(self, button_attr):
        try:
            getattr(self, button_attr).destroy()
        except:
            pass
    ...
    # Assigner le bouton à l'attribut approprié
    setattr(self, button_attr, button)
    
    # Mettre à jour la variable de compatibilité
    self.playlist_back_btn = button
    ...
    # Mettre à jour la visibilité des boutons retour selon l'état de chaque onglet
...
def _show_playlist_content(self, playlist_data, target_tab="sorties"):
    ...
    def load_playlist_content():
        try:
            ...
            if not playlist_url:
                self.root.after(0, lambda: _show_playlist_error("URL de playlist non trouvée"))
                return
            ...
            with YoutubeDL(playlist_opts) as ydl:
                ...
                if playlist_info and 'entries' in playlist_info:
                    ...
                    self.root.after(0, lambda: _show_playlist_error("Impossible de charger le contenu"))
