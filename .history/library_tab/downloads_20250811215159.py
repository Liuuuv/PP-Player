...
def show_downloads_content(self):
    ...
    self.downloads_scrollbar = ttk.Scrollbar(
        self.library_content_frame,
        orient="vertical"
        # La commande sera configurée selon le mode (classique ou virtuel)
    )
    # La configuration yscrollcommand sera faite selon le mode
    ...
    # Variables pour la virtualisation
    self.virtual_item_height = 60  # Hauteur fixe par élément
    self.virtual_visible_items = 0  # Nombre d'éléments visibles
    self.virtual_start_index = 0   # Premier élément visible
    self.virtual_end_index = 0     # Dernier élément visible
    self.virtual_total_height = 0  # Hauteur totale virtuelle
    self.virtual_widgets = []      # Pool de widgets réutilisables
    self.virtual_max_widgets = 25  # Nombre maximum de widgets à créer
    
    # Les bindings seront configurés selon le mode utilisé
    
    # Initialiser la liste de tous les fichiers téléchargés
    self.all_downloaded_files = []
    self.filtered_files = []  # Liste filtrée pour la virtualisation
...
def _display_filtered_downloads(self, files_to_display):
    """Affiche une liste filtrée de fichiers téléchargés (version virtualisée)"""
    # Stocker la liste filtrée pour la virtualisation
    self.filtered_files = files_to_display.copy()
    
    # Décider si utiliser la virtualisation ou l'affichage classique
    if len(files_to_display) > VIRTUALISATION_THRESHOLD:  # Seuil pour activer la virtualisation
        self._init_virtual_list()
    else:
        # Pour les petites listes, utiliser l'ancien système optimisé
        self._display_classic_list(files_to_display)

def _display_classic_list(self, files_to_display):
    """Affichage classique pour les petites listes"""
    ...
    # Configurer le mode classique AVANT d'afficher les éléments
    self._setup_classic_mode()
    
    # Afficher directement avec chargement différé des miniatures
    ...
    # Forcer la mise à jour de la scrollregion après l'affichage
...
def _setup_classic_mode(self):
    """Configure le canvas et la scrollbar pour le mode classique"""
    # Nettoyer les bindings de virtualisation
    self.downloads_canvas.unbind("<Configure>")
    self.downloads_canvas.unbind("<MouseWheel>")
    self.downloads_canvas.unbind("<Button-4>")
    self.downloads_canvas.unbind("<Button-5>")
    
    # Configurer la scrollbar pour le mode classique
    self.downloads_canvas.configure(yscrollcommand=self.downloads_scrollbar.set)
    self.downloads_scrollbar.configure(command=self.downloads_canvas.yview)
    
    # Binding pour mettre à jour la scrollregion
    ...
    # Restaurer les bindings de la molette pour le mode classique
...
def _init_virtual_list(self):
    """Initialise la liste virtualisée"""
    # Vider le container actuel
    for widget in self.downloads_container.winfo_children():
        widget.destroy()
    
    # Configurer le mode virtuel
    self._setup_virtual_mode()
    
    # Calculer les dimensions
    self.virtual_total_height = len(self.filtered_files) * self.virtual_item_height
    
    # Calculer le nombre d'éléments visibles
    canvas_height = self.downloads_canvas.winfo_height()
    if canvas_height > 1:  # S'assurer que le canvas est initialisé
        self.virtual_visible_items = min(
            (canvas_height // self.virtual_item_height) + 2,  # +2 pour le buffer
            len(self.filtered_files)
        )
    else:
        self.virtual_visible_items = min(10, len(self.filtered_files))  # Valeur par défaut
    
    # Créer le pool de widgets
    self._create_virtual_widgets()
...
def _setup_virtual_mode(self):
    """Configure le canvas et la scrollbar pour le mode virtuel"""
    # Nettoyer les bindings classiques
    self.downloads_container.unbind("<Configure>")
    
    # Configurer la scrollbar pour le mode virtuel
    self.downloads_canvas.configure(yscrollcommand=self._update_scrollbar)
    self.downloads_scrollbar.configure(command=self._on_virtual_scroll)
    
    # Bindings pour la virtualisation
    self.downloads_canvas.bind("<Configure>", self._on_canvas_configure)
    self.downloads_canvas.bind("<MouseWheel>", self._on_mouse_wheel)
    self.downloads_canvas.bind("<Button-4>", self._on_mouse_wheel)
    self.downloads_canvas.bind("<Button-5>", self._on_mouse_wheel)
...
def _add_download_item(self, filepath):
    ...
    try:
        ...
        title_label = tk.Label(
            ...
            anchor='nw',
            justify='left',
            wraplength=0  # Pas de limite de largeur pour prendre toute la place
        )
        title_label.grid(row=0, column=2, sticky='nsew', padx=(0, 2), pady=4)
...
def _load_next_thumbnail(self):
    ...
    for widget in self.downloads_container.winfo_children():
        if hasattr(widget, 'filepath') and widget.filepath == filepath:
            # Trouver le label de miniature et de durée
            for child in widget.winfo_children():
                if isinstance(child, tk.Label) and hasattr(child, 'filepath'):
                    if child.filepath == filepath:
                        if child.cget('text') == "⏵":  # C'est le label de miniature
                            self._load_cached_thumbnail(filepath, child)
                        elif child.cget('text') == "--:--":  # C'est le label de durée
                            duration = self._get_cached_duration(filepath)
                            child.config(text=duration)
            break
...
def _create_virtual_widgets(self):
    """Crée le pool de widgets réutilisables pour la virtualisation"""
    self.virtual_widgets = []
    
    for i in range(min(self.virtual_max_widgets, len(self.filtered_files))):
        widget = self._create_virtual_widget_template()
        widget.pack_forget()  # Cacher initialement
        self.virtual_widgets.append(widget)

def _create_virtual_widget_template(self):
    """Crée un widget template pour la virtualisation"""
    # Frame principal
    item_frame = tk.Frame(
        self.downloads_container,
        bg=COLOR_BACKGROUND,
        ...
        highlightbackground=COLOR_BACKGROUND_HIGHLIGHT,
        highlightthickness=1,
        height=self.virtual_item_height
    )
    ...
    # Trait queue (initialement caché)
    queue_indicator = tk.Frame(item_frame, bg='black', width=3)
    queue_indicator.grid(row=0, column=0, sticky='ns', padx=(2, 2), pady=2)
    queue_indicator.grid_remove()  # Caché par défaut
    ...
    # Stocker les références aux sous-widgets
...
def _update_virtual_display(self):
    """Met à jour l'affichage virtuel selon la position de scroll"""
    if not self.filtered_files:
        return
    
    # Calculer les indices des éléments à afficher
    self.virtual_end_index = min(
        self.virtual_start_index + len(self.virtual_widgets),
        len(self.filtered_files)
    )
    
    # Mettre à jour chaque widget visible
    for i, widget in enumerate(self.virtual_widgets):
        file_index = self.virtual_start_index + i
        
        if file_index < len(self.filtered_files):
            # Afficher et configurer le widget
            filepath = self.filtered_files[file_index]
            self._configure_virtual_widget(widget, filepath, file_index)
            widget.pack(fill="x", padx=CARD_FRAME_PADX, pady=CARD_FRAME_PADY)
        else:
            # Cacher le widget s'il n'y a pas de données
            widget.pack_forget()

def _configure_virtual_widget(self, widget, filepath, file_index):
    """Configure un widget virtuel avec les données d'un fichier"""
    filename = os.path.basename(filepath)
    
    # Vérifier si c'est la chanson en cours
    is_current_song = (len(self.main_playlist) > 0 and 
                        self.current_index < len(self.main_playlist) and 
                        self.main_playlist[self.current_index] == filepath)
    
    # Vérifier si c'est dans la queue
    is_in_queue = False
    if hasattr(self, 'queue_items') and hasattr(self, 'main_playlist'):
        for i, main_filepath in enumerate(self.main_playlist):
            if main_filepath == filepath and i in self.queue_items:
    ...
    # Configurer le trait de queue
    if is_in_queue:
        widget.queue_indicator.grid()
    else:
        widget.queue_indicator.grid_remove()
    
    # Configurer la miniature
    widget.thumbnail_label.configure(bg=bg_color, text="⏵")
    widget.thumbnail_label.filepath = filepath
    
    # Configurer le titre
    truncated_title = self._truncate_text_for_display(filename, max_width_pixels=310, font_family='TkDefaultFont', font_size=9)
    widget.title_label.configure(text=truncated_title, bg=bg_color)
    
    # Configurer la durée
    widget.duration_label.configure(text="--:--", bg=bg_color)
    widget.duration_label.filepath = filepath
    
    # Configurer le bouton de suppression
    widget.delete_btn.configure(bg=bg_color)
    
    # Stocker le filepath dans le widget principal
    widget.filepath = filepath
    
    # Charger la miniature et la durée de manière asynchrone
    self.root.after_idle(lambda: self._load_virtual_item_data(widget, filepath))
    
    # Configurer les événements
    self._bind_virtual_widget_events(widget, filepath)

def _load_virtual_item_data(self, widget, filepath):
    """Charge les données (miniature et durée) pour un widget virtuel"""
    try:
        # Charger la miniature depuis le cache
        if hasattr(widget, 'thumbnail_label'):
            self._load_cached_thumbnail(filepath, widget.thumbnail_label)
        
        # Charger la durée depuis le cache
        if hasattr(widget, 'duration_label'):
            duration = self._get_cached_duration(filepath)
            widget.duration_label.configure(text=duration)
    except Exception as e:
        print(f"Erreur chargement données virtuelles: {e}")

def _bind_virtual_widget_events(self, widget, filepath):
    """Configure les événements pour un widget virtuel"""
    def on_double_click(event):
        if not (event.state & 0x1 or self.selected_items):  # Pas de Shift ni sélection
            self._add_download_to_playlist(filepath)
            if filepath in self.main_playlist:
                self.current_index = self.main_playlist.index(filepath)
                self.play_track()
    ...
    def on_delete_double_click(event):
        if event.state & 0x4:  # Ctrl enfoncé
            self._delete_from_downloads(filepath, widget)
        else:
    ...
    widget.title_label.bind("<Double-1>", on_double_click)
    widget.delete_btn.bind("<Double-1>", on_delete_double_click)
    
    # Drag and drop
    self.drag_drop_handler.setup_drag_drop(widget, file_path=filepath, item_type="file")

def _on_canvas_configure(self, event):
    """Appelé quand le canvas change de taille"""
    if hasattr(self, 'filtered_files') and len(self.filtered_files) > VIRTUALISATION_THRESHOLD:
        # Recalculer le nombre d'éléments visibles
        canvas_height = event.height
        old_visible = self.virtual_visible_items
        self.virtual_visible_items = min(
            (canvas_height // self.virtual_item_height) + 2,
            len(self.filtered_files)
        )
        
        # Si le nombre d'éléments visibles a changé, recréer les widgets
        if self.virtual_visible_items != old_visible:
            self._create_virtual_widgets()
            self._update_virtual_display()

def _on_virtual_scroll(self, *args):
    """Gère le scroll de la liste virtualisée"""
    if not hasattr(self, 'filtered_files') or len(self.filtered_files) <= VIRTUALISATION_THRESHOLD:
        return
    
    if args[0] == "scroll":
        # Scroll par unité
        delta = int(args[1])
        self.virtual_start_index = max(0, min(
            self.virtual_start_index + delta,
            len(self.filtered_files) - len(self.virtual_widgets)
        ))
    elif args[0] == "moveto":
        # Scroll vers une position absolue
        fraction = float(args[1])
        max_start = max(0, len(self.filtered_files) - len(self.virtual_widgets))
        self.virtual_start_index = int(fraction * max_start)
    
    self._update_virtual_display()
    self._update_scrollbar()

def _on_mouse_wheel(self, event):
    """Gère le scroll de la molette pour la liste virtualisée"""
    if not hasattr(self, 'filtered_files') or len(self.filtered_files) <= VIRTUALISATION_THRESHOLD:
        return
    
    # Calculer le delta selon le système
    if event.num == 4 or event.delta > 0:
        delta = -3  # Scroll vers le haut
    elif event.num == 5 or event.delta < 0:
        delta = 3   # Scroll vers le bas
    else:
        return
    
    # Appliquer le scroll
    old_start = self.virtual_start_index
    self.virtual_start_index = max(0, min(
        self.virtual_start_index + delta,
        len(self.filtered_files) - len(self.virtual_widgets)
    ))
    
    if self.virtual_start_index != old_start:
        self._update_virtual_display()
        self._update_scrollbar()
...
def _update_scrollbar(self, *args):
    ...
    if len(self.filtered_files) == 0:
        self.downloads_scrollbar.set(0, 1)
        return
    
    # Calculer la position relative
    total_items = len(self.filtered_files)
    visible_items = len(self.virtual_widgets)
    
    if total_items <= visible_items:
        self.downloads_scrollbar.set(0, 1)
    else:
        start_fraction = self.virtual_start_index / (total_items - visible_items)
        end_fraction = (self.virtual_start_index + visible_items) / total_items
        self.downloads_scrollbar.set(start_fraction, end_fraction)

def _update_scrollbar_range(self):
    """Configure la plage de la scrollbar"""
    if hasattr(self, 'filtered_files') and len(self.filtered_files) > VIRTUALISATION_THRESHOLD:
        self._update_scrollbar()
    else:
        self.downloads_scrollbar.set(0, 1)