...
def _display_filtered_downloads(self, files_to_display):
    ...
    print(f"DEBUG: _display_filtered_downloads appelé avec {len(files_to_display)} fichiers")
        ...
        if hasattr(self, 'downloads_container') and self.downloads_container.winfo_exists():
            for widget in self.downloads_container.winfo_children():
                try:
                    ...
                    continue
        print("DEBUG: Widgets détruits")
    except tk.TclError:
        # Container détruit, ignorer
        print("DEBUG: Container détruit, widgets ignorés")
        pass
...
def _restore_search_binding(self):
    ...
    if hasattr(self, 'safe_after'):
        self.safe_after(50, restore_delayed)
    else:
        self.root.after(50, restore_delayed)
...
def _update_downloads_queue_visual(self):
    ...
    try:
        ...
        for widget in self.downloads_container.winfo_children():
            if hasattr(widget, 'filepath'):  # C'est un frame d'élément de téléchargement
...
def _refresh_downloads_library(self):
    ...
    print("DEBUG: _refresh_downloads_library appelé - DÉBUT")
    import traceback
    print("DEBUG: Stack trace:")
    traceback.print_stack()
    print("DEBUG: _refresh_downloads_library - TRAITEMENT EN COURS")
    try:
    ...
    print("DEBUG: _refresh_downloads_library - FIN")
...
def _on_library_search_change(self, event):
    ...
    print("DEBUG: _on_library_search_change appelé - DÉSACTIVÉ TEMPORAIREMENT")
    
    # DÉSACTIVER TEMPORAIREMENT pour éviter la boucle infinie
    # Cette fonction sera réactivée une fois le problème résolu
    return
    ...
    if hasattr(self, '_refreshing_downloads') and self._refreshing_downloads:
        print("DEBUG: Refresh en cours, ignorer l'événement de recherche")
        return
    
    import traceback
    print("DEBUG: Stack trace de _on_library_search_change:")
    traceback.print_stack()
...
def _perform_library_search(self):
    ...
    try:
        if not (hasattr(self, 'library_search_entry') and self.library_search_entry.winfo_exists()):
            print("DEBUG: Widget de recherche détruit, ignorer")
            return
    except:
        print("DEBUG: Erreur vérification widget, ignorer")
        return
    
    import traceback
    print("DEBUG: Stack trace de _perform_library_search:")
    traceback.print_stack()
    search_term = self.library_search_entry.get().lower().strip()
    print(f"DEBUG: Terme de recherche: '{search_term}'")
...
def _clear_library_search(self):
    ...
    print("DEBUG: _clear_library_search appelé")
    import traceback
    print("DEBUG: Stack trace de _clear_library_search:")
    traceback.print_stack()
