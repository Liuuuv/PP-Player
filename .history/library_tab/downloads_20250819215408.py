...
def show_downloads_content(self):
    ...
    # Bind de la molette de souris (sera configuré par _setup_virtual_scroll)
    # Les bindings virtuels seront ajoutés lors de l'affichage des fichiers
...
def _display_filtered_downloads(self, files_to_display, preserve_scroll=False):
    """Affiche une liste filtrée de fichiers téléchargés avec chargement virtuel"""
    print(f"🔄 DEBUG: _display_filtered_downloads appelée avec {len(files_to_display)} fichiers")
    
    # Marquer qu'on est en train de faire un refresh pour éviter la boucle infinie
    self._refreshing_downloads = True
    
    # Initialiser les variables de chargement virtuel
    self._virtual_files_list = files_to_display.copy()  # Liste complète des fichiers
    self._virtual_start_index = 0  # Index de début de la fenêtre virtuelle
    self._virtual_loaded_widgets = {}  # Dictionnaire {index: widget} des widgets chargés
    
    print(f"📊 DEBUG: Configuration virtuelle - initial_load: {getattr(self, 'virtual_initial_load_count', 'NON_DÉFINI')}, scroll_load: {getattr(self, 'virtual_scroll_load_count', 'NON_DÉFINI')}")
    ...
    if not preserve_scroll and hasattr(self, 'downloads_canvas'):
        try:
            if self.downloads_canvas.winfo_exists():
                ...
                self._virtual_start_index = 0  # Reset de l'index virtuel
        except tk.TclError:
    ...
    # Configurer le scroll virtuel
    print(f"🔧 DEBUG: Configuration du scroll virtuel...")
    self._setup_virtual_scroll()
    
    # Charger la première fenêtre de fichiers
    print(f"📥 DEBUG: Chargement de la première fenêtre virtuelle...")
    self._load_virtual_window()
    
    # Marquer la fin du refresh
    self._refreshing_downloads = False
    print(f"✅ DEBUG: _display_filtered_downloads terminée")
...
# ==================== SYSTÈME DE CHARGEMENT VIRTUEL ====================

def _setup_virtual_scroll(self):
    """Configure le système de scroll virtuel"""
    print(f"🖱️ DEBUG: Configuration des bindings de scroll virtuel...")
    
    # Supprimer les anciens bindings de scroll s'ils existent
    if hasattr(self, 'downloads_canvas'):
        try:
            ...
            print(f"🗑️ DEBUG: Anciens bindings supprimés")
        except:
            pass
    
    # Ajouter les nouveaux bindings pour le scroll virtuel
    self._bind_virtual_mousewheel(self.downloads_canvas, self.downloads_canvas)
    self._bind_virtual_mousewheel(self.downloads_container, self.downloads_canvas)
    print(f"✅ DEBUG: Nouveaux bindings virtuels ajoutés")

def _bind_virtual_mousewheel(self, widget, canvas):
    """Bind la molette de souris avec gestion virtuelle"""
    def on_mousewheel(event):
        # Vérifier si on doit gérer le scroll virtuel
        if hasattr(self, '_virtual_files_list') and len(self._virtual_files_list) > self.virtual_initial_load_count:
            print(f"🔄 DEBUG: Scroll virtuel détecté - {len(self._virtual_files_list)} fichiers > {self.virtual_initial_load_count}")
            self._handle_virtual_scroll(event)
        else:
            print(f"📜 DEBUG: Scroll normal - {len(getattr(self, '_virtual_files_list', []))} fichiers <= {getattr(self, 'virtual_initial_load_count', 'NON_DÉFINI')}")
            # Scroll normal pour les petites listes
...
def _handle_virtual_scroll(self, event):
    """Gère le scroll avec chargement/déchargement virtuel"""
    if not hasattr(self, '_virtual_files_list') or not hasattr(self, '_virtual_start_index'):
        print(f"❌ DEBUG: Variables virtuelles manquantes")
        return
    ...
    if hasattr(event, 'delta'):
        ...
        scroll_units = scroll_direction
    
    print(f"🖱️ DEBUG: Scroll détecté - direction: {'HAUT' if scroll_direction < 0 else 'BAS'}")
    
    # D'abord, effectuer le scroll visuel normal
    self.downloads_canvas.yview_scroll(scroll_units, "units")
    
    # Ensuite, vérifier si on doit changer de fenêtre virtuelle
    ...
    try:
        ...
        # Calculer les seuils pour le changement de fenêtre
        total_files = len(self._virtual_files_list)
        max_start = max(0, total_files - self.virtual_initial_load_count)
        
        # Seuils pour déclencher le changement de fenêtre
        threshold_up = 0.1    # Si on scroll en haut et qu'on est dans les 10% du haut
        threshold_down = 0.9  # Si on scroll en bas et qu'on est dans les 90% du bas
        
        should_change_window = False
        new_start_index = self._virtual_start_index
        
        if scroll_direction < 0 and scroll_top < threshold_up:  # Scroll vers le haut
            if self._virtual_start_index > 0:
                new_start_index = max(0, self._virtual_start_index - self.virtual_scroll_load_count)
                should_change_window = True
                print(f"� DEBUG: Seuil haut atteint, changement de fenêtre")
        
        elif scroll_direction > 0 and scroll_bottom > threshold_down:  # Scroll vers le bas
            if self._virtual_start_index < max_start:
                new_start_index = min(max_start, self._virtual_start_index + self.virtual_scroll_load_count)
                should_change_window = True
                print(f"🔽 DEBUG: Seuil bas atteint, changement de fenêtre")
        
        if should_change_window and new_start_index != self._virtual_start_index:
            print(f"🔄 DEBUG: Changement de fenêtre - ancien: {self._virtual_start_index}, nouveau: {new_start_index}")
            self._virtual_start_index = new_start_index
            self._load_virtual_window()
        
    except Exception as e:
        print(f"❌ DEBUG: Erreur lors du calcul de scroll: {e}")
        # En cas d'erreur, faire juste le scroll normal
        pass

def _load_virtual_window(self):
    """Charge la fenêtre virtuelle actuelle"""
    if not hasattr(self, '_virtual_files_list'):
        print(f"❌ DEBUG: _virtual_files_list manquante")
        return
    
    # Calculer les indices de la fenêtre
    start_idx = self._virtual_start_index
    end_idx = min(start_idx + self.virtual_initial_load_count, len(self._virtual_files_list))
    
    print(f"📥 DEBUG: Chargement fenêtre virtuelle - indices {start_idx} à {end_idx} ({end_idx - start_idx} fichiers)")
    
    # Vider le container
    try:
        if hasattr(self, 'downloads_container') and self.downloads_container.winfo_exists():
            widgets_count = len(self.downloads_container.winfo_children())
            print(f"🗑️ DEBUG: Suppression de {widgets_count} widgets existants")
            for widget in self.downloads_container.winfo_children():
                try:
                    if widget.winfo_exists():
                        widget.destroy()
    ...
    # Réinitialiser le dictionnaire des widgets chargés
    self._virtual_loaded_widgets = {}
    
    # Charger les fichiers de la fenêtre actuelle
    files_to_load = self._virtual_files_list[start_idx:end_idx]
    
    print(f"🎵 DEBUG: Chargement de {len(files_to_load)} fichiers...")
    
    # Afficher les fichiers avec chargement rapide
    for i, filepath in enumerate(files_to_load):
        widget = self._add_download_item_fast(filepath)
        self._virtual_loaded_widgets[start_idx + i] = widget
    
    print(f"✅ DEBUG: {len(self._virtual_loaded_widgets)} widgets chargés")
    
    # Forcer la mise à jour de la scrollbar
    self._update_virtual_scrollbar()
    
    # Lancer le chargement différé des miniatures
    self._start_thumbnail_loading(files_to_load, self.downloads_container)

def _update_virtual_scrollbar(self):
    """Met à jour la scrollbar pour le chargement virtuel"""
    try:
        if hasattr(self, 'downloads_container') and hasattr(self, 'downloads_canvas'):
            if self.downloads_container.winfo_exists() and self.downloads_canvas.winfo_exists():
                # Forcer la mise à jour du layout
                ...
                # Calculer la hauteur totale virtuelle
                if hasattr(self, '_virtual_files_list'):
                    total_files = len(self._virtual_files_list)
                    loaded_files = len(self._virtual_loaded_widgets)
                    
                    print(f"📏 DEBUG: Mise à jour scrollbar - total: {total_files}, chargés: {loaded_files}")
                    
                    if loaded_files > 0:
                        # Estimer la hauteur d'un élément
                        bbox = self.downloads_canvas.bbox("all")
                        if bbox:
                            loaded_height = bbox[3] - bbox[1]
                            estimated_item_height = loaded_height / loaded_files
                            total_estimated_height = total_files * estimated_item_height
                            
                            print(f"📐 DEBUG: Hauteur estimée - item: {estimated_item_height:.1f}px, total: {total_estimated_height:.1f}px")
                            
                            # Mettre à jour la région de scroll avec la hauteur estimée
                            self.downloads_canvas.configure(scrollregion=(0, 0, 0, total_estimated_height))
                            
                            # NE PAS ajuster automatiquement la position du scroll
                            # Laisser l'utilisateur contrôler le scroll manuellement
                            print(f"✅ DEBUG: Scrollregion mise à jour")
                        else:
                            print(f"⚠️ DEBUG: Pas de bbox disponible")
                            # Fallback normal
                            self.downloads_canvas.configure(scrollregion=self.downloads_canvas.bbox("all"))
                else:
                    print(f"⚠️ DEBUG: Pas de _virtual_files_list")
                    # Fallback normal
                    self.downloads_canvas.configure(scrollregion=self.downloads_canvas.bbox("all"))
    except tk.TclError as e:
        print(f"❌ DEBUG: Erreur scrollbar: {e}")
        pass