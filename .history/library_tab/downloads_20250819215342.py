...
def _handle_virtual_scroll(self, event):
    ...
    # Calculer le nouvel index de début
    old_start_index = self._virtual_start_index
    scroll_amount = self.virtual_scroll_load_count
    max_start = max(0, len(self._virtual_files_list) - self.virtual_initial_load_count)
    
    if scroll_direction < 0:  # Scroll vers le haut
        new_start_index = max(0, self._virtual_start_index - scroll_amount)
    else:  # Scroll vers le bas
        new_start_index = min(max_start, self._virtual_start_index + scroll_amount)
    
    print(f"📊 DEBUG: Index - ancien: {old_start_index}, nouveau: {new_start_index}, max_start: {max_start}")
    
    # Si l'index a changé, recharger la fenêtre
    if new_start_index != old_start_index:
        print(f"🔄 DEBUG: Rechargement de la fenêtre virtuelle...")
        self._virtual_start_index = new_start_index
        self._load_virtual_window()
    else:
        print(f"📜 DEBUG: Scroll normal dans la fenêtre actuelle")
        # Scroll normal dans la fenêtre actuelle
        self.downloads_canvas.yview_scroll(scroll_direction, "units")
