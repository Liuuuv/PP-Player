...
def show_downloads_content(self):
    ...
    self._bind_mousewheel(self.downloads_canvas, self.downloads_canvas, func=tools.on_mousewheel_lazy)
    # self._bind_mousewheel(self.downloads_container, self.downloads_canvas, func=tools.on_mousewheel_lazy)
...
def load_downloaded_files(self):
    ...
    print("ATTENTION ON A ROGNÉ LE NOMBRE DE FICHIERS TÉLÉCHARGEMENTS PIPI")
    ...
    for filename in os.listdir(downloads_dir):
        if filename.lower().endswith(audio_extensions):
            ...
            if len(self.all_downloaded_files) > 30:
                break
...
def _display_files_batch_optimized(self, files_to_display, start_index, total_files, batch_size=50):
    ...
    self._load_more_songs_below(unload=False, canvas=self.downloads_canvas)
...
def _add_downloads_playlist_item_at_position(self, filepath, song_index=None, position='bottom'):
    """Ajoute un élément de playlist à une position spécifique (top ou bottom)"""
    try:
        if position == 'bottom':
            # Pour le bas, utiliser la fonction normale
            return self._add_song_item(filepath, self.downloads_container,song_index=song_index)
        
        elif position == 'top':
            # Pour le haut, simplement créer l'élément
            # L'ordre sera corrigé par _reorder_playlist_items() après
            item_frame = self._add_song_item(filepath, self.downloads_container,song_index=song_index)
