...
def _play_playlist_from_song(self, playlist_name, song_index):
    ...
    print(f"🎵 DEBUG PLAYLIST: _play_playlist_from_song appelé - playlist: {playlist_name}, index: {song_index}")
    
    if playlist_name not in self.playlists:
        print(f"❌ DEBUG PLAYLIST: Playlist {playlist_name} non trouvée")
        return
    
    # Copier la playlist dans la main playlist
    print(f"📋 DEBUG PLAYLIST: Copie de {len(self.playlists[playlist_name])} chansons dans main_playlist")
    ...
    print(f"🎯 DEBUG PLAYLIST: current_index défini à {self.current_index}")
    
    # Lancer la lecture
    print(f"▶️ DEBUG PLAYLIST: Appel de play_track()")
    try:
        self.play_track()
        print(f"✅ DEBUG PLAYLIST: play_track() terminé avec succès")
    except Exception as e:
        print(f"❌ DEBUG PLAYLIST: Erreur lors de la lecture: {e}")
        import traceback
        traceback.print_exc()
        return
    
    # Rafraîchir l'affichage de la playlist principale (avec un délai pour éviter les conflits)
    print(f"🔄 DEBUG PLAYLIST: Programmation du rafraîchissement de l'affichage")
    try:
        self.safe_after(100, self._refresh_playlist_display)
        print(f"✅ DEBUG PLAYLIST: safe_after programmé")
    except:
        # Si safe_after n'est pas disponible, utiliser after normal
        print(f"⚠️ DEBUG PLAYLIST: safe_after échoué, utilisation de after normal")
