...
def _show_playlist_content_window(self, playlist_name):
    ...
    for i, filepath in enumerate(songs):
        self._add_playlist_window_item(container, filepath, playlist_name, i)
...
def _add_playlist_window_item(self, container, filepath, playlist_name, song_index):
    """Ajoute un élément de playlist avec le même style que les téléchargements"""
    try:
        ...
        # Configuration de la grille en 6 colonnes : numéro, miniature, texte, durée, bouton jouer, bouton supprimer
        ...
        item_frame.columnconfigure(4, minsize=80, weight=0)  # Bouton jouer
        item_frame.columnconfigure(5, minsize=40, weight=0)  # Bouton supprimer
        ...
        # 3. Texte (colonne 2)
        truncated_title = self._truncate_text_for_display(filename, max_width_pixels=300, font_family='TkDefaultFont', font_size=9)
        ...
        # 5. Bouton "Jouer depuis la playlist" (colonne 4)
        play_btn = tk.Button(
            item_frame,
        ...
        play_btn.grid(row=0, column=4, sticky='ns', padx=(0, 5), pady=8)
        play_btn.config(command=lambda f=filepath, pn=playlist_name: self._play_from_playlist(f, pn))
        create_tooltip(play_btn, "Jouer cette chanson\nAjoute la chanson à la liste de lecture et la joue")
        
        # 6. Bouton de suppression (colonne 5)
        delete_btn = tk.Button(
            item_frame,
            image=self.icons['delete'],
            bg=bg_color,
        ...
        def on_delete_double_click(event):
            ...
            if event.state & 0x4:  # Ctrl est enfoncé
                ...
                self._remove_from_playlist(filepath, playlist_name, item_frame, event)
        ...
        def on_item_double_click(event):
            # Double-clic pour jouer la musique
            self._play_from_playlist(filepath, playlist_name)
        
        # Lier les événements aux éléments cliquables
        for widget in [item_frame, thumbnail_label, title_label, duration_label, number_label]:
            widget.bind("<Double-1>", on_item_double_click)
        
    except Exception as e:
        print(f"Erreur lors de l'ajout de l'élément playlist: {e}")