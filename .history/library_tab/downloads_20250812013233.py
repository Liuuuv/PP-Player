...
def _show_no_results_message(self):
    ...
    # Garder le scroll actif - créer un frame qui prend suffisamment d'espace
    # pour que la scrollbar soit pleine (même couleur de fond que le canvas)
    no_results_frame = tk.Frame(
        self.downloads_container,
        bg='#3d3d3d',  # Même couleur que le canvas
        height=800  # Hauteur importante pour que la scrollbar soit pleine
    )
    no_results_frame.pack(fill="both", expand=True)
    no_results_frame.pack_propagate(False)  # Empêcher le frame de se redimensionner
    
    # Créer un frame central pour le contenu
    center_frame = tk.Frame(no_results_frame, bg='#3d3d3d')
    center_frame.place(relx=0.5, rely=0.5, anchor="center")
    
    # Debug : vérifier si l'icône existe
    if hasattr(self, 'icons') and 'none' in self.icons:
        print("Debug: L'icône 'none' existe et va être affichée")
        # Afficher l'image none.png avec le bon fond
        icon_label = tk.Label(
            center_frame,
        ...
        print("Debug: Image ajoutée au center_frame")
    else:
        print("Debug: L'icône 'none' n'existe pas")
        # Fallback avec emoji
        ...
        placeholder_label.pack(pady=(0, 30))
    
    # Afficher le texte "Aucun résultat"
    text_label = tk.Label(
        center_frame,
        text="Aucun résultat",
        bg='#3d3d3d',  # Même couleur que le canvas
