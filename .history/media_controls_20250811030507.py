        ...
        try:
            if self.loop and not self.loop.is_closed():
                asyncio.run_coroutine_threadsafe(
                    self._async_update_media_info(title, artist, album), 
                    self.loop
                )
        except Exception as e:
            print(f"Erreur mise à jour info média: {e}")
    
    async def _async_update_media_info(self, title, artist, album):
        """Met à jour les informations du média de manière asynchrone"""
        try:
            updater = self.smtc.display_updater
            
            # Nettoyer d'abord l'updater
            updater.clear_all()
            
            # Définir le type de média
            updater.type = 0  # Music
            
            # Définir seulement le titre pour éviter les erreurs de paramètres
            if title and len(str(title).strip()) > 0:
                try:
                    updater.music_properties.title = str(title).strip()
                except:
                    # Si ça échoue, on essaie avec un titre par défaut
                    updater.music_properties.title = "Lecture en cours"
            ...
            # En cas d'erreur, on essaie une mise à jour minimale
            try:
                updater = self.smtc.display_updater
                updater.clear_all()
                updater.type = 0
