...
class MusicPlayer:
    ...
    def _smart_scroll_to_single_result(self):
        """Ajuste intelligemment le scroll pour afficher un résultat unique de recherche"""
        if not hasattr(self, 'downloads_canvas') or not hasattr(self, 'search_result_frame'):
            return
            
        if not self.search_result_frame or not self.search_result_frame.winfo_exists():
            return
        ...
        try:
            ...
            result_y = self.search_result_frame.winfo_y()
            result_height = self.search_result_frame.winfo_height()
            
            if container_height <= canvas_height:
                # Tout tient dans le canvas, pas besoin de scroll
                return
            
            # Calculer la position cible pour centrer le résultat
            # On veut que le résultat soit visible, idéalement au centre
            target_center = result_y + (result_height / 2)
            canvas_center = canvas_height / 2
            
            # Position de scroll pour centrer le résultat
            target_scroll_top = max(0, (target_center - canvas_center) / container_height)
            
            # Si le scroll précédent était très bas, essayer de rester proche
            if self.last_downloads_scroll_position > 0.7:
                # L'utilisateur était en bas, essayer de garder une position basse si possible
                max_scroll = 1.0 - (canvas_height / container_height)
                available_bottom_space = max_scroll - target_scroll_top
                
                if available_bottom_space > 0.1:  # Il y a de la place en bas
                    # Ajuster vers le bas mais sans dépasser
                    adjustment = min(0.2, available_bottom_space)
                    target_scroll_top += adjustment
            
            # S'assurer que la position est dans les limites valides
            target_scroll_top = max(0.0, min(1.0, target_scroll_top))
            
            # Appliquer le scroll avec une petite animation
            self._animate_downloads_scroll(target_scroll_top)
            
        except Exception as e:
            print(f"Erreur ajustement scroll intelligent: {e}")
