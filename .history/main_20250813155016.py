...
class MusicPlayer:
    def __init__(self, root):
        ...
        self.playlist_back_btn = None  # Bouton retour pour les playlists
    ...
    def on_artist_tab_changed(self, event):
        ...
        try:
            ...
            # Si du contenu de playlist était affiché et qu'on change d'onglet, masquer le bouton retour
            if self.playlist_content_active:
                current_tab_id = self.artist_notebook.select()
                if current_tab_id:
                    current_tab_text = self.artist_notebook.tab(current_tab_id, "text")
                    
                    # Si on n'est plus dans l'onglet où le contenu était affiché, masquer le bouton
                    if current_tab_text != self.current_playlist_content_tab:
                        self._hide_playlist_back_button()
                    else:
                        # On est revenu dans l'onglet avec le contenu, réafficher le bouton
    ...
    def _show_artist_content(self, artist_name, video_data):
        ...
        return artist_tab.core._display_artist_interface(self, artist_name, video_data)
    
    def _save_current_search_state(self):
        """Sauvegarde l'état actuel des résultats de recherche"""
        return artist_tab.core._save_current_search_state(self)
    ...
    def _add_artist_result(self, video, index, container):
        ...
        return artist_tab.songs._add_artist_result(self, video, index, container)
    ...
    def _hide_search_elements(self):
        ...
        return artist_tab.core._hide_search_elements(self)
    
    def _return_to_search(self):
        """Retourne instantanément à l'affichage de recherche normal"""
        return artist_tab.core._return_to_search(self)
