...
class MusicPlayer:
    ...
    def _add_playlist_item(self, filepath, thumbnail_path=None):
        ...
        try:
            ...
            # Variable pour s'assurer qu'on ne tronque qu'une seule fois
            text_truncated = False
            
            # Fonction pour mettre à jour le texte tronqué (une seule fois)
            def update_truncated_text():
                nonlocal text_truncated
                if text_truncated:
                    return  # Déjà tronqué, ne pas refaire
                
                text_frame.update_idletasks()
                available_width = text_frame.winfo_width() - 2
                ...
                if available_width > 30:
                    ...
                    text_truncated = True  # Marquer comme fait
                else:
                    # Si pas encore prêt, réessayer une fois
                    self.root.after(100, update_truncated_text)
            
            # Une seule mise à jour après construction
    ...
    def select_playlist_item(self, item_frame=None, index=None):
        ...
        for child in self.playlist_container.winfo_children():
            if hasattr(child, 'selected'):
                ...
                self._set_item_colors(child, '#3d3d3d')  # Couleur normale
        ...
        if item_frame:
            ...
            self._set_item_colors(item_frame, '#5a9fd8')  # Couleur de surbrillance (bleu plus visible)
    ...
    def _set_item_colors(self, item_frame, bg_color):
        """Change récursivement la couleur de fond de tous les éléments d'un item de playlist"""
        is_selected = bg_color != '#3d3d3d'  # Détermine si c'est la sélection ou la désélection
        
        def set_colors_recursive(widget, color):
            # Changer la couleur du widget actuel s'il le supporte
            if hasattr(widget, 'config'):
                try:
                    widget.config(bg=color)
                    # Si c'est un Label avec du texte, changer aussi la couleur du texte
                    if isinstance(widget, tk.Label) and hasattr(widget, 'cget'):
                        if is_selected:
                            widget.config(fg='white', font=('TkDefaultFont', 9, 'bold'))  # Texte en gras quand sélectionné
                        else:
                            widget.config(fg='white', font=('TkDefaultFont', 9, 'normal'))  # Texte normal
                except:
                    pass  # Certains widgets ne supportent pas bg ou fg
