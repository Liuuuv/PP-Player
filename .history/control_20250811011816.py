...
def on_progress_press(self, event):
    ...
    # Show waveform during drag: always if button is activated, or temporarily if button is deactivated
    if not self.waveform_canvas.winfo_height():
        self.waveform_canvas.config(height=80)
        self.waveform_canvas.pack(fill=tk.X, pady=(0, 10))
        ...
        # If waveform is already visible, just update it
        pos = self.progress.get()
        self.draw_waveform_around(pos)
...
def on_progress_drag(self, event):
    ...
    if self.user_dragging:
        ...
        # Always draw waveform during drag (it's shown temporarily if button is deactivated)
        self.draw_waveform_around(pos)
...
def on_progress_release(self, event):
    ...
    try:
        ...
        # Hide waveform if the show_waveform button is deactivated
        if not self.show_waveform_current:
            self.waveform_canvas.config(height=0)
            self.waveform_canvas.pack(fill=tk.X, pady=0)
        else:
            # Keep waveform visible and update it if button is activated
            self.draw_waveform_around(self.current_time)
