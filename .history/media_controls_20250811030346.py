# Module pour les contrôles multimédia Windows
import asyncio
import threading
import pygame
...
class WindowsMediaControls:
    ...
    def _handle_play(self):
        """Gère le bouton Play"""
        try:
            # Vérifier que pygame est initialisé
    ...
    def update_playback_status(self, status):
        ...
        if not self.is_initialized or not self.smtc:
            return
        ...
        try:
            if self.loop and not self.loop.is_closed():
                asyncio.run_coroutine_threadsafe(
                    self._async_update_media_info(title, artist, album, thumbnail_path), 
                    self.loop
                )
        except Exception as e:
            print(f"Erreur mise à jour info média: {e}")
    
    async def _async_update_media_info(self, title, artist, album, thumbnail_path):
        """Met à jour les informations du média de manière asynchrone"""
        try:
            updater = self.smtc.display_updater
            
            # Définir les informations de base
            updater.type = 0  # Music
            updater.music_properties.title = title or "Titre inconnu"
            updater.music_properties.artist = artist or "Artiste inconnu"
            updater.music_properties.album_title = album or ""
            
            # Pour l'instant, on évite les miniatures qui causent des erreurs
            # TODO: Implémenter les miniatures plus tard
            
            # Appliquer les mises à jour
            await updater.update_async()
            
        except Exception as e:
            print(f"Erreur async update media info: {e}")
...
def initialize_media_controls(music_player):
    ...
    try:
        return WindowsMediaControls(music_player)
    except Exception as e:
        print(f"Erreur initialisation contrôles multimédia: {e}")
        return None