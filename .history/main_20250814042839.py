...
class MusicPlayer:
    ...
    def _enable_scroll_propagation(self, widget):
        ...
        def propagate_scroll_event(event):
            # Utiliser le système de scroll existant de l'application
            # Trouver l'onglet actuel et son canvas
            try:
                if hasattr(self, 'notebook') and self.notebook.select():
                    current_tab = self.notebook.tab(self.notebook.select(), "text")
                    
                    # Déterminer quel canvas utiliser selon l'onglet
                    canvas = None
                    if current_tab == "Playlist" and hasattr(self, 'playlist_canvas'):
                        canvas = self.playlist_canvas
                    elif current_tab == "Recherche" and hasattr(self, 'search_canvas'):
                        canvas = self.search_canvas
                    elif current_tab == "Téléchargements" and hasattr(self, 'downloads_canvas'):
                        canvas = self.downloads_canvas
                    elif current_tab == "Bibliothèque" and hasattr(self, 'library_canvas'):
                        canvas = self.library_canvas
                    
                    # Si un canvas est trouvé, lui envoyer l'événement de scroll
