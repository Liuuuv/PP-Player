...
def _add_to_specific_playlist(self, filepath, playlist_name):
    ...
    if playlist_name == "Main Playlist":
        # Pour la main playlist, utiliser la fonction sécurisée
        _safe_add_to_main_playlist(self, filepath)
    else:
        # Pour les autres playlists
        if filepath not in self.playlists[playlist_name]:
            self.playlists[playlist_name].append(filepath)
...
def _safe_add_to_main_playlist(self, filepath):
    ...
    try:
        if hasattr(self, 'root') and self.root.winfo_exists():
            # Vérifier si le fichier n'est pas déjà dans la playlist
            if filepath not in self.main_playlist:
                self.main_playlist.append(filepath)
                # Utiliser _add_song_item pour avoir les métadonnées et miniatures
                if hasattr(self, 'playlist_container') and self.playlist_container.winfo_exists():
                    self._add_song_item(filepath, self.playlist_container, 
                                      song_index=len(self.main_playlist)-1, 
                                      item_type="main_playlist")
                    self._update_downloads_queue_visual()
                if hasattr(self, 'status_bar'):
                    self.status_bar.config(text=f"Ajouté à la liste de lecture: {os.path.basename(filepath)}")
            else:
                if hasattr(self, 'status_bar'):
                    self.status_bar.config(text=f"Déjà dans la liste de lecture: {os.path.basename(filepath)}")
    except tk.TclError:
