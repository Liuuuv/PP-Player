...
class MusicPlayer:
    def __init__(self, root):
        ...
        # Désactiver la barre d'espace sur tous les boutons
        self.disable_space_on_buttons()
    ...
    def on_space_pressed(self, event):
        ...
        # Vérifier si le focus n'est pas sur un champ de saisie ou un bouton
        focused_widget = self.root.focus_get()
        if focused_widget and isinstance(focused_widget, (tk.Entry, tk.Text, tk.Button, ttk.Button)):
            # Si le focus est sur un champ de saisie ou un bouton, ne pas intercepter la barre d'espace
            # Pour les boutons, on empêche aussi leur activation
            if isinstance(focused_widget, (tk.Button, ttk.Button)):
                return "break"  # Empêcher l'activation du bouton
            return
    ...
    def disable_space_on_buttons(self):
        """Désactive la barre d'espace sur tous les boutons de l'interface"""
        def disable_space_on_widget(widget):
            """Désactive la barre d'espace sur un widget et ses enfants"""
            try:
                # Si c'est un bouton, désactiver la barre d'espace
                if isinstance(widget, (tk.Button, ttk.Button)):
                    widget.bind('<KeyPress-space>', lambda e: "break")
                    widget.bind('<KeyRelease-space>', lambda e: "break")
                
                # Parcourir récursivement tous les enfants
                for child in widget.winfo_children():
                    disable_space_on_widget(child)
            except:
                pass
        
        # Programmer l'exécution après la création complète de l'interface
        def apply_disable():
            disable_space_on_widget(self.root)
        
        # Exécuter après un délai pour s'assurer que tous les widgets sont créés
        self.root.after(500, apply_disable)
