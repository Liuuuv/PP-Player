...
class AIMenuSystem:
    ...
    def __init__(self, main_app):
        ...
        self.ai_icon = self.main_app.icons["activate_ai_small"]
    ...
    def generate_ai_insights_content(self, parent_frame):
        ...
        try:
            ...
            if self.ai_system and hasattr(self.ai_system, 'user_behavior_data'):
                ...
                # Prédictions récentes (si disponibles)
                if hasattr(self.ai_system, 'recent_predictions'):
                    ...
                    predictions_frame.pack(fill="x", padx=10, pady=5)
                    
                    recent_predictions = getattr(self.ai_system, 'recent_predictions', [])
                    if recent_predictions:
                        for pred in recent_predictions[-5:]:  # 5 dernières prédictions
                            pred_text = f"Chanson: {pred.get('song', 'Unknown')[:30]}..."
                            if 'skip_probability' in pred:
                                pred_text += f" | Skip: {pred['skip_probability']:.2%}"
                            if 'like_probability' in pred:
                                pred_text += f" | Like: {pred['like_probability']:.2%}"
                        ...
                        tk.Label(
                            predictions_frame,
                            text="Aucune prédiction récente disponible",
                            bg=COLOR_WINDOW_BACKGROUND,
                            fg="#cccccc",
                            font=("Arial", 10)
                        ).pack(anchor="w", padx=10, pady=2)
