...
class MusicPlayer:
    ...
    def _add_main_playlist_item(self, filepath, thumbnail_path=None, song_index=None):
        ...
        try:
            ...
            # INDICATEUR DE QUEUE : Barre verticale foncée à gauche si dans la queue
            is_in_queue = hasattr(self, 'play_queue') and filepath in self.play_queue
            if is_in_queue:
                queue_indicator = tk.Frame(
                    item_frame,
                    bg='#1a1a1a',  # Barre très foncée
                    width=4,  # Largeur de la barre
                    height=50
                )
                queue_indicator.pack(side='left', fill='y', padx=(0, 2))
                queue_indicator.pack_propagate(False)  # Empêcher le redimensionnement automatique
