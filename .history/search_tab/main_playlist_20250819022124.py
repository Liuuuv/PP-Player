...
def _check_and_unload_items(self, current_index):
        ...
        try:
            ...
            if not next_is_loaded and (next_index < preserve_start or next_index > preserve_end):
                if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
                    print(f"🔄 PRELOAD: Musique suivante {next_index} non chargée, ajout à la zone de préservation")
                
                # Étendre la zone de préservation pour inclure la musique suivante
                if next_index > preserve_end:
                    # La musique suivante est après la zone actuelle
                    new_end = next_index + get_main_playlist_config('preserve_items_after_current')
                    new_end = min(len(self.main_playlist) - 1, new_end)
                    self._append_progressive_items(preserve_end + 1, new_end + 1)
                elif next_index < preserve_start:
                    # La musique suivante est avant la zone actuelle (cas rare)
                    new_start = max(0, next_index - get_main_playlist_config('preserve_items_before_current'))
                    self._append_progressive_items(new_start, preserve_start)
