            ...
            def scroll_after_progressive_load():
                try:
                    ...
                    if relative_index is not None:
                        ...
                        # Vérifier si on doit décharger des éléments après le scroll
                        if USE_NEW_CONFIG and get_main_playlist_config('enable_smart_unloading'):
                            # Attendre un peu que le scroll soit terminé avant de décharger
                            self.root.after(100, self._check_and_unload_items)
...
def _progressive_load_system(self):
        ...
        try:
            ...
            # Vérifier si on doit décharger des éléments après le chargement
            if USE_NEW_CONFIG and get_main_playlist_config('enable_smart_unloading'):
...
def _check_and_unload_items(self):
        """Vérifie si le nombre d'éléments chargés dépasse le seuil et décharge les éléments inutiles"""
        try:
            ...
            # Obtenir le nombre maximum d'éléments chargés avant déchargement
            max_loaded = get_main_playlist_config('max_loaded_items')
            
            # Compter les éléments actuellement chargés
            try:
                children = self.playlist_container.winfo_children()
            ...
            if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
                print(f"🧹 SMART UNLOAD: {loaded_count} éléments chargés > {max_loaded} maximum")
                
            # Obtenir l'index de la chanson en cours de lecture
            current_index = getattr(self, 'current_index', 0)
            
            # Obtenir la position de scroll actuelle pour déterminer ce que l'utilisateur voit
            try:
                scroll_top, scroll_bottom = self.playlist_canvas.yview()
            except:
                scroll_top, scroll_bottom = 0, 1
                
            # Estimer les indices visibles basés sur la position de scroll
            visible_start_ratio = scroll_top
            visible_end_ratio = scroll_bottom
            
            # Estimer les indices visibles (approximation)
            total_items = len(self.main_playlist)
            visible_start_index = int(visible_start_ratio * total_items)
            visible_end_index = int(visible_end_ratio * total_items)
            
            # Déterminer si l'utilisateur regarde des éléments avant la chanson courante
            user_viewing_before_current = visible_end_index < current_index
            
            # Obtenir les paramètres de préservation
            ...
            preserve_view = get_main_playlist_config('preserve_current_view')
            
            # Déterminer les indices à préserver
            if preserve_view and user_viewing_before_current:
                # L'utilisateur regarde des éléments avant la chanson courante
                # Préserver la vue actuelle et quelques éléments après la chanson courante
                preserve_start = max(0, visible_start_index - 5)  # 5 éléments de marge
                preserve_end = min(total_items, current_index + preserve_after)
            else:
                # Préserver les éléments autour de la chanson courante
                preserve_start = max(0, current_index - preserve_before)
                preserve_end = min(total_items, current_index + preserve_after)
                
            if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
                print(f"🔍 Préservation: indices {preserve_start} à {preserve_end}")
                print(f"👁️ Vue actuelle: indices {visible_start_index} à {visible_end_index}")
                print(f"🎵 Chanson courante: indice {current_index}")
                
            # Identifier les éléments à décharger (ceux qui sont avant preserve_start)
            to_unload = []
            for widget in children:
                if hasattr(widget, 'song_index'):
                    if widget.song_index < preserve_start:
                        to_unload.append(widget)
                        
            # Décharger les éléments identifiés
            if to_unload:
                if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
                    print(f"🗑️ Déchargement de {len(to_unload)} éléments (indices < {preserve_start})")
                ...
                # Mettre à jour la région de scroll après le déchargement
                self.root.after(10, self._update_canvas_scroll_region)
                
        except Exception as e:
            if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
                print(f"❌ Erreur lors du déchargement intelligent: {e}")
