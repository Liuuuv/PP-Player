...
def _calculate_smart_window(self):
        ...
        try:
            ...
            # Calculer l'union des deux zones (fenêtre minimale à garder)
...
def _smart_load_unload(self):
        ...
        try:
            ...
            target_start, target_end = self._calculate_smart_window()
            if target_start is None or target_end is None:
                return
            
            # Obtenir la fenêtre actuellement chargée
            current_start = getattr(self, '_last_window_start', -1)
            current_end = getattr(self, '_last_window_end', -1)
            
            if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
                print(f"Smart load/unload: actuel {current_start}-{current_end} → cible {target_start}-{target_end}")
            
            # Vérifier s'il faut mettre à jour
            if current_start == target_start and current_end == target_end:
                return  # Pas de changement nécessaire
            
            # Étape 1: Décharger les éléments inutiles
            if get_main_playlist_config('auto_unload_unused'):
                self._unload_unused_items(target_start, target_end, current_start, current_end)
            
            # Étape 2: Charger les nouveaux éléments nécessaires
            self._load_required_items(target_start, target_end, current_start, current_end)
