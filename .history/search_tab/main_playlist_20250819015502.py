            ...
            def scroll_after_progressive_load():
                try:
                    ...
                    if relative_index is not None:
                        ...
                        # Vérifier si on doit décharger des éléments maintenant que l'utilisateur
                        # est revenu à la chanson courante
...
def _progressive_load_system(self):
        ...
        try:
            ...
            # Charger de start_from jusqu'à target_end
...
def _check_and_unload_items(self):
        """Vérifie si le nombre d'éléments chargés dépasse le seuil et décharge les éléments si nécessaire"""
        try:
            ...
            # Si le nombre d'éléments chargés est inférieur au seuil, ne rien faire
            ...
            try:
                ...
                visible_items_count = int(loaded_count * visible_ratio)
                
                # Estimer les indices visibles
                visible_indices = []
                for child in children:
                    if hasattr(child, 'song_index'):
                        # Vérifier si l'élément est visible dans la vue actuelle
                        child_y = child.winfo_y()
                        canvas_height = self.playlist_canvas.winfo_height()
                        scroll_y = scroll_top * self.playlist_container.winfo_height()
                        
                        if (child_y + child.winfo_height() > scroll_y and 
                            child_y < scroll_y + canvas_height):
                            visible_indices.append(child.song_index)
            except:
                # En cas d'erreur, considérer que l'utilisateur regarde autour de la chanson courante
                visible_indices = list(range(
                    max(0, current_index - 5),
                    min(len(self.main_playlist), current_index + 6)
                ))
            
            # Déterminer si l'utilisateur regarde des musiques avant la chanson courante
            viewing_before_current = any(idx < current_index for idx in visible_indices)
            
            # Créer une liste des éléments à décharger
            to_unload = []
            
            # Priorité de déchargement selon la configuration
            unload_priority = get_main_playlist_config('unload_priority')
            
            # Collecter les indices des éléments chargés
            loaded_indices = []
            for child in children:
                if hasattr(child, 'song_index'):
                    loaded_indices.append(child.song_index)
            
            # Trier les indices selon la priorité de déchargement
            if unload_priority == 'before_current':
                # Décharger d'abord les éléments avant la chanson courante
                if viewing_before_current:
                    # Si l'utilisateur regarde des musiques avant la courante,
                    # décharger celles encore plus avant qu'il ne voit pas
                    min_visible = min(visible_indices) if visible_indices else current_index
                    to_unload = [idx for idx in loaded_indices if idx < min_visible]
                else:
                    # Sinon, décharger toutes les musiques avant la courante
                    to_unload = [idx for idx in loaded_indices if idx < current_index]
            elif unload_priority == 'after_current':
                # Décharger d'abord les éléments après la chanson courante
                max_visible = max(visible_indices) if visible_indices else current_index
                to_unload = [idx for idx in loaded_indices if idx > max_visible]
            else:  # 'furthest'
                # Décharger les éléments les plus éloignés de la chanson courante
                to_unload = sorted(loaded_indices, key=lambda idx: abs(idx - current_index), reverse=True)
            
            # Limiter le nombre d'éléments à décharger
            items_to_keep = max_loaded
            items_to_unload = loaded_count - items_to_keep
            
            if to_unload:
                # Limiter la liste des éléments à décharger
                to_unload = to_unload[:items_to_unload]
                
                if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
                    print(f"🧹 UNLOADING: Déchargement de {len(to_unload)} éléments (indices: {to_unload})")
                
                # Décharger les éléments
                for child in list(children):  # Copie pour éviter les problèmes de modification pendant l'itération
                    if hasattr(child, 'song_index') and child.song_index in to_unload:
                        child.destroy()
            ...
            if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
                print(f"❌ Erreur check_and_unload: {e}")
