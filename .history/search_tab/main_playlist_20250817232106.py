...
def _refresh_main_playlist_display(self, force_full_refresh=False):
        ...
        try:
            ...
            # Vérifier si les optimisations sont activées
            optimizations_enabled = get_config("enable_optimizations", True)
            print("OPTIS? ", optimizations_enabled)

            # Décider du mode d'affichage selon la taille et la configuration
            if not optimizations_enabled or not should_use_windowing(playlist_size):
...
def _update_canvas_scroll_region(self):
        ...
        try:
            ...
            # Alternative si bbox("all") ne fonctionne pas
            if self.playlist_canvas.bbox("all") is None:
                # Calculer manuellement la hauteur basée sur le nombre d'enfants
                children_count = len(self.playlist_container.winfo_children())
                if children_count > 0:
                    # Estimer la hauteur : nombre d'éléments × hauteur approximative par élément
                    estimated_height = children_count * 60  # 60px par élément approximativement
