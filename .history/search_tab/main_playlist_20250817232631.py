...
def _refresh_full_playlist_display(self):
        ...
        try:
            ...
            self.root.after(10, lambda: self._update_canvas_scroll_region())
...
def _refresh_windowed_playlist_display(self, force_recreate=False):
        ...
        try:
            ...
            if USE_NEW_CONFIG:
                ...
                window_size = get_window_size(playlist_size)
...
def _update_canvas_scroll_region(self):
        ...
        try:
            ...
            # Forcer la mise à jour de la géométrie du container
            self.playlist_container.update_idletasks()
            
            # Méthode 1: Utiliser bbox("all") si disponible
            bbox = self.playlist_canvas.bbox("all")
            if bbox is not None:
                self.playlist_canvas.configure(scrollregion=bbox)
                if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
                    print(f"Scroll region mise à jour avec bbox: {bbox}")
                return
            
            # Méthode 2: Calculer manuellement la hauteur
            children = self.playlist_container.winfo_children()
            if children:
                # Obtenir la hauteur réelle du container
                try:
                    # Forcer une mise à jour complète
                    self.root.update_idletasks()
                    container_height = self.playlist_container.winfo_reqheight()
                    
                    if container_height > 1:
                        self.playlist_canvas.configure(scrollregion=(0, 0, 0, container_height))
                        if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
                            print(f"Scroll region mise à jour avec hauteur calculée: {container_height}")
                        return
                except:
                    pass
                
                # Méthode 3: Estimation basée sur le nombre d'éléments
                ...
                estimated_height = children_count * item_height
                self.playlist_canvas.configure(scrollregion=(0, 0, 0, estimated_height))
                
                if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
                    print(f"Scroll region mise à jour avec estimation: {estimated_height} ({children_count} × {item_height})")
            else:
