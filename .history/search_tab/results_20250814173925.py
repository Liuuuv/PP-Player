...
def _on_youtube_canvas_configure(self, event):
    ...
    print(f"DEBUG: _on_youtube_canvas_configure appelé")
    ...
    try:
        if hasattr(self, 'youtube_canvas') and self.youtube_canvas.winfo_exists():
            ...
            print(f"DEBUG: Position scroll: top={scroll_top}, bottom={scroll_bottom}")
            
            # Si on est proche du bas (90% ou plus), charger plus de résultats
            if scroll_bottom > 0.9:
                print(f"DEBUG: Proche du bas, vérification si on peut charger plus")
                if self._should_load_more_results():
                    print(f"DEBUG: Chargement de plus de résultats")
                    self._load_more_search_results()
                else:
                    print(f"DEBUG: Pas de chargement nécessaire")
            else:
                print(f"DEBUG: Pas encore en bas du scroll")
    except Exception as e:
        print(f"DEBUG: Erreur dans _on_youtube_canvas_configure: {e}")
...
def _should_load_more_results(self):
    ...
    try:
        print(f"DEBUG: _should_load_more_results appelé")
        
        if not hasattr(self, 'has_more_results'):
            print(f"DEBUG: has_more_results n'existe pas")
            return False
        
        print(f"DEBUG: has_more_results = {self.has_more_results}")
        if not self.has_more_results:
            print(f"DEBUG: Pas plus de résultats disponibles")
            return False
            
        if hasattr(self, 'is_loading_more'):
            print(f"DEBUG: is_loading_more = {self.is_loading_more}")
            if self.is_loading_more:
                print(f"DEBUG: Chargement déjà en cours")
                return False
        
        print(f"DEBUG: Peut charger plus de résultats")
        return True
    except Exception as e:
        print(f"DEBUG: Erreur vérification chargement: {e}")
        return False
