...
def _setup_infinite_scroll(self):
        ...
        try:
            ...
            # Binding pour détecter le scroll avec la molette
            def on_scroll_event(event):
                ...
                # Puis vérifier si on doit charger plus d'éléments
                self.root.after(100, self._check_infinite_scroll)
            
            # Remplacer temporairement le binding de scroll
            if not hasattr(self, '_original_scroll_bound'):
                self._original_scroll_bound = True
                # Le binding est déjà fait dans setup.py, on ajoute juste notre vérification
...
def _add_main_playlist_item_at_position(self, filepath, song_index=None, position='bottom'):
        ...
        try:
            # Créer l'élément comme d'habitude
            item_frame = self._create_playlist_item_frame(filepath, song_index)
            
            if position == 'top':
                # Insérer au début
                item_frame.pack(side=tk.TOP, fill=tk.X, padx=2, pady=1, before=self.playlist_container.winfo_children()[0] if self.playlist_container.winfo_children() else None)
            else:
                # Ajouter à la fin (comportement par défaut)
...
def _create_playlist_item_frame(self, filepath, song_index=None):
        """Crée un frame pour un élément de playlist (version simplifiée de _add_main_playlist_item)"""
        try:
            # Utiliser la fonction existante mais retourner juste le frame
