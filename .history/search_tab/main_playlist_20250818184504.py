...
def select_current_song_smart(self, auto_scroll=True, force_reload=False):
    ...
    try:
        ...
        # Vérifier si la chanson courante est déjà visible et sélectionnée
        # Si oui, on s'assure juste que son état selected est bien à True
        try:
            children = self.playlist_container.winfo_children()
            for child in children:
                if hasattr(child, 'song_index') and child.song_index == self.current_index:
                    # La chanson est déjà visible, s'assurer qu'elle est sélectionnée
                    child.selected = True
                    ...
                    if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
                        print(f"✅ Chanson {self.current_index} déjà visible et sélectionnée")
                    return
        except Exception:
        ...
        if USE_NEW_CONFIG and get_main_playlist_config('enable_progressive_loading'):
            ...
            if self._is_index_already_loaded(self.current_index):
                ...
                if relative_index is not None:
                    ...
                    try:
                        ...
                        for child in children:
                            try:
                                if hasattr(child, 'selected') and child.winfo_exists():
                                    child.selected = False
                                    self._reset_frame_appearance(child)
                            except Exception:
                                continue
                    except Exception:
                        pass
            ...
            self.root.after(100, auto_scroll_after_progressive_load)
        ...
        # ANCIEN SYSTÈME FENÊTRÉ
            ...
            def auto_scroll_after_reload():
                try:
                    ...
                    pass
            self.root.after(100, auto_scroll_after_reload)
        ...
        try:
            ...
            for child in children:
                try:
                    if child.winfo_exists() and hasattr(child, 'config'):
                        # Réinitialiser l'attribut selected
            ...
            if (hasattr(self, '_last_window_start') and hasattr(self, '_last_window_end') and
                ...
                if 0 <= relative_index < len(children):
                    ...
                    if hasattr(widget, 'winfo_exists') and widget.winfo_exists():
                        # Définir l'attribut selected à True
                        widget.selected = True
                        self._highlight_current_song_widget(widget)
            
            # Vérification supplémentaire : parcourir tous les éléments pour trouver celui avec song_index = current_index
            for child in children:
                try:
                    if (child.winfo_exists() and hasattr(child, 'song_index') and 
                        child.song_index == self.current_index):
                        # Définir l'attribut selected à True
                        child.selected = True
                        self._highlight_current_song_widget(child)
                        break
                except:
                    continue
...
def _set_item_colors(self, item_frame, bg_color):
        ...
        try:
            if hasattr(item_frame, 'config'):
                ...
                # Mettre à jour l'attribut selected en fonction de la couleur
                if bg_color == COLOR_SELECTED or bg_color == '#5a9fd8':  # Couleur de surbrillance (bleu)
                    if not hasattr(item_frame, 'selected'):
                        item_frame.selected = True
                    else:
                        item_frame.selected = True
                else:
                    if hasattr(item_frame, 'selected'):
                        item_frame.selected = False
...
def _highlight_current_song_widget(self, widget):
        ...
        try:
            # S'assurer que l'attribut selected est défini et mis à True
            if not hasattr(widget, 'selected'):
                widget.selected = True
            else:
                widget.selected = True
...
def _refresh_main_playlist_display_async(self):
        ...
        try:
            ...
            # Mettre à jour la sélection de la chanson courante après le rafraîchissement
            if hasattr(self, 'main_playlist') and len(self.main_playlist) > 0 and hasattr(self, 'current_index'):
                # Utiliser un petit délai pour s'assurer que l'interface est prête
                self.root.after(50, lambda: self.select_current_song_smart(auto_scroll=False))
