...
def _display_batch_results(self, batch_number):
    ...
    print(f"DEBUG: _display_batch_results appelée avec batch_number={batch_number}")
    # Vérifier si la recherche a été annulée
    if self.search_cancelled:
        print(f"DEBUG: Recherche annulée, arrêt de _display_batch_results")
        return
    ...
    print(f"DEBUG: batch_size={batch_size}, start_index={start_index}, end_index={end_index}")
    print(f"DEBUG: Nombre total de résultats disponibles: {len(self.all_search_results)}")
    ...
    print('BLOUPIODD ', self.search_results_count)
...
def _show_search_results(self):
    ...
    print("DEBUG: === DÉBUT _show_search_results ===")
    try:
        ...
        if hasattr(self, 'scrollbar') and self.scrollbar is not None:
            try:
                if self.scrollbar.winfo_exists():
                    self.scrollbar.pack(side="right", fill="y")
                    print("DEBUG: scrollbar affichée")
                else:
                    print("DEBUG: scrollbar n'existe pas")
            except Exception as e:
                print(f"DEBUG: Erreur affichage scrollbar: {e}")
        else:
            print("DEBUG: Pas de scrollbar")
        ...
        if hasattr(self, 'youtube_canvas') and self.youtube_canvas is not None:
            try:
                if self.youtube_canvas.winfo_exists():
                    self.youtube_canvas.pack(side="left", fill="both", expand=True)
                    print("DEBUG: youtube_canvas affiché")
                    # S'assurer que la connexion canvas-scrollbar est correcte
                    self._ensure_canvas_scrollbar_connection()
                else:
                    print("DEBUG: youtube_canvas n'existe pas")
            except Exception as e:
                print(f"DEBUG: Erreur affichage youtube_canvas: {e}")
        else:
            print("DEBUG: Pas de youtube_canvas")
        ...
        if hasattr(self, 'results_container'):
            if self.results_container and self.results_container.winfo_exists():
                children = self.results_container.winfo_children()
                print(f"DEBUG: Container a {len(children)} enfants")
            else:
                print("DEBUG: Container de résultats n'existe pas")
        else:
            print("DEBUG: Pas de results_container")
        
        # Mettre à jour le compteur de résultats
        self._update_results_counter()
        print("DEBUG: === FIN _show_search_results ===")
    except Exception as e:
    ...
    try:
        if hasattr(self, 'youtube_canvas') and hasattr(self, 'scrollbar'):
            if (self.youtube_canvas and self.youtube_canvas.winfo_exists() and 
                ...
                print("DEBUG: Connexion canvas-scrollbar avec détection de scroll restaurée")
            else:
                ...
                if self.youtube_canvas and self.youtube_canvas.winfo_exists():
                    ...
                    print("DEBUG: Canvas déconnecté de la scrollbar détruite")
    except Exception as e:
        print(f"DEBUG: Erreur lors de la configuration canvas-scrollbar: {e}")
...
def _start_new_search(self):
    ...
    if _try_restore_from_cache(self, query):
        print(f"DEBUG: Recherche '{query}' restaurée depuis le cache instantanément!")
        return
    
    print(f"DEBUG: Recherche '{query}' non trouvée dans le cache, recherche réseau...")
...
def _perform_initial_search(self, query):
    ...
    print(f"DEBUG: _perform_initial_search appelée avec query='{query}', initial_search_count={self.initial_search_count}")
    try:
        # Vérifier si la recherche a été annulée avant de commencer
        if self.search_cancelled:
            print(f"DEBUG: Recherche annulée avant de commencer")
            return
        ...
        print(f"DEBUG: Options de recherche: {search_opts}")
        ...
        with YoutubeDL(search_opts) as ydl:
            ...
            print(f"DEBUG: Résultats bruts obtenus: {len(results['entries'])}")
            print(f"DEBUG: Résultats après filtrage: {len(filtered_results)}")
            ...
            self.total_available_results = len(self.all_search_results)
            print(f"DEBUG: has_more_results défini à: {self.has_more_results}")
            print(f"DEBUG: total_available_results défini à: {self.total_available_results}")
            ...
            print(f"DEBUG: Appel de _display_batch_results(1)")
            self._display_batch_results(1)
...
def _save_current_search_state(self):
        ...
        print(f"DEBUG: État de recherche sauvegardé dans le cache:")
...
def _try_restore_from_cache(self, search_query):
    ...
    print(f"DEBUG: Restauration depuis le cache pour '{search_query}'")
    ...
    try:
        ...
        print(f"DEBUG: Variables restaurées:")
        ...
        print(f"DEBUG: Appel de _display_search_results_from_cache avec {len(self.all_search_results)} résultats")
        try:
            self._display_search_results_from_cache(self.all_search_results)
            print("DEBUG: _display_search_results_from_cache terminé avec succès")
        except Exception as e:
            print(f"DEBUG: Erreur dans _display_search_results_from_cache: {e}")
            import traceback
            traceback.print_exc()
            ...
            def restore_scroll():
                try:
                    if hasattr(self, 'youtube_canvas') and self.youtube_canvas.winfo_exists():
                        ...
                        print(f"DEBUG: Position de scroll restaurée: {scroll_top}")
                except Exception as e:
                    print(f"DEBUG: Erreur restauration scroll: {e}")
        ...
        print(f"DEBUG: Erreur lors de la restauration depuis le cache: {e}")
        return False

def _display_search_results_from_cache(self, results):
    """Affiche les résultats de recherche depuis le cache avec optimisation"""
    print(f"DEBUG: === DÉBUT _display_search_results_from_cache ===")
    print(f"DEBUG: Paramètre results: {type(results)}")
    print(f"DEBUG: Nombre de résultats: {len(results) if results else 0}")
    
    # Désactiver temporairement le chargement automatique
    self._cache_loading_in_progress = True
    print("DEBUG: Chargement automatique désactivé temporairement")
    
    if not results:
        print("DEBUG: Aucun résultat à afficher, sortie")
        self._cache_loading_in_progress = False
        return
    
    # Afficher quelques exemples de résultats
    for i, result in enumerate(results[:3]):
        print(f"DEBUG: Résultat {i}: {result.get('title', 'Sans titre')[:50]}...")
    
    print(f"DEBUG: Affichage optimisé de {len(results)} résultats depuis le cache")
    
    # Nettoyer d'abord
    print("DEBUG: Nettoyage des résultats existants...")
    self._clear_results()
    
    # S'assurer que le container existe
    print("DEBUG: Vérification du container...")
    _ensure_results_container_exists(self)
    ...
    def display_batch(batch_start):
        try:
            ...
            print(f"DEBUG: Affichage lot cache {batch_start}-{batch_end-1} sur {len(results)} résultats")
            
            # Vérifier que le container existe
            if not hasattr(self, 'results_container') or not self.results_container.winfo_exists():
                print("DEBUG: Container de résultats n'existe pas, création...")
                _ensure_results_container_exists(self)
            ...
            for i in range(batch_start, batch_end):
                if i < len(results):
                    video = results[i]
                    print(f"DEBUG: Ajout résultat cache {i}: {video.get('title', 'Sans titre')[:50]}...")
                    try:
                        self._add_search_result(video, i)
                        print(f"DEBUG: Résultat cache {i} ajouté avec succès")
                    except Exception as e:
                        print(f"DEBUG: Erreur ajout résultat cache {i}: {e}")
            
            # Mettre à jour le compteur
            self.search_results_count = batch_end
            print(f"DEBUG: Compteur mis à jour: {self.search_results_count}")
            
            # Programmer le lot suivant s'il y en a un
            if batch_end < len(results):
                print(f"DEBUG: Programmation lot suivant dans {delay}ms")
                self.safe_after(delay, lambda: display_batch(batch_end))
            else:
                # Affichage terminé
                print(f"DEBUG: Affichage depuis cache terminé - {len(results)} résultats")
                # Mettre à jour la scrollregion
                self._update_scroll_region()
                # Afficher les résultats
                self._show_search_results()
                print("DEBUG: _show_search_results() appelé après cache")
                
                # Réactiver le chargement automatique après un délai
                def reactivate_auto_loading():
                    self._cache_loading_in_progress = False
                    print("DEBUG: Chargement automatique réactivé")
...
def _check_scroll_position(self):
    ...
    print(f"DEBUG: _check_scroll_position appelée")
    if self._should_load_more_results():
        print(f"DEBUG: _should_load_more_results = True, chargement de plus de résultats...")
        self._load_more_search_results()
    else:
        print(f"DEBUG: _should_load_more_results = False, pas de chargement")

def _should_load_more_results(self):
    """Vérifie si on doit charger plus de résultats - version optimisée"""
    print(f"DEBUG: _should_load_more_results - Vérifications:")
    print(f"  - is_loading_more: {self.is_loading_more}")
    print(f"  - is_searching: {self.is_searching}")
    print(f"  - current_search_query: '{self.current_search_query}'")
    print(f"  - search_results_count: {self.search_results_count}")
    print(f"  - max_search_results: {self.max_search_results}")
    print(f"  - current_search_batch: {self.current_search_batch}")
    print(f"  - max_search_batchs: {self.max_search_batchs}")
    print(f"  - has_more_results: {getattr(self, 'has_more_results', False)}")
    print(f"  - cache_loading_in_progress: {getattr(self, '_cache_loading_in_progress', False)}")
    ...
    if (self.is_loading_more or 
        ...
        print(f"DEBUG: Conditions de base non remplies, retour False")
        return False
    # Vérifier si on a encore des résultats à charger
    if not getattr(self, 'has_more_results', False):
        print(f"DEBUG: Pas de résultats supplémentaires disponibles, retour False")
        return False
    ...
    try:
        ...
        if abs(scroll_bottom - self._last_scroll_bottom) < 0.01:
            print(f"DEBUG: Position scroll n'a pas assez changé, retour False")
            return False
        ...
        if scroll_bottom > 0.85:  # 85%
            print(f"DEBUG: Position scroll > 85%, retour True")
            return True
        
        print(f"DEBUG: Position scroll <= 85%, retour False")
        return False
    except Exception as e:
        print(f"DEBUG: Erreur dans _should_load_more_results: {e}")
        return False
...
def _display_search_results(self, results, scroll_position=None):
    ...
    try:
        ...
        if batches_needed > 2:  # Seulement pour les gros chargements
            print(f"DEBUG: Restauration rapide - {num_results} résultats, {batches_needed} batches")
            ...
            if self.youtube_canvas.winfo_exists() and self.results_container.winfo_exists():
                ...
                if bbox:
                    ...
                    if bbox[3] > 1000:  # Seulement pour les grandes scrollregions
                        print(f"DEBUG: Grande scrollregion mise à jour: hauteur={bbox[3]}")
    except Exception as e:
        print(f"DEBUG: Erreur lors de la mise à jour de la scrollregion: {e}")
...
def _restore_scroll_position(self, scroll_position):
    ...
    try:
        if hasattr(self, 'youtube_canvas') and self.youtube_canvas.winfo_exists():
            ...
            print(f"DEBUG: Scrollregion actuelle: {scrollregion}")
