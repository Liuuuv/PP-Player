...
def _setup_infinite_scroll(self):
        ...
        try:
            ...
            if not hasattr(self, 'playlist_canvas'):
                print("❌ DEBUG: Pas de playlist_canvas")
                return
            
            print(f"✅ DEBUG: playlist_canvas trouvé: {type(self.playlist_canvas)}")
            ...
            print("✅ DEBUG: Binding <Configure> ajouté")
...
def _adjust_canvas_scroll_position(self, start_index, end_index, center_index):
        ...
        try:
            ...
            total_songs = len(self.main_playlist)
            if total_songs == 0:
                print("❌ DEBUG: Playlist vide, pas d'ajustement")
                return
            
            # Calculer où devrait être le centre de la fenêtre visible
            # Si on affiche les éléments start_index à end_index, 
            # le centre devrait être à center_index
            
            # Position relative du centre dans la fenêtre affichée
            relative_center = center_index - start_index
            window_size = end_index - start_index
            
            print(f"📊 DEBUG: Centre relatif: {relative_center}/{window_size}")
            
            # Calculer la position de scroll cible
            # On veut que le centre soit visible au milieu de la zone visible
            if window_size > 0:
                # Position du centre dans la fenêtre (0.0 = haut, 1.0 = bas)
                center_position_in_window = relative_center / window_size
                
                # Ajuster pour centrer l'élément dans la zone visible
                # Si on a une zone visible qui montre par exemple 10 éléments,
                # on veut que l'élément central soit au milieu
                visible_height_ratio = 0.5  # Approximation de la hauteur visible
                
                # Position de scroll cible (0.0 à 1.0)
                target_scroll_top = center_position_in_window - (visible_height_ratio / 2)
                target_scroll_top = max(0.0, min(1.0, target_scroll_top))
                
                print(f"🎯 DEBUG: Position scroll cible: {target_scroll_top:.3f}")
                
                # Appliquer la nouvelle position de scroll
                self.playlist_canvas.yview_moveto(target_scroll_top)
                
                print(f"✅ DEBUG: Position de scroll ajustée à {target_scroll_top:.3f}")
                
                # Vérifier la nouvelle position
                new_top, new_bottom = self.playlist_canvas.yview()
                print(f"📊 DEBUG: Nouvelle position réelle: top={new_top:.3f}, bottom={new_bottom:.3f}")
