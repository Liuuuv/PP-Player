...
def _add_main_playlist_item(self, filepath, thumbnail_path=None, song_index=None):
        ...
        try:
            ...
            for widget in widgets_to_bind:
                ...
                # Ajouter un gestionnaire pour la molette de souris qui transmet l'événement au canvas principal
                widget.bind("<MouseWheel>", lambda e: self._forward_mousewheel_to_canvas(e))
                widget.bind("<Button-4>", lambda e: self._forward_mousewheel_to_canvas(e))  # Linux
                widget.bind("<Button-5>", lambda e: self._forward_mousewheel_to_canvas(e))  # Linux
...
def _forward_mousewheel_to_canvas(self, event):
        """Transmet l'événement de la molette de souris directement au canvas principal"""
        try:
            if hasattr(self, 'playlist_canvas') and self.playlist_canvas:
                # Stopper la propagation de l'événement original
                event.widget.unbind("<MouseWheel>")
                event.widget.unbind("<Button-4>")
                event.widget.unbind("<Button-5>")
                
                # Créer un nouvel événement et l'envoyer directement au canvas
                if hasattr(self, '_on_mousewheel'):
                    self._on_mousewheel(event, self.playlist_canvas)
                
                # Rétablir le binding après un court délai
                self.root.after(100, lambda: self._rebind_mousewheel(event.widget))
                
                # Empêcher la propagation de l'événement original
                return "break"
        except Exception as e:
            print(f"Erreur transmission molette: {e}")
            
def _rebind_mousewheel(self, widget):
        """Rétablit le binding de la molette après un délai"""
        try:
            if widget and widget.winfo_exists():
                widget.bind("<MouseWheel>", lambda e: self._forward_mousewheel_to_canvas(e))
                widget.bind("<Button-4>", lambda e: self._forward_mousewheel_to_canvas(e))
                widget.bind("<Button-5>", lambda e: self._forward_mousewheel_to_canvas(e))
        except Exception:
            pass
            
def _on_mousewheel(self, event, canvas):
        """Gère le défilement avec la molette de souris (version locale)"""
        try:
            # Utiliser la fonction globale si elle existe
            if hasattr(self, 'root') and hasattr(self.root, '_on_mousewheel'):
                return self.root._on_mousewheel(event, canvas)
