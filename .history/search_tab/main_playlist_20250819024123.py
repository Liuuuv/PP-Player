                ...
                try:
                    ...
                    if relative_index is not None:
                        ...
                        if USE_NEW_CONFIG and get_main_playlist_config('enable_smart_unloading'):
                            ...
                            # Utiliser une méthode sécurisée pour appeler _check_and_unload_items
                            def safe_check_and_unload():
                                try:
                                    if hasattr(self, '_check_and_unload_items'):
                                        print("DEBUG: Déchargement après find.png")
                                        self._check_and_unload_items(self.current_index)
                                    else:
                                        print("DEBUG: Méthode _check_and_unload_items non disponible")
                                except Exception as e:
                                    print(f"DEBUG: Erreur lors du déchargement après find.png: {e}")
                            
                            self.root.after(100, safe_check_and_unload)
        ...
        try:
            ...
            if USE_NEW_CONFIG and get_main_playlist_config('enable_smart_unloading'):
                ...
                def safe_check_and_unload():
                    try:
                        if hasattr(self, '_check_and_unload_items'):
                            print("DEBUG: Déchargement après auto-recentrage")
                            self._check_and_unload_items(current_index)
                        else:
                            print("DEBUG: Méthode _check_and_unload_items non disponible dans auto-recentrage")
                    except Exception as e:
                        print(f"DEBUG: Erreur lors du déchargement après auto-recentrage: {e}")
                    ...
                    if hasattr(self, 'current_index') and self.main_playlist:
                        ...
                        if 0 <= current_index < len(self.main_playlist):
                            ...
                            if not next_is_loaded:
                                ...
                                # Forcer le déchargement des éléments inutiles
                                def safe_check_and_unload():
                                    try:
                                        print("DEBUG: Déchargement après chargement préventif")
                                        self._check_and_unload_items(current_index)
                                    except Exception as e:
                                        print(f"DEBUG: Erreur lors du déchargement après chargement préventif: {e}")
            ...
            if hasattr(self, 'current_index') and self.main_playlist:
                ...
                if current_index < first_visible_index or current_index > last_visible_index:
                    ...
                    if hasattr(self, '_is_index_already_loaded') and not self._is_index_already_loaded(current_index):
                        print(f"DEBUG: Musique actuelle non chargée, déclenchement du chargement/déchargement")
                        if hasattr(self, '_check_and_unload_items'):
                            self._check_and_unload_items(current_index)
