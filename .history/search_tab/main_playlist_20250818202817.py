...
def _add_main_playlist_item(self, filepath, thumbnail_path=None, song_index=None):
        ...
        try:
            ...
            # Stocker l'index de la chanson dans le widget pour pouvoir le retrouver
            ...
            for widget in widgets_to_bind:
                ...
                widget.bind("<Enter>", lambda event: self.root.after(50, on_enter(event)))
...
def select_current_song_smart(self, auto_scroll=True, force_reload=False):
    """Sélection intelligente de la chanson courante avec système incrémental"""
    try:
        if not self.main_playlist or self.current_index >= len(self.main_playlist):
            return
            
        print(f"🎮 SELECTION CHANSON: {self.current_index}")
        
        # Vérifier si la chanson actuelle est visible
        start_index = getattr(self, '_display_start_index', 0)
        displayed_count = getattr(self, '_displayed_items_count', 0)
        end_index = start_index + displayed_count
        
        if start_index <= self.current_index < end_index:
            # Chanson visible, juste mettre en surbrillance
            children = self.playlist_container.winfo_children()
            relative_index = self.current_index - start_index
            if 0 <= relative_index < len(children):
                self._highlight_current_song_widget(children[relative_index])
                if auto_scroll:
                    # Scroll vers la chanson si nécessaire
                    self._scroll_to_visible_item(relative_index)
                print(f"✅ Chanson visible à l'index relatif {relative_index}")
                return
        
        # Chanson non visible, déclencher la vérification de visibilité
        print(f"🔄 Chanson non visible, rechargement nécessaire")
        self._check_current_song_visibility()
            
    except Exception as e:
        print(f"❌ Erreur select current smart: {e}")

def _scroll_to_visible_item(self, relative_index):
        """Fait défiler vers un élément visible dans la liste"""
        try:
            ...
            if 0 <= relative_index < len(children):
                ...
                # Obtenir la position du widget dans le conteneur
                widget_top = widget.winfo_y()
                widget_height = widget.winfo_height()
                container_height = self.playlist_container.winfo_height()
                canvas_height = self.playlist_canvas.winfo_height()
                
                # Calculer la position relative nécessaire pour centrer l'élément
                if container_height > 0:
                    scroll_position = (widget_top + widget_height/2 - canvas_height/2) / container_height
                    scroll_position = max(0, min(1, scroll_position))  # Limiter entre 0 et 1
                    
                    self.playlist_canvas.yview_moveto(scroll_position)
                    
        except Exception as e:
            print(f"Erreur lors du scroll vers l'élément: {e}")
...
def add_to_main_playlist(self, filepath, thumbnail_path=None, song_index=None, show_status=True, allow_duplicates=False):
    """Fonction centralisée pour ajouter une musique à la main playlist avec chargement incrémental
    ...
    if allow_duplicates or filepath not in self.main_playlist:
        ...
        # Avec le nouveau système incrémental, on ne rafraîchit que si nécessaire
        # Si l'élément ajouté est dans la plage visible, l'afficher immédiatement
        start_index = getattr(self, '_display_start_index', 0)
        displayed_count = getattr(self, '_displayed_items_count', 0)
        new_index = len(self.main_playlist) - 1
        
        if start_index <= new_index < start_index + displayed_count:
            # L'élément est dans la plage visible, l'afficher
            self._add_main_playlist_item(filepath, thumbnail_path, new_index)
            self._displayed_items_count += 1
...
def _refresh_main_playlist_display(self, force_full_refresh=False):
        """Rafraîchit l'affichage de la main playlist avec chargement incrémental simple"""
        try:
            ...
            # Si pas de playlist, vider l'affichage
            if not self.main_playlist:
                for child in self.playlist_container.winfo_children():
                    child.destroy()
                self._displayed_items_count = 0
                return
            
            # Initialiser les variables de chargement incrémental
            if not hasattr(self, '_displayed_items_count'):
                self._displayed_items_count = 0
            ...
            # Commencer par afficher la musique actuelle + 10 suivantes
            start_index = max(0, getattr(self, 'current_index', 0))
            items_to_show = min(11, len(self.main_playlist) - start_index)
            ...
            self._displayed_items_count = items_to_show
            self._display_start_index = start_index
            
            # Configurer le scroll pour charger plus d'éléments
            self._setup_incremental_scroll()
            
            # Mettre en surbrillance la chanson courante
            if hasattr(self, 'current_index') and self.current_index >= start_index:
                try:
                    children = self.playlist_container.winfo_children()
                    relative_index = self.current_index - start_index
                    if 0 <= relative_index < len(children):
...
def _setup_incremental_scroll(self):
        """Configure le système de scroll incrémental pour charger plus d'éléments"""
        try:
            if not hasattr(self, 'playlist_canvas') or not self.playlist_canvas.winfo_exists():
                return
            ...
            def on_scroll(*args):
                # Vérifier si on est en bas et charger plus d'éléments
                self._check_scroll_load_more()
                
                # Vérifier si la musique actuelle n'est plus visible
                self._check_current_song_visibility()
            
            # Bind les événements de scroll
            if hasattr(self, 'playlist_scrollbar'):
                original_command = self.playlist_scrollbar.cget('command')
                
                def enhanced_scroll(*args):
                    if original_command:
                        self.playlist_canvas.yview(*args)
                    on_scroll()
                
                self.playlist_scrollbar.config(command=enhanced_scroll)
            
            # Bind la molette de souris
            def on_mousewheel(event):
                ...
                self.playlist_canvas.yview_scroll(int(delta), "units")
                on_scroll()
                return "break"
            
            self.playlist_canvas.bind("<MouseWheel>", on_mousewheel)
            self.playlist_canvas.bind("<Button-4>", on_mousewheel)
            self.playlist_canvas.bind("<Button-5>", on_mousewheel)
            
        except Exception as e:
            print(f"Erreur lors de la configuration du scroll incrémental: {e}")

def _check_scroll_load_more(self):
        """Vérifie si on doit charger plus d'éléments en bas de liste"""
        try:
            if not hasattr(self, 'playlist_canvas') or not self.playlist_canvas.winfo_exists():
                return
            ...
            # Si on est proche du bas (90% ou plus), charger 10 éléments de plus
            if scroll_bottom > 0.9:
                start_index = getattr(self, '_display_start_index', 0)
                displayed_count = getattr(self, '_displayed_items_count', 0)
                
                next_start = start_index + displayed_count
                next_count = min(10, len(self.main_playlist) - next_start)
                
                if next_count > 0:
                    # Ajouter les nouveaux éléments en bas
                    for i in range(next_start, next_start + next_count):
                        if i < len(self.main_playlist):
                            filepath = self.main_playlist[i]
                            self._add_main_playlist_item(filepath, song_index=i)
                    
                    self._displayed_items_count += next_count
...
def _check_current_song_visibility(self):
        """Vérifie si la musique actuelle est visible et charge si nécessaire"""
        try:
            ...
            # Si la musique actuelle n'est plus dans la plage affichée
            if self.current_index >= end_index:
                # Charger 10 nouvelles musiques centrées sur la musique actuelle
                new_start = max(0, self.current_index - 5)
                new_count = min(11, len(self.main_playlist) - new_start)
                ...
                self._display_start_index = new_start
                self._displayed_items_count = new_count
                
                # Mettre en surbrillance la chanson courante
                children = self.playlist_container.winfo_children()
                relative_index = self.current_index - new_start
                if 0 <= relative_index < len(children):
                    self._highlight_current_song_widget(children[relative_index])
            ...
            print(f"Erreur lors de la vérification de visibilité: {e}")
...
def _update_canvas_scroll_region(self):
        ...
        try:
            ...
            try:
                current_scroll_pos = self.playlist_canvas.yview()
            except:
            ...
            try:
                ...
                if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
                    print(f"Erreur lors de update_idletasks: {e}")
                return
...
def _setup_infinite_scroll(self):
        ...
        try:
            ...
            def on_scroll_position_change(*args):
                ...
                # Mettre à jour l'affichage basé sur la nouvelle position
                self._update_display_based_on_scroll_position()
            
            # Connecter le callback à la scrollbar avec une meilleure gestion
            if hasattr(self, 'playlist_scrollbar') and self.playlist_scrollbar:
                # Sauvegarder la fonction originale de yview
                original_yview = self.playlist_canvas.yview
                
                # Créer une nouvelle fonction qui appelle à la fois yview et notre callback
                def enhanced_yview(*args):
                    original_yview(*args)
                    on_scroll_position_change()
                
                # Configurer la scrollbar avec notre nouvelle fonction
                self.playlist_scrollbar.config(command=enhanced_yview)
                
                # Ajouter des bindings spécifiques pour la scrollbar
                self.playlist_scrollbar.bind("<B1-Motion>", lambda e: on_scroll_position_change())
                self.playlist_scrollbar.bind("<ButtonRelease-1>", lambda e: on_scroll_position_change())
            ...
            # Ajouter des bindings pour le drag de la scrollbar
            self.playlist_canvas.bind("<B1-Motion>", lambda e: self._mark_user_scrolling())
            self.playlist_canvas.bind("<ButtonRelease-1>", lambda e: self._update_display_based_on_scroll_position())
...
def _update_display_based_on_scroll_position(self):
        ...
        try:
            ...
            # Réduire le seuil pour une mise à jour plus réactive
            threshold = 3  # Mettre à jour si on a bougé de plus de 3 éléments
            ...
            if (abs(new_start - current_start) > threshold or 
                ...
                # Utiliser after_idle pour éviter les problèmes de synchronisation
                self.root.after_idle(lambda: self._update_windowed_display(new_start, new_end, center_index))
...
def _update_windowed_display(self, start_index, end_index, center_index):
        ...
        try:
            ...
            existing_children = self.playlist_container.winfo_children()
            
            # Créer un dictionnaire des widgets existants par index pour réutilisation
            existing_widgets = {}
            for child in existing_children:
                if hasattr(child, 'song_index'):
                    existing_widgets[child.song_index] = child
            
            # Vider le container actuel mais sans détruire immédiatement les widgets
            # pour éviter le scintillement
            for child in existing_children:
                child.pack_forget()
            
            # Ajouter les nouveaux éléments ou réutiliser les existants
            for i in range(start_index, end_index):
                if i < len(self.main_playlist):
                    filepath = self.main_playlist[i]
                    
                    # Vérifier si on peut réutiliser un widget existant
                    if i in existing_widgets and existing_widgets[i].winfo_exists():
                        # Réutiliser le widget existant
                        widget = existing_widgets[i]
                        widget.pack(fill='x', pady=2, padx=5)
                        # Le retirer du dictionnaire pour ne pas le détruire plus tard
                        del existing_widgets[i]
                    else:
                        # Créer un nouveau widget
                        self._add_main_playlist_item(filepath, song_index=i)
            
            # Détruire les widgets qui ne sont plus utilisés
            ...
            if (hasattr(self, 'current_index') and 
                ...
                try:
                    ...
                    children = self.playlist_container.winfo_children()
                    for child in children:
                        if hasattr(child, 'song_index') and child.song_index == self.current_index:
                            self.select_playlist_item(child, auto_scroll=False)
                            break
                except Exception as e:
                    if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
                        print(f"Erreur lors de la mise en surbrillance: {e}")
...
def _setup_progressive_scroll_detection(self):
        ...
        try:
            ...
            # Ajouter le binding pour détecter le scroll vers le bas
            self.playlist_canvas.bind("<MouseWheel>", self._on_progressive_scroll)
            self.playlist_canvas.bind("<Button-4>", self._on_progressive_scroll) # Linux
            self.playlist_canvas.bind("<Button-5>", self._on_progressive_scroll) # Linux
            
            if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
                print("📜 Détection scroll progressif configurée")
...
def _on_progressive_scroll(self, event=None):
        ...
        try:
            ...
            try:
                ...
                if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
                    print(f"📜 Scroll position: {scroll_position:.2f}, seuil: {threshold}")
                ...
                if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
                    print(f"⚠️ Erreur détection position scroll: {e}")
                
        except Exception as e:
            if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
                print(f"❌ Erreur gestion scroll progressif: {e}")
