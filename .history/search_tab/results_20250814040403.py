...
def _display_batch_results(self, batch_number):
    ...
    # Afficher tous les résultats du batch en une fois avec un délai minimal
    if batch_results and not self.search_cancelled:
        self.root.after(10, lambda: self._display_batch_results_fast(batch_results))
...
def _display_batch_results_fast(self, batch_results):
    ...
    for i, (video, idx) in enumerate(batch_results):
        ...
        # Afficher avec un délai minimal pour éviter de bloquer l'UI
        delay = i * 5  # 5ms entre chaque résultat au lieu de 100ms
...
def _try_restore_from_cache(self, search_query):
    ...
    try:
        ...
        # Couleur verte pour indiquer le cache
        try:
            if hasattr(self, 'status_bar'):
                self.status_bar.config(fg='#00ff00')  # Vert
                # Revenir à la couleur normale après 3 secondes
                self.safe_after(3000, reset_status_color)
        except:
            pass
...
def _display_search_results_from_cache(self, results):
    ...
    # Affichage par lots ultra-rapide pour le cache (lots plus gros)
    batch_size = 15  # Plus gros lots pour le cache
    delay = 2  # Délai très court entre les lots
    ...
    def display_batch(batch_start):
        try:
            ...
            for i in range(batch_start, batch_end):
                if i < len(results):
                    self._add_search_result(results[i], i)
