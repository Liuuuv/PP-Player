...
def _add_main_playlist_item(self, filepath, thumbnail_path=None, song_index=None):
        ...
        try:
            ...
            # Attribut pour suivre l'état de survol global de l'élément
            item_frame.hover_state = False
            # ID pour le délai d'entrée
            item_frame.enter_after_id = None
            
            # Définir les widgets à lier avant de les utiliser dans les fonctions
            widgets_to_bind = [item_frame]
            if show_numbers and number_label:
                widgets_to_bind.append(number_label)
            widgets_to_bind.extend([thumbnail_label, text_frame, title_label, duration_label])
            # Filtrer les widgets None
            widgets_to_bind = [w for w in widgets_to_bind if w is not None]
            
            def on_enter(event):
                """Rend l'item plus clair au survol avec un délai pour éviter les conflits"""
                # Annuler tout délai d'entrée précédent
                if hasattr(item_frame, 'enter_after_id') and item_frame.enter_after_id:
                    item_frame.after_cancel(item_frame.enter_after_id)
                    item_frame.enter_after_id = None
                
                # Programmer l'entrée avec un court délai pour permettre à on_leave de s'exécuter d'abord
                def delayed_enter():
                    if item_frame.is_dragging:
                        return  # Ne pas changer la couleur si on est en train de drag
                    
                    # Mettre à jour l'état de survol
                    item_frame.hover_state = True
                    ...
                    # Réinitialiser l'ID du délai
                    item_frame.enter_after_id = None
                
                # Définir un délai court (5ms) pour permettre à on_leave de s'exécuter d'abord
                item_frame.enter_after_id = item_frame.after(5, delayed_enter)

            def on_leave(event):
                """Restaure la couleur originale quand la souris quitte l'item"""
                # Obtenir le widget sous la souris après l'événement leave
                x, y = event.widget.winfo_pointerxy()
                widget_under_mouse = event.widget.winfo_containing(x, y)
                
                # Vérifier si le widget sous la souris fait partie des widgets liés à cet élément
                is_still_in_item = False
                for widget in widgets_to_bind:
                    if widget is widget_under_mouse or (widget.winfo_ismapped() and widget.winfo_containing(x, y) is widget_under_mouse):
                        is_still_in_item = True
                        break
                
                # Si la souris est toujours sur un widget de l'élément, ne rien faire
                if is_still_in_item:
                    return
                
                # Annuler tout délai d'entrée en cours
                if hasattr(item_frame, 'enter_after_id') and item_frame.enter_after_id:
                    item_frame.after_cancel(item_frame.enter_after_id)
                    item_frame.enter_after_id = None
            ...
            # widgets_to_bind est déjà défini plus haut
