...
def _save_current_search_state(self):
 ...
 État de recherche sauvegardé dans le cache:")
...
def _try_restore_from_cache(self, search_query):
 ...
 Appel de _display_search_results_from_cache avec {len(self.all_search_results)} résultats")
        try:
            self._display_search_results_from_cache(self.all_search_results)
    
        except Exception as e:
     Erreur dans _display_search_results_from_cache: {e}")
            import traceback
            traceback.print_exc()
        ...
        if interface_state and interface_state.get('scroll_position'):
            def restore_scroll():
             ...
             Position de scroll restaurée: {scroll_top}")
 ...
 Erreur lors de la restauration depuis le cache: {e}")
        return False
...
def _show_current_song_thumbnail(self):
 ...
 thumbnail_frame n'existe pas comme attribut")
        # Essayer de recréer la frame
        if hasattr(self, '_recreate_thumbnail_frame'):
     tentative de recréation via _recreate_thumbnail_frame")
            self._recreate_thumbnail_frame()
        else:
     impossible de recréer, _recreate_thumbnail_frame n'existe pas")
            return
        

    if not self.thumbnail_frame.winfo_exists():
 thumbnail_frame existe comme attribut mais widget détruit")
        # Essayer de recréer la frame
        if hasattr(self, '_recreate_thumbnail_frame'):
     tentative de recréation via _recreate_thumbnail_frame")
            self._recreate_thumbnail_frame()
     ...
     impossible de recréer, _recreate_thumbnail_frame n'existe pas")
        ...
        pass
 thumbnail_frame existe et le widget est valide")
 ...
 Label miniature chanson créé et packé avec succès")
 ...
 Tentative de chargement de la vraie miniature")

        
    else:
 Aucune chanson en cours, affichage miniature par défaut")
 ...
 Label miniature par défaut créé et packé avec succès")
...
def _return_to_search(self):
 ...
 === DÉBUT _return_to_search ===")
     ...
     Widgets dans youtube_results_frame AVANT nettoyage: {len(children)}")
 ...
 Suppression du bouton retour playlist...")
     ...
     attribut artist_tab_back_btn remis à None")
        ...
        if hasattr(self, 'youtube_results_frame'):
            try:
                children = self.youtube_results_frame.winfo_children()
         Widgets dans youtube_results_frame APRÈS suppression widgets artiste: {len(children)}")
                for i, child in enumerate(children):
         ...
         Erreur lors du listage des widgets après suppression: {e}")
        
        # CORRECTION: Nettoyer tous les frames enfants non essentiels de youtube_results_frame
 Nettoyage des frames enfants non essentiels...")
        if hasattr(self, 'youtube_results_frame'):
            try:
                ...
                for child in children:
                    try:
                     ...
                     Conservation du widget essentiel: {child.__class__.__name__}")
                            continue
                        
                        # Supprimer tous les autres frames (main_container et autres)
                 Suppression du widget non essentiel: {child.__class__.__name__} (name: {child.winfo_name()})")
                        child.destroy()
                        
                    except Exception as e:
                 Erreur lors de la suppression d'un widget: {e}")
            except Exception as e:
         Erreur lors du nettoyage des frames: {e}")
        
        # Debug: Lister les widgets APRÈS nettoyage complet
        if hasattr(self, 'youtube_results_frame'):
            try:
                children = self.youtube_results_frame.winfo_children()
         Widgets dans youtube_results_frame APRÈS nettoyage complet: {len(children)}")
                for i, child in enumerate(children):
                    try:
                        widget_class = child.__class__.__name__
                        widget_info = f"  - Widget {i}: {widget_class}"
         ...
         Erreur lors du listage final: {e}")
        
        # Restaurer l'état original des résultats de recherche
        had_search_results = False
        if hasattr(self, 'saved_search_state'):
     Restauration de l'état de recherche - Query: '{self.saved_search_state.get('search_query', '')}', Results: {len(self.saved_search_state.get('search_results', []))}")
 ...
 Vérification et affichage de thumbnail_frame...")
        if hasattr(self, 'thumbnail_frame'):
            try:
                if self.thumbnail_frame.winfo_exists():
             thumbnail_frame existe, forçage du pack...")
             ...
             thumbnail_frame forcée visible: {self.thumbnail_frame.winfo_ismapped()}")
                else:
             thumbnail_frame détruite, recréation...")
                    # thumbnail_frame détruite, la recréer
                    self._recreate_thumbnail_frame()
            except tk.TclError as e:
         TclError avec thumbnail_frame: {e}, recréation...")
                # thumbnail_frame détruite, la recréer
                self._recreate_thumbnail_frame()
        else:
     thumbnail_frame n'existe pas, création...")
        ...
        if hasattr(self, 'youtube_results_frame'):
            try:
                children = self.youtube_results_frame.winfo_children()
         Widgets dans youtube_results_frame ÉTAT FINAL: {len(children)}")
                for i, child in enumerate(children):
                    try:
                        widget_class = child.__class__.__name__
                        widget_info = f"  - Widget {i}: {widget_class}"
 ...
 === FIN _return_to_search ===")
        
    except Exception as e:
 ERREUR DANS _return_to_search: {e}")
...
def _add_search_result(self, video, index):
 ...
 Ajout du résultat de recherche {index}: {video.get('title', 'Sans titre')}")
        if self.search_cancelled:
            return
        try:
                ...
                if artist or album:
                    ...
                    if artist:
                        ...
                        def on_artist_click(event, artist_name=artist, video_data=video):
                            try:
                         Clic sur artiste: {artist_name}")
                         Video data keys: {list(video_data.keys())}")
...
def _recreate_thumbnail_frame(self):
    ...
    if hasattr(self, 'youtube_results_frame'):
        ...
        children = self.youtube_results_frame.winfo_children()
 Widgets dans youtube_results_frame AVANT recréation: {len(children)}")
        for i, child in enumerate(children):
    ...
    if hasattr(self, 'youtube_results_frame'):
        ...
        children = self.youtube_results_frame.winfo_children()
 Widgets dans youtube_results_frame APRÈS recréation: {len(children)}")
        for i, child in enumerate(children):
            widget_class = child.__class__.__name__
            widget_info = f"  - Widget {i}: {widget_class}"
            if hasattr(child, 'winfo_name'):
...
def _display_search_results(self, results, scroll_position=None):
    ...
    try:
 _display_search_results appelée avec {len(results)} résultats")
     ...
     Restauration rapide - {num_results} résultats, {batches_needed} batches")
        ...
        if scroll_position:
     Restauration de la position de scroll: {scroll_position}")
 ...
 Restauration de {num_results} résultats programmée via _display_batch_results")
        
    except Exception as e:
 Erreur dans _display_search_results: {e}")
    ...
    if hasattr(self, 'youtube_canvas') and hasattr(self, 'results_container'):
        if self.youtube_canvas.winfo_exists() and self.results_container.winfo_exists():
            ...
            if bbox:
             ...
             Grande scrollregion mise à jour: hauteur={bbox[3]}")
...
def _restore_scroll_position(self, scroll_position):
 ...
 Application de la position de scroll: top={scroll_top}, bottom={scroll_bottom}")
 ...
 Scrollregion actuelle: {scrollregion}")
        
        # Appliquer la position de scroll
        self.youtube_canvas.yview_moveto(scroll_top)
 Position de scroll restaurée avec succès")
