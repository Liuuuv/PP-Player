...
def generate_waveform_preview(self, filepath):
    ...
    try:
        ...
        # Debug info
        print(f"DEBUG generate_waveform_preview:")
        print(f"  - Samples shape: {samples.shape}")
        print(f"  - Max value: {max_val}")
        print(f"  - Non-zero samples: {np.count_nonzero(self.waveform_data_raw)}/{len(self.waveform_data_raw)}")
        print(f"  - First 10 values: {self.waveform_data_raw[:10]}")
        print(f"  - Middle 10 values: {self.waveform_data_raw[len(self.waveform_data_raw)//2:len(self.waveform_data_raw)//2+10]}")
...
def get_adaptive_waveform_data(self, canvas_width=None):
    ...
    print(f"DEBUG get_adaptive_waveform_data:")
    print(f"  - song_length: {self.song_length}")
    print(f"  - target_resolution: {target_resolution}")
    print(f"  - waveform_data_raw length: {len(self.waveform_data_raw)}")
...
def draw_waveform_around(self, time_sec, window_sec=5):
    if self.waveform_data_raw is None:
        print("DEBUG draw_waveform_around: waveform_data_raw is None")
        return
    
    print(f"DEBUG draw_waveform_around: waveform_data_raw shape: {self.waveform_data_raw.shape}")
    print(f"DEBUG draw_waveform_around: waveform_data_raw non-zero: {np.count_nonzero(self.waveform_data_raw)}")

    total_length = self.song_length
    if total_length == 0:
        print("DEBUG draw_waveform_around: song_length is 0")
        return
    ...
    if adaptive_data is None:
        print("DEBUG draw_waveform_around: adaptive_data is None")
        return
    
    print(f"DEBUG draw_waveform_around: adaptive_data shape: {adaptive_data.shape}")
    print(f"DEBUG draw_waveform_around: adaptive_data non-zero: {np.count_nonzero(adaptive_data)}")
