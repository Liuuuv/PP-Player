                ...
                if hasattr(self, 'playlist_canvas') and self.playlist_canvas:
                    ...
                    def forward_wheel(event, canvas=self.playlist_canvas):
                        ...
                        if hasattr(event, 'delta') and event.delta:
                            ...
                            print("canvas.yview_scroll(int(-1*(event.delta/120)), units)")
                        else:
...
def _clear_main_playlist(self, event=None):
    ...
    self.reset_main_playlist()
    self.go_to_top(self.playlist_canvas)
...
def _update_canvas_scroll_region(self):
        ...
        try:
            ...
            # Inclure le nombre d'enfants dans la vérification de changement
            current_state = (current_width, current_height, children_count)
            
            if (not hasattr(self, '_last_container_state') or 
                self._last_container_state != current_state):
                # Forcer la mise à jour de la géométrie seulement si nécessaire
                self.playlist_container.update_idletasks()
                self._last_container_state = current_state
                
                if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
                    print(f"📐 Géométrie mise à jour: {current_width}x{current_height}, {children_count} enfants")
            ...
            children = self.playlist_container.winfo_children()
            # children_count déjà calculé plus haut
        ...
        try:
            ...
            try:
                ...
                if scroll_top <= scroll_threshold:
                    # Proche du haut, charger plus d'éléments au-dessus
                    # if not self.main_playlist_is_loading_more_items:
                    self._load_more_songs_above()
                    # self.main_playlist_is_loading_more_items = True
                elif scroll_bottom >= (1.0 - scroll_threshold):
                    # Proche du bas, charger plus d'éléments en-dessous
...
def _load_more_songs_above(self):
        ...
        try:
            ...
            if hasattr(self, '_loading_above_in_progress') and self._loading_above_in_progress:
                if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
            ...
            self._loading_above_in_progress = True
            ...
            def reset_loading_flag():
                self._loading_above_in_progress = False
                if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
                    print("✅ Flag de chargement vers le haut réinitialisé")
            
            # Délai plus long pour éviter les chargements répétés
            self.root.after(500, reset_loading_flag)
...
def _extend_window_up(self, new_start):
        ...
        try:
            ...
            for i in range(new_start, current_start):
                if i >= 0 and i < len(self.main_playlist):
                    filepath = self.main_playlist[i]
...
def _extend_window_down(self, new_end):
        ...
        try:
            ...
            # Forcer la mise à jour de la géométrie avant de recalculer la région de scroll
            self.playlist_container.update_idletasks()
            
            # Mettre à jour la région de scroll avec un délai pour s'assurer que les nouveaux éléments sont rendus
            def update_scroll_region_delayed():
                try:
                    self._update_canvas_scroll_region()
                    if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
                        print(f"✅ Région de scroll mise à jour après extension vers le bas")
                except Exception as e:
                    print(f"❌ Erreur mise à jour région scroll: {e}")
            
            self.root.after(10, update_scroll_region_delayed)
            
            # Forcer aussi la mise à jour de la scrollbar
            def force_scrollbar_update():
                try:
                    if hasattr(self, 'playlist_canvas') and self.playlist_canvas.winfo_exists():
                        # Forcer la mise à jour de la scrollbar en déclenchant un événement de scroll
                        current_view = self.playlist_canvas.yview()
                        self.playlist_canvas.yview_moveto(current_view[0])
                        
                        if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
                            print(f"🔄 Scrollbar forcée à se mettre à jour")
                except Exception as e:
                    print(f"❌ Erreur force scrollbar update: {e}")
            
            self.root.after(20, force_scrollbar_update)
