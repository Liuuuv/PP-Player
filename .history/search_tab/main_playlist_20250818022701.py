...
def _display_main_playlist(self):
        """Affiche la main playlist avec optimisation windowing si nécessaire"""
        try:
            ...
            # Vérifier si on doit utiliser le windowing
            try:
                from search_tab.config import should_use_windowing
                use_windowing = should_use_windowing(playlist_size)
                print(f"🪟 DEBUG: Windowing activé: {use_windowing}")
            except ImportError:
                use_windowing = False
                print("⚠️ DEBUG: Configuration windowing non disponible, affichage complet")
            
            if use_windowing:
                # Utiliser le système de windowing optimisé
                self._display_windowed_playlist_original()
            else:
                # Affichage complet pour petites playlists
                self._display_full_playlist_original()
...
def _display_full_playlist_original(self):
        """Affiche tous les éléments de la playlist (mode complet original)"""
        try:
            print("📋 DEBUG: Affichage complet de la playlist")
            ...
            current_playing_index = getattr(self, 'current_song_index', -1)
            for index, filepath in enumerate(self.main_playlist):
                is_current = (index == current_playing_index)
                self._add_main_playlist_item(filepath, song_index=index, is_current_playing=is_current)
            
            # Forcer la mise à jour de la région de scroll
            self._update_scroll_region_simple()
            
            print(f"✅ DEBUG: {len(self.main_playlist)} éléments affichés (mode complet)")
            
        except Exception as e:
            print(f"❌ DEBUG: Erreur affichage complet: {e}")

def _display_windowed_playlist_original(self):
        """Affiche la playlist avec windowing original (10 avant + 10 après)"""
        try:
            print("🪟 DEBUG: Affichage avec windowing original")
            
            # Déterminer la position centrale (chanson en cours ou milieu)
            current_playing_index = getattr(self, 'current_song_index', 0)
            playlist_size = len(self.main_playlist)
            
            # Calculer la fenêtre : 10 avant + 1 courante + 10 après
            songs_before = 10
            songs_after = 10
            
            start_index = max(0, current_playing_index - songs_before)
            end_index = min(playlist_size, current_playing_index + songs_after + 1)
            
            # Ajuster si on est près des bords pour garder 21 éléments si possible
            window_size = 21
            if end_index - start_index < window_size and playlist_size > window_size:
                if start_index == 0:
                    end_index = min(playlist_size, window_size)
                elif end_index == playlist_size:
                    start_index = max(0, playlist_size - window_size)
            
            print(f"🪟 DEBUG: Fenêtre [{start_index}:{end_index}] autour de la chanson {current_playing_index}")
            
            # Vider le container
            for widget in self.playlist_container.winfo_children():
                widget.destroy()
            
            # Ajouter l'indicateur du début si nécessaire
            if start_index > 0:
                self._add_simple_indicator(f"... {start_index} musiques précédentes", "prev", start_index)
            
            # Ajouter les éléments de la fenêtre
            for index in range(start_index, end_index):
                if index < playlist_size:
                    filepath = self.main_playlist[index]
                    is_current = (index == current_playing_index)
                    self._add_main_playlist_item(filepath, song_index=index, is_current_playing=is_current)
            
            # Ajouter l'indicateur de fin si nécessaire
            if end_index < playlist_size:
                remaining = playlist_size - end_index
                self._add_simple_indicator(f"... {remaining} musiques suivantes", "next", end_index)
            
            # Mettre à jour la région de scroll
            self._update_scroll_region_simple()
            
            print(f"✅ DEBUG: Fenêtre affichée avec {end_index - start_index} éléments")
            
        except Exception as e:
            print(f"❌ DEBUG: Erreur affichage windowed: {e}")

def _add_simple_indicator(self, text, direction, target_index):
        """Ajoute un indicateur simple de navigation"""
        try:
            ...
            def navigate():
                if direction == "prev":
                    # Aller vers les musiques précédentes
                    new_center = max(0, target_index - 15)
                else:  # next
                    # Aller vers les musiques suivantes
                    new_center = min(len(self.main_playlist) - 1, target_index + 15)
                
                # Changer la position de vue (pas la position de lecture)
                self.current_song_index = new_center
                print(f"🔄 DEBUG: Navigation vers {direction}, nouveau centre: {new_center}")
                self._display_windowed_playlist_original()
            
            indicator_label.bind("<Button-1>", lambda e: navigate())
            
            # Effet visuel au survol
            def on_enter(e):
                indicator_label.config(fg='#aaaaaa', bg='#3d3d3d')
            
            def on_leave(e):
                indicator_label.config(fg='#888888', bg='#2d2d2d')
            
            indicator_label.bind("<Enter>", on_enter)
            indicator_label.bind("<Leave>", on_leave)
            
        except Exception as e:
            print(f"❌ DEBUG: Erreur création indicateur simple: {e}")

def _update_scroll_region_simple(self):
        """Met à jour la région de scroll de manière simple"""
        try:
            ...
            # Configurer la région de scroll
            self.playlist_canvas.configure(scrollregion=(0, 0, 0, total_height))
            
            new_region = self.playlist_canvas.cget('scrollregion')
            print(f"📏 DEBUG: Nouvelle région de scroll: {new_region}")
            
        except Exception as e:
            print(f"❌ DEBUG: Erreur mise à jour région scroll simple: {e}")

def _display_windowed_playlist(self):
        """Affiche la playlist avec le système de windowing optimisé"""
        try:
            print("🪟 DEBUG: Affichage avec windowing")
            
            # Utiliser la position de vue (pas la position de lecture)
            view_center = getattr(self, 'playlist_view_center', getattr(self, 'current_song_index', 0))
            current_playing_index = getattr(self, 'current_song_index', 0)
            
            start_index, end_index = self._calculate_smart_window_bounds(view_center)
            
            print(f"🪟 DEBUG: Fenêtre [{start_index}:{end_index}] centrée sur vue {view_center}, lecture {current_playing_index}")
            
            # Vider le container
            for widget in self.playlist_container.winfo_children():
                widget.destroy()
            
            # Ajouter les indicateurs et éléments de la fenêtre
            self._add_smart_window_indicators(start_index, end_index, view_center)
            
            # Ajouter les éléments de la fenêtre
            for index in range(start_index, min(end_index, len(self.main_playlist))):
                filepath = self.main_playlist[index]
                # Marquer si c'est la chanson en cours de lecture
                is_current_playing = (index == current_playing_index)
                self._add_main_playlist_item(filepath, song_index=index, is_current_playing=is_current_playing)
            
            # Mettre à jour la région de scroll
            self._update_scroll_region_windowed(start_index, end_index)
...
def _calculate_window_bounds(self, center_index):
        """Calcule les bornes de la fenêtre d'affichage (ancienne version)"""
        try:
            from search_tab.config import get_main_playlist_config, get_optimal_window_size
            
            playlist_size = len(self.main_playlist)
            window_size = get_optimal_window_size(playlist_size)
            songs_before = get_main_playlist_config('songs_before_current')
            songs_after = get_main_playlist_config('songs_after_current')
            
            # Calculer les bornes
            start_index = max(0, center_index - songs_before)
            end_index = min(playlist_size, center_index + songs_after + 1)
            ...
            print(f"❌ DEBUG: Erreur calcul bornes: {e}")
            # Fallback
            return max(0, center_index - 10), min(len(self.main_playlist), center_index + 11)

def _calculate_smart_window_bounds(self, view_center):
        """Calcule les bornes intelligentes pour éviter les interruptions"""
        try:
            from search_tab.config import get_main_playlist_config
            
            playlist_size = len(self.main_playlist)
            songs_before = get_main_playlist_config('songs_before_current')  # 10
            songs_after = get_main_playlist_config('songs_after_current')    # 10
            
            # Fenêtre de base : 10 avant + 1 centre + 10 après
            base_start = max(0, view_center - songs_before)
            base_end = min(playlist_size, view_center + songs_after + 1)
            
            # Vérifier s'il faut étendre pour éviter les interruptions
            current_window = getattr(self, '_current_window_bounds', None)
            
            if current_window:
                current_start, current_end = current_window
                
                # Si l'utilisateur navigue dans la fenêtre actuelle, l'étendre intelligemment
                if current_start <= view_center <= current_end:
                    # Étendre la fenêtre pour permettre la navigation continue
                    extended_buffer = 25  # Buffer pour navigation fluide
                    
                    smart_start = max(0, min(base_start, current_start - extended_buffer))
                    smart_end = min(playlist_size, max(base_end, current_end + extended_buffer))
                    
                    print(f"🧠 DEBUG: Extension intelligente [{smart_start}:{smart_end}] pour navigation fluide")
                    
                    # Limiter la taille maximale pour éviter les problèmes de performance
                    max_window_size = 100
                    if smart_end - smart_start > max_window_size:
                        # Recentrer sur la vue actuelle
                        half_max = max_window_size // 2
                        smart_start = max(0, view_center - half_max)
                        smart_end = min(playlist_size, smart_start + max_window_size)
                        print(f"🔧 DEBUG: Limitation taille fenêtre à [{smart_start}:{smart_end}]")
                    
                    self._current_window_bounds = (smart_start, smart_end)
                    return smart_start, smart_end
            
            # Nouvelle fenêtre ou saut important
            print(f"🆕 DEBUG: Nouvelle fenêtre de base [{base_start}:{base_end}]")
            self._current_window_bounds = (base_start, base_end)
            return base_start, base_end
            
        except Exception as e:
            print(f"❌ DEBUG: Erreur calcul bornes intelligentes: {e}")
            # Fallback sécurisé
            start = max(0, view_center - 10)
            end = min(len(self.main_playlist), view_center + 11)
            self._current_window_bounds = (start, end)
            return start, end

def _add_window_indicators(self, start_index, end_index):
        """Ajoute les indicateurs de navigation (... X musiques précédentes) - ancienne version"""
        try:
            from search_tab.config import get_main_playlist_config
            
            if not get_main_playlist_config('show_navigation_indicators'):
                return
            ...
            # Indicateur du début
            if start_index > 0:
                self._add_navigation_indicator(f"... {start_index} musiques précédentes", "prev", start_index)
            
            # Indicateur de la fin (ajouté après les éléments)
            if end_index < playlist_size:
                remaining = playlist_size - end_index
                # On l'ajoutera après les éléments dans _display_windowed_playlist
                self._pending_end_indicator = (f"... {remaining} musiques suivantes", "next", end_index)
            else:
                self._pending_end_indicator = None
                
        except Exception as e:
            print(f"❌ DEBUG: Erreur indicateurs: {e}")

def _add_smart_window_indicators(self, start_index, end_index, view_center):
        """Ajoute les indicateurs intelligents qui ne changent pas la position de lecture"""
        try:
            from search_tab.config import get_main_playlist_config
            ...
            # Indicateur du début
            if start_index > 0:
                self._add_smart_navigation_indicator(
                    f"... {start_index} musiques précédentes", 
                    "prev", 
                    start_index, 
                    view_center
                )
            ...
            if end_index < playlist_size:
                remaining = playlist_size - end_index
                self._pending_end_indicator = (
                    f"... {remaining} musiques suivantes", 
                    "next", 
                    end_index, 
                    view_center
                )
            ...
            print(f"❌ DEBUG: Erreur indicateurs intelligents: {e}")

def _add_navigation_indicator(self, text, direction, target_index):
        """Ajoute un indicateur de navigation cliquable (ancienne version)"""
        try:
            import tkinter as tk
            ...
            # Rendre cliquable pour navigation
            def navigate_to():
                if direction == "prev":
                    new_center = max(0, target_index - 10)
                else:  # next
                    new_center = min(len(self.main_playlist) - 1, target_index + 10)
                self.current_song_index = new_center
                self._display_windowed_playlist()
            
            indicator_label.bind("<Button-1>", lambda e: navigate_to())
            
        except Exception as e:
            print(f"❌ DEBUG: Erreur création indicateur: {e}")

def _add_smart_navigation_indicator(self, text, direction, target_index, current_view_center):
        """Ajoute un indicateur de navigation intelligent qui ne change pas la position de lecture"""
        try:
            import tkinter as tk
            ...
            # Navigation intelligente qui change seulement la VUE
            def smart_navigate():
                if direction == "prev":
                    # Aller vers les musiques précédentes
                    new_view_center = max(0, target_index - 15)
                    print(f"🔼 DEBUG: Navigation vers précédentes, nouvelle vue centrée sur {new_view_center}")
                else:  # next
                    # Aller vers les musiques suivantes
                    new_view_center = min(len(self.main_playlist) - 1, target_index + 15)
                    print(f"🔽 DEBUG: Navigation vers suivantes, nouvelle vue centrée sur {new_view_center}")
                
                # IMPORTANT: Changer seulement la position de VUE, pas la position de LECTURE
                self.playlist_view_center = new_view_center
                # Ne PAS changer self.current_song_index !
                
                # Rafraîchir l'affichage avec la nouvelle vue
                self._display_windowed_playlist()
            
            indicator_label.bind("<Button-1>", lambda e: smart_navigate())
            ...
            print(f"❌ DEBUG: Erreur création indicateur intelligent: {e}")

def _update_scroll_region_manual(self):
        """Met à jour la région de scroll manuellement"""
        try:
            self.playlist_container.update_idletasks()
            
            # Calculer la hauteur totale manuellement
            total_height = 0
            for child in self.playlist_container.winfo_children():
                total_height += child.winfo_reqheight()
            ...
            # Configurer la région de scroll manuellement
            self.playlist_canvas.configure(scrollregion=(0, 0, 0, total_height))
            
            # Vérifier la nouvelle région
            new_region = self.playlist_canvas.cget('scrollregion')
            print(f"📏 DEBUG: Nouvelle région de scroll: {new_region}")
            
        except Exception as e:
            print(f"❌ DEBUG: Erreur mise à jour région scroll: {e}")

def _update_scroll_region_windowed(self, start_index, end_index):
        """Met à jour la région de scroll pour le mode windowed"""
        try:
            # Calculer la hauteur réelle des éléments affichés
            self._update_scroll_region_manual()
            
            # Ajouter l'indicateur de fin si nécessaire
            if hasattr(self, '_pending_end_indicator') and self._pending_end_indicator:
                if len(self._pending_end_indicator) == 4:
                    # Nouvel indicateur intelligent
                    text, direction, target_index, view_center = self._pending_end_indicator
                    self._add_smart_navigation_indicator(text, direction, target_index, view_center)
                else:
                    # Ancien indicateur (fallback)
                    text, direction, target_index = self._pending_end_indicator
                    self._add_navigation_indicator(text, direction, target_index)
                # Recalculer après ajout de l'indicateur
                self._update_scroll_region_manual()
            
        except Exception as e:
            print(f"❌ DEBUG: Erreur mise à jour région windowed: {e}")

def _add_main_playlist_item(self, filepath, thumbnail_path=None, song_index=None, is_current_playing=False):
        ...
        try:
            ...
            # 1. Frame principal - couleur selon si c'est la chanson en cours
            if is_current_playing:
                # Chanson en cours de lecture - couleur spéciale
                bg_color = COLOR_SELECTED
                print(f"🎵 DEBUG: Chanson en cours (index {song_index}) mise en surbrillance")
            else:
                # Chanson normale
                bg_color = '#4a4a4a'
