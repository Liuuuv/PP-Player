...
def _save_current_search_state(self):
        ...
        print(f"DEBUG: État de recherche sauvegardé:")
        print(f"  - Query: '{self.saved_search_state['search_query']}'")
        print(f"  - Results: {len(self.saved_search_state['search_results'])}")
        print(f"  - current_search_batch: {self.saved_search_state['current_search_batch']}")
        print(f"  - has_more_results: {self.saved_search_state['has_more_results']}")
        print(f"  - search_results_count: {getattr(self, 'search_results_count', 'NOT_SET')}")
        print(f"  - all_search_results length: {len(getattr(self, 'all_search_results', []))}")
...
def _should_load_more_results(self):
    ...
    print(f"DEBUG _should_load_more_results:")
    ...
    print(f"  - has_more_results: {getattr(self, 'has_more_results', 'NOT_SET')}")
    ...
    if (self.is_loading_more or 
        ...
        print(f"DEBUG: Conditions bloquantes détectées")
        return False
    
    # Vérifier si on a encore des résultats à charger
    if not getattr(self, 'has_more_results', False):
        print(f"DEBUG: has_more_results = False, pas de chargement")
        return False
    ...
    try:
        ...
        scroll_top, scroll_bottom = self.youtube_canvas.yview()
        print(f"DEBUG: Position scroll: top={scroll_top:.2f}, bottom={scroll_bottom:.2f}")
        
        # CORRECTION: Réduire le seuil à 70% car la scrollbar ne va pas jusqu'à 100%
        # Cela est dû à un problème de scrollregion qui sera corrigé séparément
        if scroll_bottom > 0.65:  # Réduit de 80% à 65%
            print(f"DEBUG: Scroll à {scroll_bottom:.2f} > 65%, chargement autorisé!")
            return True
        
        print(f"DEBUG: Scroll à {scroll_bottom:.2f} < 65%, pas de chargement")
        return False
    except Exception as e:
...
def _display_search_results(self, results, scroll_position=None):
    ...
    try:
        ...
        print(f"DEBUG: État restauré:")
        print(f"  - batches_needed: {batches_needed}")
        print(f"  - has_more_results: {has_more}")
        print(f"  - current_search_query: '{query}'")
        ...
        if batches_needed > 1:
            for batch in range(2, batches_needed + 1):
                print(f"DEBUG: Affichage du batch {batch}")
        ...
        total_delay = num_results * SEARCH_WAIT_TIME_BETWEEN_RESULTS + 100
        ...
        if hasattr(self, 'youtube_canvas') and hasattr(self, 'results_container'):
            if self.youtube_canvas.winfo_exists() and self.results_container.winfo_exists():
                ...
                if bbox:
                    ...
                    print(f"DEBUG: Scrollregion mise à jour: {bbox}")
                else:
                    print("DEBUG: Aucune bbox trouvée pour la scrollregion")
    except Exception as e:
