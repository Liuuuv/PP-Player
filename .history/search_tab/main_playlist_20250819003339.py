...
def _refresh_windowed_playlist_display(self, force_recreate=False):
        ...
        try:
            # Éviter les rafraîchissements pendant le scroll actif
            if hasattr(self, '_processing_scroll') and self._processing_scroll:
                # Programmer un rafraîchissement différé après la fin du scroll
                if not hasattr(self, '_delayed_refresh_scheduled') or not self._delayed_refresh_scheduled:
                    self._delayed_refresh_scheduled = True
                    self.root.after(500, lambda: self._delayed_refresh())
                return
...
def _update_canvas_scroll_region(self):
        ...
        try:
            print("_update_canvas_scroll_region appelé")
...
def _setup_progressive_scroll_detection(self):
        ...
        try:
            ...
            # Au lieu de lier directement aux événements de scroll, utiliser un observateur de position
            # qui sera appelé périodiquement pour vérifier si plus d'éléments doivent être chargés
            if not hasattr(self, '_scroll_observer_active'):
                self._scroll_observer_active = True
                self._setup_scroll_position_observer()
            
            if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
                print("📜 Détection scroll progressif configurée (observateur)")
                
        except Exception as e:
            if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
                print(f"❌ Erreur config scroll progressif: {e}")
                
def _setup_scroll_position_observer(self):
        """Configure un observateur de position de scroll qui vérifie périodiquement"""
        if not hasattr(self, '_scroll_observer_active') or not self._scroll_observer_active:
            return
            
        # Vérifier la position et charger plus si nécessaire
        self._check_scroll_position_for_loading()
        
        # Programmer la prochaine vérification (toutes les 300ms)
        self.root.after(300, self._setup_scroll_position_observer)

def _check_scroll_position_for_loading(self):
        """Vérifie périodiquement la position de scroll pour charger plus d'éléments si nécessaire"""
        try:
            if not (USE_NEW_CONFIG and get_main_playlist_config('enable_progressive_loading')):
                return
            
            # Éviter les vérifications pendant le défilement actif
            if hasattr(self, '_processing_scroll') and self._processing_scroll:
                return
            ...
            try:
                ...
                # Stocker la dernière position pour détecter les changements significatifs
                last_position = getattr(self, '_last_scroll_position', 0)
                position_changed = abs(scroll_position - last_position) > 0.05
                self._last_scroll_position = scroll_position
                
                if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll') and position_changed:
                ...
                if scroll_position >= threshold:
                    ...
                    # Attendre au moins 500ms entre les chargements
...
# Garder l'ancienne fonction pour compatibilité, mais la rediriger vers la nouvelle
def _on_progressive_scroll(self, event=None):
        """Fonction de compatibilité - redirige vers le nouveau système"""
        self._check_scroll_position_for_loading()
...
def _delayed_refresh(self):
        """Effectue un rafraîchissement différé après la fin du scroll"""
        try:
            # Réinitialiser le flag de rafraîchissement différé
            self._delayed_refresh_scheduled = False
            
            # Vérifier que le scroll est bien terminé
            if hasattr(self, '_processing_scroll') and self._processing_scroll:
                # Si toujours en scroll, reprogrammer
                self.root.after(200, lambda: self._delayed_refresh())
                return
                
            # Effectuer le rafraîchissement avec force_recreate=False pour optimiser
            self._refresh_windowed_playlist_display(force_recreate=False)
            
        except Exception as e:
            print(f"Erreur lors du rafraîchissement différé: {e}")