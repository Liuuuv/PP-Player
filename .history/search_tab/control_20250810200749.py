...
def generate_waveform_preview(self, filepath):
    ...
    try:
        ...
        if max_val > 0:
            ...
            # Si toutes les valeurs sont à zéro, créer un signal plat
            self.waveform_data_raw = np.zeros_like(samples)
            
        self.waveform_data = None  # Sera calculé dynamiquement
        print(f"Waveform générée avec succès: {len(self.waveform_data_raw)} échantillons")
...
def get_adaptive_waveform_data(self, canvas_width=None):
    ...
    if self.waveform_data_raw is None:
        print("get_adaptive_waveform_data: waveform_data_raw is None")
        return None
    ...
    if self.song_length > 0:
        ...
        target_resolution = max(1000, min(20000, target_resolution))
        print(f"Target resolution calculated: {target_resolution} (song_length: {self.song_length})")
    else:
        target_resolution = 1000  # Valeur par défaut
        print(f"Using default target resolution: {target_resolution}")
    ...
    if len(self.waveform_data_raw) > target_resolution:
        ...
        result = self.waveform_data_raw[::step]
        print(f"Subsampled data: {len(self.waveform_data_raw)} -> {len(result)} (step: {step})")
        return result
    else:
        print(f"Using raw data as-is: {len(self.waveform_data_raw)} samples")
...
def show_waveform_on_clicked(self):
    ...
    print(f"Waveform button clicked. show_waveform_current: {self.show_waveform_current}")

    if self.show_waveform_current:
        print("Activating waveform display...")
        self.waveform_canvas.config(height=80)
        self.waveform_canvas.pack(fill=tk.X, pady=(0, 10))
        print(f"Canvas configured. Width: {self.waveform_canvas.winfo_width()}, Height: {self.waveform_canvas.winfo_height()}")
        print(f"Current time: {self.current_time}, Song length: {self.song_length}")
        print(f"Waveform data raw exists: {self.waveform_data_raw is not None}")
        if self.waveform_data_raw is not None:
            print(f"Waveform data raw length: {len(self.waveform_data_raw)}")
        self.draw_waveform_around(self.current_time)
        self.show_waveform_btn.config(bg="#4a8fe7", activebackground="#4a8fe7")
    else:
        print("Deactivating waveform display...")
...
def draw_waveform_around(self, time_sec, window_sec=5):
    print(f"draw_waveform_around called with time_sec={time_sec}")
    
    if self.waveform_data_raw is None:
        print("No waveform_data_raw available")
        return

    total_length = self.song_length
    if total_length == 0:
        print("Song length is 0")
        return

    print(f"Total length: {total_length}, waveform_data_raw length: {len(self.waveform_data_raw)}")
    ...
    if adaptive_data is None:
        print("No adaptive_data generated")
        return
    
    print(f"Adaptive data length: {len(adaptive_data)}")
    ...
    # Si le canvas n'a pas de hauteur valide (caché), ne pas dessiner
    if height <= 1:
        return
