...
def _show_current_song_thumbnail(self):
    ...
    try:
        if not self.thumbnail_frame.winfo_exists():
            print("DEBUG: thumbnail_frame détruit")
            return
    except tk.TclError as e:
        # Thumbnail frame détruit
        print(f"DEBUG: Erreur thumbnail_frame: {e}")
        return
    except Exception as e:
        print(f"DEBUG: Erreur inattendue thumbnail_frame: {e}")
        return
    ...
    try:
        if self.thumbnail_frame.winfo_exists():
            ...
            print(f"DEBUG: Nettoyage de {len(children)} widgets dans thumbnail_frame")
            ...
            print("DEBUG: thumbnail_frame n'existe plus lors du nettoyage")
            return
    except tk.TclError as e:
        # Thumbnail frame détruit, ignorer
        print(f"DEBUG: Erreur lors du nettoyage thumbnail_frame: {e}")
        return
        
    if len(self.main_playlist) > 0 and self.current_index < len(self.main_playlist):
        current_song = self.main_playlist[self.current_index]
        print(f"DEBUG: Affichage miniature pour: {os.path.basename(current_song)}")
        ...
        try:
            ...
            print("DEBUG: Label miniature créé avec succès")
            ...
            print(f"DEBUG: Erreur création label miniature: {e}")
            pass
        
    else:
        print("DEBUG: Aucune chanson en cours, affichage icône par défaut")
        try:
            ...
            print("DEBUG: Label par défaut créé avec succès")
        except tk.TclError as e:
            # Erreur lors de la création du label, ignorer
            print(f"DEBUG: Erreur création label par défaut: {e}")
            pass
