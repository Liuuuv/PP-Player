...
def _check_and_unload_items(self, current_index):
        ...
        try:
            ...
            if not current_is_loaded:
                if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
                    print(f"🔄 RELOAD: Musique actuelle {current_index} non chargée, rechargement forcé")
            ...
            if not next_is_loaded:
                if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
                    print(f"🔄 PRELOAD: Musique suivante {next_index} non chargée, ajout à la zone de préservation")
            ...
            prioritize_before = (user_looking_below or 
                                getattr(self, '_just_used_find_button', False) or 
                                getattr(self, '_scrolling_below_current', False))
            ...
            if prioritize_before:
                ...
                if widgets_before_current and USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
                    print(f"🧹 PRIORITÉ: Déchargement des {len(widgets_before_current)} éléments AVANT la musique actuelle")
            else:
            ...
            if widgets_to_unload:
                ...
                if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
                    print(f"🧹 SMART UNLOAD: Déchargement de {unload_count} éléments (préservation {preserve_start}-{preserve_end})")
                    print(f"  - {len(widgets_before_current)} éléments avant la musique actuelle")
                    print(f"  - {len(widgets_after_current)} éléments après la musique actuelle")
