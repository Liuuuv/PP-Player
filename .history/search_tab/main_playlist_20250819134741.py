...
def _progressive_load_system(self):
        ...
        try:
            ...
            if not current_song_visible:
                ...
                # Marquer qu'on a dû effacer l'affichage pour la musique actuelle
                self._had_to_clear_display_for_current = True
...
def _append_progressive_items(self, start_index, end_index):
        ...
        try:
            ...
            # DÉCHARGEMENT INTELLIGENT : Seulement dans des cas très spécifiques
            if loaded_count > 0:
                # Vérifier si on doit déclencher le déchargement
                should_unload = False
                
                # CAS 1: Si on vient d'utiliser le bouton "find"
                if getattr(self, '_just_used_find_button', False):
                    should_unload = True
                    print(f"DEBUG: Déchargement après bouton 'find' ({loaded_count} éléments chargés)")
                
                # CAS 2: Si la musique actuelle n'était pas visible avant le chargement
                # (on peut le détecter si on a dû effacer l'affichage avant de recharger)
                elif getattr(self, '_had_to_clear_display_for_current', False):
                    should_unload = True
                    print(f"DEBUG: Déchargement après rechargement de la musique actuelle non visible ({loaded_count} éléments chargés)")
                    # Réinitialiser le flag
                    self._had_to_clear_display_for_current = False
                
                else:
                    print(f"DEBUG: {loaded_count} nouveaux éléments chargés, mais pas de déchargement (changement normal de musique)")
                
                # Déclencher le déchargement seulement si nécessaire
                if should_unload:
                ...
                try:
                    if hasattr(self, 'current_index') and self.main_playlist:
                        ...
                        if 0 <= current_index < len(self.main_playlist):
                            ...
                            if not self._is_index_already_loaded(current_index):
                                # DÉSACTIVÉ: Déchargement automatique dans vérification périodique
                                # Le déchargement ne se fait que lors du chargement de nouvelles musiques ou après "find"
                                pass
