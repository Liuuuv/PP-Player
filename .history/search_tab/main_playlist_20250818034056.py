...
def select_current_song_smart(self, auto_scroll=True):
    """Sélectionne la chanson courante avec le système intelligent - FONCTION WRAPPER"""
    try:
        if not self.main_playlist or self.current_index >= len(self.main_playlist):
            return
        ...
        if USE_NEW_CONFIG and get_main_playlist_config('enable_smart_loading'):
            ...
            if (hasattr(self, '_last_window_start') and hasattr(self, '_last_window_end')):
                ...
                if start_index <= self.current_index < end_index:
                    # Calculer l'index relatif et sélectionner
                    relative_index = self.current_index - start_index
                    self.select_playlist_item(index=relative_index, auto_scroll=auto_scroll)
                    
                    if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
                        print(f"✅ Select current smart: chanson {self.current_index} (relatif: {relative_index})")
                else:
                    # La chanson n'est pas chargée, déclencher le smart reload d'abord
                    if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
                        print(f"🔄 Select current smart: rechargement requis pour chanson {self.current_index}")
                    
                    # Utiliser la fonction scroll_to_current qui gère le rechargement
                    self._scroll_to_current_song()
            else:
                # Pas encore de fenêtre chargée, utiliser scroll_to_current
                self._scroll_to_current_song()
        else:
            # Système classique
