...
def _add_main_playlist_item_at_position(self, filepath, song_index=None, position='bottom'):
        ...
        try:
            if position == 'bottom':
                ...
                # Trouver la position d'insertion correcte basée sur song_index
                ...
                if item_frame:
                    # Déplacer le nouvel élément à la bonne position
                    item_frame.pack_forget()
                    
                    if insert_before:
                        # Insérer avant l'élément trouvé
                        item_frame.pack(fill='x', pady=2, padx=5, before=insert_before)
                        if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
                            print(f"  → Élément {song_index} inséré avant {insert_before.song_index}")
                    else:
                        # Aucun élément avec index supérieur trouvé, ajouter à la fin
                        item_frame.pack(fill='x', pady=2, padx=5)
                        if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
                            print(f"  → Élément {song_index} ajouté à la fin")
