...
def _ensure_results_container_exists(self):
    ...
    if not hasattr(self, 'results_container') or not self.results_container.winfo_exists():
        try:
            ...
            print(f"Erreur lors de la recréation du results_container: {e}")
...
def _recreate_youtube_canvas(self):
    ...
    try:
        ...
        print(f"Erreur lors de la recréation du youtube_canvas: {e}")
        ...
        try:
            if child.winfo_exists() and hasattr(child, 'video_data'):
                ...
                if url in self.current_downloads:
                    ...
                    self._set_item_colors(child, COLOR_DOWNLOAD)
                else:
                    # Apparence normale
                    ...
                    self._set_item_colors(child, COLOR_BACKGROUND)
        except tk.TclError:
...
def _display_batch_results_fast(self, batch_results):
    ...
    for i, (video, idx) in enumerate(batch_results):
        ...
        self.root.after(delay, lambda v=video, index=idx: self._add_search_result(v, index))
...
def _display_new_results(self, new_results):
    ...
    for i, result in enumerate(new_results):
        if self.search_cancelled:
            break
        self.root.after(30 * i, lambda r=result, idx=start_index + i: self._add_search_result(r, idx))
        self.search_results_count += 1
    ...
    try:
        ...
        if hasattr(self, 'scrollbar') and self.scrollbar is not None:
            try:
                ...
                print(f"Erreur affichage scrollbar: {e}")
        else:
            pass
        ...
        if hasattr(self, 'youtube_canvas') and self.youtube_canvas is not None:
            try:
                ...
                print(f"Erreur affichage youtube_canvas: {e}")
        else:
            pass
...
def _ensure_canvas_scrollbar_connection(self):
    ...
    try:
        ...
        print(f"Erreur lors de la configuration canvas-scrollbar: {e}")
...
def _perform_initial_search(self, query):
    ...
    try:
        ...
        # Réinitialiser le flag de scroll utilisateur pour une nouvelle recherche
...
def _save_current_search_state(self):
        ...
        # DEBUG: État de recherche sauvegardé dans le cache:")
        print(f"  - Query: '{search_state['search_query']}'")
        print(f"  - Results: {len(search_state['search_results'])}")
        print(f"  - current_search_batch: {search_state['current_search_batch']}")
        print(f"  - has_more_results: {search_state['has_more_results']}")
        print(f"  - search_results_count: {search_state['search_results_count']}")
...
def _try_restore_from_cache(self, search_query):
    ...
    # DEBUG: Restauration depuis le cache pour '{search_query}'")
    print(f"  - {len(cached_state['search_results'])} résultats trouvés")
    print(f"  - has_more_results: {cached_state.get('has_more_results', False)}")
    print(f"  - total_available_results: {cached_state.get('total_available_results', 'non défini')}")
    ...
    try:
        ...
        # DEBUG: Variables restaurées:")
        print(f"  - current_search_query: '{self.current_search_query}'")
        print(f"  - has_more_results: {self.has_more_results}")
        print(f"  - total_available_results: {self.total_available_results}")
        print(f"  - search_results_count: {self.search_results_count}")
        print(f"  - current_search_batch: {self.current_search_batch}")
        ...
        # DEBUG: Appel de _display_search_results_from_cache avec {len(self.all_search_results)} résultats")
        try:
            self._display_search_results_from_cache(self.all_search_results)
    
        except Exception as e:
            # DEBUG: Erreur dans _display_search_results_from_cache: {e}")
            import traceback
            traceback.print_exc()
            ...
            def restore_scroll():
                try:
                    if hasattr(self, 'youtube_canvas') and self.youtube_canvas.winfo_exists():
                        ...
                        # DEBUG: Position de scroll restaurée: {scroll_top}")
                except Exception as e:
                    # DEBUG: Erreur restauration scroll: {e}")
        ...
        # DEBUG: Erreur lors de la restauration depuis le cache: {e}")
        return False
...
def _display_search_results_from_cache(self, results):
    ...
    self._cache_loading_in_progress = True
    
    if not results:
        self._cache_loading_in_progress = False
        return
    ...
    def display_batch(batch_start):
        try:
            ...
            for i in range(batch_start, batch_end):
                if i < len(results):
                    ...
                    try:
                        ...
                        print(f"Erreur ajout résultat cache {i}: {e}")
            ...
            if batch_end < len(results):
                ...
                # Réactiver le chargement automatique après un délai
                def reactivate_auto_loading():
                    self._cache_loading_in_progress = False
                
                self.safe_after(1000, reactivate_auto_loading)  # 1 seconde de délai
...
def _show_current_song_thumbnail(self):
    ...
    if not hasattr(self, 'thumbnail_frame'):
        # DEBUG: thumbnail_frame n'existe pas comme attribut")
        # Essayer de recréer la frame
        if hasattr(self, '_recreate_thumbnail_frame'):
            # DEBUG: tentative de recréation via _recreate_thumbnail_frame")
            self._recreate_thumbnail_frame()
        else:
            # DEBUG: impossible de recréer, _recreate_thumbnail_frame n'existe pas")
            return
        
    try:
        if not self.thumbnail_frame.winfo_exists():
            # DEBUG: thumbnail_frame existe comme attribut mais widget détruit")
            # Essayer de recréer la frame
            if hasattr(self, '_recreate_thumbnail_frame'):
                # DEBUG: tentative de recréation via _recreate_thumbnail_frame")
                self._recreate_thumbnail_frame()
            ...
            # DEBUG: thumbnail_frame existe et le widget est valide")
    except tk.TclError as e:
        # DEBUG: TclError avec thumbnail_frame: {e}")
        ...
        # DEBUG: Exception dans vérification thumbnail_frame: {e}")
        return
    ...
    if len(self.main_playlist) > 0 and self.current_index < len(self.main_playlist):
        ...
        try:
            ...
            # DEBUG: Label miniature chanson créé et packé avec succès")
            ...
            # DEBUG: Tentative de chargement de la vraie miniature")
        except tk.TclError as e:
            # Erreur lors de la création du label, ignorer
            # DEBUG: TclError lors de la création du label chanson: {e}")
        except Exception as e:
            # DEBUG: Exception lors de la création du label chanson: {e}")
        
    else:
        # DEBUG: Aucune chanson en cours, affichage miniature par défaut")
        try:
            ...
            # DEBUG: Label miniature par défaut créé et packé avec succès")
        except tk.TclError as e:
            # Erreur lors de la création du label, ignorer
            # DEBUG: TclError lors de la création du label par défaut: {e}")
        except Exception as e:
            # DEBUG: Exception lors de la création du label par défaut: {e}")
    ...
    try:
        ...
        if hasattr(self, 'youtube_results_frame'):
            try:
                ...
                # DEBUG: Widgets dans youtube_results_frame AVANT nettoyage: {len(children)}")
                for i, child in enumerate(children):
                    try:
                        ...
                        # DEBUG:   - Widget {i}: ERREUR - {e}")
            except Exception as e:
                # DEBUG: Erreur lors du listage des widgets avant: {e}")
        ...
        # DEBUG: Suppression du notebook artiste...")
        if hasattr(self, 'artist_notebook'):
            try:
                if self.artist_notebook.winfo_exists():
                    # DEBUG: artist_notebook existe, destruction...")
                    self.artist_notebook.destroy()
                    # DEBUG: artist_notebook détruit")
                else:
                    # DEBUG: artist_notebook n'existe plus")
                delattr(self, 'artist_notebook')
                # DEBUG: attribut artist_notebook supprimé")
            except Exception as e:
                # DEBUG: Erreur suppression artist_notebook: {e}")
        
        # Supprimer la croix si elle existe
        # DEBUG: Suppression du bouton croix artiste...")
        if hasattr(self, 'artist_close_btn'):
            try:
                if self.artist_close_btn.winfo_exists():
                    # DEBUG: artist_close_btn existe, destruction...")
                    self.artist_close_btn.destroy()
                    # DEBUG: artist_close_btn détruit")
                else:
                    # DEBUG: artist_close_btn n'existe plus")
                delattr(self, 'artist_close_btn')
                # DEBUG: attribut artist_close_btn supprimé")
            except Exception as e:
                # DEBUG: Erreur suppression artist_close_btn: {e}")
        
        # Supprimer le bouton retour playlist si il existe
        # DEBUG: Suppression du bouton retour playlist...")
        if hasattr(self, 'artist_tab_back_btn') and self.artist_tab_back_btn:
            try:
                if self.artist_tab_back_btn.winfo_exists():
                    # DEBUG: artist_tab_back_btn existe, destruction...")
                    self.artist_tab_back_btn.destroy()
                    # DEBUG: artist_tab_back_btn détruit")
                else:
                    # DEBUG: artist_tab_back_btn n'existe plus")
                self.artist_tab_back_btn = None
                # DEBUG: attribut artist_tab_back_btn remis à None")
            except Exception as e:
                # DEBUG: Erreur suppression artist_tab_back_btn: {e}")
        
        # Debug: Lister tous les widgets présents dans youtube_results_frame APRÈS suppression des widgets artiste
        if hasattr(self, 'youtube_results_frame'):
            try:
                children = self.youtube_results_frame.winfo_children()
                # DEBUG: Widgets dans youtube_results_frame APRÈS suppression widgets artiste: {len(children)}")
                for i, child in enumerate(children):
                    try:
                        widget_class = child.__class__.__name__
                        widget_info = f"  - Widget {i}: {widget_class}"
                ...
                # DEBUG: Erreur lors du listage des widgets après suppression: {e}")
        
        # CORRECTION: Nettoyer tous les frames enfants non essentiels de youtube_results_frame
        # DEBUG: Nettoyage des frames enfants non essentiels...")
        if hasattr(self, 'youtube_results_frame'):
            try:
                ...
                for child in children:
                    try:
                        ...
                        if (child == getattr(self, 'thumbnail_frame', None) or
                            ...
                            # DEBUG: Conservation du widget essentiel: {child.__class__.__name__}")
                            continue
                        
                        # Supprimer tous les autres frames (main_container et autres)
                        # DEBUG: Suppression du widget non essentiel: {child.__class__.__name__} (name: {child.winfo_name()})")
                        child.destroy()
                        
                    except Exception as e:
                        # DEBUG: Erreur lors de la suppression d'un widget: {e}")
            except Exception as e:
                # DEBUG: Erreur lors du nettoyage des frames: {e}")
        
        # Debug: Lister les widgets APRÈS nettoyage complet
        if hasattr(self, 'youtube_results_frame'):
            try:
                children = self.youtube_results_frame.winfo_children()
                # DEBUG: Widgets dans youtube_results_frame APRÈS nettoyage complet: {len(children)}")
                for i, child in enumerate(children):
                    try:
                        widget_class = child.__class__.__name__
                        widget_info = f"  - Widget {i}: {widget_class}"
                ...
                # DEBUG: Erreur lors du listage final: {e}")
        
        # Restaurer l'état original des résultats de recherche
        had_search_results = False
        if hasattr(self, 'saved_search_state'):
            # DEBUG: Restauration de l'état de recherche - Query: '{self.saved_search_state.get('search_query', '')}', Results: {len(self.saved_search_state.get('search_results', []))}")
        ...
        # DEBUG: Vérification et affichage de thumbnail_frame...")
        if hasattr(self, 'thumbnail_frame'):
            try:
                if self.thumbnail_frame.winfo_exists():
                    # DEBUG: thumbnail_frame existe, forçage du pack...")
                    ...
                    # DEBUG: thumbnail_frame forcée visible: {self.thumbnail_frame.winfo_ismapped()}")
                else:
                    # DEBUG: thumbnail_frame détruite, recréation...")
                    # thumbnail_frame détruite, la recréer
                    self._recreate_thumbnail_frame()
            except tk.TclError as e:
                # DEBUG: TclError avec thumbnail_frame: {e}, recréation...")
                # thumbnail_frame détruite, la recréer
                self._recreate_thumbnail_frame()
        else:
            # DEBUG: thumbnail_frame n'existe pas, création...")
        ...
        if hasattr(self, 'youtube_results_frame'):
            try:
                children = self.youtube_results_frame.winfo_children()
                # DEBUG: Widgets dans youtube_results_frame ÉTAT FINAL: {len(children)}")
                for i, child in enumerate(children):
                    try:
                        widget_class = child.__class__.__name__
                        widget_info = f"  - Widget {i}: {widget_class}"
        ...
        # DEBUG: === FIN _return_to_search ===")
        
    except Exception as e:
        # DEBUG: ERREUR DANS _return_to_search: {e}")
...
def _add_search_result(self, video, index):
        ...
        # DEBUG: Ajout du résultat de recherche {index}: {video.get('title', 'Sans titre')}")
        if self.search_cancelled:
            return
        try:
            ...
            result_frame.is_selected = False
                ...
                if artist or album:
                    ...
                    if artist:
                        ...
                        artist_label = tk.Label(
                            ...
                            text=artist,
                            bg=COLOR_BACKGROUND,
                            fg=COLOR_ARTIST_NAME,  # Couleur bleue pour indiquer que c'est cliquable
                        ...
                        def on_artist_click(event, artist_name=artist, video_data=video):
                            try:
                                # DEBUG: Clic sur artiste: {artist_name}")
                                # DEBUG: Video data keys: {list(video_data.keys())}")
                                ...
                                # DEBUG: Erreur ouverture artist_tab: {e}")
                                import traceback
                                traceback.print_exc()
            ...
            # Configurer les propriétés pour le drag-and-drop (seulement pour les vidéos)
            if not is_channel:
            ...
            def on_result_ctrl_click(event, frame=result_frame):
                ...
                if "https://www.youtube.com/channel/" in video_url or "https://www.youtube.com/@" in video_url:
                    ...
                    import webbrowser
                    webbrowser.open(video_url)
                    self.status_bar.config(text="Vidéo ouverte dans le navigateur")
            
            def on_result_right_click(event):
                self._on_result_right_click(event, frame=result_frame)
            ...
            widgets_to_bind = [result_frame, thumbnail_label, text_frame, title_label, duration_label, metadata_frame, metadata_label]
            ...
            for widget in widgets_to_bind:
                ...
                widget.bind("<Control-ButtonPress-1>", on_result_ctrl_click)
                widget.bind("<ButtonPress-3>", on_result_right_click)
            ...
            def force_motion_bindings():
                widgets_to_fix = [result_frame, thumbnail_label, text_frame, title_label, duration_label, metadata_frame, metadata_label]
                
                for widget in widgets_to_fix:
                    if widget and widget.winfo_exists():
                        widget.bind("<B1-Motion>", lambda e, f=result_frame: self.drag_drop_handler._on_drag_motion(e, f))
                        widget.bind("<ButtonRelease-1>", lambda e, f=result_frame: self.drag_drop_handler._on_drag_release(e, f))
                        widget.bind("<B3-Motion>", lambda e, f=result_frame: self.drag_drop_handler._on_drag_motion(e, f))
                        widget.bind("<ButtonRelease-3>", lambda e, f=result_frame: self.drag_drop_handler._on_drag_release(e, f))
...
def _on_result_right_click(self, event, frame):
        ...
        if hasattr(frame, 'video_data'):
            ...
            # Afficher le menu unifié
            self.show_unified_context_menu(video, event, item_type="youtube")
...
def _recreate_thumbnail_frame(self):
    ...
    if hasattr(self, 'youtube_results_frame'):
        try:
            children = self.youtube_results_frame.winfo_children()
            # DEBUG: Widgets dans youtube_results_frame AVANT recréation: {len(children)}")
            for i, child in enumerate(children):
                try:
                    widget_class = child.__class__.__name__
                    widget_info = f"  - Widget {i}: {widget_class}"
            ...
            # DEBUG: Erreur lors du listage des widgets: {e}")
    ...
    try:
        ...
        self.thumbnail_frame.pack_propagate(True)
        self.thumbnail_frame.update_idletasks()
        # DEBUG: thumbnail_frame recréée et packée avec succès")

        # Debug: Lister tous les widgets présents APRÈS recréation
        if hasattr(self, 'youtube_results_frame'):
            try:
                children = self.youtube_results_frame.winfo_children()
                # DEBUG: Widgets dans youtube_results_frame APRÈS recréation: {len(children)}")
                for i, child in enumerate(children):
                    try:
                        widget_class = child.__class__.__name__
                        widget_info = f"  - Widget {i}: {widget_class}"
                ...
                # DEBUG: Erreur lors du listage des widgets après: {e}")

    except Exception as e:
        # DEBUG: Erreur lors de la recréation de thumbnail_frame: {e}")
...
def _should_load_more_results(self):
    """Vérifie si on doit charger plus de résultats - version optimisée"""
    # Vérifications rapides d'abord
    if (self.is_loading_more or 
        self.is_searching or
        not self.current_search_query or 
        self.search_results_count >= self.max_search_results or
        self.current_search_batch >= self.max_search_batchs or
        getattr(self, '_cache_loading_in_progress', False)):
        return False
    
    # Vérifier si on a encore des résultats à charger
    if not getattr(self, 'has_more_results', False):
        return False
    
    # Empêcher le chargement automatique immédiat après une recherche
    # Attendre au moins 2 secondes après la fin de la recherche initiale
    if hasattr(self, 'search_start_time') and self.search_start_time:
        time_since_search = time.time() - self.search_start_time
        if time_since_search < 2.0:  # 2 secondes de délai
            return False
    
    # Vérifier si l'utilisateur a réellement scrollé (pas juste l'affichage automatique)
    if not getattr(self, '_user_has_scrolled', False):
        return False
    ...
    try:
        ...
        # Cache de la dernière position pour éviter les calculs redondants
        if not hasattr(self, '_last_scroll_bottom'):
            self._last_scroll_bottom = 0.0
        
        # Si la position n'a pas beaucoup changé, pas besoin de recalculer
        if abs(scroll_bottom - self._last_scroll_bottom) < 0.01:
            return False
        
        self._last_scroll_bottom = scroll_bottom
        
        # CORRECTION: Réduire le seuil à 70% car la scrollbar ne va pas jusqu'à 100%
        if scroll_bottom > 0.85:  # 85%
            return True
...
def _display_search_results(self, results, scroll_position=None):
    ...
    try:
        # DEBUG: _display_search_results appelée avec {len(results)} résultats")
        
        # Restaurer complètement l'état de recherche
        self.all_search_results = results
        self.search_results_count = 0
        
        # Calculer le nombre de batches nécessaires basé sur le nombre de résultats
        num_results = len(results)
        batches_needed = (num_results - 1) // 10 + 1  # Nombre de batches de 10 résultats
        has_more = getattr(self, 'has_more_results', False)
        query = getattr(self, 'current_search_query', '')
        
        # Debug réduit pour les performances
        if batches_needed > 2:  # Seulement pour les gros chargements
            # DEBUG: Restauration rapide - {num_results} résultats, {batches_needed} batches")
        ...
        # Si on a plus de 10 résultats, afficher les batches suivants rapidement
        if batches_needed > 1:
            for batch in range(2, batches_needed + 1):
                # Délai réduit pour un affichage plus rapide
                self.root.after(20 * batch, lambda b=batch: self._display_batch_results(b))
        
        # Mettre à jour current_search_batch pour refléter le nombre de batches affichés
        self.current_search_batch = batches_needed
        
        # Forcer la mise à jour de la scrollregion après que tous les résultats soient ajoutés
        # Calcul optimisé : 5ms par résultat + 50ms de marge
        total_delay = num_results * 5 + 50
        self.root.after(total_delay, self._update_canvas_scrollregion)
        
        # Restaurer la position de scroll après que tous les résultats soient ajoutés
        if scroll_position:
            # DEBUG: Restauration de la position de scroll: {scroll_position}")
        ...
        # DEBUG: Restauration de {num_results} résultats programmée via _display_batch_results")
        
    except Exception as e:
        # DEBUG: Erreur dans _display_search_results: {e}")
...
def _get_cached_thumbnail(self, video_id, thumbnail_url):
    ...
    # Vérifier si la miniature est en cache
    if video_id in self.thumbnail_cache:
        return self.thumbnail_cache[video_id]
    
    # Si pas en cache, retourner None pour un chargement asynchrone
    return None
...
def _cache_thumbnail(self, video_id, thumbnail_image):
    ...
    # Limiter la taille du cache (garder seulement les 100 dernières)
    ...
    self.thumbnail_cache[video_id] = thumbnail_image

def _update_canvas_scrollregion(self):
    """Force la mise à jour de la scrollregion du canvas"""
    try:
        if hasattr(self, 'youtube_canvas') and hasattr(self, 'results_container'):
            if self.youtube_canvas.winfo_exists() and self.results_container.winfo_exists():
                # Forcer la mise à jour de tous les widgets
                self.results_container.update_idletasks()
                self.youtube_canvas.update_idletasks()
                
                # Recalculer la scrollregion
                bbox = self.youtube_canvas.bbox("all")
                if bbox:
                    self.youtube_canvas.configure(scrollregion=bbox)
                    # Debug réduit pour les performances
                    if bbox[3] > 1000:  # Seulement pour les grandes scrollregions
                        # DEBUG: Grande scrollregion mise à jour: hauteur={bbox[3]}")
    except Exception as e:
        # DEBUG: Erreur lors de la mise à jour de la scrollregion: {e}")
...
def _restore_scroll_position(self, scroll_position):
    ...
    try:
        if hasattr(self, 'youtube_canvas') and self.youtube_canvas.winfo_exists():
            ...
            # DEBUG: Application de la position de scroll: top={scroll_top}, bottom={scroll_bottom}")
            ...
            # DEBUG: Scrollregion actuelle: {scrollregion}")
            
            # Appliquer la position de scroll
            self.youtube_canvas.yview_moveto(scroll_top)
            # DEBUG: Position de scroll restaurée avec succès")
        else:
            # DEBUG: Canvas non disponible pour restaurer le scroll")
    except Exception as e:
        # DEBUG: Erreur lors de la restauration du scroll: {e}")
