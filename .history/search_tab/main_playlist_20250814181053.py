...
def add_to_main_playlist(self, filepath, thumbnail_path=None, song_index=None, show_status=True, allow_duplicates=False):
    ...
    """
    # Utiliser la fonction de base pour ajouter à la playlist
    added = self.add_file_to_main_playlist(filepath, show_status=show_status)
    
    if added or allow_duplicates:
        # Mettre à jour l'interface seulement si ajouté
        self._add_song_item(filepath, self.playlist_container, song_index=song_index, item_type="main_playlist")
...
def _refresh_playlist_display(self):
        ...
        try:
            ...
            for i, filepath in enumerate(self.main_playlist):
                self._add_song_item(filepath, self.playlist_container, song_index=i, item_type="main_playlist")
