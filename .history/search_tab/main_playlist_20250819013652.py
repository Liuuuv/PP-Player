...
def _setup_progressive_scroll_detection(self):
        ...
        try:
            print("DEBUG: _setup_progressive_scroll_detection appelé")
            
            if not (USE_NEW_CONFIG and get_main_playlist_config('enable_progressive_loading')):
                print("DEBUG: Chargement progressif désactivé dans la configuration")
                return
                
            if not (hasattr(self, 'playlist_canvas') and self.playlist_canvas):
                print("DEBUG: Playlist canvas non disponible")
                return
            ...
            print("DEBUG: Détection scroll progressif configurée (via _check_infinite_scroll)")
                
        except Exception as e:
            print(f"DEBUG: Erreur config scroll progressif: {e}")

def _on_progressive_scroll(self, event=None):
        """Gère le scroll pour le chargement progressif"""
        try:
            print("DEBUG: _on_progressive_scroll appelé")
            
            if not (USE_NEW_CONFIG and get_main_playlist_config('enable_progressive_loading')):
                print("DEBUG: Chargement progressif désactivé")
                return
            ...
            try:
                ...
                print(f"DEBUG: Scroll position: {scroll_position:.3f}, seuil: {threshold}")
                
                # Si on atteint le seuil, charger plus d'éléments
                if scroll_position >= threshold:
                    print("DEBUG: Seuil atteint, chargement de plus d'éléments")
                    self._load_more_on_scroll()
                else:
                    print("DEBUG: Seuil non atteint, pas de chargement supplémentaire")
                    
            except Exception as e:
                print(f"DEBUG: Erreur détection position scroll: {e}")
                
        except Exception as e:
            print(f"DEBUG: Erreur gestion scroll progressif: {e}")
...
def _check_infinite_scroll(self):
        ...
        try:
            print("DEBUG: _check_infinite_scroll appelé")
