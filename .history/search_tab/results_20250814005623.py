            ...
            if not is_channel:
                ...
                if artist or album:
                    ...
                    if artist:
                        ...
                        # Binding pour le double-clic sur l'artiste (ouvrir la page de la chaîne)
                        def on_artist_double_click(event, video_data=video):
                            try:
                                import webbrowser
                                channel_url = video_data.get('channel_url', '')
                                uploader_url = video_data.get('uploader_url', '')
                                
                                # Essayer d'abord channel_url, puis uploader_url
                                url_to_open = channel_url or uploader_url
                                
                                if url_to_open:
                                    print(f"DEBUG: Ouverture de la chaîne: {url_to_open}")
                                    webbrowser.open(url_to_open)
                                    self.status_bar.config(text=f"Ouverture de la chaîne de {artist}")
                                else:
                                    print(f"DEBUG: Aucune URL de chaîne trouvée pour {artist}")
                                    self.status_bar.config(text=f"URL de chaîne non disponible pour {artist}")
                            except Exception as e:
                                print(f"DEBUG: Erreur ouverture chaîne: {e}")
                                self.status_bar.config(text=f"Erreur lors de l'ouverture de la chaîne")
                        
                        artist_label.bind("<Button-1>", on_artist_click)
                        artist_label.bind("<Double-Button-1>", on_artist_double_click)
                        create_tooltip(artist_label, f"Clic: voir toutes les vidéos de {artist}\nDouble-clic: ouvrir la page de la chaîne")
