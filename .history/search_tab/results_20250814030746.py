...
def _display_search_results(self, results, scroll_position=None):
    ...
    try:
        ...
        # Calculer le nombre de batches déjà affichés basé sur le nombre de résultats
        # Si on a 10 résultats ou moins, c'est le batch 1
        # Si on a plus, calculer le nombre de batches
        num_results = len(results)
        if num_results <= 10:
            self.current_search_batch = 1
        else:
            self.current_search_batch = (num_results - 1) // 10 + 1
        
        print(f"DEBUG: Restauration batch {self.current_search_batch} avec {num_results} résultats")
        ...
        if num_results > 10:
            for batch in range(2, self.current_search_batch + 1):
