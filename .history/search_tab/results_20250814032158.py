...
def _display_search_results(self, results, scroll_position=None):
    ...
    try:
        ...
        # Le current_search_batch est déjà restauré dans _return_to_search
        # Ne pas le recalculer ici pour préserver l'état exact
        num_results = len(results)
        current_batch = getattr(self, 'current_search_batch', 1)
        has_more = getattr(self, 'has_more_results', False)
        query = getattr(self, 'current_search_query', '')
        
        print(f"DEBUG: État restauré:")
        print(f"  - current_search_batch: {current_batch}")
        ...
        if num_results > 10:
            for batch in range(2, current_batch + 1):
