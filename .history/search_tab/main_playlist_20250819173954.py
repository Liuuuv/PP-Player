...
def _extend_window_up(self, new_start):
        ...
        try:
            ...
            # Ajouter les nouveaux éléments au début dans l'ordre décroissant
            # pour maintenir l'ordre chronologique correct
            for i in range(current_start - 1, new_start - 1, -1):
...
def _add_main_playlist_item_at_position(self, filepath, song_index=None, position='bottom'):
        ...
        try:
            if position == 'bottom':
                ...
                # Pour le haut, créer l'élément manuellement pour contrôler le placement
                # Sauvegarder les enfants existants
                existing_children = list(self.playlist_container.winfo_children())
                
                # Créer le nouvel élément (il sera automatiquement ajouté à la fin)
                item_frame = self._add_main_playlist_item(filepath, song_index=song_index)
                
                if item_frame and existing_children:
                    # Déplacer le nouvel élément au début
                    item_frame.pack_forget()
                    # Insérer avant le premier élément existant
                    item_frame.pack(fill='x', pady=2, padx=5, before=existing_children[0])
                    
                    if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
                        print(f"  → Élément {song_index} inséré au début")
