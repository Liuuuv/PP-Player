...
def _scroll_to_current_song(self, event=None):
    """Fait défiler la liste de lecture vers la chanson en cours - Compatible système intelligent"""
    ...
    try:
        # Si le système intelligent est activé, FORCER le rechargement autour de la chanson courante
        if USE_NEW_CONFIG and get_main_playlist_config('enable_smart_loading'):
            if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
                print(f"🎯 SCROLL TO CURRENT: Forcer rechargement autour chanson {self.current_index}")
            
            # Calculer la fenêtre 10+1+10 autour de la chanson courante
            ...
            # FORCER le rechargement de cette fenêtre
            self._force_reload_window(start_index, end_index)
            
            # Attendre un peu puis sélectionner l'item (il sera maintenant chargé)
            def select_after_reload():
                try:
                    # Calculer l'index relatif dans la fenêtre chargée
                    ...
                    if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
                        print(f"✅ Scroll to current réussi: chanson {self.current_index} (relative: {relative_index})")
                        
                except Exception as e:
                    if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
                        print(f"❌ Erreur select après reload: {e}")
            
            # Exécuter après un délai pour que le rechargement soit terminé
            self.root.after(100, select_after_reload)
