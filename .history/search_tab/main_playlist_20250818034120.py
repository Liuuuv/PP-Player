...
def _scroll_to_current_song(self, event=None):
    """Bouton find.png : Scroll DIRECT vers chanson courante SANS rechargement d'affichage"""
    ...
    try:
        if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
            print(f"🎯 FIND BUTTON: Scroll direct vers chanson {self.current_index} SANS rechargement")
        
        # Si le système intelligent est activé, d'abord vérifier si on a une fenêtre chargée
        if USE_NEW_CONFIG and get_main_playlist_config('enable_smart_loading'):
            # Si la chanson courante est dans la fenêtre déjà chargée, juste scroller
            if (hasattr(self, '_last_window_start') and hasattr(self, '_last_window_end')):
                start_index = self._last_window_start
                end_index = self._last_window_end
                
                if start_index <= self.current_index < end_index:
                    # Chanson courante déjà chargée : scroll direct avec animation
                    ...
                    if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
                        print(f"✅ Find scroll direct: chanson {self.current_index} (relatif: {relative_index})")
                    return
            
            # Chanson pas dans la fenêtre chargée : FORCER le rechargement PUIS scroll
            if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
                print(f"🔄 Find button: Rechargement requis pour chanson {self.current_index}")
            
            # Calculer et charger la fenêtre autour de la chanson courante
            ...
            # Scroll avec animation après rechargement
            def scroll_after_reload():
                try:
                    ...
                    if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
                        print(f"✅ Find scroll après rechargement: chanson {self.current_index} (relatif: {relative_index})")
                        
                except Exception as e:
                    if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
                        print(f"❌ Erreur scroll après reload: {e}")
            
            # Délai minimal pour le rechargement
...
def select_current_song_smart(self, auto_scroll=True, force_reload=False):
    """Auto-scroll intelligent : Scroll vers chanson courante avec ease in/out"""
    try:
        if not self.main_playlist or self.current_index >= len(self.main_playlist):
            return
            
        if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
            print(f"🎮 AUTO-SCROLL SMART: Vers chanson {self.current_index}, animation={auto_scroll}, force={force_reload}")
        
        # Si le système intelligent est activé
        if USE_NEW_CONFIG and get_main_playlist_config('enable_smart_loading'):
            # Vérifier si la chanson courante est dans la fenêtre chargée
            if (hasattr(self, '_last_window_start') and hasattr(self, '_last_window_end') and not force_reload):
                start_index = self._last_window_start
                end_index = self._last_window_end
                
                # Si la chanson courante est dans la fenêtre chargée
                if start_index <= self.current_index < end_index:
                    # Scroll direct avec animation ease in/out
                    relative_index = self.current_index - start_index
                    self.select_playlist_item(index=relative_index, auto_scroll=auto_scroll)
                    
                    if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
                        print(f"✅ Auto-scroll direct: chanson {self.current_index} (relatif: {relative_index})")
                    return
            
            # La chanson n'est pas chargée OU force_reload : rechargement + scroll animé
            if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
                action = "forcé" if force_reload else "requis"
                print(f"🔄 Auto-scroll rechargement {action} pour chanson {self.current_index}")
            
            # Calculer et charger la fenêtre autour de la nouvelle chanson
            songs_before = get_main_playlist_config('songs_before_current')
            songs_after = get_main_playlist_config('songs_after_current')
            ...
            # Auto-scroll avec animation après rechargement
            def auto_scroll_after_reload():
                try:
                    ...
                    if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
                        print(f"✅ Auto-scroll après rechargement: chanson {self.current_index} (relatif: {relative_index})")
                        
                except Exception as e:
                    if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
                        print(f"❌ Erreur auto-scroll après reload: {e}")
            
            # Délai optimisé pour l'auto-scroll
            ...
            if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
                print(f"✅ Auto-scroll classique: chanson {self.current_index}")
...
def _smart_load_unload(self):
        """Effectue le chargement/déchargement intelligent RÉEL"""
        try:
            if not (USE_NEW_CONFIG and get_main_playlist_config('enable_smart_loading')):
                return
            ...
            # Simple et efficace : 10+1+10 autour de la chanson courante - PROTECTION INDEX
            current_index = getattr(self, 'current_index', 0)
            total_songs = len(self.main_playlist)
            
            # SÉCURITÉ: Protéger contre les index invalides
            current_index = max(0, min(current_index, total_songs - 1))
            
            if current_index < 0 or current_index >= total_songs:
                if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
                    print(f"❌ ABORT SMART RELOAD: Index invalide {current_index} (total: {total_songs})")
                return
            
            # Calculer la fenêtre 10+1+10 avec protection rigoureuse
            songs_before = get_main_playlist_config('songs_before_current') 
            songs_after = get_main_playlist_config('songs_after_current')
            
            # PROTECTION ABSOLUE: Jamais d'index négatif ou trop grand
            ...
            if target_start < 0:
                target_start = 0
            if target_end > total_songs:
                target_end = total_songs
            if target_start >= target_end:
                if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
                    print(f"❌ ABORT SMART RELOAD: Fenêtre invalide {target_start}-{target_end}")
                return
            
            if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
                print(f"🔄 SMART RELOAD: Chanson {current_index}, fenêtre {target_start}-{target_end}")
            
            # FORCER le rechargement complet (méthode simple et sûre)
            ...
            if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
                print(f"❌ Erreur smart load/unload: {e}")

def _force_reload_window(self, start_index, end_index):
        """Force le rechargement d'une fenêtre spécifique (décharge tout, recharge la fenêtre)"""
        try:
            if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
                print(f"🔥 FORCE RELOAD: Déchargement complet puis rechargement {start_index}-{end_index}")
            ...
            # Étape 2: RECHARGER la fenêtre cible
            charges = 0
            for i in range(start_index, end_index):
                if i < len(self.main_playlist):
                    filepath = self.main_playlist[i]
