...
def _scroll_to_current_song(self, event=None):
    ...
    try:
        ...
        if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
            print(f"🎯 FIND BUTTON: Vers chanson {self.current_index}")
        ...
        if USE_NEW_CONFIG and get_main_playlist_config('enable_progressive_loading'):
            ...
            if self._is_index_already_loaded(self.current_index):
                ...
                if relative_index is not None:
                    ...
                    if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
                        print(f"✅ Find scroll direct progressif: chanson {self.current_index} (relatif: {relative_index})")
                    return
            
            # Chanson pas encore chargée : déclencher le chargement progressif
            if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
                print(f"🔄 Find button: Chargement progressif requis pour chanson {self.current_index}")
                ...
                try:
                    ...
                    if relative_index is not None:
                        ...
                        if USE_NEW_CONFIG and get_main_playlist_config('enable_smart_unloading'):
                            ...
                            # Définir un flag pour indiquer qu'on vient d'utiliser le bouton find
                            self._just_used_find_button = True
                            print("DEBUG: Flag _just_used_find_button activé")
                            
                            # Déclencher le déchargement après un court délai
                            self.root.after(100, lambda: print("DEBUG: Déchargement après find.png"))
                        
                        if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
                            print(f"✅ Find scroll après chargement progressif: chanson {self.current_index}")
                except Exception as e:
                    if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
                        print(f"❌ Erreur find scroll après chargement progressif: {e}")
            
            self.root.after(50, scroll_after_progressive_load)
...
def _update_canvas_scroll_region(self):
        ...
        try:
            ...
            if children_count > 0:
                ...
                elif enable_infinite and total_songs > children_count:
                    # Mode classique : région virtuelle pour toutes les musiques
...
def _auto_center_on_current_song(self):
        ...
        try:
            ...
            if USE_NEW_CONFIG and get_main_playlist_config('enable_smart_unloading'):
                # Attendre un peu pour s'assurer que le scroll est terminé
                # Définir un flag pour indiquer qu'on vient d'utiliser le recentrage auto
                self._just_used_find_button = True  # Même comportement que find.png
                print("DEBUG: Flag _just_used_find_button activé (auto-recentrage)")
                
                # Déclencher le déchargement après un court délai
                self.root.after(100, lambda: print("DEBUG: Déchargement après auto-recentrage"))
...
def _progressive_load_system(self):
        ...
        try:
            ...
            if USE_NEW_CONFIG and get_main_playlist_config('enable_smart_unloading'):
                # Utiliser une méthode sécurisée pour le déchargement
                try:
                    # Vérifier si la méthode existe dans l'objet actuel
                    if '_check_and_unload_items' in dir(self):
                        self._check_and_unload_items(current_index)
                    else:
                        print("DEBUG: Méthode _check_and_unload_items non disponible dans cet objet")
                except Exception as e:
                    print(f"DEBUG: Erreur lors du déchargement: {e}")
            
            # Calculer combien charger (courante + X suivantes)
            initial_load = get_main_playlist_config('initial_load_after_current')
            target_end = min(total_songs, current_index + initial_load + 1)
...
def _is_index_already_loaded(self, index):
        ...
        try:
            print(f"DEBUG: Vérification si index {index} est déjà chargé")
            
            # Optimisation : utiliser un cache pour les index chargés
            if not hasattr(self, '_loaded_indexes_cache') or self._loaded_indexes_cache is None:
                # Initialiser ou rafraîchir le cache
                print("DEBUG: Initialisation/rafraîchissement du cache des index chargés")
                self._loaded_indexes_cache = set()
                children = self.playlist_container.winfo_children()
                for child in children:
                    if hasattr(child, 'song_index'):
                        self._loaded_indexes_cache.add(child.song_index)
                print(f"DEBUG: Cache initialisé avec {len(self._loaded_indexes_cache)} index")
            
            # Vérifier dans le cache
            is_loaded = index in self._loaded_indexes_cache
            print(f"DEBUG: Index {index} chargé: {is_loaded}")
            return is_loaded
        except Exception as e:
...
def _invalidate_loaded_indexes_cache(self):
    ...
    if hasattr(self, '_loaded_indexes_cache'):
        print("DEBUG: Invalidation du cache des index chargés")
...
def _setup_current_song_check(self):
        ...
        try:
            ...
            def check_current_song_loaded():
                try:
                    print("DEBUG: Vérification périodique des musiques chargées")
                    if hasattr(self, 'current_index') and self.main_playlist:
                        ...
                        if 0 <= current_index < len(self.main_playlist):
                            ...
                            print(f"DEBUG: Musique actuelle (index {current_index}) chargée: {current_is_loaded}")
                            
                            if not current_is_loaded:
                                # Forcer le chargement/déchargement
                                print(f"DEBUG: Chargement forcé de la musique actuelle (index {current_index})")
                                # Utiliser le chargement progressif au lieu du déchargement direct
                                if hasattr(self, '_progressive_load_system'):
                                    self._progressive_load_system()
                            
                            # Vérifier également si la musique suivante est chargée
                            next_index = (current_index + 1) % len(self.main_playlist)
                            next_is_loaded = self._is_index_already_loaded(next_index)
                            print(f"DEBUG: Musique suivante (index {next_index}) chargée: {next_is_loaded}")
                            
                            if not next_is_loaded:
                                # Charger la musique suivante si elle n'est pas déjà chargée
                                # Cela évite le problème où la musique en lecture est la dernière affichée
                                print(f"DEBUG: Chargement préventif de la musique suivante (index {next_index})")
                                
                                # Charger la musique suivante et quelques-unes après
                                load_count = get_main_playlist_config('load_more_on_scroll')
                                
                                # Si on est à la fin de la playlist, charger depuis le début (boucle)
                                if next_index == 0:
                                    # On est à la dernière musique, charger depuis le début
                                    print(f"DEBUG: Chargement depuis le début (index 0 à {load_count-1})")
                                    self._append_progressive_items(0, load_count)
                                else:
                                    # Charger la musique suivante et quelques-unes après
                                    end_index = min(len(self.main_playlist), next_index + load_count)
                                    print(f"DEBUG: Chargement de la suite (index {next_index} à {end_index-1})")
                                    self._append_progressive_items(next_index, end_index)
                except Exception as e:
                    print(f"DEBUG: Erreur vérification musique courante: {e}")
            ...
            print("DEBUG: Vérification périodique configurée")
            
        except Exception as e:
            print(f"DEBUG: Erreur configuration vérification musique courante: {e}")
...
def _load_more_on_scroll(self):
        ...
        try:
            print("DEBUG: Chargement de plus d'éléments sur scroll")
            if not self.main_playlist:
                print("DEBUG: Playlist vide, rien à charger")
                return
            ...
            print(f"DEBUG: Dernier élément chargé: {last_loaded}, total: {total_songs}")
            
            # Vérifier si on a déjà chargé trop d'éléments
            currently_loaded = len(self.playlist_container.winfo_children()) if hasattr(self, 'playlist_container') else 0
            max_loaded = get_main_playlist_config('max_loaded_items')
            
            if currently_loaded >= max_loaded:
                print(f"DEBUG: Trop d'éléments déjà chargés ({currently_loaded}/{max_loaded}), déchargement requis avant de charger plus")
                # Forcer le déchargement avant de charger plus
                current_index = getattr(self, 'current_index', 0)
                if USE_NEW_CONFIG and get_main_playlist_config('enable_smart_unloading'):
                    try:
                        if '_check_and_unload_items' in dir(self):
                            ...
                            print("DEBUG: Méthode _check_and_unload_items non disponible")
                    except Exception as e:
                        print(f"DEBUG: Erreur lors du déchargement préventif: {e}")
            
            # Si on a déjà tout chargé, vérifier si on doit charger depuis le début (boucle)
            if last_loaded >= total_songs - 1:
                print("DEBUG: Fin de playlist atteinte, chargement depuis le début")
                ...
                print(f"DEBUG: Premiers éléments déjà chargés: {first_loaded}")
                
                # Si les premiers éléments ne sont pas chargés, les charger
                if not first_loaded:
                    print(f"DEBUG: Chargement depuis le début (index 0 à {load_more_count-1})")
                    ...
                    current_index = getattr(self, 'current_index', 0)
                    if USE_NEW_CONFIG and get_main_playlist_config('enable_smart_unloading'):
                        print("DEBUG: Déchargement après chargement depuis le début")
                        try:
                            if '_check_and_unload_items' in dir(self):
                                self._check_and_unload_items(current_index)
                            else:
            ...
            load_more_count = min(
                get_main_playlist_config('load_more_on_scroll'),  # Nombre configuré
                get_main_playlist_config('max_load_per_scroll', 10)  # Limite par scroll
            )
            
            # Limiter le nombre d'éléments à charger en fonction de ce qui est déjà chargé
            max_to_load = max(0, max_loaded - currently_loaded)
            if max_to_load < load_more_count:
                print(f"DEBUG: Limitation du chargement à {max_to_load} éléments (max: {max_loaded}, actuel: {currently_loaded})")
                load_more_count = max(1, max_to_load)  # Charger au moins 1 élément
            
            start_from = last_loaded + 1
            end_at = min(total_songs, start_from + load_more_count)
            
            print(f"DEBUG: Chargement de {start_from} à {end_at-1} (+{end_at-start_from} musiques)")
            ...
            if start_from > current_index:
                ...
                self._scrolling_below_current = True
                print(f"DEBUG: Utilisateur scrolle en dessous de la musique actuelle (index {current_index})")
            else:
                self._scrolling_below_current = False
                print(f"DEBUG: Utilisateur scrolle au-dessus de la musique actuelle (index {current_index})")
            
            # Vérifier si on doit décharger des éléments (trop d'éléments chargés)
            if USE_NEW_CONFIG and get_main_playlist_config('enable_smart_unloading'):
                print("DEBUG: Déchargement après chargement sur scroll")
                # Utiliser une méthode sécurisée pour le déchargement
                try:
                    # Vérifier si la méthode existe dans l'objet actuel
                    if '_check_and_unload_items' in dir(self):
            ...
            print(f"DEBUG: Erreur chargement supplémentaire: {e}")

def _check_and_unload_items(self, current_index):
        """Vérifie si on doit décharger des éléments et les décharge si nécessaire"""
        try:
            print(f"DEBUG: _check_and_unload_items appelé pour index {current_index}")
            
            # Vérifier si le déchargement intelligent est activé
            if not (USE_NEW_CONFIG and get_main_playlist_config('enable_smart_unloading')):
                print("DEBUG: Déchargement intelligent désactivé dans la configuration")
                return
                
            # Obtenir les widgets actuellement chargés
            children = self.playlist_container.winfo_children()
            loaded_count = len(children)
            print(f"DEBUG: {loaded_count} éléments actuellement chargés")
            
            # Vérifier si la musique actuelle est chargée
            current_is_loaded = self._is_index_already_loaded(current_index)
            print(f"DEBUG: Musique actuelle (index {current_index}) chargée: {current_is_loaded}")
            
            # Obtenir le seuil de déchargement
            max_loaded_items = get_main_playlist_config('max_loaded_items')
            print(f"DEBUG: Seuil de déchargement: {max_loaded_items} éléments")
            
            # Forcer le déchargement si la musique actuelle n'est pas chargée
            # ou si on a dépassé le seuil
            force_unload = not current_is_loaded
            if not force_unload and loaded_count <= max_loaded_items:
                print(f"DEBUG: Pas besoin de décharger (force_unload: {force_unload}, loaded_count: {loaded_count})")
                return
                
            # Déterminer la position de vue actuelle (ce que l'utilisateur voit)
            try:
                scroll_top, scroll_bottom = self.playlist_canvas.yview()
                canvas_height = self.playlist_canvas.winfo_height()
                item_height = get_main_playlist_config('item_height_estimate')
                
                # Estimer les index visibles
                visible_items_count = int(canvas_height / item_height)
                ...
                last_visible_index = min(total_items - 1, first_visible_index + visible_items_count)
                
                print(f"DEBUG: Vue actuelle: {first_visible_index}-{last_visible_index} (scroll: {scroll_top:.3f}-{scroll_bottom:.3f})")
            except Exception as e:
                # En cas d'erreur, utiliser l'index courant comme référence
                first_visible_index = max(0, current_index - 5)
                last_visible_index = min(len(self.main_playlist) - 1, current_index + 5)
                print(f"DEBUG: Erreur détection vue, utilisation fallback: {first_visible_index}-{last_visible_index}")
                print(f"DEBUG: Erreur: {e}")
            
            # Déterminer les zones à préserver
            ...
            # Préserver la vue actuelle si configuré
            if get_main_playlist_config('preserve_current_view'):
                # Étendre la zone de préservation pour inclure la vue actuelle
                preserve_start = min(first_visible_index, current_index - preserve_before)
                preserve_end = max(last_visible_index, current_index + preserve_after)
                print(f"DEBUG: Préservation vue actuelle + zone courante: {preserve_start}-{preserve_end}")
            else:
                # Sinon, préserver seulement autour de la position courante
                preserve_start = current_index - preserve_before
                preserve_end = current_index + preserve_after
                print(f"DEBUG: Préservation zone courante uniquement: {preserve_start}-{preserve_end}")
                
            # S'assurer que les limites sont valides
            preserve_start = max(0, preserve_start)
            preserve_end = min(len(self.main_playlist) - 1, preserve_end)
            
            # Si la musique actuelle n'est pas chargée, forcer le chargement
            if not current_is_loaded:
                print(f"DEBUG: Musique actuelle {current_index} non chargée, rechargement forcé")
            ...
            # Si la musique suivante n'est pas dans la zone de préservation, l'ajouter
            if not next_is_loaded:
                print(f"DEBUG: Musique suivante {next_index} non chargée, ajout à la zone de préservation")
                
                # Toujours charger la musique suivante, même si on est à la fin de la playlist
                # Cela garantit que lorsqu'on passe à la suivante, elle est déjà chargée
                load_count = get_main_playlist_config('load_more_on_scroll')
            ...
            # Collecter les widgets à décharger, en séparant ceux avant et après la musique actuelle
            ...
            for child in children:
                if hasattr(child, 'song_index'):
                    ...
                    # Décharger si en dehors de la zone de préservation
                    if index < preserve_start or index > preserve_end:
                        if index < current_index:
                            widgets_before_current.append(child)
                        else:
                            widgets_after_current.append(child)
            
            print(f"DEBUG: Widgets à décharger - avant: {len(widgets_before_current)}, après: {len(widgets_after_current)}")
            
            # Déterminer si l'utilisateur regarde en dessous de la musique actuelle
            user_looking_below = False
            try:
                scroll_top, scroll_bottom = self.playlist_canvas.yview()
                # Estimer l'index du premier élément visible
                visible_start = int(scroll_top * len(self.main_playlist))
                if visible_start > current_index:
                    user_looking_below = True
                    print(f"DEBUG: Utilisateur regarde en dessous de la musique actuelle (visible: {visible_start}, actuelle: {current_index})")
                else:
                    print(f"DEBUG: Utilisateur regarde au niveau ou au-dessus de la musique actuelle (visible: {visible_start}, actuelle: {current_index})")
            except Exception as e:
                print(f"DEBUG: Erreur détection position utilisateur: {e}")
            
            # Priorité de déchargement : d'abord les éléments avant la musique actuelle
            # si l'utilisateur regarde en dessous, scrolle vers le bas, ou si on vient d'utiliser find.png
            just_used_find = getattr(self, '_just_used_find_button', False)
            scrolling_below = getattr(self, '_scrolling_below_current', False)
            prioritize_before = (user_looking_below or just_used_find or scrolling_below)
            
            print(f"DEBUG: Flags - user_looking_below: {user_looking_below}, just_used_find: {just_used_find}, scrolling_below: {scrolling_below}")
            print(f"DEBUG: Priorité déchargement avant: {prioritize_before}")
            
            # Décharger les widgets
            widgets_to_unload = []
            if prioritize_before:
                # Décharger d'abord les éléments avant la musique actuelle
                widgets_to_unload = widgets_before_current + widgets_after_current
                if widgets_before_current:
                    print(f"DEBUG: PRIORITÉ: Déchargement des {len(widgets_before_current)} éléments AVANT la musique actuelle")
            else:
                # Décharger de manière équilibrée
                widgets_to_unload = widgets_before_current + widgets_after_current
                print(f"DEBUG: Déchargement équilibré: {len(widgets_to_unload)} éléments")
            ...
            if getattr(self, '_just_used_find_button', False):
                ...
                print("DEBUG: Flag _just_used_find_button réinitialisé")
            if getattr(self, '_scrolling_below_current', False):
                self._scrolling_below_current = False
                print("DEBUG: Flag _scrolling_below_current réinitialisé")
                
            # Invalider le cache des index chargés
            self._invalidate_loaded_indexes_cache()
            print("DEBUG: Cache des index chargés invalidé")
                
            # Mettre à jour la région de scroll
            self.root.after(10, self._update_canvas_scroll_region)
            print("DEBUG: Mise à jour de la région de scroll programmée")
            
        except Exception as e:
            print(f"DEBUG: ERREUR dans _check_and_unload_items: {e}")
            import traceback
            traceback.print_exc()
                
        except Exception as e:
            if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
                print(f"❌ Erreur déchargement intelligent: {e}")
...
def _trigger_smart_reload_on_song_change(self):
        ...
        try:
            ...
            if song_changed or force_reload:
                ...
                print(f"⚪ Pas de changement (chanson {current_index}), pas de reload")
...
def _check_infinite_scroll(self):
        ...
        try:
            print("DEBUG: Vérification du scroll infini")
            ...
            if hasattr(self, '_last_infinite_check_time'):
                ...
                if current_time - self._last_infinite_check_time < 0.1:  # 100ms entre les vérifications
                    print("DEBUG: Vérification trop fréquente, ignorée")
                    return
            ...
            if not (hasattr(self, 'playlist_canvas') and self.playlist_canvas.winfo_exists()):
                print("DEBUG: Canvas de playlist non disponible")
                return
            
            # Vérifier si on doit utiliser le chargement progressif
            if USE_NEW_CONFIG and get_main_playlist_config('enable_progressive_loading'):
                # Appeler la fonction de chargement progressif
                print("DEBUG: Appel de _on_progressive_scroll")
                self._on_progressive_scroll()
            
            # Vérifier si on doit utiliser le scroll infini
            if not (USE_NEW_CONFIG and get_main_playlist_config('enable_infinite_scroll')):
                print("DEBUG: Scroll infini désactivé")
                return
            
            # Obtenir la position actuelle du scroll
            try:
                scroll_top, scroll_bottom = self.playlist_canvas.yview()
                print(f"DEBUG: Position de scroll: {scroll_top:.3f}-{scroll_bottom:.3f}")
            except Exception as e:
                print(f"DEBUG: Erreur récupération position scroll: {e}")
                return
            
            threshold = get_main_playlist_config('scroll_threshold') if USE_NEW_CONFIG else 0.1
            print(f"DEBUG: Seuil de déclenchement: {threshold}")
            
            # Vérifier si on est proche du bas (charger des éléments suivants)
            if scroll_bottom >= (1.0 - threshold):
                print("DEBUG: Seuil bas atteint, chargement de plus d'éléments")
                self._load_more_on_scroll()
            else:
                print("DEBUG: Seuil non atteint, pas de chargement supplémentaire")
            ...
            if hasattr(self, 'current_index') and self.main_playlist:
                ...
                # Estimer l'index du premier et dernier élément visible
                first_visible_index = int(scroll_top * total_items)
                last_visible_index = int(scroll_bottom * total_items)
                
                print(f"DEBUG: Éléments visibles: {first_visible_index}-{last_visible_index}, musique actuelle: {current_index}")
                
                # Si la musique actuelle n'est pas visible, vérifier si on doit charger/décharger
                if current_index < first_visible_index or current_index > last_visible_index:
                    print(f"DEBUG: Musique actuelle (index {current_index}) n'est plus visible")
                    
                    # Vérifier si la musique actuelle est chargée
                    if hasattr(self, '_is_index_already_loaded') and not self._is_index_already_loaded(current_index):
                        print(f"DEBUG: Musique actuelle non chargée, déclenchement du chargement progressif")
                        # Utiliser le chargement progressif au lieu du déchargement direct
                        if hasattr(self, '_progressive_load_system'):
                            self._progressive_load_system()
                
        except Exception as e:
            print(f"DEBUG: Erreur vérification scroll infini: {e}")
            import traceback
            traceback.print_exc()
