            ...
            if not is_channel:
                ...
                if artist or album:
                    ...
                    if artist:
                        ...
                        # Binding pour le double-clic sur l'artiste (ouvrir l'artiste dans l'app)
                        def on_artist_double_click(event, artist_name=artist, video_data=video):
                            try:
                                print(f"DEBUG: Double-clic sur artiste: {artist_name}")
                                # Utiliser la même fonction que le clic simple pour ouvrir l'artiste dans l'app
                                self._show_artist_content(artist_name, video_data)
                                self.status_bar.config(text=f"Affichage du contenu de {artist_name}")
                            except Exception as e:
                                print(f"DEBUG: Erreur ouverture artiste: {e}")
                                self.status_bar.config(text=f"Erreur lors de l'ouverture de l'artiste")
                        
                        artist_label.bind("<Button-1>", on_artist_click)
                        artist_label.bind("<Double-Button-1>", on_artist_double_click)
                        create_tooltip(artist_label, f"Clic: voir toutes les vidéos de {artist}\nDouble-clic: ouvrir l'artiste dans l'app")
