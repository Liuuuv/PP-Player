...
def _show_current_song_thumbnail(self):
    ...
    print("DEBUG: _show_current_song_thumbnail dans results.py appelée")
    
    # Vérifier que thumbnail_frame existe et est accessible
    if not hasattr(self, 'thumbnail_frame'):
        print("DEBUG: thumbnail_frame n'existe pas - tentative de recréation")
        # Essayer de recréer la frame
        if hasattr(self, '_recreate_thumbnail_frame'):
            self._recreate_thumbnail_frame()
        else:
            print("DEBUG: impossible de recréer thumbnail_frame")
            return
        
    try:
        if not self.thumbnail_frame.winfo_exists():
            print("DEBUG: thumbnail_frame détruite - tentative de recréation")
            # Essayer de recréer la frame
            if hasattr(self, '_recreate_thumbnail_frame'):
                self._recreate_thumbnail_frame()
            else:
        ...
        print("DEBUG: TclError - thumbnail_frame détruite - tentative de recréation")
        ...
        print(f"DEBUG: Exception dans _show_current_song_thumbnail: {e}")
        return
