...
def _display_search_results(self, results, scroll_position=None):
    ...
    try:
        ...
        # Configurer la scrollbar après que tous les résultats soient ajoutés
        total_delay = len(results) * SEARCH_WAIT_TIME_BETWEEN_RESULTS + 100
        self.root.after(total_delay, lambda: self._configure_scrollbar_after_results())
        ...
        if scroll_position:
            ...
            # Attendre que tous les résultats soient ajoutés et que la scrollbar soit configurée
...
def _configure_scrollbar_after_results(self):
    """Configure la scrollbar après que tous les résultats soient ajoutés"""
    try:
        print("DEBUG: Configuration de la scrollbar après ajout des résultats")
        
        if hasattr(self, 'scrollbar') and hasattr(self, 'youtube_canvas'):
            if self.scrollbar.winfo_exists() and self.youtube_canvas.winfo_exists():
                # Forcer la mise à jour du canvas pour calculer la scrollregion
                ...
                # Vérifier la scrollregion
                scrollregion = self.youtube_canvas.cget('scrollregion')
                print(f"DEBUG: Scrollregion après configuration: {scrollregion}")
                
                print("DEBUG: Scrollbar configurée avec succès après ajout des résultats")
            else:
                print("DEBUG: Scrollbar ou canvas non disponible pour configuration")
        else:
            print("DEBUG: Attributs scrollbar ou canvas manquants")
            
    except Exception as e:
        print(f"DEBUG: Erreur lors de la configuration de la scrollbar: {e}")
