        ...
        try:
            ...
            if children_count > 0:
                ...
                if (USE_NEW_CONFIG and get_main_playlist_config('enable_smart_loading') and 
                    ...
                    # Calculer la hauteur totale virtuelle pour toute la playlist
                    total_virtual_height = total_songs * item_height
                    
                    # Utiliser la hauteur virtuelle totale pour la région de scroll
                    # Cela permet d'avoir une scrollbar qui représente correctement la taille totale
                    self.playlist_canvas.configure(scrollregion=(0, 0, 0, total_virtual_height))
                    
                    print(f"DEBUG: Scroll region intelligente: {total_virtual_height}px pour {total_songs} musiques totales ({children_count} chargées)")
                    ...
                    print(f"DEBUG: Scroll region normale: {displayed_height}px pour {children_count} éléments")
...
def _auto_center_on_current_song(self):
        ...
        try:
            ...
            if USE_NEW_CONFIG and get_main_playlist_config('enable_smart_unloading'):
                # Attendre un peu pour s'assurer que le scroll est terminé
                self.root.after(100, lambda: self._check_and_unload_items(current_index))
...
def _append_progressive_items(self, start_index, end_index):
        ...
        try:
            ...
            # Charger le reste des éléments
            for i in range(start_index, min(end_index, len(self.main_playlist))):
                if i != current_index and not self._is_index_already_loaded(i):  # Éviter de recharger l'index actuel
                    filepath = self.main_playlist[i]
                    try:
                ...
                try:
                    if hasattr(self, 'current_index') and self.main_playlist:
                        ...
                        if 0 <= current_index < len(self.main_playlist):
                            ...
                            if not self._is_index_already_loaded(next_index):
                                ...
                                if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
                                    print(f"🔄 PRELOAD: Chargement préventif de la musique suivante (index {next_index})")
...
def _check_and_unload_items(self, current_index):
        ...
        try:
            ...
            if not force_unload and loaded_count <= max_loaded_items:
                print(f"DEBUG: Pas besoin de décharger (chargés: {loaded_count}, max: {max_loaded_items})")
                return
            ...
            try:
                ...
                print(f"DEBUG: Éléments visibles: {first_visible_index} à {last_visible_index}")
            except Exception as e:
                ...
                print(f"DEBUG: Erreur calcul visibilité: {e}, utilisation fallback {first_visible_index}-{last_visible_index}")
            ...
            print(f"DEBUG: Zone de préservation: {preserve_start} à {preserve_end}")
            ...
            # Déterminer si l'utilisateur regarde au-dessus ou en dessous de la musique actuelle
            user_looking_below = False
            user_looking_above = False
            try:
                scroll_top, scroll_bottom = self.playlist_canvas.yview()
                # Estimer l'index du premier élément visible
                visible_start = int(scroll_top * len(self.main_playlist))
                visible_end = int(scroll_bottom * len(self.main_playlist))
                
                if visible_start > current_index:
                    user_looking_below = True
                    print(f"DEBUG: Utilisateur regarde en dessous de la musique actuelle")
                elif visible_end < current_index:
                    user_looking_above = True
                    print(f"DEBUG: Utilisateur regarde au-dessus de la musique actuelle")
                else:
                    print(f"DEBUG: Musique actuelle visible à l'écran")
            except Exception as e:
            ...
            # Ne pas décharger les éléments au-dessus si l'utilisateur les regarde
            preserve_above = user_looking_above
            
            # Décharger les widgets
            widgets_to_unload = []
            if prioritize_before and not preserve_above:
                ...
                # Ne pas décharger les éléments au-dessus car l'utilisateur les regarde
...
def _check_infinite_scroll(self):
        ...
        try:
            ...
            current_index = getattr(self, 'current_index', 0)
            if hasattr(self, 'main_playlist') and self.main_playlist and current_index < len(self.main_playlist):
                try:
                    # Vérifier si la musique actuelle est chargée
                    current_is_loaded = self._is_index_already_loaded(current_index)
                    
                    # Si la musique actuelle n'est pas chargée, la charger
                    if not current_is_loaded:
                        ...
                        print(f"DEBUG: Chargement forcé de la musique actuelle et des éléments autour ({preserve_start}-{preserve_end})")
                        
                        # Charger la musique actuelle et quelques éléments autour
                        self._append_progressive_items(preserve_start, preserve_end + 1)
                        
                        # Vérifier si la musique actuelle est maintenant chargée
                        if self._is_index_already_loaded(current_index):
                            print(f"DEBUG: Musique actuelle {current_index} maintenant chargée")
                            
                            # Scroll vers la musique actuelle
                            relative_index = self._find_relative_index_in_loaded(current_index)
                            if relative_index is not None:
                                print(f"DEBUG: Scroll vers la musique actuelle (index relatif {relative_index})")
                                self.select_playlist_item(index=relative_index, auto_scroll=True)
                        else:
                            print(f"DEBUG: ERREUR: Musique actuelle {current_index} toujours pas chargée après tentative!")
                            
                            # Forcer le chargement direct de la musique actuelle
                            try:
                                filepath = self.main_playlist[current_index]
                                print(f"DEBUG: Forçage direct du chargement de la musique actuelle: {os.path.basename(filepath)}")
                                self._add_main_playlist_item(filepath, song_index=current_index)
                            except Exception as e:
                                print(f"DEBUG: Erreur forçage direct: {e}")
                        
                        # Décharger les éléments inutiles
                        if USE_NEW_CONFIG and get_main_playlist_config('enable_smart_unloading'):
                            self.root.after(100, lambda: self._check_and_unload_items(current_index))
                except Exception as e:
                    print(f"DEBUG: Erreur vérification musique actuelle: {e}")
                    import traceback
                    traceback.print_exc()
            ...
            if scroll_top <= threshold:
                print(f"DEBUG: Proche du haut ({scroll_top:.3f}), chargement des éléments précédents")
            ...
            # Vérifier si on est proche du bas (charger des éléments suivants)
                ...
                print(f"DEBUG: Proche du bas ({scroll_bottom:.3f}), chargement des éléments suivants")
                self._load_more_songs_below()
                
        except Exception as e:
            print(f"DEBUG: Erreur lors de la vérification du scroll infini: {e}")
            import traceback
            traceback.print_exc()
