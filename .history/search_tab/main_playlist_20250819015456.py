    ...
    try:
        ...
        if USE_NEW_CONFIG and get_main_playlist_config('enable_progressive_loading'):
            ...
            def scroll_after_progressive_load():
                try:
                    # Si la chanson n'est toujours pas chargée, forcer le chargement
                    if not self._is_index_already_loaded(self.current_index):
                        # Forcer le chargement de la chanson courante et quelques-unes autour
                        buffer = 5  # Charger 5 chansons avant et après
                        start_idx = max(0, self.current_index - buffer)
                        end_idx = min(len(self.main_playlist), self.current_index + buffer + 1)
                        
                        # Forcer le chargement de ces éléments
                        self._append_progressive_items(start_idx, end_idx)
                        
                        if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
                            print(f"🔄 Chargement forcé pour find.png: {start_idx} à {end_idx-1}")
                    
                    # Trouver l'index relatif et scroller
                    relative_index = self._find_relative_index_in_loaded(self.current_index)
                    if relative_index is not None:
                        self.select_playlist_item(index=relative_index, auto_scroll=True)
...
def _append_progressive_items(self, start_index, end_index):
        ...
        try:
            ...
            # Vérifier si on doit décharger des éléments
            if loaded_count > 0 and USE_NEW_CONFIG and not get_main_playlist_config('never_unload'):
                self._check_and_unload_items()
...
def _load_more_on_scroll(self):
        ...
        try:
            ...
            # Vérifier si on doit décharger des éléments
            if USE_NEW_CONFIG and not get_main_playlist_config('never_unload'):
                self._check_and_unload_items()
...
def _check_and_unload_items(self):
        """Vérifie si le nombre d'éléments chargés dépasse le seuil et décharge les éléments non visibles"""
        try:
            ...
            # Vérifier si on dépasse le seuil de déchargement
            max_loaded = get_main_playlist_config('max_loaded_items')
            if loaded_count <= max_loaded:
                return
                
            # Obtenir l'index de la chanson en cours
            current_index = getattr(self, 'current_index', 0)
            ...
            try:
                ...
                scroll_top, scroll_bottom = 0, 1
                
            # Déterminer les éléments visibles
            visible_items = []
            for widget in children:
                if not hasattr(widget, 'song_index'):
                    continue
                    
                # Calculer la position relative du widget dans le canvas
                try:
                    widget_y = widget.winfo_y() / self.playlist_container.winfo_height()
                    widget_height = widget.winfo_height() / self.playlist_container.winfo_height()
                    widget_bottom = widget_y + widget_height
                    
                    # Vérifier si le widget est visible dans la vue actuelle
                    if (widget_y <= scroll_bottom and widget_bottom >= scroll_top):
                        visible_items.append(widget.song_index)
                except:
                    # Si on ne peut pas calculer, considérer comme non visible
                    pass
            
            # Déterminer la stratégie de déchargement
            strategy = get_main_playlist_config('unload_strategy')
            preserve_visible = get_main_playlist_config('preserve_visible_items')
            
            # Trier les widgets par index pour les décharger dans l'ordre
            widgets_to_unload = []
            
            if strategy == 'before_current':
                # Décharger les éléments avant la chanson courante
                for widget in children:
                    if not hasattr(widget, 'song_index'):
                        continue
                    if widget.song_index < current_index:
                        # Ne pas décharger les éléments visibles si demandé
                        if preserve_visible and widget.song_index in visible_items:
                            continue
                        widgets_to_unload.append(widget)
                        
            elif strategy == 'after_current':
                # Décharger les éléments après la chanson courante
                for widget in children:
                    if not hasattr(widget, 'song_index'):
                        continue
                    if widget.song_index > current_index:
                ...
                # Décharger intelligemment en préservant les éléments autour de la position actuelle
                # Calculer la distance par rapport à la chanson courante et à la vue actuelle
                for widget in children:
                    if not hasattr(widget, 'song_index'):
                        continue
                        
                    # Ne jamais décharger la chanson courante
                    if widget.song_index == current_index:
                        continue
                    ...
                    # Calculer la distance par rapport à la chanson courante
                    distance_to_current = abs(widget.song_index - current_index)
                    
                    # Ajouter à la liste de déchargement avec priorité basée sur la distance
                    widgets_to_unload.append((widget, distance_to_current))
                
                # Trier par distance (plus loin = plus prioritaire pour déchargement)
                widgets_to_unload.sort(key=lambda x: x[1], reverse=True)
                widgets_to_unload = [w[0] for w in widgets_to_unload]
            
            # Calculer combien d'éléments décharger
            to_unload_count = loaded_count - max_loaded
            widgets_to_unload = widgets_to_unload[:to_unload_count]
            
            # Décharger les widgets
            if widgets_to_unload:
                unload_indices = [w.song_index for w in widgets_to_unload if hasattr(w, 'song_index')]
                if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
                    print(f"🗑️ Déchargement de {len(widgets_to_unload)} éléments: {unload_indices}")
