...
def _add_main_playlist_item(self, filepath, thumbnail_path=None, song_index=None):
        ...
        try:
            print("_add_main_playlist_item appelée ", filepath, song_index)
            # Vérifier que le main_playlist_container existe et est accessible
            if not (hasattr(self, 'main_playlist_container') and self.main_playlist_container.winfo_exists()):
            ...
            def on_playlist_item_double_click(event):
                ...
                print('DOUBLE CLICK ON MAIN PLAYLIST ITEM')
            ...
            for widget in widgets_to_bind:
                ...
                if hasattr(self, 'main_playlist_canvas') and self.main_playlist_canvas:
                    # Fonction locale pour transmettre l'événement
                    def forward_wheel(event, canvas=self.main_playlist_canvas):
...
def forward_wheel(self, event):
    canvas = self.main_playlist_canvas
    # Optimisation: Limiter la fréquence des événements
    if hasattr(self, '_last_wheel_time'):
        current_time = time.time()
        if current_time - self._last_wheel_time < 0.01:  # 10ms entre les scrolls
...
def select_playlist_item(self, item_frame=None, index=None, auto_scroll=True, is_manual=False):
    ...
    try:
        if hasattr(self, 'main_playlist_container') and self.main_playlist_container.winfo_exists():
            # Accès sécurisé aux enfants pour la désélection
            try:
                children_for_deselect = self.main_playlist_container.winfo_children()
            except tk.TclError:
            ...
            if index is not None:
                try:
                    children = self.main_playlist_container.winfo_children()
            ...
            if item_frame.winfo_exists():
                ...
                if auto_scroll:
                    try:
                        ...
                        if (hasattr(self, 'main_playlist_container') and self.main_playlist_container.winfo_exists() and
                            hasattr(self, 'main_playlist_canvas') and self.main_playlist_canvas.winfo_exists()):
                            
                            container_height = self.main_playlist_container.winfo_height()
                            if container_height > 0:
                                ...
                                container_height = self.main_playlist_container.winfo_height()
                                if container_height > 0:
                                    self.main_playlist_canvas.yview_moveto(item_y / container_height)
...
def _clear_main_playlist(self, event=None):
    ...
    self.reset_main_playlist()
    self.go_to_top(self.main_playlist_canvas)
...
def _scroll_to_current_song(self, event=None, is_manual=False):
    ...
    try:
        ...
        self.select_playlist_item(index=self.current_index, auto_scroll=True, is_manual=is_manual)
...
def add_to_main_playlist(self, filepath, thumbnail_path=None, song_index=None, show_status=True, allow_duplicates=False):
    ...
    """
    print(f"Ajout de {filepath} à la main playlist add_to_main_playlist ", allow_duplicates, filepath in self.main_playlist)
...
def _smooth_scroll_to_position(self, target_position, duration, is_manual):
    ...
    try:
        ...
        if not (hasattr(self, 'main_playlist_canvas') and self.main_playlist_canvas.winfo_exists()):
            return
        ...
        try:
            ...
            current_top, current_bottom = self.main_playlist_canvas.yview()
            ...
            try:
                if hasattr(self, 'main_playlist_canvas') and self.main_playlist_canvas.winfo_exists():
                    self.main_playlist_canvas.yview_moveto(target_position)
            except tk.TclError:
                # Canvas détruit, ignorer
                pass
            return
        ...
        def animate_step():
            ...
            try:
                ...
                self.main_playlist_canvas.yview_moveto(current_position)
            except tk.TclError:
...
def reset_main_playlist(self):
    ...
    try:
        if hasattr(self, 'main_playlist_container') and self.main_playlist_container.winfo_exists():
            for widget in self.main_playlist_container.winfo_children():
                try:
...
def _refresh_main_playlist_display(self, force_full_refresh=False):
        ...
        try:
            ...
            if not hasattr(self, 'main_playlist_container'):
                return
                
            if not self.main_playlist_container.winfo_exists():
                return
...
def _refresh_windowed_playlist_display(self, force_recreate=False):
        ...
        try:
            ...
            can_reuse = (not force_recreate and
                        ...
                        hasattr(self, 'main_playlist_container') and self.main_playlist_container.winfo_exists() and
                        len(self.main_playlist_container.winfo_children()) > 0)
            ...
            if (len(self.main_playlist) > 0 and 
                ...
                try:
                    ...
                    widgets = self.main_playlist_container.winfo_children()
...
def _update_canvas_scroll_region(self):
        ...
        try:
            ...
            if not (hasattr(self, 'main_playlist_canvas') and hasattr(self, 'main_playlist_container')):
                return
                
            if not (self.main_playlist_canvas.winfo_exists() and self.main_playlist_container.winfo_exists()):
                return
            ...
            current_width = self.main_playlist_container.winfo_width()
            current_height = self.main_playlist_container.winfo_height()
            ...
            if (not hasattr(self, '_last_container_size') or 
                ...
                self.main_playlist_container.update_idletasks()
                self._last_container_size = (current_width, current_height)
            ...
            if children_count > 0:
                ...
                if (USE_NEW_CONFIG and get_main_playlist_config('enable_smart_loading') and 
                    ...
                    self.main_playlist_canvas.configure(scrollregion=(0, 0, 0, displayed_height))
                    ...
                    self.main_playlist_canvas.configure(scrollregion=(0, 0, 0, virtual_height))
                ...
                self.main_playlist_canvas.configure(scrollregion=(0, 0, 0, 0))
...
def _setup_infinite_scroll(self):
        ...
        try:
            ...
            # self.main_playlist_canvas.bind('<Configure>', self._on_playlist_canvas_configure)
            ...
            if hasattr(self, 'playlist_scrollbar') and self.playlist_scrollbar:
                self.playlist_scrollbar.config(command=lambda *args: [
                    self.main_playlist_canvas.yview(*args),
                    on_scroll_position_change()
                ])
            
            # Aussi connecter au canvas directement
            # self.main_playlist_canvas.bind('<MouseWheel>', self._on_scroll_with_update)
            # self.main_playlist_canvas.bind('<Button-4>', self._on_scroll_with_update)  # Linux
            # self.main_playlist_canvas.bind('<Button-5>', self._on_scroll_with_update)  # Linux
...
def _on_dynamic_scroll(self, event):
        ...
        try:
            ...
            try:
                ...
                scroll_top, scroll_bottom = self.main_playlist_canvas.yview()
                ...
                if scroll_top <= scroll_threshold and not loading_up:
                    ...
                    # self.main_playlist_canvas.yview()
                    # self.main_playlist_canvas.yview_scroll(0, "units")
...
#                 self._on_mousewheel(event, self.main_playlist_canvas)
...
def _update_windowed_display(self, start_index, end_index, center_index):
        ...
        try:
            ...
            # Vider le container actuel
            for child in self.main_playlist_container.winfo_children():
                child.destroy()
...
def _auto_center_on_current_song(self):
        ...
        try:
            ...
            if total_songs > 1:
                ...
                self.main_playlist_canvas.yview_moveto(scroll_position)
...
def _get_current_view_position(self):
        ...
        try:
            if not hasattr(self, 'main_playlist_canvas') or not self.main_playlist_canvas.winfo_exists():
                return None
...
def _progressive_load_system(self):
        ...
        try:
            ...
            currently_loaded = len(self.main_playlist_container.winfo_children()) if hasattr(self, 'main_playlist_container') else 0
            ...
            if not hasattr(self, '_last_window_end') or self._last_window_end < target_end:
...
def _append_progressive_items(self, start_index, end_index):
        ...
        try:
            ...
            if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll') and loaded_count > 0:
                ...
                total_loaded = len(self.main_playlist_container.winfo_children())
...
def _setup_progressive_scroll_detection(self):
        ...
        try:
            ...
            if not (hasattr(self, 'main_playlist_canvas') and self.main_playlist_canvas):
                return
...
#                 if scroll_position >= threshold:
#                     print("self.main_playlist_is_loading_more_items ", self.main_playlist_is_loading_more_items)
#                     if not self.main_playlist_is_loading_more_items:                        
#                         self._load_more_on_scroll()
#                         self._check_and_unload_items(self.current_index)
#                         self.main_playlist_is_loading_more_items = True
...
def _adjust_scroll_after_unload(self, unload_count, previous_scroll_position):
    ...
    try:
        ...
        if current_children_count > 0:
            ...
            try:
                scroll_region = self.main_playlist_canvas.cget('scrollregion')
                if scroll_region:
                    # Format: "x1 y1 x2 y2"
                    ...
                    if len(parts) >= 4:
                        ...
                        if total_height > 0:
                            ...
                            self.main_playlist_canvas.yview_moveto(new_scroll_position)
                ...
                try:
                    self.main_playlist_canvas.yview_moveto(max(0.0, previous_scroll_position * 0.9))
                except:
                    pass
        else:
            print("DEBUG: Aucun enfant restant, scroll en haut")
            self.main_playlist_canvas.yview_moveto(0.0)
    ...
    try:
        ...
        if scroll_top <= threshold:
            ...
            def apply_minimal_adjustment():
                try:
                    self.main_playlist_canvas.yview_moveto(new_position)
                    if USE_NEW_CONFIG and get_main_playlist_config('debug_scroll'):
...
def _reorder_playlist_items(self):
        ...
        try:
            if not hasattr(self, 'main_playlist_container') or not self.main_playlist_container.winfo_exists():
                return
            
            # Récupérer tous les enfants avec leur song_index
            children = list(self.main_playlist_container.winfo_children())
            indexed_children = []
