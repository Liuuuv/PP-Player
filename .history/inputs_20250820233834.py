...
class ImportDialog:
    ...
    def _process_html_file(self, html_file_path, max_duration):
        ...
        def process_in_thread():
            try:
                ...
                self._process_videos_in_waves(youtube_links, max_duration, batch_size, html_file_path)
    ...
    def _process_videos_in_waves(self, youtube_links, max_duration, batch_size, html_file_path):
        """Traite les vidéos par vagues : vérifier X → télécharger → vérifier X suivants → télécharger..."""
        def process_wave_thread():
            ...
            for wave_start in range(0, total_links, batch_size):
                ...
                # Phase 1 : Vérifier les durées de cette vague
                valid_videos_wave = []
                skipped_videos_wave = []
                ...
                # Phase 2 : Télécharger les vidéos valides de cette vague
                if valid_videos_wave:
                    ...
                    for j, (url, title, video_data) in enumerate(valid_videos_wave):
                        try:
                            self.music_player.root.after(0, lambda w=wave_num, tw=total_waves, j=j+1, vv=len(valid_videos_wave): 
                                self.music_player.status_bar.config(
                                    text=f"⬇️ Vague {w}/{tw} - Téléchargement {j}/{vv}: {title[:50]}..."))
                    ...
                    # Pause plus longue entre les vagues
                    if wave_end < total_links:
                        self.music_player.root.after(0, lambda: 
                            self.music_player.status_bar.config(text="⏸️ Pause entre les vagues..."))
                        time.sleep(10)
        ...
        threading.Thread(target=process_wave_thread, daemon=True).start()
