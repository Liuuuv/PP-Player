...
def _show_result_context_menu(self, item, event):
    ...
    context_menu.add_command(
        ...
        command=lambda video=item: self._download_youtube_video(video, add_to_main_playlist=True)
    )
    context_menu.add_command(
...
def _download_youtube_video(self, video, add_to_playlist=False, callback=None):
    ...
    if url:
        ...
        threading.Thread(
            ...
            args=(url, add_to_playlist, callback),  # url et add_to_playlist
            daemon=True
        ).start()
