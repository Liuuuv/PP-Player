...
class ToolTip:
    ...
    def show_tip(self, event=None):
        ...
        # Rendre la fenêtre tooltip complètement transparente aux événements de souris
        try:
            tw.wm_attributes('-topmost', True)
            # Rendre la fenêtre transparente aux événements de souris
...
def create_tooltip(widget, text, delay=700, wraplength=180):
    """
    ...
    """
    try:
        # return # tooltips désactivés
        balloon = Pmw.Balloon(widget,  initwait=delay)
        # for child in widget.winfo_children()+[widget]:
        #     balloon.bind(child, text)
        balloon.bind(widget, text)
        return balloon
        return ToolTip(widget, text, delay, wraplength)
    except Exception as e:
        print(f"Erreur lors de la création du tooltip : {e}")
        return None