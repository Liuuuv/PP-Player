    ...
    try:
        ...
        if item_type == "search_result":
            ...
            def on_search_result_click(event):
                ...
                # Clic normal - télécharger et ajouter à la playlist
                if hasattr(self, '_download_and_add_to_playlist'):
                    self._download_and_add_to_playlist(video)
            
            def on_search_result_right_click(event):
                # Menu contextuel pour les résultats de recherche
                if hasattr(self, '_show_search_result_menu'):
                    self._show_search_result_menu(event, video)
