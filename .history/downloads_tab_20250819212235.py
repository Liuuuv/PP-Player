...
from config import DOWNLOADS_INITIAL_LOAD_COUNT, DOWNLOADS_SCROLL_LOAD_COUNT
...
def setup_downloads_tab(self):
    ...
    self.downloads_display_start = 0  # Index de début d'affichage
    self.downloads_display_end = 0    # Index de fin d'affichage
    self.downloads_scroll_threshold = 0.8  # Seuil de scroll pour charger plus d'éléments
    self.downloads_is_loading = False  # Flag pour éviter les chargements multiples
    ...
    # Bind mousewheel avec chargement paresseux
    def _on_downloads_mousewheel(event):
        self.downloads_canvas.yview_scroll(int(-1*(event.delta/120)), "units")
        # Vérifier si on doit charger/décharger des éléments
        self._check_downloads_scroll_position()
    
    self.downloads_canvas.bind("<MouseWheel>", _on_downloads_mousewheel)
    
    # Bind scroll de la scrollbar
    def _on_downloads_scrollbar_scroll(*args):
        self.downloads_canvas.yview(*args)
        self._check_downloads_scroll_position()
    
    downloads_scrollbar.configure(command=_on_downloads_scrollbar_scroll)
...
def create_download_item_widget(self, download_item):
    ...
    # S'assurer que l'overlay est en arrière-plan et les autres widgets au-dessus
    progress_overlay.lower()  # Mettre l'overlay en arrière-plan
...
def update_downloads_display(self):
    """Met à jour l'affichage des téléchargements avec chargement paresseux"""
    ...
    if not self.download_manager.download_queue:
        ...
        self.downloads_display_start = 0
        self.downloads_display_end = 0
    else:
        self.no_downloads_label.pack_forget()
        
        # Initialiser les indices d'affichage
        total_downloads = len(self.download_manager.download_queue)
        self.downloads_display_start = 0
        self.downloads_display_end = min(DOWNLOADS_INITIAL_LOAD_COUNT, total_downloads)
        ...
        # Créer des espaces réservés pour tous les téléchargements
        self._create_downloads_placeholders()
        
        # Charger seulement les premiers éléments
        self._load_downloads_range(self.downloads_display_start, self.downloads_display_end)
...
def _create_downloads_placeholders(self):
    """Crée des espaces réservés pour tous les téléchargements"""
    # Créer un frame placeholder pour chaque téléchargement
    self.downloads_placeholders = []
    for i, download_item in enumerate(self.download_manager.download_queue):
        placeholder = tk.Frame(
            self.downloads_scrollable_frame,
            bg='#2d2d2d',
            height=70,  # Hauteur approximative d'un élément de téléchargement
            relief='flat'
        )
        placeholder.pack(fill="x", padx=CARD_FRAME_PADX, pady=CARD_FRAME_PADY)
        placeholder.pack_propagate(False)  # Maintenir la hauteur fixe
        self.downloads_placeholders.append(placeholder)

def _load_downloads_range(self, start_idx, end_idx):
    """Charge les téléchargements dans la plage spécifiée"""
    if self.downloads_is_loading:
        return
    
    self.downloads_is_loading = True
    
    try:
        for i in range(start_idx, min(end_idx, len(self.download_manager.download_queue))):
            download_item = self.download_manager.download_queue[i]
            
            # Remplacer le placeholder par le vrai widget
            if i < len(self.downloads_placeholders):
                placeholder = self.downloads_placeholders[i]
                
                # Créer le widget réel dans le placeholder
                self._create_download_widget_in_placeholder(download_item, placeholder)
    finally:
        self.downloads_is_loading = False

def _create_download_widget_in_placeholder(self, download_item, placeholder):
    """Crée un widget de téléchargement dans un placeholder"""
    # Supprimer le contenu du placeholder
    ...
    # Configurer le placeholder comme frame principal
    ...
    # Stocker la référence
    download_item.widget_frame = placeholder
    
    # Configuration de la grille
    placeholder.columnconfigure(0, minsize=4, weight=0)   # Indicateur de statut
    placeholder.columnconfigure(1, minsize=100, weight=0) # Miniature plus large
    placeholder.columnconfigure(2, weight=1)              # Texte (prend l'espace restant)
    placeholder.columnconfigure(3, minsize=80, weight=0)  # Progression plus large
    placeholder.columnconfigure(4, minsize=50, weight=0)  # Bouton annuler plus large
    placeholder.rowconfigure(0, minsize=60, weight=0)     # Plus de hauteur
    
    # 1. Indicateur de statut (trait vertical coloré)
    status_color = '#4CAF50' if download_item.completed else '#f44336' if download_item.error else '#2196F3'
    status_indicator = tk.Frame(placeholder, bg=status_color, width=3)
    status_indicator.grid(row=0, column=0, sticky='ns', padx=(2, 2), pady=2)
    status_indicator.grid_propagate(False)
    
    # 2. Miniature de la vidéo
    thumbnail_label = tk.Label(
        placeholder,
        bg=bg_color,
        width=12,
    ...
    # 3. Frame pour le texte (titre + statut)
    text_frame = tk.Frame(placeholder, bg=bg_color)
    text_frame.grid(row=0, column=2, sticky='nsew', padx=(0, 2), pady=4)
    text_frame.columnconfigure(0, weight=1)
    ...
    progress_overlay = tk.Frame(placeholder, bg='#1b5e20')
    progress_overlay.place(x=0, y=0, relheight=1.0, width=0)
...
def _unload_downloads_range(self, start_idx, end_idx):
    """Décharge les téléchargements dans la plage spécifiée"""
    for i in range(start_idx, min(end_idx, len(self.download_manager.download_queue))):
        download_item = self.download_manager.download_queue[i]
        
        # Supprimer le widget du dictionnaire
        ...
        # Remettre le placeholder vide
        if i < len(self.downloads_placeholders):
            placeholder = self.downloads_placeholders[i]
            
            # Supprimer tout le contenu du placeholder
            for child in placeholder.winfo_children():
                child.destroy()
            
            # Remettre le placeholder en état initial
            placeholder.configure(
                bg='#2d2d2d',
                highlightthickness=0
            )

def _check_downloads_scroll_position(self):
    """Vérifie la position de scroll et charge/décharge des éléments si nécessaire"""
    if self.downloads_is_loading or not self.download_manager.download_queue:
        return
    
    try:
        # Obtenir la position de scroll actuelle
        scroll_top = self.downloads_canvas.canvasy(0)
        scroll_bottom = scroll_top + self.downloads_canvas.winfo_height()
        
        # Calculer quels éléments devraient être visibles
        total_height = self.downloads_scrollable_frame.winfo_reqheight()
        if total_height <= 0:
            return
        
        item_height = 70 + CARD_FRAME_PADY * 2  # Hauteur approximative d'un élément
        total_items = len(self.download_manager.download_queue)
        
        # Calculer les indices visibles avec une marge
        visible_start = max(0, int(scroll_top / item_height) - 5)
        visible_end = min(total_items, int(scroll_bottom / item_height) + 5)
        
        # Déterminer la nouvelle plage à charger
        new_start = max(0, visible_start - DOWNLOADS_SCROLL_LOAD_COUNT // 2)
        new_end = min(total_items, visible_end + DOWNLOADS_SCROLL_LOAD_COUNT // 2)
        
        # Limiter la plage totale chargée
        max_loaded = DOWNLOADS_INITIAL_LOAD_COUNT + DOWNLOADS_SCROLL_LOAD_COUNT * 2
        if new_end - new_start > max_loaded:
            # Centrer la plage autour de la zone visible
            center = (visible_start + visible_end) // 2
            new_start = max(0, center - max_loaded // 2)
            new_end = min(total_items, new_start + max_loaded)
        
        # Vérifier si on doit changer la plage chargée
        if new_start != self.downloads_display_start or new_end != self.downloads_display_end:
            # Décharger les éléments qui ne sont plus dans la plage
            if new_start > self.downloads_display_start:
                self._unload_downloads_range(self.downloads_display_start, new_start)
            if new_end < self.downloads_display_end:
                self._unload_downloads_range(new_end, self.downloads_display_end)
            
            # Charger les nouveaux éléments
            if new_start < self.downloads_display_start:
                self._load_downloads_range(new_start, self.downloads_display_start)
            if new_end > self.downloads_display_end:
                self._load_downloads_range(self.downloads_display_end, new_end)
            
            # Mettre à jour les indices
            self.downloads_display_start = new_start
            self.downloads_display_end = new_end
