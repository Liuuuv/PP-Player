...
def show_cache_menu(self):
    ...
    try:
        ...
        # Ajouter un titre
        cache_menu.add_command(label="🗂️ Gestion du cache", state='disabled')
        cache_menu.add_separator()
        
        # Options de nettoyage du cache
        cache_menu.add_command(
            label="🔍 Vider le cache des recherches",
            command=self._clear_search_cache
        )
        
        cache_menu.add_command(
            label="🎤 Vider le cache des artistes",
            command=self._clear_artist_cache
        )
        
        cache_menu.add_command(
            label="🖼️ Vider le cache des miniatures",
            command=self._clear_thumbnail_cache
        )
        
        cache_menu.add_command(
            label="📋 Vider le cache des playlists",
