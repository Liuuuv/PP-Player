...
class MusicPlayer:
    def __init__(self, root):
        ...
        # Variables pour l'animation des points de statut
        self.status_animation_timer = None
        self.status_animation_dots = 0  # 0, 1, 2, 3 pour ".", "..", "...", ""
        self.status_base_text = ""  # Texte de base sans les points
        self.status_is_animating = False
    ...
    def play_track(self):
        try:
            ...
            self.start_status_animation("Playing")
    ...
    def search_youtube(self):
        ...
        self._clear_results()
        self.search_list = []
        self.start_status_animation("Recherche en cours")
        self.root.update()
        ...
        try:
            ...
            with YoutubeDL(search_opts) as ydl:
                ...
                if not self.all_search_results:
                    self.root.after(0, lambda: (self.stop_status_animation(), self.status_bar.config(text="Aucun résultat trouvé")))
                    self.root.after(0, self._show_current_song_thumbnail)
                    return
            ...
            self.root.after(0, lambda: (self.stop_status_animation(), self.status_bar.config(text=f"Erreur recherche: {e}")))
        finally:
            self.is_searching = False
            self.is_loading_more = False
    ...
    def start_status_animation(self, base_text):
        """Démarre l'animation des points pour un texte de statut"""
        self.status_base_text = base_text
        self.status_is_animating = True
        self.status_animation_dots = 0
        self._animate_status_dots()
    
    def stop_status_animation(self):
        """Arrête l'animation des points de statut"""
        self.status_is_animating = False
        if self.status_animation_timer:
            self.root.after_cancel(self.status_animation_timer)
            self.status_animation_timer = None
    
    def _animate_status_dots(self):
        """Anime les points du texte de statut"""
        if not self.status_is_animating:
            return
        
        # Cycle des points: ".", "..", "...", ""
        dots_patterns = [".", "..", "...", ""]
        dots = dots_patterns[self.status_animation_dots]
        
        # Mettre à jour le texte
        self.status_bar.config(text=f"{self.status_base_text}{dots}")
        
        # Passer au pattern suivant
        self.status_animation_dots = (self.status_animation_dots + 1) % 4
        
        # Programmer la prochaine animation (500ms)
        self.status_animation_timer = self.root.after(500, self._animate_status_dots)
    ...
    def _display_batch_results(self, batch_number):
        ...
        self.root.after(0, lambda: (
            self.stop_status_animation(),
            self.status_bar.config(text=f"{self.search_results_count}/{len(self.all_search_results)} résultats affichés (lot {batch_number})")
        ))
    ...
    def _download_youtube_thread(self, url):
        try:
            ...
            self.root.after(0, lambda e=e: (self.stop_status_animation(), self.status_bar.config(text=f"Erreur: {str(e)}")))
    ...
    def _download_progress_hook(self, d):
        ...
        if d['status'] == 'downloading':
            ...
            self.root.after(0, lambda: self.start_status_animation(f"Téléchargement {percent} à {speed}"))
