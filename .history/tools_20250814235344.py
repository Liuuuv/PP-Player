    ...
    try:
        ...
        def on_enter(event):
            ...
            if not item_frame.selected:  # Ne pas changer la couleur si l'item est sélectionné
                ...
                hover_color = self._lighten_color(bg_color, 0.2)  # 20% plus clair
                self._set_item_colors(item_frame, hover_color)
        
        def on_leave(event):
            """Restaure la couleur originale quand la souris quitte l'item"""
            if not item_frame.selected:  # Ne pas changer la couleur si l'item est sélectionné
                self._set_item_colors(item_frame, bg_color)
        
        # Bind du clic sur l'artiste
        artist_label.bind("<Button-1>", on_artist_click)
        # Ajouter les événements de survol pour l'artiste aussi
        artist_label.bind("<Enter>", on_enter)
        artist_label.bind("<Leave>", on_leave)
