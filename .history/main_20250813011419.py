...
class MusicPlayer:
    ...
    def _create_artist_tabs(self):
        ...
        # Positionner la croix comme stats.png et output.png (à droite des onglets, plus à droite que output)
        close_btn.place(in_=self.artist_notebook, relx=1.0, rely=0.0, anchor="ne", x=-70, y=-5)
    ...
    def _add_artist_result(self, video, index, container):
        ...
        try:
            ...
            # Configuration de la grille (plus compact avec 2 lignes pour titre+durée)
            result_frame.columnconfigure(0, minsize=60, weight=0)  # Miniature plus petite
            result_frame.columnconfigure(1, weight=1)              # Titre et durée
            result_frame.columnconfigure(2, minsize=80, weight=0)  # Date
            ...
            duration_under_title_label = tk.Label(
                text_frame,
                text=duration_text,
                bg='#4a4a4a',
                fg='#aaaaaa',
            ...
            duration_under_title_label.grid(row=1, column=0, sticky='ew', pady=(0, 1))
            ...
            if upload_date:
                try:
                    # Convertir YYYYMMDD en DD/MM/YY
                    if len(upload_date) >= 8 and upload_date.isdigit():
                        year = upload_date[:4]
                        month = upload_date[4:6]
                        day = upload_date[6:8]
                        date_text = f"{day}/{month}/{year[2:]}"
                    else:
                        date_text = upload_date[:10] if len(upload_date) >= 10 else upload_date
                except:
                    date_text = "?"
            ...
            date_label.grid(row=0, column=2, sticky='ns', padx=(0, 5), pady=3)
            ...
            duration_label.grid(row=0, column=3, sticky='ns', padx=(0, 5), pady=3)
