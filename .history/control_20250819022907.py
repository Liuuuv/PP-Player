...
def next_track(self):
    ...
    if hasattr(self, '_is_index_already_loaded') and hasattr(self, '_check_and_unload_items'):
        try:
            ...
            if not next_is_loaded and hasattr(self, '_append_progressive_items'):
            ...
            self.safe_after(50, lambda: self._check_and_unload_items(next_index))
            
        except Exception as e:
            if hasattr(self, 'debug_scroll') and self.debug_scroll:
                print(f"❌ Erreur vérification musique suivante: {e}")
