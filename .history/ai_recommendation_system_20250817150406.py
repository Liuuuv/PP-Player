...
class MusicAIRecommendationSystem:
    ...
    def on_song_end(self, song_path, was_skipped=False, listening_duration=None):
        ...
        if song_data['was_skipped']:
            self.user_behavior_data['skip_patterns'].append({
                'song_path': song_path,
                'timestamp': end_time,
                'listening_ratio': listening_ratio,
                'skip_reason': 'manual' if was_skipped else 'early_stop'
            })
            print(f"🤖 IA: Skip détecté - raison: {'manuel' if was_skipped else 'arrêt précoce'}")
