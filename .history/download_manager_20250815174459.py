...
def download_youtube_video(app, url, title=None, video_data=None, callback_on_complete=None, callback_params=None):
    ...
    def download_thread():
        nonlocal title, video_data  # Permettre la modification des variables de la fonction parent
        try:
            ...
            app.root.after(0, lambda: setattr(app, '_temp_download_result', 
                app.add_download_to_tab(url, title, video_data)))
            ...
            if not download_added:
                print(f"Téléchargement déjà en cours ou terminé: {title}")
                return False
            
            # Étape 3: Effectuer le téléchargement réel
            app.root.after(0, lambda: app.status_bar.config(text=f"Téléchargement: {title}"))
            ...
            def progress_hook(d):
                if d['status'] == 'downloading':
                    ...
                    upload_date = video_data.get('upload_date') if video_data else None
                    ...
                    app.root.after(0, lambda: app.status_bar.config(text=f"Téléchargé: {title}"))
