...
def show_unified_context_menu(self, item, event=None, item_type="file"):
    ...
    if item_type == "file":
        ...
        # Options pour vidéo YouTube (mêmes que pour les fichiers locaux)
        menu.add_command(
            label="📂 Ouvrir le dossier",
            command=lambda: self._open_downloads_folder()
        )
        ...
        # Option de téléchargement (équivalent de "supprimer" pour les fichiers locaux)
        menu.add_command(
            label="⬇️ Télécharger maintenant",
            command=lambda: self._download_youtube_video(item),
            foreground='#44ff44'  # Vert pour téléchargement
        )
