...
class MusicPlayer:
    ...
    def _should_load_more_results(self):
        ...
        try:
            ...
            if scroll_bottom > 0.8:
                print(f"Scroll détecté: {scroll_bottom:.2f}, chargement de plus de résultats...")
                return True
    ...
    def _perform_batch_search(self, query, batch_number):
        ...
        try:
            search_opts = {
                ...
                'quiet': True,
                'no_warnings': True,
                'max_downloads': 15,  # Un peu plus pour avoir 10 après filtrage
                'ignoreerrors': True
            }
            
            with YoutubeDL(search_opts) as ydl:
                # Recherche de 15 résultats pour avoir 10 après filtrage
                ...
                batch_results = video_results[:10]
                ...
                self.root.after(0, lambda: self.status_bar.config(
                    text=f"{self.search_results_count} résultats trouvés (lot {batch_number})"
                ))
    ...
    def _load_more_search_results(self):
        ...
        print(f"_load_more_search_results appelée - Résultats: {self.search_results_count}/{len(self.all_search_results)}")
        ...
        print(f"Chargement de {count} résultats à partir de l'index {start_index}")
