...
class MusicPlayer:
    ...
    def _recreate_thumbnail_frame(self):
        ...
        try:
            ...
            self.thumbnail_frame = tk.Frame(
                ...
                name='thumbnail_frame'  # Nommer explicitement la frame
            )
            # Afficher la frame thumbnail EN PREMIER (avec before pour être au-dessus)
            self.thumbnail_frame.pack(fill='both', pady=5, padx=5, anchor="center")
            
            # S'assurer qu'elle soit au premier plan (lift la met au-dessus)
            self.thumbnail_frame.lift()
            
            print("DEBUG: thumbnail_frame recréée, packée et mise au premier plan avec succès")
    ...
    def _return_to_search(self):
        ...
        try:
            ...
            if hasattr(self, 'playlist_back_btn'):
                try:
                    if self.playlist_back_btn.winfo_exists():
                        print("DEBUG: playlist_back_btn existe, destruction...")
                        self.playlist_back_btn.destroy()
                        print("DEBUG: playlist_back_btn détruit")
                    else:
                        print("DEBUG: playlist_back_btn n'existe plus")
                    delattr(self, 'playlist_back_btn')
                    print("DEBUG: attribut playlist_back_btn supprimé")
                except Exception as e:
                    print(f"DEBUG: Erreur suppression playlist_back_btn: {e}")
            ...
            # SOLUTION: Supprimer TOUTES les frames restantes qui ne sont pas thumbnail_frame, youtube_canvas, ou scrollbar
            print("DEBUG: NETTOYAGE COMPLET - Suppression de toutes les frames artiste restantes...")
            if hasattr(self, 'youtube_results_frame'):
                try:
                    children_to_remove = []
                    for child in self.youtube_results_frame.winfo_children():
                        try:
                            widget_class = child.__class__.__name__
                            widget_name = child.winfo_name() if hasattr(child, 'winfo_name') else "sans_nom"
                            
                            # Garder seulement les widgets essentiels : Canvas (youtube_canvas) et Scrollbar
                            # Supprimer toutes les Frame qui ne sont pas thumbnail_frame
                            should_remove = False
                            
                            if widget_class == 'Frame':
                                # Supprimer toutes les frames sauf si c'est thumbnail_frame (par le nom)
                                if widget_name != 'thumbnail_frame':
                                    should_remove = True
                                    print(f"DEBUG: Frame résiduelle détectée pour suppression: {widget_name}")
                            elif widget_class not in ['Canvas', 'Scrollbar']:
                                # Supprimer tout ce qui n'est pas Canvas, Scrollbar ou Frame
                                should_remove = True
                                print(f"DEBUG: Widget non-essentiel détecté pour suppression: {widget_class} ({widget_name})")
                            
                            if should_remove:
                                children_to_remove.append(child)
                        except Exception as e:
                            print(f"DEBUG: Erreur lors de l'analyse du widget: {e}")
                    
                    # Supprimer les frames résiduelles
                    for child in children_to_remove:
                        try:
                            if child.winfo_exists():
                                print(f"DEBUG: Suppression de la frame résiduelle: {child.winfo_name()}")
                                child.destroy()
                        except Exception as e:
                            print(f"DEBUG: Erreur lors de la suppression de la frame résiduelle: {e}")
                    
                    print(f"DEBUG: {len(children_to_remove)} frames résiduelles supprimées")
                    
                except Exception as e:
                    print(f"DEBUG: Erreur lors du nettoyage complet: {e}")
