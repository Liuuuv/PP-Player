...
class MusicPlayer:
    ...
    def _add_playlist_item(self, filepath, thumbnail_path=None):
        """Ajoute un élément à la playlist avec bouton poubelle flottant"""
        try:
            filename = os.path.basename(filepath)
            
            # Frame principal avec placement relatif
            item_frame = ttk.Frame(
                self.playlist_container,
                style='TFrame',
                height=50  # Hauteur fixe pour chaque item
            )
            item_frame.pack(fill='x', pady=2)
            item_frame.pack_propagate(False)  # Empêche le redimensionnement automatique
            
            # Canvas pour le placement absolu
            canvas = tk.Canvas(
                item_frame,
                bg='#3d3d3d',
                highlightthickness=0,
            ...
            # canvas.create_window(5, 25, anchor='w', window=thumbnail_label)
            canvas.create_window(5, 10, anchor='w', window=thumbnail_label)
            
            # Texte (position absolue)
            text_label = tk.Label(
                canvas,
                text=filename,
                bg='#3d3d3d',
            ...
            # canvas.create_window(90, 25, anchor='w', window=text_label, width=300)
            canvas.create_window(20, 10, anchor='w', window=text_label, width=30)
            
            # Bouton poubelle (position absolue par-dessus)
            delete_btn = tk.Button(
                ...
                relief='raised',
                borderwidth=1,
                command=lambda f=filepath: self._remove_playlist_item(f, item_frame)
            )
            # Positionné à droite avec un décalage pour chevaucher
            # canvas.create_window(380, 25, anchor='e', window=delete_btn)
            canvas.create_window(20, 10, anchor='e', window=delete_btn)
            
            # Gestion des événements
            def select_item(e):
                canvas.configure(bg='#4a4a4a')
                thumbnail_label.configure(bg='#4a4a4a')
                text_label.configure(bg='#4a4a4a')
                
            canvas.bind("<Button-1>", select_item)
            canvas.bind("<Double-1>", lambda e, f=filepath: self._play_playlist_item(f))
