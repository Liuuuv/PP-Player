...
def _display_thumbnail(self, label, photo):
    ...
    print(f"DEBUG: _display_thumbnail appelé")
    try:
        # Vérifier si le widget existe encore avant de l'utiliser
        if label.winfo_exists():
            print(f"DEBUG: Label existe, configuration de l'image")
            label.configure(image=photo)
            label.image = photo  # Garder une référence
            print(f"DEBUG: Image configurée avec succès")
        else:
            print(f"DEBUG: Label n'existe plus lors de l'affichage")
    except tk.TclError as e:
        # Le widget a été détruit, ignorer silencieusement
        print(f"DEBUG: TclError lors de l'affichage: {e}")
        pass
    except Exception as e:
        print(f"DEBUG: Erreur affichage thumbnail: {e}")
        import traceback
        traceback.print_exc()
...
def _load_thumbnail(self, label, url):
    ...
    print(f"DEBUG: _load_thumbnail appelé avec URL: {url}")
    try:
        import requests
        from io import BytesIO
        ...
        try:
            if not label.winfo_exists():
                print("DEBUG: Label n'existe plus, abandon du chargement")
                return
        except tk.TclError:
            # Le widget a été détruit
            print("DEBUG: TclError - widget détruit")
            return
        
        # Corriger l'URL si elle commence par //
        if url.startswith('//'):
            url = 'https:' + url
            print(f"DEBUG: URL corrigée: {url}")
        
        print(f"DEBUG: Téléchargement de la miniature depuis: {url}")
        response = requests.get(url, timeout=5)
        print(f"DEBUG: Réponse reçue, status: {response.status_code}, taille: {len(response.content)} bytes")
        
        img_data = BytesIO(response.content)
        img = Image.open(img_data)
        print(f"DEBUG: Image ouverte, taille originale: {img.size}")
        ...
        print(f"DEBUG: Redimensionnement vers: {thumbnail_size}")
        img.thumbnail(thumbnail_size, Image.Resampling.LANCZOS)
        photo = ImageTk.PhotoImage(img)
        print(f"DEBUG: PhotoImage créé")
        
        # Vérifier à nouveau avant d'afficher
        try:
            if label.winfo_exists():
                print(f"DEBUG: Affichage de la miniature dans le label")
                self.root.after(0, lambda: self._display_thumbnail(label, photo))
            else:
                print("DEBUG: Label n'existe plus au moment de l'affichage")
        except tk.TclError:
            # Le widget a été détruit entre temps
            print("DEBUG: TclError lors de l'affichage")
            pass
    except Exception as e:
        print(f"DEBUG: Erreur chargement thumbnail: {e}")
        import traceback
        traceback.print_exc()
...
def _download_youtube_thread(self, url, add_to_playlist=True):
    ...
    try:
        ...
        # Ajouter à l'onglet téléchargements SEULEMENT si le fichier n'existe pas déjà
        ...
        if hasattr(self, 'current_download_url'):
            self.root.after(0, lambda: self.download_manager.mark_error(self.current_download_url, f"Erreur: {str(e)}"))
    finally:
...
def _add_song_item(self, filepath_or_video, container, playlist_name=None, song_index=None, item_type="downloads", video_index=None):
    ...
    try:
        ...
        if item_type == "search_result":
            ...
            def on_search_result_right_click(event):
                ...
                if hasattr(self, 'selected_items') and self.selected_items:
                    ...
                    # Menu contextuel pour les résultats de recherche
                    if hasattr(self, '_on_result_right_click'):
                        self._on_result_right_click(event, item_frame)
        ...
        if item_type == "search_result" and video and video.get('thumbnails'):
            print(f"DEBUG: Chargement miniature pour résultat de recherche: {title}")
            # Stocker la référence pour le chargement différé
            thumbnail_label.video_data = video
                ...
                try:
                    ...
                    if thumbnails:
                        ...
                        if best_thumbnail:
                            ...
                            print(f"DEBUG: URL miniature trouvée: {thumbnail_url}")
                            
                            # Utiliser la fonction appropriée selon le type
                            if is_channel:
                                print(f"DEBUG: Chargement miniature circulaire pour chaîne")
                                ...
                                print(f"DEBUG: Chargement miniature rectangulaire pour vidéo")
                            ...
                            print(f"DEBUG: Aucune URL de miniature valide trouvée")
                    else:
                        print(f"DEBUG: Aucune miniature disponible pour cette vidéo")
                except Exception as e:
                    print(f"DEBUG: Erreur chargement miniature recherche: {e}")
...
def _download_and_add_to_playlists(self, video_url, playlist_names):
        ...
        try:
            ...
            if existing_file:
                ...
                # Rafraîchir l'affichage
                self.safe_after(0, self._refresh_playlist_display)
                return
