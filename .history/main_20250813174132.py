...
class MusicPlayer:
    def __init__(self, root):
        ...
        # Variables pour la gestion des boutons retour dans les onglets artiste
        self.playlist_back_btn_sorties = None  # Bouton retour pour l'onglet Sorties
        self.playlist_back_btn_playlists = None  # Bouton retour pour l'onglet Playlists
        self.playlist_content_active_sorties = False  # True si du contenu de playlist est affiché dans Sorties
        self.playlist_content_active_playlists = False  # True si du contenu de playlist est affiché dans Playlists
        
        # Variables de compatibilité (pour ne pas casser le code existant)
        self.playlist_back_btn = None  # Référence au bouton actuel (sera mise à jour dynamiquement)
        self.current_playlist_content_tab = None  # Onglet où du contenu de playlist est affiché ("Sorties" ou "Playlists")
        self.playlist_content_active = False  # True si du contenu de playlist est affiché
    ...
    def on_artist_tab_changed(self, event):
        ...
        try:
            if not hasattr(self, 'artist_notebook') or not self.artist_notebook.winfo_exists():
                return
            ...
            if current_tab_id:
                ...
                # Mettre à jour les variables de compatibilité selon l'onglet actuel
                if current_tab_text == "Sorties":
                    self.playlist_back_btn = self.playlist_back_btn_sorties
                    self.playlist_content_active = self.playlist_content_active_sorties
                    self.current_playlist_content_tab = "Sorties" if self.playlist_content_active_sorties else None
                elif current_tab_text == "Playlists":
                    self.playlist_back_btn = self.playlist_back_btn_playlists
                    self.playlist_content_active = self.playlist_content_active_playlists
                    self.current_playlist_content_tab = "Playlists" if self.playlist_content_active_playlists else None
                
                # Afficher/masquer les boutons selon l'état de chaque onglet
                self._update_back_buttons_visibility()
                
        except Exception as e:
            print(f"Erreur dans on_artist_tab_changed: {e}")
    
    def _update_back_buttons_visibility(self, current_tab_text=None):
        """Met à jour la visibilité des boutons retour selon l'état de chaque onglet"""
        try:
            # Gérer chaque onglet indépendamment selon son état
            if self.playlist_content_active_sorties:
                self._show_playlist_back_button_for_tab("sorties")
            else:
                self._hide_playlist_back_button_for_tab("sorties")
                
            if self.playlist_content_active_playlists:
                self._show_playlist_back_button_for_tab("playlists")
            else:
                self._hide_playlist_back_button_for_tab("playlists")
                
        except Exception as e:
            print(f"Erreur dans _update_back_buttons_visibility: {e}")
    
    def _show_playlist_back_button_for_tab(self, tab_name):
        """Affiche le bouton retour pour un onglet spécifique"""
        try:
            button = None
            if tab_name == "sorties":
                button = self.playlist_back_btn_sorties
            elif tab_name == "playlists":
                button = self.playlist_back_btn_playlists
            
            if button is not None:
                try:
                    if button.winfo_exists():
                        button.place(in_=button.master, relx=1.0, rely=0.0, anchor="ne", x=-40, y=5)
                        button.tkraise()
                except:
                    pass
        except Exception as e:
            print(f"Erreur lors de l'affichage du bouton retour pour {tab_name}: {e}")
    
    def _show_playlist_back_button(self):
        """Affiche le bouton retour pour les playlists"""
        try:
            if hasattr(self, 'playlist_back_btn') and self.playlist_back_btn and self.playlist_back_btn.winfo_exists():
                self.playlist_back_btn.place(in_=self.playlist_back_btn.master, relx=1.0, rely=0.0, anchor="ne", x=-40, y=5)
                self.playlist_back_btn.tkraise()
        except Exception as e:
    ...
    def _reset_playlist_content_state(self, tab_name=None):
        ...
        if tab_name is None or tab_name == "sorties":
            self.playlist_content_active_sorties = False
            self._hide_playlist_back_button_for_tab("sorties")
        
        if tab_name is None or tab_name == "playlists":
            self.playlist_content_active_playlists = False
            self._hide_playlist_back_button_for_tab("playlists")
        
        # Mettre à jour les variables de compatibilité et la visibilité des boutons
        if hasattr(self, 'artist_notebook') and self.artist_notebook.winfo_exists():
            try:
                ...
                if current_tab_id:
                    ...
                    # Mettre à jour la visibilité des boutons selon l'état de chaque onglet
                    self._update_back_buttons_visibility()
            except:
                pass
    
    def _hide_playlist_back_button_for_tab(self, tab_name):
        """Masque le bouton retour pour un onglet spécifique"""
        try:
            button = None
            if tab_name == "sorties":
                button = self.playlist_back_btn_sorties
            ...
            print(f"Erreur lors du masquage du bouton retour pour {tab_name}: {e}")
    ...
    def add_to_multiple_playlists(self, playlist_names):
        ...
        return library_tab.playlists.add_to_multiple_playlists(self, playlist_names)
    ...
    def _refresh_playlist_display(self):
        ...
        return search_tab.main_playlist._refresh_display(self)
    ...
    def _show_artist_content(self, artist_name, video_data):
        ...
        return artist_tab.core._show_artist_content(self, artist_name, video_data)
    ...
    def _display_results_in_batches(self, items, container, item_type, batch_size=3):
        ...
        return artist_tab.core._display_results_in_batches(self, items, container, item_type, batch_size)
    ...
    def _find_artist_channel_id(self):
        ...
        return artist_tab.core._find_artist_channel_id(self)

    def _search_artist_videos_with_id(self):
        """Recherche les vidéos de l'artiste depuis l'onglet Vidéos de sa chaîne"""
        return artist_tab.songs._search_artist_videos_with_id(self)
    ...
    def _add_artist_playlist_result(self, playlist, index, container, target_tab="sorties"):
        ...
        return artist_tab.core._add_artist_playlist_result(self, playlist, index, container, target_tab)
    ...
    def _show_playlist_content(self, playlist_data, target_tab="sorties"):
        ...
        return library_tab.playlists._show_playlist_content(self, playlist_data, target_tab)

    def _show_playlist_loading(self, playlist_title, target_tab="sorties"):
        """Affiche un message de chargement pour la playlist"""
        return artist_tab.playlists._show_playlist_loading(self, playlist_title, target_tab)
