...
def _show_single_file_menu(self, event, filepath):
    ...
    if not is_youtube_video:
        context_menu.add_command(
            ...
            command=lambda: self.add_to_main_playlist(filepath)
        )
        context_menu.add_command(
            label="⏭️ Lire ensuite",
            command=lambda: self.drag_drop_handler._add_to_queue_first(filepath)
        )
        context_menu.add_command(
            label="⏰ Lire bientôt", 
            command=lambda: self.drag_drop_handler._add_to_queue(filepath)
        )
        context_menu.add_separator()
    ...
    for playlist_name in self.playlists.keys():
        if playlist_name != "Main Playlist":
            if is_youtube_video:
                context_menu.add_command(
                    label=playlist_name,
                    command=lambda p=playlist_name: self._download_and_add_to_playlist(filepath, p)
                )
            else:
                context_menu.add_command(
                    label=playlist_name,
                    command=lambda p=playlist_name: self._add_to_specific_playlist(filepath, p)
                )
    ...
    context_menu.add_command(
        label="Créer nouvelle playlist...",
        command=lambda: self._create_new_playlist_dialog(filepath, is_youtube_video)
    )
