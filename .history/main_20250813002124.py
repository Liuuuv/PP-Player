                        ...
                        for url in channel_urls_to_try:
                            ...
                            try:
                                ...
                                if "404" in error_msg or "Not Found" in error_msg:
                                    print(f"Chaîne introuvable pour {url}")
                                    # Si l'URL contient des caractères encodés problématiques, le signaler
                                    if '%' in url and any(x in url for x in ['%E', '%C', '%D']):
                                        print(f"  -> URL avec encodage détecté, cela peut être la cause de l'erreur 404")
                                elif "Unable to download API page" in error_msg:
                                    print(f"API YouTube indisponible pour {url}")
                                else:
