    ...
    def draw_slider(self, force_redraw=False):
        ...
        try:
            ...
            if not self.canvas_objects or force_redraw:
                ...
                # DEBUG: Afficher les calculs de la barre bleue pendant le drag
                if self.dragging:
                    print(f"[DEBUG] BLUE BAR UPDATE - current_value: {self.current_value}, to: {self.to}, progress: {actual_progress:.3f}")
                
                if actual_progress > 0:
                    actual_fill_x = track_start_x + actual_progress * track_width
                    if self.dragging:
                        print(f"  actual_fill_x: {actual_fill_x}, track_start_x: {track_start_x}, track_width: {track_width}")
                    if 'fill' in self.canvas_objects:
                        self.canvas.coords(self.canvas_objects['fill'],
                                         track_start_x, track_y - self.track_height//2,
                                         actual_fill_x, track_y + self.track_height//2)
                        if self.dragging:
                            print(f"  -> Updated fill coords: ({track_start_x}, {track_y - self.track_height//2}) to ({actual_fill_x}, {track_y + self.track_height//2})")
                    else:
