                ...
                if self.playlist_content_container.winfo_exists():
                    ...
                    for child in self.playlist_content_container.winfo_children():
                        try:
                            # Vérifier que le child existe encore
                            if child.winfo_exists() and hasattr(child, 'selected'):
                                print(f"DEBUG: Traitement du child: {child}")
                                print(f"DEBUG: Type du child: {type(child)}")
                                
                                # Vérifier s'il y a des images dans ce frame
                                if hasattr(child, 'winfo_children'):
                                    for subchild in child.winfo_children():
                                        if hasattr(subchild, 'cget'):
                                            try:
                                                image_info = subchild.cget('image')
                                                if image_info:
                                                    print(f"DEBUG: Image trouvée dans le frame: {image_info}")
                                            except:
                                                pass
                                        # Vérifier si c'est un Label avec du texte qui pourrait contenir 'delete'
                                        if hasattr(subchild, 'cget'):
                                            try:
                                                text_info = subchild.cget('text')
                                                if text_info and 'delete' in str(text_info).lower():
                                                    print(f"DEBUG: Texte 'delete' trouvé: {text_info}")
                                            except:
                                                pass
                                
                                child.selected = False
                                if hasattr(self, '_set_item_colors'):
                                    print(f"DEBUG: Appel de _set_item_colors sur le frame")
                                    self._set_item_colors(child, '#4a4a4a')
                                else:
                                    print(f"DEBUG: _set_item_colors non disponible, utilisation de config")
                                    try:
                                        child.config(bg='#4a4a4a')
                                    except:
                                        pass
