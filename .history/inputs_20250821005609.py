    ...
    def _process_html_file(self, html_file_path, max_duration):
        ...
        def process_in_thread():
            try:
                ...
                # Démarrer une session de logs (version simplifiée)
                session_id = None
                try:
                    if hasattr(self.import_logger, 'start_session'):
                        session_id = self.import_logger.start_session('HTML', html_file_path)
                        print(f"✅ Session créée: {session_id}")
                    else:
                        print("⚠️ Logger sans start_session, on continue...")
                        session_id = "manual_session"
                except Exception as e:
                    print(f"❌ Erreur création session: {e}, on continue quand même...")
                    session_id = "fallback_session"
                
                try:
                    if hasattr(self.import_logger, 'set_total_links'):
                ...
                try:
                    if hasattr(self.import_logger, 'set_pending_urls'):
