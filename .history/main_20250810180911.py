    ...
    def show_selection_menu(self, event):
        ...
        for playlist_name in self.playlists.keys():
            if playlist_name != "Main Playlist":
                ...
                # Permettre de cliquer sur le frame pour cocher/décocher
                def toggle_checkbox(event, v=var):
                    v.set(not v.get())
                
                # Bind le clic sur le frame
                playlist_frame.bind("<Button-1>", toggle_checkbox)
