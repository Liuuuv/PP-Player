...
class MusicPlayer:
    ...
    def _display_playlist_content(self, videos, playlist_title, target_tab="sorties"):
        ...
        # Bouton retour (icône back, même taille que la croix)
        back_command = self._return_to_playlists_tab if target_tab == "playlists" else self._return_to_playlists
        
        if hasattr(self, 'icons') and 'back_small' in self.icons:
            back_button = tk.Button(
                target_frame,
                image=self.icons['back_small'],
                bg='#3d3d3d',
        ...
        # Positionner le bouton à gauche de la croix
        back_button.place(in_=target_frame, relx=1.0, rely=0.0, anchor="ne", x=-30, y=5)
        back_button.tkraise()  # Mettre le bouton au premier plan
