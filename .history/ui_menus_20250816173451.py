...
def _update_playlists_paths(old_playlists_file, new_playlists_file, old_downloads_folder, new_downloads_folder):
    """Met à jour les chemins des fichiers dans playlists.json après un déplacement de dossier"""
    import json
    
    try:
        # Charger l'ancien fichier playlists.json
        with open(old_playlists_file, 'r', encoding='utf-8') as f:
            playlists = json.load(f)
        
        # Normaliser les chemins pour éviter les problèmes de séparateurs
        old_downloads_folder = os.path.normpath(old_downloads_folder)
        new_downloads_folder = os.path.normpath(new_downloads_folder)
        
        updated_playlists = {}
        
        # Mettre à jour les chemins pour chaque playlist
        for playlist_name, songs in playlists.items():
            updated_songs = []
            
            for song_path in songs:
                # Normaliser le chemin du fichier
                normalized_song_path = os.path.normpath(song_path)
                
                # Gérer les chemins relatifs qui commencent par "downloads"
                if song_path.startswith("downloads"):
                    # Enlever le préfixe "downloads" et créer le nouveau chemin absolu
                    relative_path = song_path[len("downloads"):].lstrip(os.sep).lstrip("/").lstrip("\\")
                    new_song_path = os.path.join(new_downloads_folder, relative_path)
                    new_song_path = os.path.normpath(new_song_path)
                    
                    # Vérifier si le fichier existe dans le nouveau dossier
                    if os.path.exists(new_song_path):
                        updated_songs.append(new_song_path)
                        print(f"Chemin relatif mis à jour: {song_path} -> {new_song_path}")
                    else:
                        print(f"Fichier non trouvé après déplacement: {new_song_path}")
                
                # Vérifier si le chemin commence par l'ancien dossier de téléchargements (chemins absolus)
                elif normalized_song_path.startswith(old_downloads_folder):
                    # Remplacer l'ancien chemin par le nouveau
                    relative_path = os.path.relpath(normalized_song_path, old_downloads_folder)
                    new_song_path = os.path.join(new_downloads_folder, relative_path)
                    new_song_path = os.path.normpath(new_song_path)
                    ...
                    if os.path.exists(new_song_path):
                        updated_songs.append(new_song_path)
                        print(f"Chemin absolu mis à jour: {song_path} -> {new_song_path}")
                    else:
                        print(f"Fichier non trouvé après déplacement: {new_song_path}")
                else:
                    # Si le chemin ne commence pas par l'ancien dossier, le garder tel quel
                    # (peut être un fichier externe à la bibliothèque)
                    if os.path.exists(normalized_song_path):
                        updated_songs.append(normalized_song_path)
                    else:
                        print(f"Fichier externe non trouvé: {normalized_song_path}")
            
            # Ajouter la playlist seulement si elle contient des chansons valides
            if updated_songs:
                updated_playlists[playlist_name] = updated_songs
            else:
                print(f"Playlist '{playlist_name}' supprimée car aucun fichier valide trouvé")
        
        # Sauvegarder le fichier mis à jour
        with open(new_playlists_file, 'w', encoding='utf-8') as f:
            json.dump(updated_playlists, f, ensure_ascii=False, indent=2)
        
        print(f"Fichier playlists.json mis à jour avec {len(updated_playlists)} playlists")
        
    except Exception as e:
        print(f"Erreur lors de la mise à jour des chemins dans playlists.json: {e}")
        raise
