...
class MusicPlayer:
    ...
    def _create_artist_tabs(self):
        ...
        # Créer le notebook dans le results_container, au-dessus des résultats existants
        self.artist_notebook = ttk.Notebook(self.results_container, takefocus=0)
        
        # Placer le notebook en premier (au-dessus)
        if self.saved_search_state['results_children']:
            # Temporairement retirer tous les widgets existants
            for child in self.saved_search_state['results_children']:
                child.pack_forget()
            
            # Ajouter le notebook en premier avec une hauteur fixe
            self.artist_notebook.pack(side=tk.TOP, fill=tk.X, padx=5, pady=(5, 0))
            self.artist_notebook.configure(height=300)  # Hauteur fixe pour les onglets
            
            # Remettre les widgets existants en dessous
            for child in self.saved_search_state['results_children']:
                child.pack(side=tk.TOP, fill=tk.BOTH, expand=True)
        else:
            self.artist_notebook.pack(side=tk.TOP, fill=tk.X, padx=5, pady=5)
            self.artist_notebook.configure(height=300)  # Hauteur fixe pour les onglets
    ...
    def _return_to_search(self):
        ...
        try:
            ...
            # Restaurer l'ordre original des widgets si nécessaire
            if hasattr(self, 'saved_search_state') and self.saved_search_state['results_children']:
                # Retirer tous les widgets
                for child in self.results_container.winfo_children():
                    child.pack_forget()
                
                # Remettre les widgets dans l'ordre original
                for child in self.saved_search_state['results_children']:
