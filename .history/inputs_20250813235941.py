...
def on_global_volume_up(self, event):
    ...
    # Programmer la répétition si les touches sont maintenues
    ...
    # Programmer la répétition après 500ms puis toutes les 100ms
    def repeat_volume_up():
        if hasattr(self, '_volume_keys_pressed') and self._volume_keys_pressed.get('up', False):
            ...
            self._volume_repeat_id = self.root.after(100, repeat_volume_up)
    
    # Marquer la touche comme pressée
    if not hasattr(self, '_volume_keys_pressed'):
        self._volume_keys_pressed = {}
    self._volume_keys_pressed['up'] = True
    
    self._volume_repeat_id = self.root.after(500, repeat_volume_up)
    return "break"
...
def on_global_volume_down(self, event):
    ...
    def repeat_volume_down():
        if hasattr(self, '_volume_keys_pressed') and self._volume_keys_pressed.get('down', False):
            current_vol = self.volume * 100
            new_vol = max(0, current_vol - 5)
            self.volume_slider.set(new_vol)
            self.status_bar.config(text=f"Volume: {int(new_vol)}%")
            self._volume_repeat_id = self.root.after(100, repeat_volume_down)
    ...
    self._volume_repeat_id = self.root.after(500, repeat_volume_down)
    return "break"

def on_global_volume_key_release(self, event):
    """Appelé quand les touches de volume sont relâchées"""
    if hasattr(self, '_volume_keys_pressed'):
        if event.keysym == 'Up':
