            ...
            if hasattr(self, 'youtube_results_frame'):
                try:
                    ...
                    for child in self.youtube_results_frame.winfo_children():
                        try:
                            # Garder seulement les widgets essentiels (canvas, scrollbar)
                            # Supprimer toutes les autres frames qui pourraient être des résidus d'artiste
                            if (child.__class__.__name__ == 'Frame' and 
                                hasattr(child, 'winfo_name') and 
                                child.winfo_name() != 'thumbnail_frame'):  # Ne pas toucher à thumbnail_frame si elle existe déjà
                                print(f"DEBUG: Frame résiduelle détectée pour suppression: {child.winfo_name()}")
