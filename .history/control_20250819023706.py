...
def next_track(self):
    ...
    next_index = (self.current_index + 1) % len(self.main_playlist)
    self.current_index = next_index
    self.play_track()
    
    # Vérifier si la musique suivante est chargée
    try:
        print(f"DEBUG: Passage à la musique suivante (index {next_index})")
        
        # Vérifier si les méthodes nécessaires sont disponibles
        has_is_index_loaded = hasattr(self, '_is_index_already_loaded')
        has_check_unload = hasattr(self, '_check_and_unload_items')
        has_progressive_load = hasattr(self, '_progressive_load_system')
        has_append_items = hasattr(self, '_append_progressive_items')
        
        print(f"DEBUG: Méthodes disponibles - is_index_loaded: {has_is_index_loaded}, check_unload: {has_check_unload}, progressive_load: {has_progressive_load}, append_items: {has_append_items}")
        
        if has_is_index_loaded:
            # Vérifier si la musique actuelle est chargée
            is_loaded = self._is_index_already_loaded(next_index)
            print(f"DEBUG: Musique actuelle (index {next_index}) chargée: {is_loaded}")
            
            if not is_loaded and has_progressive_load:
                # Si la musique suivante n'est pas chargée, forcer le chargement
                print("DEBUG: Déclenchement du chargement progressif")
                self.safe_after(10, self._progressive_load_system)
            
            # Vérifier également si la musique suivante est chargée
            next_next_index = (next_index + 1) % len(self.main_playlist)
            next_is_loaded = self._is_index_already_loaded(next_next_index)
            print(f"DEBUG: Musique suivante (index {next_next_index}) chargée: {next_is_loaded}")
            
            if not next_is_loaded and has_append_items:
                # Charger la musique suivante de la suivante
                load_count = 10  # Nombre d'éléments à charger
                ...
                if next_next_index == 0:
                    ...
                    self.safe_after(20, lambda: self._append_progressive_items(0, load_count))
                else:
                    # Charger la musique suivante et quelques-unes après
                    end_index = min(len(self.main_playlist), next_next_index + load_count)
                    print(f"DEBUG: Chargement de la suite (index {next_next_index} à {end_index-1})")
                    self.safe_after(20, lambda: self._append_progressive_items(next_next_index, end_index))
            
            # Forcer le déchargement intelligent dans tous les cas
            if has_check_unload:
                print(f"DEBUG: Déclenchement du déchargement intelligent")
                
                # Définir un flag pour indiquer qu'on vient de changer de musique
                if hasattr(self, '_just_used_find_button'):
                    self._just_used_find_button = True
                
                # Utiliser une fonction sécurisée pour le déchargement
                def safe_check_and_unload():
                    try:
                        print("DEBUG: Exécution du déchargement intelligent")
                        self._check_and_unload_items(next_index)
                    except Exception as e:
                        print(f"DEBUG: Erreur lors du déchargement intelligent: {e}")
                
                self.safe_after(50, safe_check_and_unload)
            else:
                print("DEBUG: Méthode _check_and_unload_items non disponible")
        else:
            print("DEBUG: Méthode _is_index_already_loaded non disponible")
            
    except Exception as e:
        print(f"DEBUG: Erreur vérification musique suivante: {e}")
