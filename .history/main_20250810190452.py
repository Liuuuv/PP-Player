...
class MusicPlayer:
    ...
    def _filter_results_by_content_type(self, results):
        ...
        for entry in results:
            ...
            # Déterminer si c'est une chaîne d'artiste (pour miniature ronde)
            is_artist_channel = self._is_artist_content(title, uploader)
            
            # Vérifier le filtre Track (chansons individuelles et autres contenus non-artistes)
            if self.filter_track.get() and not is_artist_channel:
                should_include = True
            
            # Vérifier le filtre Artist (chaînes d'artistes)
            if self.filter_artist.get() and is_artist_channel:
                should_include = True
