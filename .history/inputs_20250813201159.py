...
def on_escape_pressed(self, event):
    ...
    if hasattr(self, 'artist_mode') and self.artist_mode:
        ...
        if hasattr(self, 'artist_tab_back_btn') and self.artist_tab_back_btn and self.artist_tab_back_btn.winfo_exists():
            # Déterminer l'onglet actuel et appeler la bonne fonction de retour
            try:
                if hasattr(self, 'artist_notebook') and self.artist_notebook.winfo_exists():
                    ...
                    if current_tab_id:
                        ...
                        if current_tab_text == "Sorties" and self.artist_tab_active_sorties:
                            self._return_to_releases()
                            return "break"
                        elif current_tab_text == "Playlists" and self.artist_tab_active_playlists:
                            self._return_to_playlists()
                            return "break"
                
                # Fallback: utiliser le bouton retour si on n'arrive pas à déterminer l'onglet
                self.artist_tab_back_btn.invoke()
            except:
                # En cas d'erreur, utiliser le bouton retour
                self.artist_tab_back_btn.invoke()
            return "break"
        else:
