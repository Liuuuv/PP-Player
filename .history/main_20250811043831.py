...
class MusicPlayer:
    ...
    def _update_stats_bar(self):
        ...
        if self.last_youtube_search_time > 0:
            stats_parts.append(f"YT: {self.last_youtube_search_time:.2f}s")
        
        if self.last_library_search_time > 0:
            stats_parts.append(f"Lib: {self.last_library_search_time:.3f}s")
        
        # Afficher les compteurs de recherches
        if self.total_youtube_searches > 0 or self.total_library_searches > 0:
            stats_parts.append(f"Total: YT({self.total_youtube_searches}) Lib({self.total_library_searches})")
        
        # Joindre toutes les parties avec des séparateurs
