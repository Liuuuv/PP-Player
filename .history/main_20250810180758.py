...
class MusicPlayer:
    ...
    def on_escape_pressed(self, event):
        ...
        # Vérifier sur quel onglet on se trouve
    ...
    def show_selection_menu(self, event):
        """Affiche une fenêtre avec des checkboxes pour sélectionner les playlists"""
        if not self.selected_items:
            return
        
        # Créer une nouvelle fenêtre
        menu_window = tk.Toplevel(self.root)
        menu_window.title("Ajouter à des playlists")
        menu_window.geometry("300x400")
        menu_window.configure(bg='#2d2d2d')
        ...
        x = event.x_root - 150
        y = event.y_root - 200
        menu_window.geometry(f"300x400+{x}+{y}")
        ...
        # Titre
        title_text = "Télécharger et ajouter à:" if has_youtube_videos else "Ajouter à:"
        title_label = tk.Label(
            menu_window,
            text=title_text,
            bg='#2d2d2d',
        ...
        # Sous-titre avec le nombre d'éléments sélectionnés
        count_label = tk.Label(
            menu_window,
            text=f"{len(self.selected_items)} élément(s) sélectionné(s)",
            bg='#2d2d2d',
            fg='#cccccc',
            font=('TkDefaultFont', 9)
        )
        ...
        checkbox_frame = tk.Frame(menu_window, bg='#2d2d2d')
        checkbox_frame.pack(fill=tk.BOTH, expand=True, padx=20, pady=(0, 10))
        
        # Canvas et scrollbar pour les checkboxes
        canvas = tk.Canvas(checkbox_frame, bg='#3d3d3d', highlightthickness=0)
        ...
        # Ajouter les checkboxes pour chaque playlist (sauf Main Playlist)
        for playlist_name in self.playlists.keys():
            if playlist_name != "Main Playlist":
                var = tk.BooleanVar()
                checkbox_vars[playlist_name] = var
                
                # Frame pour chaque playlist (pour permettre le clic sur le nom)
                playlist_frame = tk.Frame(scrollable_frame, bg='#3d3d3d')
                playlist_frame.pack(fill='x', pady=2, padx=10)
                ...
                # Permettre de cliquer sur le nom pour cocher/décocher
                def toggle_checkbox(v=var):
                    v.set(not v.get())
                
                # Bind le clic sur le frame et le label
                playlist_frame.bind("<Button-1>", lambda e, v=var: toggle_checkbox(v))
                checkbox.bind("<Button-1>", lambda e: None)  # Le checkbox gère déjà son propre clic
        
        # Ajouter une option pour créer une nouvelle playlist
        new_playlist_var = tk.BooleanVar()
        new_playlist_frame = tk.Frame(scrollable_frame, bg='#3d3d3d')
        new_playlist_frame.pack(fill='x', pady=5, padx=10)
        
        new_playlist_checkbox = tk.Checkbutton(
            new_playlist_frame,
            text="Créer nouvelle playlist:",
            variable=new_playlist_var,
            bg='#3d3d3d',
            fg='white',
            selectcolor='#4a4a4a',
            activebackground='#4a4a4a',
        ...
        new_playlist_checkbox.pack(anchor='w')
        
        new_playlist_entry = tk.Entry(
            new_playlist_frame,
            bg='#4a4a4a',
            fg='white',
            insertbackground='white',
        ...
        new_playlist_entry.pack(fill='x', pady=(5, 0))
        
        canvas.pack(side="left", fill="both", expand=True)
        scrollbar.pack(side="right", fill="y")
        
        # Frame pour les boutons
        buttons_frame = tk.Frame(menu_window, bg='#2d2d2d')
        ...
        def confirm_selection():
            ...
            # Vérifier les playlists existantes cochées
            for playlist_name, var in checkbox_vars.items():
                if var.get():
                    selected_playlists.append(playlist_name)
            
            # Vérifier si on veut créer une nouvelle playlist
            if new_playlist_var.get():
                new_name = new_playlist_entry.get().strip()
                if new_name:
                    selected_playlists.append(new_name)
            ...
            if selected_playlists:
                if has_youtube_videos:
                    self.download_and_add_to_multiple_playlists(selected_playlists)
                else:
                    self.add_to_multiple_playlists(selected_playlists)
