...
def on_global_seek_forward(self, event):
    ...
    try:
        if pygame.mixer.music.get_busy() and self.song_length > 0:
            ...
            new_position = min(self.song_length, self.current_time + 5)
            ...
            print(f"DEBUG: Avance de 5s - Position: {new_position:.1f}s ({progress_percent:.1f}%)")
            self.status_bar.config(text=f"Avancé de 5s - {new_position:.0f}s")
...
def on_global_seek_backward(self, event):
    ...
    try:
        if pygame.mixer.music.get_busy():
            ...
            print(f"DEBUG: Recule de 5s - Position: {new_position}s")
            self.status_bar.config(text=f"Reculé de 5s - {new_position:.0f}s")
        else:
            print("DEBUG: Aucune musique en cours")
    except Exception as e:
