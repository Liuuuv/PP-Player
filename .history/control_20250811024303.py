...
def play_pause(self):
    ...
    # S'assurer que le mixeur est initialisé
    if not tools.ensure_mixer_initialized():
        return
...
def next_track(self):
    ...
    if self.current_index >= len(self.main_playlist) - 1 and self.loop_mode != 1:
        # Arrêter la lecture
        if tools.ensure_mixer_initialized():
...
def on_progress_release(self, event):
    ...
    try:
        if not self.paused and tools.ensure_mixer_initialized():
