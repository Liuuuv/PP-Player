...
class MusicPlayer:
    def __init__(self, root):
        ...
        self.last_waveform_update = 0  # Pour contrôler la fréquence de mise à jour de la waveform
        self.waveform_update_interval = 0.1  # Mettre à jour la waveform toutes les 100ms
        self.waveform_red_line_id = None  # ID de la ligne rouge pour pouvoir la supprimer/redessiner
        self.last_waveform_data = None  # Cache des dernières données de waveform dessinées
    ...
    def update_time(self):
        while True:
            if pygame.mixer.music.get_busy() and not self.paused and not self.user_dragging:
                ...
                if self.show_waveform_current:
                    # Ne mettre à jour la waveform qu'à intervalles réguliers pour éviter le clignotement
                    current_time_for_waveform = time.time()
                    if current_time_for_waveform - self.last_waveform_update >= self.waveform_update_interval:
                        self.draw_waveform_around(self.current_time)
                        self.last_waveform_update = current_time_for_waveform
                else:
                    self.waveform_canvas.delete("all")
            self.root.update()
            time.sleep(0.01)
