...
class DragDropHandler:
    ...
    def _setup_child_bindings(self, parent_frame):
        ...
        for child in parent_frame.winfo_children():
            if isinstance(child, (tk.Label, tk.Frame)):
                print(f"🔧 Configurant bindings motion pour {child.__class__.__name__} - {getattr(child, 'text', 'N/A')[:15]}")
    ...
    def _on_drag_motion(self, event, frame):
        ...
        if not frame.drag_enabled:
            print(f"🔴 MOTION IGNORÉ: {event.widget.__class__.__name__} - drag_enabled=False")
            return
        
        if not hasattr(frame, 'motion_debug_count'):
            frame.motion_debug_count = 0
        frame.motion_debug_count += 1
        if frame.motion_debug_count == 1:  # Afficher seulement le premier message
            print(f"🔵 MOTION OK: {event.widget.__class__.__name__} - drag_enabled=True")
