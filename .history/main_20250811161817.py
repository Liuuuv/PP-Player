...
class MusicPlayer:
    ...
    def _remove_playlist_item(self, filepath, frame, event=None):
        """Supprime un élément de la main playlist en préservant l'ordre"""
        try:
            # Vérifier si Ctrl est enfoncé pour supprimer du dossier downloads
            if event and (event.state & 0x4):  # Ctrl est enfoncé
                ...
                # Sauvegarder l'état actuel pour préserver l'ordre
                was_playing = pygame.mixer.music.get_busy()
                current_song_path = None
                if len(self.main_playlist) > 0 and self.current_index < len(self.main_playlist):
                    current_song_path = self.main_playlist[self.current_index]
                
                # Supprimer de la liste
                self.main_playlist.pop(index)
                
                # Mettre à jour l'index courant en préservant l'ordre
                if index < self.current_index:
                    # La chanson supprimée était avant la chanson en cours
                    # Décrémenter l'index pour maintenir la même chanson en cours
                    self.current_index -= 1
                elif index == self.current_index:
                    # La chanson supprimée était la chanson en cours
                    pygame.mixer.music.stop()
                    
                    # Si il reste des chansons
                    if len(self.main_playlist) > 0:
                        # Garder le même index si possible, sinon prendre la dernière
                        ...
                        # Afficher la miniature de la chanson en cours
                        self._show_current_song_thumbnail()
                # Si index > self.current_index, rien à faire car l'ordre est préservé
