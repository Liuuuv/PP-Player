...
class MusicPlayer:
    ...
    def _add_playlist_item(self, filepath, thumbnail_path=None):
        ...
        try:
            ...
            # Frame principal
            item_frame = ttk.Frame(self.playlist_container, height=20)
            item_frame.pack(fill='x', pady=2)
            
            # Canvas pour placement précis
            canvas = tk.Canvas(item_frame, bg='#3d3d3d', highlightthickness=0)
            canvas.pack(fill='both', expand=True)
            
            # Miniature
            thumbnail_label = tk.Label(canvas, bg='#3d3d3d', width=80, height=20)
            canvas.create_window(30, 25, anchor='w', window=thumbnail_label)
            ...
            # Calcul dynamique de la longueur max basée sur la largeur disponible
            def get_elided_text(label, full_text, max_width):
                label.config(text=full_text)
                while label.winfo_reqwidth() > max_width and len(full_text) > 3:
            ...
            # Calcul après affichage
            def adjust_text():
                max_width = text_frame.winfo_width() + 20  # Marge pour le bouton
                elided_text = get_elided_text(title_label, filename, max_width)
                title_label.config(text=elided_text)
            
            text_frame.bind("<Configure>", lambda e: adjust_text())
            
            # Bouton poubelle
            delete_btn = tk.Button(canvas, image=self.icons["delete"], bg='#ff4444')
            canvas.create_window(380, 10, anchor='e', window=delete_btn)
