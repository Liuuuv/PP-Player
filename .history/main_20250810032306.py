...
class MusicPlayer:
    def __init__(self, root):
        ...
        self.preview_active = False  # True quand une prévisualisation est en cours
        self.preview_filepath = None  # Chemin du fichier en prévisualisation
        self.preview_original_state = None  # État original de la lecture (paused, current_time, etc.)
        self.preview_channel = None  # Canal pygame pour la prévisualisation
    ...
    def _add_download_item(self, filepath):
        ...
        try:
            ...
            def on_item_double_click(event):
                ...
                if event.state & 0x4:  # Ctrl est enfoncé
                    # Vérifier si Alt est aussi enfoncé pour supprimer (Ctrl+Alt+double-clic)
                    if event.state & 0x20000:  # Alt est aussi enfoncé
                        self._delete_from_downloads(filepath, item_frame)
                    else:
                        # Ctrl seul : prévisualisation
                        self.start_preview(filepath)
            ...
            def on_item_button_release(event):
                """Gère le relâchement du bouton pour arrêter la prévisualisation"""
                if self.preview_active and self.preview_filepath == filepath:
                    self.stop_preview()
            ...
            item_frame.bind("<Control-Double-1>", on_item_ctrl_double_click)
            item_frame.bind("<ButtonRelease-1>", on_item_button_release)
            thumbnail_label.bind("<Button-2>", on_item_click)
            thumbnail_label.bind("<Double-1>", on_item_double_click)
            thumbnail_label.bind("<Control-Double-1>", on_item_ctrl_double_click)
            thumbnail_label.bind("<ButtonRelease-1>", on_item_button_release)
            title_label.bind("<Button-2>", on_item_click)
            title_label.bind("<Double-1>", on_item_double_click)
            title_label.bind("<Control-Double-1>", on_item_ctrl_double_click)
            title_label.bind("<ButtonRelease-1>", on_item_button_release)
            duration_label.bind("<Button-2>", on_item_click)
            duration_label.bind("<Double-1>", on_item_double_click)
            duration_label.bind("<Control-Double-1>", on_item_ctrl_double_click)
            duration_label.bind("<ButtonRelease-1>", on_item_button_release)
    ...
    def _add_main_playlist_item(self, filepath, thumbnail_path=None, song_index=None):
        ...
        try:
            ...
            def on_playlist_item_double_click(event):
                # Vérifier si Ctrl est enfoncé pour la prévisualisation
                if event.state & 0x4:  # Ctrl est enfoncé
                    self.start_preview(filepath)
                elif event.state & 0x1:  # Shift est enfoncé - ne rien faire sur double-clic avec Shift
                    pass
                else:
            ...
            def on_playlist_item_button_release(event):
                """Gère le relâchement du bouton pour arrêter la prévisualisation"""
                if self.preview_active and self.preview_filepath == filepath:
                    self.stop_preview()
            ...
            item_frame.bind("<Control-Double-1>", on_playlist_item_ctrl_double_click)
            item_frame.bind("<ButtonRelease-1>", on_playlist_item_button_release)
            thumbnail_label.bind("<Button-1>", on_playlist_item_click)
            thumbnail_label.bind("<Double-1>", on_playlist_item_double_click)
            thumbnail_label.bind("<Control-Double-1>", on_playlist_item_ctrl_double_click)
            thumbnail_label.bind("<ButtonRelease-1>", on_playlist_item_button_release)
            title_label.bind("<Button-1>", on_playlist_item_click)
            title_label.bind("<Double-1>", on_playlist_item_double_click)
            title_label.bind("<Control-Double-1>", on_playlist_item_ctrl_double_click)
            title_label.bind("<ButtonRelease-1>", on_playlist_item_button_release)
            duration_label.bind("<Button-1>", on_playlist_item_click)
            duration_label.bind("<Double-1>", on_playlist_item_double_click)
            duration_label.bind("<Control-Double-1>", on_playlist_item_ctrl_double_click)
            duration_label.bind("<ButtonRelease-1>", on_playlist_item_button_release)
            ...
            if show_numbers:
                ...
                number_label.bind("<Control-Double-1>", on_playlist_item_ctrl_double_click)
                number_label.bind("<ButtonRelease-1>", on_playlist_item_button_release)
    ...
    def start_preview(self, filepath):
        """Démarre la prévisualisation d'un fichier audio"""
        try:
            ...
            self.preview_original_state = {
                'paused': self.paused,
                'current_time': self.current_time,
                'current_song': self.main_playlist[self.current_index] if self.main_playlist else None,
            ...
            # Charger et jouer l'extrait (à partir de 30 secondes ou du début si plus court)
            audio = MP3(filepath)
            duration = audio.info.length
            start_time = min(30, duration * 0.3)  # Commencer à 30s ou 30% de la durée
            
            # Charger le fichier de prévisualisation
            pygame.mixer.music.load(filepath)
            pygame.mixer.music.play(start=start_time)
            pygame.mixer.music.set_volume(self.volume * 0.7)  # Volume réduit pour la prévisualisation
            ...
            self.status_bar.config(text=f"Prévisualisation: {os.path.basename(filepath)}")
            
        except Exception as e:
            print(f"Erreur lors du démarrage de la prévisualisation: {e}")
            self.status_bar.config(text=f"Erreur prévisualisation: {e}")
    
    def stop_preview(self):
        """Arrête la prévisualisation et restaure l'état précédent"""
        try:
            ...
            if self.preview_original_state:
                if self.preview_original_state['current_song'] and self.preview_original_state['was_playing']:
                    # Recharger et reprendre la musique précédente
                    pygame.mixer.music.load(self.preview_original_state['current_song'])
                    if not self.preview_original_state['paused']:
                        pygame.mixer.music.play(start=self.preview_original_state['current_time'])
                        self.base_position = self.preview_original_state['current_time']
                        self.paused = False
                        self.play_button.config(image=self.icons["pause"], text="Pause")
                    else:
                        self.paused = True
                    ...
                    self.current_time = self.preview_original_state['current_time']
                    self.progress.config(value=self.current_time)
                    self.current_time_label.config(
                        text=time.strftime('%M:%S', time.gmtime(self.current_time))
                    )
                    
                    self.status_bar.config(text="Playing" if not self.paused else "Paused")
                else:
                    self.status_bar.config(text="Stopped")
            
            # Réinitialiser les variables de prévisualisation
            self.preview_active = False
            self.preview_filepath = None
            self.preview_original_state = None
            
        except Exception as e:
            print(f"Erreur lors de l'arrêt de la prévisualisation: {e}")
            self.preview_active = False
            self.preview_filepath = None
            self.preview_original_state = None
