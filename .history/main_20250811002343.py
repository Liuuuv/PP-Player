...
class MusicPlayer:
    ...
    def _clear_main_playlist(self, event=None):
        ...
        # Vider la liste principale et la playlist principale
...
def _scroll_to_current_song(self, event=None):
    """Fait défiler la liste de lecture vers la chanson actuellement jouée"""
    ...
    # Chercher le widget correspondant à la chanson actuelle
    current_filepath = self.main_playlist[self.current_index]
    
    # Parcourir les widgets de la playlist pour trouver celui qui correspond
    for widget in self.playlist_container.winfo_children():
        if hasattr(widget, 'filepath') and widget.filepath == current_filepath:
            # Calculer la position relative du widget dans le container
            ...
            if container_height > canvas_height:
                # Calculer la position de scroll (0.0 à 1.0)
                scroll_position = widget_y / (container_height - canvas_height)
                ...
                self.status_bar.config(text=f"Navigation vers: {os.path.basename(current_filepath)}")
            else:
                self.status_bar.config(text="Pas besoin de faire défiler")
            return
    
    self.status_bar.config(text="Musique en cours non trouvée dans la liste")
