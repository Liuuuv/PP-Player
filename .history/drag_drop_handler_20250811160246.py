...
class DragDropHandler:
    ...
    def _add_file_to_playlist(self, file_path):
        ...
        if file_path:
            ...
            # Toujours ajouter à la fin - l'utilisateur fait un drag explicite pour ajouter
            # Même si c'est déjà en dernière position, on respecte l'intention de l'utilisateur
    ...
    def _add_playlist_item_to_main(self, file_path):
        ...
        if file_path:
            ...
            # Vérifier si c'est déjà la dernière musique de la playlist ET qu'elle existe toujours
            if (self.music_player.main_playlist and 
                len(self.music_player.main_playlist) > 0 and
                self.music_player.main_playlist[-1] == file_path and
