...
def next_track(self):
    ...
    # Auto-scroll si activé et pas de scroll manuel détecté
    if hasattr(self, 'auto_scroll_enabled') and self.auto_scroll_enabled and not getattr(self, 'manual_scroll_detected', False):
        # Utiliser la fonction existante avec auto_scroll=True pour avoir l'animation ease in/out
        self.root.after(100, lambda: self.select_playlist_item(index=self.current_index, auto_scroll=True))
