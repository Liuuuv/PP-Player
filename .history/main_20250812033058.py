...
class MusicPlayer:
        ...
        def search_releases():
            try:
                ...
                with YoutubeDL(search_opts) as ydl:
                    ...
                    if hasattr(self, 'current_artist_channel_url') and self.current_artist_channel_url:
                        try:
                            ...
                            if channel_info and 'entries' in channel_info:
                                ...
                                # Filtrer et garder seulement les playlists valides
                                playlists = [p for p in playlists if p and p.get('id') and 'playlist' in p.get('_type', '')]
                                all_playlists.extend(playlists[:15])  # Prendre les 15 premières
                        except Exception as e:
                            print(f"Erreur extraction playlists chaîne: {e}")
                    
                    # Si pas de playlists trouvées, faire une recherche classique
                    if not all_playlists:
                        search_queries = [
                            f"ytsearch15:{self.current_artist_name} playlist",
                            f"ytsearch10:{self.current_artist_name} album",
                        ...
                        for query in search_queries:
                            try:
                                ...
                                if results and 'entries' in results:
                                    ...
                                    # Filtrer pour les playlists ou contenus qui semblent être des sorties
                                    playlists = [r for r in items if (
                                        'playlist' in r.get('_type', '') or
                                        any(keyword in r.get('title', '').lower() 
                                            for keyword in ['album', 'ep', 'playlist', 'collection', 'compilation'])
                                    )]
                    ...
                    for playlist in all_playlists:
                        ...
                        if playlist_id and playlist_id not in unique_playlists:
                            ...
                            if not playlist.get('webpage_url') and playlist_id:
                                if 'playlist' in playlist.get('_type', ''):
                    ...
                    # Limiter à 12 playlists max
                    final_playlists = final_playlists[:12]
    ...
    def _show_playlist_content(self, playlist_data):
            ...
            try:
                ...
                with YoutubeDL(playlist_opts) as ydl:
                    ...
                    if playlist_info and 'entries' in playlist_info:
                        ...
                        self.root.after(0, lambda: self._show_playlist_error("Impossible de charger le contenu de la playlist"))
        ...
        self._show_playlist_loading(playlist_data.get('title', 'Playlist'))
