...
def _start_text_animation(self, full_title, frame):
    ...
    truncated_title = tools._truncate_text_for_display(self, full_title, max_width_pixels=frame.max_width, font_family=self.get_label_font(frame), font_size=self.get_label_font_size(frame))
...
def _animate_title_step(self, frame):
    ...
    if frame.scroll_position >= len(frame.full_text) + 8:  # +8 pour les espaces ajoutés
        ...
        truncated_title = tools._truncate_text_for_display(self, frame.full_text, max_width_pixels=max_width_pixels, font_family='Helvetica', font_size=font_size)
        frame.config(text=truncated_title)
