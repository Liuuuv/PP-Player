...
def create_ui(self):
    ...
    # CORRECTION: Forcer l'affichage de la playlist au démarrage si elle existe
    def force_display_playlist():
        if hasattr(self, '_display_main_playlist') and hasattr(self, 'main_playlist') and len(self.main_playlist) > 0:
            print(f"🔄 DEBUG: Affichage forcé de la playlist au démarrage ({len(self.main_playlist)} éléments)")
            self._display_main_playlist()
        else:
            print("⚠️ DEBUG: Pas de playlist à afficher au démarrage")
    
    # Programmer l'affichage après que tout soit initialisé
    self.root.after(1000, force_display_playlist)
...
def setup_search_tab(self):
    ...
    # CORRECTION CRITIQUE: Ne PAS packer le container - il est déjà dans le canvas !
