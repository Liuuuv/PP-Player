...
def on_global_play_pause(self, event):
    ...
    print("DEBUG: on_global_play_pause appelé !")
    try:
        if hasattr(self, 'play_pause'):
            print("DEBUG: play_pause existe, appel en cours...")
            self.play_pause()
            print("DEBUG: Ctrl+Alt+0 (pavé numérique) - play_pause() appelé avec succès")
        else:
            print("DEBUG: play_pause n'existe pas !")
    except Exception as e:
        print(f"DEBUG: Erreur Ctrl+Alt+0: {e}")
        import traceback
        traceback.print_exc()
    return "break"
...
def on_global_volume_up(self, event):
    ...
    print(f"DEBUG: Volume up - {int(new_volume)}%")
    ...
    def repeat_volume_up():
        if self._volume_keys_pressed.get('up', False):
            ...
            if current_vol < 100:  # Seulement si pas au maximum
                ...
                print(f"DEBUG: Volume up repeat - {int(new_vol)}%")
...
def on_global_volume_down(self, event):
    ...
    self.volume_slider.set(new_volume)  # Met à jour le slider visuellement
    self.status_bar.config(text=f"Volume: {int(new_volume)}%")
    print(f"DEBUG: Volume down - {int(new_volume)}%")
    ...
    def repeat_volume_down():
        if self._volume_keys_pressed.get('down', False):
            ...
            if current_vol > 0:  # Seulement si pas au minimum
                ...
                self.volume_slider.set(new_vol)  # Met à jour le slider visuellement
                self.status_bar.config(text=f"Volume: {int(new_vol)}%")
                print(f"DEBUG: Volume down repeat - {int(new_vol)}%")
                # Continuer la répétition
...
def on_global_volume_key_release(self, event):
    ...
    if event.keysym == 'Up':
        ...
        print("DEBUG: Touche Volume Up relâchée")
    elif event.keysym == 'Down':
        self._volume_keys_pressed['down'] = False
        print("DEBUG: Touche Volume Down relâchée")
...
class ImportDialog:
    ...
    def _download_single_existing(self, url):
        """Télécharge une seule vidéo en utilisant les fonctions existantes"""
        # Utiliser la fonction existante de téléchargement
        thread = threading.Thread(
            target=self.music_player._download_youtube_selection,
            args=([url], "Main Playlist"),
            daemon=True
        )
        thread.start()
    
    def _download_playlist_existing(self, url):
        """Télécharge une playlist en utilisant les fonctions existantes"""
        def extract_playlist_urls():
            try:
                self.music_player.status_bar.config(text="Extraction des URLs de la playlist...")
                
                # Utiliser yt-dlp pour extraire les URLs de la playlist
                ydl_opts = {
                    'extract_flat': True,
                    'quiet': True,
                    'no_warnings': True,
                ...
                with YoutubeDL(ydl_opts) as ydl:
                    ...
                    if 'entries' in info:
                        ...
                        if urls:
                            # Utiliser la fonction existante pour télécharger toutes les URLs
                            self.music_player._download_youtube_selection(urls, "Main Playlist")
                        else:
                            self.music_player.status_bar.config(text="Aucune vidéo trouvée dans la playlist")
                    else:
                        # Ce n'est pas une playlist, traiter comme une vidéo unique
                        self._download_single_existing(url)
                        
            except Exception as e:
                self.music_player.status_bar.config(text=f"Erreur extraction playlist: {str(e)}")
        
        thread = threading.Thread(target=extract_playlist_urls, daemon=True)
        thread.start()
...
def on_global_seek_forward(self, event):
    ...
    try:
        if pygame.mixer.music.get_busy() and self.song_length > 0:
            ...
            # Calculer le pourcentage pour le curseur (0-100)
            progress_percent = (new_time / self.song_length) * 100
            
            print(f"DEBUG: Avance de 5s - De {self.current_time:.1f}s à {new_time:.1f}s ({progress_percent:.1f}%)")
            
            # Utiliser set_position qui gère le repositionnement de la musique
            self.set_position(progress_percent)
            
            self.status_bar.config(text=f"Avancé de 5s - {new_time:.0f}s")
        else:
            print("DEBUG: Aucune musique en cours ou durée inconnue")
            print(f"DEBUG: music.get_busy()={pygame.mixer.music.get_busy()}, song_length={self.song_length}")
    except Exception as e:
        print(f"DEBUG: Erreur seek forward: {e}")
        import traceback
        traceback.print_exc()
    return "break"
...
def on_global_seek_backward(self, event):
    ...
    try:
        if pygame.mixer.music.get_busy() and self.song_length > 0:
            ...
            new_position = max(0, self.current_time - 5)
            
            # Calculer le pourcentage pour le curseur
            progress_percent = (new_position / self.song_length) * 100
            
            # Mettre à jour la position via set_position (comme si on avait bougé le curseur)
            self.set_position(progress_percent)
            
            print(f"DEBUG: Recule de 5s - Position: {new_position:.1f}s ({progress_percent:.1f}%)")
            self.status_bar.config(text=f"Reculé de 5s - {new_position:.0f}s")
            
            # Mettre à jour le curseur visuellement
            if hasattr(self, 'progress'):
                self.progress.set(progress_percent)
        else:
            print("DEBUG: Aucune musique en cours ou durée inconnue")
    except Exception as e:
        print(f"DEBUG: Erreur seek backward: {e}")
    return "break"
