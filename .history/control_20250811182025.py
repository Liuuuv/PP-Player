...
def play_pause(self):
    ...
    if pygame.mixer.music.get_busy() and not self.paused:
        ...
        if self.main_playlist and self.current_index < len(self.main_playlist):
            song_name = os.path.basename(self.main_playlist[self.current_index])
...
def prev_track(self):
    ...
    if (hasattr(self, 'queue_items') and 
        ...
        self.main_playlist[self.current_index] in self.queue_items):
        self.queue_items.discard(self.main_playlist[self.current_index])
...
def next_track(self):
    ...
    # Nettoyer la queue si la chanson actuelle en faisait partie (ancien système)
    if (hasattr(self, 'queue_items') and 
        self.current_index < len(self.main_playlist) and
        self.main_playlist[self.current_index] in self.queue_items):
        self.queue_items.discard(self.main_playlist[self.current_index])
    
    # NOUVEAU : Vérifier s'il y a des musiques dans la queue
    if hasattr(self, 'play_queue') and self.play_queue:
        # Jouer la première musique de la queue
        next_song = self.play_queue.pop(0)  # Retirer de la queue
        
        # Trouver cette musique dans la main_playlist pour mettre à jour current_index
        try:
            self.current_index = self.main_playlist.index(next_song)
            self.play_track()
            
            # Rafraîchir l'affichage pour retirer l'indicateur de queue
            if hasattr(self, '_refresh_playlist_display'):
                self._refresh_playlist_display()
            
            return
        except ValueError:
            # Si la musique n'est plus dans la playlist, continuer normalement
            pass
    
    # Comportement normal : passer à la chanson suivante de la playlist
