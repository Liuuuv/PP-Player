...
class ImportDialog:
                ...
                for wave_start in range(0, total_links, batch_size):
                    ...
                    for i, url in enumerate(wave_links):
                        ...
                        try:
                            ...
                            if session_id:
                                self.logger.log("INFO", f"Vérification {current_check}/{total_links}: {url}")
                    ...
                    if valid_videos_wave:
                        ...
                        for j, (url, title, video_data) in enumerate(valid_videos_wave):
                            ...
                            # Attendre si la session est en pause
                            while session_id and self.logger.is_paused():
                                print("⏸️ Session en pause, attente...")
                                self.music_player.root.after(0, lambda: 
                                    self.music_player.status_bar.config(text="⏸️ Session en pause"))
                                time.sleep(1)
                            ...
                            try:
                                ...
                                if session_id:
                                    self.logger.log("INFO", f"Début du téléchargement: {title}")
    ...
    def _download_video_sync(self, url, title, video_data):
        ...
        download_success = [False]  # Liste pour pouvoir modifier depuis le callback
        
        def on_download_complete(success):
            download_success[0] = success
            download_complete.set()
        
        # Lancer le téléchargement asynchrone avec callback
        self.music_player.download_video_async(url, title, video_data, on_download_complete)
