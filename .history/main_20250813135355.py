...
class MusicPlayer:
    ...
    def search_youtube(self):
        # Si on est en mode artiste, revenir à la recherche normale et attendre un peu
        if hasattr(self, 'artist_mode') and self.artist_mode:
            self._return_to_search()
            # Attendre que la fermeture soit complète avant de lancer la recherche
            self.root.after(100, lambda: self._continue_search_after_artist_close())
            return
    ...
    def _continue_search_after_artist_close(self):
        """Continue la recherche après la fermeture de la page artiste"""
        # Vérifier qu'on n'est plus en mode artiste
        if hasattr(self, 'artist_mode') and self.artist_mode:
            # Si on est encore en mode artiste, attendre encore un peu
            self.root.after(50, lambda: self._continue_search_after_artist_close())
            return
        
        # Maintenant on peut lancer la recherche en toute sécurité
        self._start_new_search()
    ...
    def _add_search_result(self, video, index):
        ...
        try:
            ...
            # Vérifier qu'on n'est pas en mode artiste (éviter les conflits de widgets)
            if hasattr(self, 'artist_mode') and self.artist_mode:
                return
