...
def play_pause(self):
    ...
    if pygame.mixer.music.get_busy() and not self.paused:
        ...
        # Attendre que pygame démarre réellement la lecture pour synchroniser
        max_wait = 50  # 50ms maximum
        wait_step = 1  # 1ms par étape
        for _ in range(max_wait):
            if pygame.mixer.music.get_busy():
                break
            time.sleep(wait_step / 1000.0)
        
        # Synchroniser base_position avec la position de reprise
...
def on_progress_press(self, event):
    ...
    if self.show_waveform_current:
        ...
        pos = self.progress.get_value_from_x(event.x)
...
def on_progress_drag(self, event):
    ...
    if self.user_dragging:
        pos = self.progress.get_value_from_x(event.x)  # En secondes
...
def on_progress_release(self, event):
    ...
    try:
        if not self.paused:
            ...
            # Attendre que pygame démarre réellement la lecture pour synchroniser
            # Ceci assure que get_pos() sera cohérent avec notre base_position
            max_wait = 50  # 50ms maximum
            wait_step = 1  # 1ms par étape
            for _ in range(max_wait):
                if pygame.mixer.music.get_busy():
            ...
            # Synchroniser base_position avec la nouvelle position de départ
            # pygame.mixer.music.get_pos() va repartir de 0, donc base_position = pos
            self.base_position = pos
        else:
            # En pause, juste mettre à jour la position courante
            self.base_position = pos
