...
class MusicPlayer:
    ...
    def _add_song_item(self, *args, **kwargs):
        """Délègue à la fonction _add_song_item de tools.py"""
        return tools._add_song_item(self, *args, **kwargs)
    
    # Fonctions de base pour le téléchargement et l'ajout
    def download_youtube_video(self, *args, **kwargs):
        """Télécharge une vidéo YouTube"""
        return core_functions.download_youtube_video(self, *args, **kwargs)
    
    def add_file_to_main_playlist(self, *args, **kwargs):
        """Ajoute un fichier à la main playlist"""
        return core_functions.add_file_to_main_playlist(self, *args, **kwargs)
    
    def add_file_to_playlist(self, *args, **kwargs):
        """Ajoute un fichier à une playlist spécifique"""
        return core_functions.add_file_to_playlist(self, *args, **kwargs)
    
    def add_file_to_queue(self, *args, **kwargs):
        """Ajoute un fichier à la queue"""
        return core_functions.add_file_to_queue(self, *args, **kwargs)
    
    def play_file_after_current(self, *args, **kwargs):
        """Insère un fichier pour qu'il soit lu après la chanson actuelle"""
        return core_functions.play_file_after_current(self, *args, **kwargs)
    
    def refresh_ui_after_changes(self, *args, **kwargs):
        """Rafraîchit l'interface utilisateur après des modifications"""
        return core_functions.refresh_ui_after_changes(self, *args, **kwargs)
    
    def download_youtube_video_async(self, *args, **kwargs):
        """Télécharge une vidéo YouTube de manière asynchrone"""
        return core_functions.download_youtube_video_async(self, *args, **kwargs)
    
    def download_multiple_youtube_videos(self, *args, **kwargs):
        """Télécharge plusieurs vidéos YouTube"""
        return core_functions.download_multiple_youtube_videos(self, *args, **kwargs)
    
    def download_multiple_youtube_videos_async(self, *args, **kwargs):
        """Télécharge plusieurs vidéos YouTube de manière asynchrone"""
        return core_functions.download_multiple_youtube_videos_async(self, *args, **kwargs)
    ...
    def on_test_downloads(self, event):
        ...
        print("DEBUG: Test des téléchargements activé")
    ...
    def add_download_to_tab(self, url, title, video_data=None):
        ...
        return downloads_tab.add_download_to_tab(self, url, title, video_data)
    ...
    def _download_youtube_video(self, video):
        """Télécharge une vidéo YouTube"""
        return ui_menus._download_youtube_video(self, video)
    ...
    def _add_main_playlist_item(self, filepath, thumbnail_path=None, song_index=None):
        ...
        print('AOAOOAOAOAOAOOAAAAAAAAAAAAAAAAAAAAAAAAA')
    ...
    def _safe_add_search_result(self, video, index):
        ...
        return search_tab.results._safe_add_search_result(self, video, index)
    ...
    def _show_playlist_menu(self, video, frame):
        ...
        return search_tab.results._show_playlist_menu(self, video, frame)
    
    def _add_video_to_playlist(self, video, frame, playlist_name):
        """Ajoute une vidéo à une playlist spécifique"""
        return search_tab.results._add_video_to_playlist(self, video, frame, playlist_name)
    
    def _show_youtube_result_menu(self, event, video, frame):
        """Affiche le même menu que les fichiers locaux mais pour les résultats YouTube"""
        try:
            # Créer le menu contextuel avec le même style que les fichiers locaux
            ...
            if len(title) > 40:
                title = title[:37] + "..."
            context_menu.add_command(label=f"Télécharger et ajouter à:", state='disabled')
            context_menu.add_separator()
            
            # Option pour ajouter à la liste de lecture principale
            ...
            for playlist_name in self.playlists.keys():
                if playlist_name != "Main Playlist":
                    context_menu.add_command(
                        label=playlist_name,
                        command=lambda p=playlist_name: self._add_video_to_playlist(video, frame, p)
                    )
            ...
            # Option pour télécharger seulement
            context_menu.add_separator()
            context_menu.add_command(
            ...
            # Option pour ouvrir sur YouTube
            context_menu.add_separator()
            context_menu.add_command(
                label="🔗 Ouvrir sur YouTube",
                command=lambda: self._open_youtube_video(video)
            )
            ...
            print(f"Erreur affichage menu YouTube: {e}")
            import traceback
            traceback.print_exc()
    
    def _open_youtube_video(self, video):
        """Ouvre une vidéo YouTube dans le navigateur"""
        try:
            import webbrowser
            url = video.get('webpage_url') or f"https://www.youtube.com/watch?v={video.get('id')}"
            webbrowser.open(url)
        except Exception as e:
            print(f"Erreur ouverture YouTube: {e}")
    ...
    def _refresh_playlist_display(self):
        ...
        return search_tab.main_playlist._refresh_playlist_display(self)
