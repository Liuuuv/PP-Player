...
class ToolTip:
    ...
    def show_tip(self, event=None):
        ...
        # Les laisser passer au widget parent
        def pass_scroll_event(event):
            # Propager l'événement au widget parent
            try:
                parent = self.widget.winfo_toplevel()
                parent.event_generate(event.type, x=event.x_root, y=event.y_root, 
                                    delta=getattr(event, 'delta', 0))
            except:
                pass
            return "break"
        
        # Bind les événements de scroll sur la fenêtre tooltip et son label
        tw.bind('<MouseWheel>', pass_scroll_event)
        tw.bind('<Button-4>', pass_scroll_event)  # Linux
        tw.bind('<Button-5>', pass_scroll_event)  # Linux
        label.bind('<MouseWheel>', pass_scroll_event)
        label.bind('<Button-4>', pass_scroll_event)  # Linux
        label.bind('<Button-5>', pass_scroll_event)  # Linux
