...
class MusicPlayer:
    def __init__(self, root):
        ...
        self.last_youtube_search_time = 0.0  # Dernier temps de recherche YouTube
        self.last_library_search_time = 0.0  # Dernier temps de recherche bibliothèque
        self.total_youtube_searches = 0  # Nombre total de recherches YouTube
        self.total_library_searches = 0  # Nombre total de recherches bibliothèque
    ...
    def _update_stats_bar(self):
        """Met à jour la barre de statistiques avec les temps de recherche"""
        if not hasattr(self, 'stats_bar'):
            return
            
        stats_text = ""
        
        # Afficher les temps de recherche s'ils existent
        if self.last_youtube_search_time > 0:
            stats_text += f"YT: {self.last_youtube_search_time:.2f}s"
        
        if self.last_library_search_time > 0:
            if stats_text:
                stats_text += " | "
            stats_text += f"Lib: {self.last_library_search_time:.3f}s"
        
        # Afficher les compteurs de recherches
        if self.total_youtube_searches > 0 or self.total_library_searches > 0:
            if stats_text:
                stats_text += " | "
            stats_text += f"Recherches: YT({self.total_youtube_searches}) Lib({self.total_library_searches})"
        
        self.stats_bar.config(text=stats_text)
    ...
    def _perform_library_search(self):
        ...
        if not search_term:
            ...
            if self.library_search_start_time:
                ...
                self.last_library_search_time = search_duration
                self.total_library_searches += 1
                print(f"Recherche bibliothèque terminée en {search_duration:.3f}s - {len(filtered_files)} résultats trouvés")
        ...
        try:
            ...
            with YoutubeDL(search_opts) as ydl:
                ...
                if self.search_start_time and not self.search_cancelled:
                    ...
                    self.last_youtube_search_time = search_duration
                    self.total_youtube_searches += 1
                    self.root.after(0, lambda: self._safe_status_update(
                        f"Recherche terminée en {search_duration:.2f}s - {len(self.all_search_results)} résultats trouvés"
                    ))
                ...
                if results and 'entries' in results:
                    ...
                    if new_results and not self.search_cancelled:
                        ...
                        if start_time:
                            ...
                            self.last_youtube_search_time = fetch_duration  # Mettre à jour le temps de la dernière recherche
                            self.root.after(0, lambda: self._safe_status_update(
                                f"Chargement terminé en {fetch_duration:.2f}s - {len(new_results)} nouveaux résultats"
                            ))
                            self.root.after(0, self._update_stats_bar)
                    else:
                        ...
                        if start_time:
                            fetch_duration = time.time() - start_time
                            self.root.after(0, lambda: self._safe_status_update(
                                f"Tous les résultats chargés en {fetch_duration:.2f}s"
                            ))
                        else:
