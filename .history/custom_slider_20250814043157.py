...
class CustomVolumeSlider(tk.Frame):
    ...
    def __init__(self, parent, from_=0, to=100, value=50, command=None, orient='horizontal', 
        ...
        # Rendre transparent aux événements de scroll
        self._make_scroll_transparent()
    ...
    def _make_scroll_transparent(self):
        """Rend le widget transparent aux événements de scroll"""
        def ignore_scroll(event):
            # Ne rien faire, laisser l'événement passer
            return
        
        # Appliquer à tous les widgets du slider
        widgets_to_make_transparent = [self, self.canvas, self.title_label, self.slider_frame, self.value_label]
        
        for widget in widgets_to_make_transparent:
            try:
                # Bind les événements de scroll pour qu'ils ne fassent rien
                widget.bind('<MouseWheel>', ignore_scroll)
                widget.bind('<Button-4>', ignore_scroll)  # Linux
                widget.bind('<Button-5>', ignore_scroll)  # Linux
            except:
