    ...
    def _process_html_file(self, html_file_path, max_duration):
        ...
        def process_in_thread():
            try:
                ...
                for i, url in enumerate(youtube_links):
                    try:
                        ...
                        ydl_opts = {
                            ...
                            'ignoreerrors': True,  # Continuer même en cas d'erreur
                        }
                        ...
                        # Catégoriser les erreurs pour un meilleur diagnostic
                ...
                # Télécharger les vidéos valides par paquets
                if valid_videos:
                    ...
                    self.music_player.root.after(0, lambda bs=batch_size: self.music_player.status_bar.config(
                        text=f"Démarrage du téléchargement de {len(valid_videos)} vidéos par paquets de {bs}..."))
                    
                    self._download_videos_in_batches(valid_videos, batch_size)
                else:
                    self.music_player.root.after(0, lambda: self.music_player.status_bar.config(
