...
def _add_to_specific_playlist_with_youtube_support(self, filepath, playlist_name, youtube_item=None):
    ...
    if youtube_item:
        # C'est une vidéo YouTube, télécharger d'abord puis ajouter
        self._download_and_add_to_playlist(youtube_item, playlist_name)
    else:
        # C'est un fichier local, ajouter directement
        self.add_file_to_playlist(filepath, playlist_name)
        self.refresh_ui_after_changes()
...
def _download_and_add_to_playlist(self, youtube_item, playlist_name):
    ...
    url = youtube_item.get('webpage_url', '')
    title = youtube_item.get('title', 'Titre inconnu')
    ...
    if url:
        def on_download_complete(result):
            if result['success']:
                ...
                self.refresh_ui_after_changes()
                self.status_bar.config(text=f"Téléchargé et ajouté à '{playlist_name}': {os.path.basename(result['filepath'])}")
            else:
                self.status_bar.config(text=f"Erreur téléchargement: {result.get('error', 'Erreur inconnue')}")
