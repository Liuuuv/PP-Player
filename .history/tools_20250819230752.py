...
def on_mousewheel_lazy(self, event, canvas):
    ...
    self._check_infinite_scroll(event, canvas)
    
    # Empêcher la propagation pour éviter le double traitement
    return "break"

def _check_infinite_scroll(self, event, canvas):
    ...
    try:
        ...
        # if not (hasattr(self, 'hovered_canvas') and self.hovered_canvas.winfo_exists()):
        #     return
        ...
        if get_config('enable_dynamic_scroll'):
            ...
            _on_dynamic_scroll(self, event, canvas)
...
def _on_dynamic_scroll(self, event, canvas):
    ...
    try:
        ...
        try:
            ...
            if scroll_top <= scroll_threshold and not loading_up:
                ...
                if hasattr(event, 'delta') and event.delta:
                    if event.delta >= 0:
                        self._load_more_songs_above(canvas)
                        if get_config('debug_scroll'):
                ...
                if hasattr(event, 'delta') and event.delta:
                    if event.delta <= 0:
                        self._load_more_songs_below(canvas=canvas)
                        if get_config('debug_scroll'):
...
def _load_more_songs_above(self, canvas):
    ...
    try:
        ...
        self._extend_window_up(new_start, canvas)
...
def _load_more_songs_below(self, unload=True, canvas=None):
    ...
    try:
        ...
        if not hasattr(self, '_last_window_end'):
            print("Erreur: _last_window_end non initialisé")
            return
        
        if canvas == self.playlist_canvas:
        current_end = self.playlist_canvas_last_window_end
        ...
        self._extend_window_down(new_end, canvas)
        if unload:
            _check_and_unload_items(self, self.current_index, canvas)
...
def _extend_window_up(self, new_start, canvas):
    ...
    try:
        ...
        for i in range(new_start, current_start):
            if i < len(self.main_playlist):
                ...
                if canvas == self.playlist_canvas:
                    ...
                    filepath = self.all_downloaded_files[i]
                self._add_item_at_position(canvas, filepath, song_index=i, position='top')
                if get_config('debug_scroll'):
        ...
        self._reorder_playlist_items(canvas)
        ...
        self._adjust_scroll_after_top_load(items_added, canvas)
...
def _check_and_unload_items(self, current_index, canvas):
...
def _update_canvas_scroll_region(self, canvas):
    ...
    try:
        ...
        if not (self.canvas.winfo_exists() and self.playlist_container.winfo_exists()):
            return
        ...
        if children_count > 0:
            item_height = get_config('item_height_estimate')
            total_songs = len(self.main_playlist)
            enable_dynamic = get_config('enable_dynamic_scroll')

            virtual_height = children_count * item_height
            self.canvas.configure(scrollregion=(0, 0, 0, virtual_height))
