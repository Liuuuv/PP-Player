    ...
    def _process_html_file(self, html_file_path, max_duration):
        ...
        def process_in_thread():
            try:
                ...
                print(f"🔍 Logger object: {self.import_logger}")
                print(f"🔍 Logger type: {type(self.import_logger)}")
                
                # Démarrer une session de logs avec timeout
                session_id = None
                try:
                    print("🔄 Appel de start_session...")
                    print(f"🔍 Paramètres: type='HTML', path='{html_file_path}'")
                    
                    # Essayer avec un timeout
                    import signal
                    
                    def timeout_handler(signum, frame):
                        raise TimeoutError("start_session a pris trop de temps")
                    ...
                    try:
                        print("🔄 Avant appel start_session...")
                        session_id = self.import_logger.start_session('HTML', html_file_path)
                        print(f"🔄 Après appel start_session, session_id: {session_id}")
                        signal.alarm(0)  # Annuler le timeout
                        print(f"✅ Session créée: {session_id}")
                    except TimeoutError:
                        signal.alarm(0)
                        print("⚠️ Timeout sur start_session, on continue sans session...")
                        session_id = f"timeout_session_{int(time.time())}"
                    ...
                    # Ne pas faire return, continuer avec une session par défaut
    ...
    def _process_videos_in_waves_sequential(self, youtube_links, max_duration, batch_size, html_file_path, session_id):
        ...
        def process_wave_thread():
            try:
                ...
                # Pause pour laisser l'interface se mettre à jour
                time.sleep(1)
