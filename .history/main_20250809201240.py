...
class MusicPlayer:
    def __init__(self, root):
        ...
        self.current_playlist_name = "Principale"  # Playlist par défaut
        self.playlists[self.current_playlist_name] = []  # Initialiser la playlist principale
        # Faire pointer self.playlist vers la playlist principale pour compatibilité
        self.playlist = self.playlists[self.current_playlist_name]
        
        # Charger les playlists sauvegardées
        self.load_playlists()
        
        # Variables pour la gestion des onglets
        self.current_tab = None  # "search" ou "library" ou None
        self.tab_deselected = False  # True si on a désélectionné un onglet
    ...
    def on_tab_changed(self, event):
        """Gère le changement d'onglet avec possibilité de désélection"""
        selected_tab = self.notebook.tab(self.notebook.select(), "text")
        
        # Vérifier si on clique sur l'onglet déjà sélectionné
        if selected_tab == self.current_tab:
            # Désélectionner l'onglet et afficher la vue par défaut
            self._show_default_view()
            self.current_tab = None
            self.tab_deselected = True
        else:
            # Sélectionner le nouvel onglet
            self.current_tab = selected_tab
            self.tab_deselected = False
    ...
    def _show_default_view(self):
        """Affiche la vue par défaut avec la liste de lecture et la miniature"""
        # Masquer les contenus des onglets
        for widget in self.search_tab.winfo_children():
            widget.pack_forget()
        for widget in self.library_tab.winfo_children():
            widget.pack_forget()
        
        # Afficher seulement la liste de lecture dans l'onglet recherche
        self._setup_default_search_view()
    
    def _show_search_view(self):
        """Affiche la vue de recherche normale"""
        # Remettre le contenu normal de l'onglet recherche
        for widget in self.search_tab.winfo_children():
            widget.pack_forget()
        self.setup_search_tab()
    
    def _show_library_view(self):
        """Affiche la vue de bibliothèque normale"""
        # Le contenu de la bibliothèque reste normal
        pass
    
    def _setup_default_search_view(self):
        """Configure la vue par défaut avec liste de lecture et miniature"""
        # Frame principal pour la vue par défaut
        default_frame = ttk.Frame(self.search_tab)
        default_frame.pack(fill=tk.BOTH, expand=True)
        
        # Frame pour la miniature (en haut)
        thumbnail_frame = ttk.Frame(default_frame)
        thumbnail_frame.pack(fill=tk.X, pady=10)
        
        # Miniature de la chanson en cours
        self.current_song_thumbnail = tk.Label(
            thumbnail_frame,
            bg='#3d3d3d',
            width=20,
            height=15,
        ...
        self.current_song_thumbnail.pack(pady=20)
        
        # Mettre à jour la miniature si une chanson est en cours
        if len(self.playlist) > 0 and self.current_index < len(self.playlist):
            self._update_current_song_thumbnail()
        
        # Frame pour la liste de lecture (en bas)
        playlist_frame = ttk.Frame(default_frame)
        playlist_frame.pack(fill=tk.BOTH, expand=True, pady=(0, 10))
        
        # Canvas et Scrollbar pour la playlist
        self.default_playlist_canvas = tk.Canvas(
            playlist_frame,
            bg='#3d3d3d',
            highlightthickness=0
        )
        self.default_playlist_scrollbar = ttk.Scrollbar(
            playlist_frame,
            orient="vertical",
            command=self.default_playlist_canvas.yview
        )
        self.default_playlist_canvas.configure(yscrollcommand=self.default_playlist_scrollbar.set)

        self.default_playlist_scrollbar.pack(side="right", fill="y")
        self.default_playlist_canvas.pack(side="left", fill="both", expand=True)

        self.default_playlist_container = ttk.Frame(self.default_playlist_canvas)
        self.default_playlist_canvas.create_window((0, 0), window=self.default_playlist_container, anchor="nw")

        self.default_playlist_container.bind(
            "<Configure>",
            lambda e: self.default_playlist_canvas.configure(
                scrollregion=self.default_playlist_canvas.bbox("all")
            )
        )
        self._bind_mousewheel(self.default_playlist_canvas, self.default_playlist_canvas)
        self._bind_mousewheel(self.default_playlist_container, self.default_playlist_canvas)
        
        # Afficher la liste de lecture
        self._populate_default_playlist()
    
    def _update_current_song_thumbnail(self):
        """Met à jour la miniature de la chanson en cours"""
        if hasattr(self, 'current_song_thumbnail') and len(self.playlist) > 0 and self.current_index < len(self.playlist):
            current_song = self.playlist[self.current_index]
            self._load_download_thumbnail(current_song, self.current_song_thumbnail)
    
    def _populate_default_playlist(self):
        """Remplit la liste de lecture dans la vue par défaut"""
        if hasattr(self, 'default_playlist_container'):
            # Vider le container
            for widget in self.default_playlist_container.winfo_children():
                widget.destroy()
            
            # Ajouter tous les éléments de la playlist
            for filepath in self.playlist:
                self._add_playlist_item_to_default(filepath)
    
    def _add_playlist_item_to_default(self, filepath):
        """Ajoute un élément de playlist dans la vue par défaut"""
        try:
            filename = os.path.basename(filepath)
            
            # Vérifier si c'est la chanson en cours
            is_current_song = (len(self.playlist) > 0 and 
                             self.current_index < len(self.playlist) and 
                             self.playlist[self.current_index] == filepath)
            
            # Frame principal
            bg_color = '#5a9fd8' if is_current_song else '#4a4a4a'
            item_frame = tk.Frame(
                self.default_playlist_container,
                bg=bg_color,
                relief='flat',
                bd=1,
            ...
            # Configuration similaire à la playlist normale
            ...
            duration_label.bind("<Double-1>", lambda e: on_item_click())
            ...
            print(f"Erreur affichage default playlist item: {e}")
    ...
    def _display_playlists(self):
        ...
        # Organiser les playlists en lignes de 3 (exclure la playlist "Principale")
        playlist_items = [(name, songs) for name, songs in self.playlists.items() if name != "Principale"]
    ...
    def _add_playlist_card(self, parent_frame, playlist_name, songs, column):
        ...
        try:
            ...
            for i in range(4):
                ...
                if i < len(songs):
                    self._load_playlist_thumbnail(songs[i], thumbnail_label)
                else:
                    # Miniature vide
                    thumbnail_label.config(text="♪", fg='#666666', font=('TkDefaultFont', 16))
            ...
            # Bouton supprimer (nécessite double-clic)
            delete_btn = tk.Button(
                buttons_frame,
                image=self.icons['delete'] if hasattr(self, 'icons') and 'delete' in self.icons else None,
                text="🗑" if not (hasattr(self, 'icons') and 'delete' in self.icons) else "",
                bg="#ff4444",
                fg="white",
                activebackground="#ff6666",
                relief="flat",
    ...
    def save_playlists(self):
        ...
        try:
            ...
            # Sauvegarder toutes les playlists sauf la principale
            playlists_to_save = {name: songs for name, songs in self.playlists.items() if name != "Principale"}
    ...
    def load_playlists(self):
        ...
        try:
            ...
            if os.path.exists(playlists_file):
                ...
                # Ajouter les playlists chargées (en gardant la playlist principale)
    ...
    def _play_from_playlist(self, filepath, playlist_name):
        ...
        # Ajouter à la playlist principale si pas déjà présent
    ...
    def _clear_youtube_search(self):
        ...
        # Si on est dans l'onglet recherche et qu'il n'y a plus de résultats, basculer vers la vue par défaut
        if self.current_tab == "Recherche":
            self._show_default_view()
            self.tab_deselected = True
            self.current_tab = None
    ...
    def _add_to_specific_playlist(self, filepath, playlist_name):
        ...
        if playlist_name == "Principale":
            # Pour la playlist principale, utiliser l'ancienne méthode
            if filepath not in self.playlist:
                self.playlist.append(filepath)
                self._add_playlist_item(filepath)
    ...
    def _add_download_to_playlist(self, filepath):
        """Ajoute un fichier téléchargé à la playlist principale (pour compatibilité)"""
        self._add_to_specific_playlist(filepath, "Principale")
    ...
    def play_track(self):
        try:
            ...
            # Mettre à jour la miniature dans la vue par défaut si elle est active
            if self.tab_deselected:
                self._update_current_song_thumbnail()
                self._populate_default_playlist()
            ...
            try:
                ...
                with YoutubeDL(self.ydl_opts) as ydl:
                    ...
                    if os.path.exists(audio_filename):
                        # Ajouter à la playlist après la chanson en cours
                        insert_position = self.current_index + 1
                        self.playlist.insert(insert_position, audio_filename)
    ...
    def _refresh_playlist_display(self):
        """Rafraîchit l'affichage de la playlist"""
        # Vider le container actuel
