...
def create_ui(self):
    ...
    self.import_button.place(in_=self.notebook, relx=1.0, rely=0.0, anchor="ne", x=-60, y=-5)
...
def setup_controls(self):
        ...
        self.progress = CustomProgressSlider(
            ...
            command=self.set_position, length=800
        )
...
def setup_keyboard_bindings(self):
        ...
        self.root.bind('<Control-Alt-m>', self.on_global_play_pause)
        self.root.bind('<Control-Alt-M>', self.on_global_play_pause)
        
        # Test de debug pour Ctrl+Alt+M
        def test_binding(event):
            print("DEBUG: Test binding Ctrl+Alt+M détecté !")
            return self.on_global_play_pause(event)
        
        self.root.bind('<Control-Alt-Key-m>', test_binding)
        self.root.bind('<Control-Alt-Key-M>', test_binding)
