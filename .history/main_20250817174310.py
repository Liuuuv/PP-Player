    ...
    def update_time(self):
        while True:
            try:
                ...
                # DEBUG: Vérifier si le thread fonctionne pendant le drag
                if hasattr(self, 'user_dragging') and self.user_dragging:
                    print(f"[DEBUG] UPDATE THREAD RUNNING - user_dragging: {self.user_dragging}")
                ...
                if pygame.mixer.music.get_busy() and not self.paused:
                    ...
                    if not self.update_suspended:
                        try:
                            # DEBUG: Afficher les infos pendant le drag
                            if hasattr(self, 'user_dragging') and self.user_dragging:
                                print(f"[DEBUG] UPDATE THREAD - DRAGGING MODE")
                                print(f"  current_time: {self.current_time}")
                                print(f"  user_dragging: {self.user_dragging}")
                                print(f"  progress has set_value: {hasattr(self.progress, 'set_value')}")
                                if hasattr(self.progress, 'dragging'):
                                    print(f"  progress.dragging: {self.progress.dragging}")
                            
                            # Utiliser set_value() pour le CustomProgressSlider
                            if hasattr(self.progress, 'set_value'):
                                self.progress.set_value(self.current_time)
                                if hasattr(self, 'user_dragging') and self.user_dragging:
                                    print(f"  -> Called set_value({self.current_time})")
                            else:
