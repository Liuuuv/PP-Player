...
class MusicPlayer:
    ...
    # create_download_item_widget supprimée - remplacée par le système de chargement paresseux
    ...
    def _update_canvas_scroll_region(self, canvas):
        ...
        return tools._update_canvas_scroll_region(self)

    def _setup_infinite_scroll(self, canvas):
        """Configure le scroll infini pour charger plus d'éléments (redirige vers dynamic_scroll)"""
        return tools._setup_dynamic_scroll(self, canvas)
    ...
    def _check_infinite_scroll(self, event=None, canvas=None):
        """Vérifie si on doit charger plus d'éléments en haut ou en bas"""
        return tools._check_infinite_scroll(self, event, canvas)

    def _load_more_songs_above(self, canvas):
        """Charge plus de musiques au-dessus de la fenêtre actuelle"""
        return tools._load_more_songs_above(self, canvas)

    def _load_more_songs_below(self, unload=False, canvas=None):
        """Charge plus de musiques en-dessous de la fenêtre actuelle"""
        return tools._load_more_songs_below(self, unload=unload, canvas=canvas)

    def _extend_window_up(self, new_start, canvas):
        """Étend la fenêtre d'affichage vers le haut"""
        return tools._extend_window_up(self, new_start, canvas)

    def _extend_window_down(self, new_end, canvas):
        """Étend la fenêtre d'affichage vers le bas"""
        return tools._extend_window_down(self, new_end, canvas)
    
    def _add_item_at_position(self, canvas, filepath, song_index=None, position='bottom'):
        """Ajoute un élément de playlist à une position spécifique (top ou bottom)"""
        if canvas == self.playlist_canvas:
            return search_tab.main_playlist._add_main_playlist_item_at_position(self, filepath, song_index, position)
        elif canvas == self.downloads_canvas:
            return library_tab.downloads._add_downloads_playlist_item_at_position(self, filepath, song_index, position)
    ...
    def _bind_scroll(self, canvas, func=None):
        ...
        if func is None:
            ...
            canvas.bind_all("<MouseWheel>", lambda e: func(self, e, canvas))
            canvas.bind_all("<Button-4>", lambda e: func(self, e, canvas))
            canvas.bind_all("<Button-5>", lambda e: func(self, e, canvas))
    
    def set_hovered_canvas(self, canvas):
        self.hovered_canvas = canvas
