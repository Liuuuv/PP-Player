...
def _add_artist_playlist_result(self, playlist, index, container, target_tab="sorties"):
        ...
        try:
            ...
            try:
                ...
                max_width = 90
            display_title = utils.truncate_text_for_display(self, title, max_width_pixels=max_width, font_size=8)
            ...
            def on_enter(event):
                ...
                if not result_frame.selected:  # Ne pas changer la couleur si l'item est sélectionné
                    # Calculer une couleur plus claire
                    hover_color = self._lighten_color(TEST_COLOR, HOVER_LIGHT_PERCENTAGE)
                    self._set_item_colors(result_frame, hover_color)
...
def _update_loading_messages(self):
    ...
    try:
        if hasattr(self, 'musiques_loading') and self.musiques_loading.winfo_exists():
            self.musiques_loading.config(text="Chargement des musiques...", fg=COLOR_TEXT)
        if hasattr(self, 'sorties_loading') and self.sorties_loading.winfo_exists():
            self.sorties_loading.config(text="Chargement des sorties...", fg=COLOR_TEXT)
        if hasattr(self, 'playlists_loading') and self.playlists_loading.winfo_exists():
            self.playlists_loading.config(text="Chargement des playlists...", fg=COLOR_TEXT)
    except:
        pass
...
def _add_artist_result(self, video, index, container):
        ...
        try:
            ...
            try:
                ...
                max_duration = INTERFACE_CONFIG.get('max_duration_show_search', 600)
            except ImportError:
                max_duration = 600  # 10 minutes par défaut
...
def _load_artist_thumbnail(self, video, thumbnail_label):
        ...
        if not hasattr(self, '_thumbnail_executor'):
            ...
            try:
                from search_tab.config import get_artist_config
                pool_size = get_artist_config('thumbnail_pool_size') or 4
            except ImportError:
                pool_size = 4
            self._thumbnail_executor = concurrent.futures.ThreadPoolExecutor(max_workers=pool_size)
        ...
        def load_thumbnail():
            try:
                ...
                if video_id:
                    ...
                    if cached_thumbnail:
                        def update_cached():
                            if safe_widget_config(thumbnail_label, image=cached_thumbnail, text=""):
                                thumbnail_label.image = cached_thumbnail
                        
                        if hasattr(self, 'safe_after'):
                            self.safe_after(0, update_cached)
                        else:
                            try:
                ...
                if response.status_code == 200:
                    ...
                    # Mettre à jour le label dans le thread principal avec vérification sécurisée
                    def update_thumbnail():
                        if safe_widget_config(thumbnail_label, image=photo, text=""):
                            thumbnail_label.image = photo  # Garder une référence
                    
                    # Utiliser safe_after au lieu de root.after
                    if hasattr(self, 'safe_after'):
                        self.safe_after(0, update_thumbnail)
                    else:
                        try:
...
def _load_playlist_count(self, playlist, count_label):
        ...
        if not hasattr(self, '_playlist_executor'):
            ...
            try:
                from search_tab.config import get_artist_config
                pool_size = get_artist_config('playlist_pool_size') or 2
            except ImportError:
                pool_size = 2
            self._playlist_executor = concurrent.futures.ThreadPoolExecutor(max_workers=pool_size)
            ...
            try:
                ...
                with YoutubeDL(count_opts) as ydl:
                    ...
                    if playlist_info and 'entries' in playlist_info:
                        ...
                        # Mettre en cache avec le gestionnaire optimisé
                        self.artist_tab_manager.cache_manager.set_playlist_content(playlist_url, entries)
                    ...
                    # Mettre à jour le label dans le thread principal avec vérification sécurisée
                    def update_count():
                        safe_widget_config(count_label, text=count_text)
                    
                    # Utiliser safe_after au lieu de root.after
                    if hasattr(self, 'safe_after'):
                        self.safe_after(0, update_count)
                    else:
                        try:
                ...
                # En cas d'erreur, afficher juste "Playlist" avec vérification sécurisée
                def update_error():
                    safe_widget_config(count_label, text="Playlist")
                
                if hasattr(self, 'safe_after'):
                    self.safe_after(0, update_error)
                else:
                    try:
...
def _load_more_results(self, content_type):
    ...
    try:
        ...
        # Utiliser la configuration centralisée pour les limites
        try:
            from search_tab.config import get_artist_config
            search_limits = get_artist_config('search_limits') or {
                'videos': 50, 'releases': 30, 'playlists': 25
            }
        except ImportError:
            search_limits = {'videos': 50, 'releases': 30, 'playlists': 25}
        
        max_limit = search_limits.get(content_type, 25)
...
def _start_artist_name_animation(self):
    ...
    try:
        ...
        max_width = 90
    truncated_name = tools._truncate_text_for_display(self, self.current_artist_name, max_width_pixels=max_width, font_family='TkDefaultFont', font_size=9)
