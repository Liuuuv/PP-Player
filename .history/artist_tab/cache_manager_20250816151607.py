...
class ArtistCacheManager:
    ...
    def __init__(self, downloads_folder=None):
        ...
        try:
            ...
            cache_file = get_cache_config('cache_file', 'search') or os.path.join(self.downloads_folder, 'search_cache.json')
        except ImportError:
            # Valeurs par défaut
            self.max_searches = 50
            self.search_expire_time = 3600
            self.max_interface_states = 20
            cache_file = os.path.join(self.downloads_folder, 'search_cache.json')
