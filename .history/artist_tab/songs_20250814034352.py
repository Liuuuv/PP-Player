...
def _search_artist_videos_with_id(self):
        ...
        try:
            ...
            if cache_key in self.artist_cache and 'videos' in self.artist_cache[cache_key]:
                ...
                self.root.after(0, lambda: _display_artist_videos(self, cached_videos))
                return
            
            # Configuration ultra-optimisée pour extraire les vidéos de la chaîne rapidement
            search_opts = {
                'extract_flat': True,  # Plus efficace et contient la durée
                'quiet': True,
                'no_warnings': True,
                'ignoreerrors': True,
                'socket_timeout': 3,    # Timeout très court
                'retries': 0,           # Pas de retry pour plus de rapidité
                'fragment_retries': 0,  # Pas de retry pour les fragments
                'playlistend': 20,      # Limiter à 20 vidéos pour plus de rapidité
                'skip_unavailable_fragments': True,  # Ignorer les fragments indisponibles
                'abort_on_unavailable_fragment': False,  # Ne pas s'arrêter sur les erreurs
                'concurrent_fragment_downloads': 1  # Limiter les téléchargements simultanés
            }
            ...
            with YoutubeDL(search_opts) as ydl:
                ...
                try:
                    ...
                    if channel_info and 'entries' in channel_info:
                        ...
                        if videos:
                            all_videos.extend(videos[:25])  # Prendre les 25 premières pour plus de rapidité
                        else:
                            pass
                    else:
                        pass
                ...
                for i, video in enumerate(all_videos):
                    ...
                    if i % 5 == 0 and self.artist_search_cancelled:
                        return
                    ...
                    # Petite pause pour permettre à l'UI de respirer
                    if i % 10 == 0:
                        import time
...
def _display_artist_videos(self, videos):
        ...
        # Remettre l'état du contenu de playlist à zéro pour tous les onglets car on affiche les musiques normales
        ...
        # Bind mousewheel avec détection de scroll en bas
        def _on_mousewheel(event):
            canvas.yview_scroll(int(-1*(event.delta/120)), "units")
            # Vérifier si on est en bas de page pour charger plus de résultats
            core._check_scroll_bottom(self, canvas, "videos")
        
        def bind_mousewheel_recursive(widget):
            """Bind mousewheel à un widget et tous ses enfants de manière robuste"""
            try:
                widget.bind("<MouseWheel>", _on_mousewheel)
                widget.bind("<Button-4>", lambda e: _on_mousewheel(type('Event', (), {'delta': 120})()))  # Linux
                widget.bind("<Button-5>", lambda e: _on_mousewheel(type('Event', (), {'delta': -120})()))  # Linux
                
                # Bind récursif pour tous les enfants
                for child in widget.winfo_children():
                    bind_mousewheel_recursive(child)
            except:
                pass
        
        # Bind sur le canvas principal
        ...
        # Bind récursif sur le frame scrollable
        bind_mousewheel_recursive(scrollable_frame)
        
        # Bind également sur le frame parent pour être sûr
        self.musiques_frame.bind("<MouseWheel>", _on_mousewheel)
        
        # Fonction pour rebinder automatiquement les nouveaux widgets
        def rebind_new_widgets():
            try:
                bind_mousewheel_recursive(scrollable_frame)
                self.safe_after(1000, rebind_new_widgets)  # Rebind toutes les secondes
            except:
                pass
        
        # Démarrer le rebinding automatique
        self.safe_after(1000, rebind_new_widgets)
        
        # Stocker le canvas pour la détection de scroll
        self.artist_videos_canvas = canvas
