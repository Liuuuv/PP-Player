...
def get_optimized_search_options(content_type="default"):
    """Retourne les options de recherche optimisées selon le type de contenu"""
    base_options = ARTIST_SEARCH_OPTIONS.copy()
    
    if content_type == "videos":
        base_options['playlistend'] = get_search_limit('max_videos')
    elif content_type == "releases":
        base_options['playlistend'] = get_search_limit('max_releases')
    elif content_type == "playlists":
        base_options['playlistend'] = get_search_limit('max_playlists')
    elif content_type == "channel_search":
        base_options['playlistend'] = 1
