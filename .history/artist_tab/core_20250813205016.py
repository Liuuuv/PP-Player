...
def _display_results_in_batches(self, items, container, item_type, batch_size=2):
        """Affiche les résultats par petits lots de façon non-bloquante pour éviter le lag de l'interface"""
        if not items:
            return
        
        # Adapter la taille des lots selon le nombre d'éléments pour optimiser les performances
        if len(items) > 20:
            batch_size = 1  # Lots plus petits pour beaucoup d'éléments
        elif len(items) > 10:
            batch_size = 2  # Taille normale
        else:
            batch_size = 3  # Lots plus gros pour peu d'éléments
        
        # Diviser les éléments en lots optimisés pour plus de fluidité
        ...
        def display_batch(batch_index):
            ...
            # Programmer le lot suivant avec un délai adaptatif pour plus de fluidité
            delay = 15 if batch_index < 3 else 25  # Plus rapide pour les premiers lots
...
def _create_artist_tabs(self):
        ...
        # Nettoyer tout le contenu de youtube_results_frame de manière optimisée
        children = list(self.youtube_results_frame.winfo_children())
        for i, widget in enumerate(children):
            try:
                widget.destroy()
                # Petite pause tous les 5 widgets pour éviter le lag
                if i % 5 == 0:
                    self.root.update_idletasks()
            except:
                pass
...
def _append_new_results(self, new_items, content_type):
    ...
    try:
        ...
        # Rebinder le scroll sur les nouveaux éléments
        _rebind_scroll_on_new_elements(self, canvas, scrollable_frame, content_type)
...
def _rebind_scroll_on_new_elements(self, canvas, scrollable_frame, content_type):
    """Rebind le scroll sur les nouveaux éléments ajoutés"""
    try:
        def _on_mousewheel(event):
            canvas.yview_scroll(int(-1*(event.delta/120)), "units")
            _check_scroll_bottom(self, canvas, content_type)
        ...
        # Rebinder sur tous les éléments du frame scrollable
        bind_mousewheel_recursive(scrollable_frame)
        
    except Exception as e:
        print(f"Erreur lors du rebinding du scroll: {e}")