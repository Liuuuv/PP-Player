...
def _create_artist_tabs(self):
    ...
    if hasattr(self, 'icons') and 'cross_small' in self.icons:
        self.artist_close_btn = tk.Button(
            ...
            width=20,
            height=20,
            command=lambda: self._return_to_search(),
            cursor='hand2',
            takefocus=0
        )
    else:
...
def _show_artist_content(self, artist_name, video_data):
    ...
    try:
        ...
        # Remettre l'état du contenu de playlist à zéro pour le nouvel artiste
        print("DEBUG: Avant _reset_playlist_content_state")
        _reset_playlist_content_state(self)
        print("DEBUG: Après _reset_playlist_content_state")
        ...
        print("DEBUG: Avant _save_current_search_state (artist_tab)")
        self._save_current_search_state()
        print("DEBUG: Après _save_current_search_state (artist_tab)")
        
        # S'assurer que la zone YouTube est visible
        print("DEBUG: Avant _show_search_results (artist_tab)")
        self._show_search_results()
        print("DEBUG: Après _show_search_results (artist_tab)")
        
        # Créer les onglets IMMÉDIATEMENT (UI d'abord, contenu après)
        print("DEBUG: Avant _create_artist_tabs (artist_tab)")
        self._create_artist_tabs()
        print("DEBUG: Après _create_artist_tabs (artist_tab)")
...
def _reset_playlist_content_state(self):
    """Remet l'état du contenu de playlist à zéro pour tous les onglets"""
    try:
        ...
        # Masquer les boutons retour s'ils existent
        ...
        if hasattr(self, 'playlist_back_btn') and self.playlist_back_btn is not None:
            try:
