...
class MusicPlayer:
    ...
    def _scroll_to_current_song(self, event=None):
        ...
        try:
            # Utiliser l'animation pour le scroll manuel vers la chanson courante
            self.select_playlist_item(index=self.current_index, animate_scroll=True)
    ...
    def select_playlist_item(self, item_frame=None, index=None, animate_scroll=False):
        """Met en surbrillance l'élément sélectionné dans la playlist
        
        Args:
            item_frame: Frame de l'élément à sélectionner
            index: Index de l'élément à sélectionner
            animate_scroll: Si True, anime le scroll vers l'élément
        """
        ...
        if item_frame:
            ...
            try:
                ...
                if container_height > 0:
                    ...
                    if animate_scroll:
                        # Animation smooth seulement si demandée
                        self._smooth_scroll_to_position(target_position)
                    else:
                        # Scroll instantané par défaut
