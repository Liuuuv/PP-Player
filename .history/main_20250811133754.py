...
class MusicPlayer:
    ...
    def _clear_results(self):
        ...
        try:
            ...
            if hasattr(self, 'results_counter_label') and self.results_counter_label.winfo_exists():
                self.results_counter_label.pack_forget()
    ...
    def _show_search_results(self):
        ...
        try:
            ...
            # Afficher le label du compteur de résultats en bas à droite
            if hasattr(self, 'results_counter_label') and self.results_counter_label.winfo_exists():
                self.results_counter_label.pack(side="bottom", fill="x", padx=5, pady=2)
    ...
    def _update_results_counter(self):
        ...
        try:
            if hasattr(self, 'results_counter_label') and self.results_counter_label.winfo_exists():
                ...
                total_count = min(len(self.all_search_results), self.max_search_results)
                
                # Si on a plus de résultats disponibles que le maximum autorisé
                if len(self.all_search_results) >= self.max_search_results:
