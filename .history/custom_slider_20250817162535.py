...
class CustomProgressSlider(tk.Frame):
    ...
    def on_click(self, event):
        ...
        # Ne pas appeler command pendant le drag - seulement au release
    ...
    def on_release(self, event):
        ...
        if self.dragging:
            ...
            value = self.get_value_from_x(event.x)
            # IMPORTANT: Mettre à jour current_value AVANT d'appeler le callback
            # car le callback utilise self.get() qui retourne current_value
            self.current_value = value
            ...
            self.draw_slider()
            # Utiliser seulement les callbacks spécifiques, pas command
