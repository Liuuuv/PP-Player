...
def _add_youtube_to_playlist(self, video, frame, playlist_name):
    ...
    if existing_file:
        ...
        threading.Thread(
            target=self._download_youtube_thread_for_playlist,
            args=(url, playlist_name, frame),
            daemon=True
        ).start()
        
        # Restaurer search_list
        self.search_list = original_search_list

def _download_youtube_thread_for_playlist(self, url, playlist_name, frame):
    """Thread pour télécharger une vidéo YouTube et l'ajouter à une playlist spécifique"""
    try:
        video = self.search_list[0]
        title = video['title']
        url = video.get('webpage_url') or f"https://www.youtube.com/watch?v={video.get('id')}"
        ...
        if existing_file:
            # Le fichier existe déjà, l'ajouter directement à la playlist
            if playlist_name == "Main Playlist":
                self.safe_after(0, lambda: self.add_to_main_playlist(existing_file))
            else:
                if existing_file not in self.playlists[playlist_name]:
                    self.playlists[playlist_name].append(existing_file)
                    self.save_playlists()
                    self.safe_after(0, lambda: self.status_bar.config(text=f"Fichier existant ajouté à '{playlist_name}': {os.path.basename(existing_file)}"))
                else:
                    self.safe_after(0, lambda: self.status_bar.config(text=f"Déjà dans '{playlist_name}': {os.path.basename(existing_file)}"))
            
            # Remettre l'apparence normale
            self.safe_after(0, lambda: self._reset_frame_appearance(frame, '#4a4a4a'))
            return
        ...
        if not os.path.exists(downloads_dir):
            try:
                ...
                self.safe_after(0, lambda e=e: self.status_bar.config(text=f"Erreur création dossier: {str(e)}"))
                return

        # Utiliser les mêmes options que _download_youtube_thread
        ydl_opts = {
            'format': 'bestaudio/best',
            'outtmpl': os.path.join(downloads_dir, f'{safe_title}.%(ext)s'),
            'postprocessors': [{
        ...
        with YoutubeDL(ydl_opts) as ydl:
            ...
            if os.path.exists(final_path):
                ...
                if os.path.exists(downloaded_file + ".jpg"):
                    ...
                    self._download_youtube_thumbnail(info, final_path)
                ...
                if playlist_name == "Main Playlist":
                    self.safe_after(0, lambda: self.add_to_main_playlist(final_path))
                else:
                    if final_path not in self.playlists[playlist_name]:
                        self.playlists[playlist_name].append(final_path)
                        self.save_playlists()
                        self.safe_after(0, lambda: self.status_bar.config(text=f"Téléchargé et ajouté à '{playlist_name}': {safe_title}"))
                    else:
                        self.safe_after(0, lambda: self.status_bar.config(text=f"Déjà dans '{playlist_name}': {safe_title}"))
                ...
                self.safe_after(0, lambda: self.status_bar.config(text=f"Erreur: fichier non trouvé après téléchargement"))
                self.safe_after(0, lambda: self._reset_frame_appearance(frame, '#ffcc00', error=True))
                
    except Exception as e:
        self.safe_after(0, lambda e=e: self.status_bar.config(text=f"Erreur: {str(e)}"))
