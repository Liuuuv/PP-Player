...
class MusicPlayer:
    ...
    def _scroll_to_current_song(self, event=None):
        """Fait défiler la liste de lecture vers la chanson en cours"""
        ...
        try:
            # Calculer la position relative de la chanson actuelle
            total_songs = len(self.main_playlist)
            if total_songs == 0:
                return
            
            # Position relative (0.0 à 1.0)
            relative_position = self.current_index / max(1, total_songs - 1)
            
            # Faire défiler le canvas vers cette position
            self.playlist_canvas.yview_moveto(relative_position)
