...
class DragDropHandler:
    ...
    def _update_drag_visual(self, frame, dx, dy):
        ...
        elif dx < -100:  # Drag vers la gauche - ajouter en dernier dans la queue
            self._set_frame_colors(frame, '#6a6a4a')  # Jaune-orange pour indiquer "ajouter en dernier dans la queue"
        else:
    ...
    def _add_to_queue_last_dragged_item(self, frame):
        """Ajoute l'élément dragué en dernier dans la queue selon son type"""
        try:
            if frame.drag_item_type == "file":
                # Fichier local
                self._add_to_queue_last_file(frame.drag_file_path)
            elif frame.drag_item_type == "youtube":
                # Résultat YouTube
                self._add_to_queue_last_youtube(frame.drag_video_data, frame)
            elif frame.drag_item_type == "playlist_item":
                # Élément de playlist (copier vers main playlist en dernier dans la queue)
                self._add_to_queue_last_file(frame.drag_file_path)
            
        except Exception as e:
            print(f"Erreur lors de l'ajout en dernier dans la queue: {e}")
            self.music_player.status_bar.config(text=f"Erreur: {str(e)}")
    
    def _add_to_queue_last_file(self, file_path):
        """Ajoute un fichier local en dernier dans la queue"""
        if file_path:
            filename = file_path.split('/')[-1] if '/' in file_path else file_path.split('\\')[-1]
            
            # Ajouter en dernier dans la queue
            self._add_to_queue_last(file_path)
            self.music_player.status_bar.config(text=f"Ajouté en dernier dans la queue: {filename}")
            
            # Marquer que la main playlist ne provient pas d'une playlist
            self.music_player.main_playlist_from_playlist = False
    
    def _add_to_queue_last_youtube(self, video_data, frame):
        """Télécharge et ajoute une vidéo YouTube en dernier dans la queue"""
        if not video_data:
            return
        ...
        try:
            ...
            # Marquer cette vidéo pour être ajoutée en dernier dans la queue après téléchargement
            if not hasattr(self.music_player, 'pending_queue_last_additions'):
                self.music_player.pending_queue_last_additions = {}
            self.music_player.pending_queue_last_additions[url] = True
            
            # Changer l'apparence pour indiquer le téléchargement
            try:
                frame.config(bg='#6a6a4a')  # Jaune-orange pour indiquer le téléchargement pour queue
            except:
                pass
            ...
            self.music_player.status_bar.config(text=f"Téléchargement pour queue (dernier): {title[:30]}...")
        except Exception as e:
    ...
    def _add_to_queue_last(self, file_path):
        """Ajoute un fichier en dernier dans la queue (à la fin de la playlist)"""
        if not file_path:
            return
        ...
        # Vérifier si la playlist était vide avant l'ajout
        was_empty = len(self.music_player.main_playlist) == 0
        
        # Ajouter le fichier à la fin de la playlist s'il n'y est pas déjà
        if file_path not in self.music_player.main_playlist:
            self.music_player.main_playlist.append(file_path)
            
            # Si la playlist était vide, ne pas marquer comme queue (première chanson)
            if was_empty:
                return
            ...
            # L'index de la nouvelle chanson est la dernière position
            new_index = len(self.music_player.main_playlist) - 1
            self.music_player.queue_items.add(new_index)
