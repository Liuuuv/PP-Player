...
class MusicPlayer:
    ...
    def setup_media_keys(self):
        ...
        def on_media_key_press(key):
            ...
            try:
                ...
                if key in [keyboard.Key.media_play_pause, keyboard.Key.media_next, keyboard.Key.media_previous]:
                    ...
                    # Retourner False pour empêcher la propagation de l'événement vers d'autres applications
                    return False
        ...
        try:
            # Utiliser suppress=False pour capturer les événements avant les autres applications
            self.media_key_listener = keyboard.Listener(
                on_press=on_media_key_press,
                suppress=False  # Ne pas supprimer complètement, mais intercepter
            )
            self.media_key_listener.daemon = True  # Thread daemon pour qu'il se ferme avec l'application
            self.media_key_listener.start()
            print("Touches multimédia activées avec priorité (Play/Pause, Next, Previous)")
        except Exception as e:
            print(f"Impossible d'activer les touches multimédia: {e}")
            # Essayer une version de fallback sans suppression
            try:
                self.media_key_listener = keyboard.Listener(on_press=on_media_key_press)
                self.media_key_listener.daemon = True
                self.media_key_listener.start()
                print("Touches multimédia activées en mode fallback")
