...
class MusicPlayer:
    ...
    def _save_duration_cache(self):
        """Sauvegarde le cache des durées sur le disque - Version indépendante"""
        try:
    ...
    def _get_cached_duration(self, filepath):
        """Récupère la durée depuis le cache ou la calcule si nécessaire - Version indépendante"""
        try:
            if hasattr(self, 'downloads_manager'):
                return self.downloads_manager.get_formatted_duration(filepath)
            else:
                # Fallback vers l'ancienne méthode si le gestionnaire n'est pas disponible
                return "??:??"
        except Exception as e:
            print(f"Erreur récupération durée pour {filepath}: {e}")
            return "??:??"
    ...
    def _on_downloads_canvas_configure(self, event):
        ...
        return library_tab.downloads._on_downloads_canvas_configure(self, event)
    
    def _on_downloads_scroll(self, *args):
        """Gère le scroll de la liste des téléchargements et charge plus d'éléments si nécessaire"""
        return library_tab.downloads._on_downloads_scroll(self, *args)
    
    def _check_load_more_downloads(self):
        """Vérifie si on doit charger plus de téléchargements"""
        return library_tab.downloads._check_load_more_downloads(self)
