...
class AIMenuSystem:
    ...
    def create_ai_button(self, parent_frame):
        ...
        try:
            # Charger les icônes
            ai_icon_path = os.path.join(os.path.dirname(__file__), 'assets', 'activate_ai.png')
            ai_active_icon_path = os.path.join(os.path.dirname(__file__), 'assets', 'activate_ai_active.png')
            
            if os.path.exists(ai_icon_path):
                self.ai_icon = tk.PhotoImage(file=ai_icon_path)
                if os.path.exists(ai_active_icon_path):
                    self.ai_active_icon = tk.PhotoImage(file=ai_active_icon_path)
                else:
                    self.ai_active_icon = self.ai_icon
            else:
                # Fallback si les icônes n'existent pas
                self.ai_icon = None
                self.ai_active_icon = None
