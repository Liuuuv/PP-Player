...
class CustomProgressSlider(tk.Frame):
    ...
    def draw_slider(self):
        ...
        try:
            ...
            if self.dragging and hasattr(self, 'drag_thumb_x'):
                ...
                print(f"Thumb position: {thumb_x}, Track start: {track_start_x}, Track width: {track_width}, Progress: {progress}, Current value: {self.current_value}, From: {self.from_}, To: {self.to}")
    ...
    def on_release(self, event):
        ...
        if self.dragging:
            ...
            value = self.get_value_from_x(event.x)
            print(f"Release thumb at: {event.x} {value}")
