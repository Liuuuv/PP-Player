    ...
    def add_selection_to_main_playlist(self):
        ...
        for filepath in self.selected_items_order:
            ...
            if not filepath.startswith("https://"):
                # Toujours ajouter, même si déjà présent (permettre les doublons)
                self.main_playlist.append(filepath)
                self._add_main_playlist_item(filepath, None, None)
                added_count += 1
        ...
        if len(self.main_playlist) == 0:
            ...
            for i, filepath in enumerate(self.selected_items_order):
                if not filepath.startswith("https://"):
                    ...
                    should_skip = False
                    if self.queue_items and filepath in self.main_playlist:
                        # Trouver la première position dans la queue
                        sorted_queue_indices = sorted(self.queue_items)
                        if sorted_queue_indices:
                            ...
                            if (first_queue_index < len(self.main_playlist) and 
                                self.main_playlist[first_queue_index] == filepath):
                                should_skip = True
                    
                    if not should_skip:
                        # Insérer à la position courante (toujours ajouter, même si déjà présent)
            ...
            if not filepath.startswith("https://"):
                ...
                if self.queue_items and filepath in self.main_playlist:
                    ...
                    if sorted_queue_indices:
                        ...
                        if (last_queue_index < len(self.main_playlist) and 
                            self.main_playlist[last_queue_index] == filepath):
                            should_skip = True
                
                if not should_skip:
                    # Insérer à la position de fin de queue (toujours ajouter, même si déjà présent)
