...
class CustomProgressSlider(tk.Frame):
    ...
    def draw_slider(self):
        ...
        try:
            ...
            # Position du thumb basée sur la valeur
    ...
    def on_drag(self, event):
        ...
        if self.dragging:
            value = self.get_value_from_x(event.x)
            # Mettre à jour la valeur sans redessiner
            self.current_value = value
            # Redessiner manuellement pour le drag
            self.draw_slider()
            self.show_time_label(event.x)
            if self.command:
                self.command(value)
    ...
    def set_value(self, value):
        ...
        # Éviter les redraws inutiles et ne pas redessiner pendant le drag
        if not self.dragging and abs(new_value - self.current_value) > 0.1:  # Seuil de 0.1%
            self.current_value = new_value
            self.draw_slider()
        elif self.dragging:
            # Pendant le drag, juste mettre à jour la valeur sans redessiner
            self.current_value = new_value
