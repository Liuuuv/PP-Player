...
class MusicPlayer:
    ...
    def create_ui(self):
        ...
        # volume_frame.grid(row=0, column=7, padx=5)
        ...
        self.colorize_frames(root)

    
    def colorize_frames(self,widget, colors=("red", "green", "blue", "yellow", "cyan", "magenta")):
        color_index = 0
        for child in widget.winfo_children():
            if isinstance(child, tk.Frame):
                child.configure(bg=colors[color_index % len(colors)])
                color_index += 1
            # Récursif pour colorer les sous-frames
            self.colorize_frames(child, colors)
