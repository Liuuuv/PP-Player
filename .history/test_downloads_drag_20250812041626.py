...
class TestDownloadsDrag:
    ...
    def create_download_item(self):
        ...
        def on_left_button_press(event):
            print(f"CLIC DÉTECTÉ sur {event.widget.__class__.__name__}")
        ...
        def on_right_button_press(event):
            print(f"CLIC DROIT DÉTECTÉ sur {event.widget.__class__.__name__}")
            self.drag_drop_handler.setup_drag_start(event, item_frame)
            widget_name = event.widget.__class__.__name__
            widget_text = getattr(event.widget, 'text', 'N/A')[:20]
        ...
        def on_motion_debug(event):
            if hasattr(item_frame, 'drag_enabled') and item_frame.drag_enabled:
                print(f"MOTION avec drag_enabled=True depuis {event.widget.__class__.__name__}")
            else:
                print(f"MOTION avec drag_enabled=False depuis {event.widget.__class__.__name__}")
        
        # Configuration du drag-and-drop AVANT les bindings
        ...
        for widget in widgets_to_bind:
            print(f"Configurant bindings pour {widget.__class__.__name__}")
            ...
            # Ajouter debug pour motion
            widget.bind("<B1-Motion>", on_motion_debug)
