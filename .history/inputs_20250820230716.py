...
class ImportDialog:
    ...
    def setup_drag_drop(self):
        ...
        # Essayer d'implémenter le drag & drop avec tkinterdnd2
        try:
            from tkinterdnd2 import DND_FILES
            self.dialog.drop_target_register(DND_FILES)
            self.dialog.dnd_bind('<<Drop>>', self.on_drop)
            
            # Ajouter un label d'instruction pour le drag & drop
            self.drag_label = tk.Label(
                self.dialog,
                text="💡 Vous pouvez aussi glisser-déposer un fichier HTML ici",
                bg='#2d2d2d',
                fg='#888888',
                font=("Arial", 8)
            )
            ...
            # Si tkinterdnd2 n'est pas disponible, afficher seulement le bouton
            pass
