...
class MusicPlayer:
    ...
    def on_artist_tab_changed(self, event):
        ...
        try:
            ...
            if current_tab_id:
                ...
                # Afficher/masquer les boutons selon l'onglet actuel
                self._update_back_buttons_visibility(current_tab_text)
                
        except Exception as e:
            print(f"Erreur dans on_artist_tab_changed: {e}")
    
    def _update_back_buttons_visibility(self, current_tab_text):
        """Met à jour la visibilité des boutons retour selon l'onglet actuel"""
        try:
            # Masquer tous les boutons d'abord
            if hasattr(self, 'playlist_back_btn_sorties') and self.playlist_back_btn_sorties is not None:
                try:
                    if self.playlist_back_btn_sorties.winfo_exists():
                        self.playlist_back_btn_sorties.place_forget()
                except:
                    pass
            
            if hasattr(self, 'playlist_back_btn_playlists') and self.playlist_back_btn_playlists is not None:
                try:
                    if self.playlist_back_btn_playlists.winfo_exists():
                        self.playlist_back_btn_playlists.place_forget()
                except:
                    pass
            
            # Afficher le bouton approprié selon l'onglet et l'état
            if current_tab_text == "Sorties" and self.playlist_content_active_sorties:
                self._show_playlist_back_button_for_tab("sorties")
            elif current_tab_text == "Playlists" and self.playlist_content_active_playlists:
    ...
    def _show_artist_content(self, artist_name, video_data):
        ...
        try:
            ...
            print("DEBUG: Avant _save_current_search_state")
            self._save_current_search_state()
            print("DEBUG: Après _save_current_search_state")
            
            # S'assurer que la zone YouTube est visible
            print("DEBUG: Avant _show_search_results")
            self._show_search_results()
            print("DEBUG: Après _show_search_results")
            
            # Créer les onglets IMMÉDIATEMENT (UI d'abord, contenu après)
            print("DEBUG: Avant _create_artist_tabs")
            self._create_artist_tabs()
            print("DEBUG: Après _create_artist_tabs")
            ...
            self.safe_after(50, lambda: self._search_artist_content_async())
    ...
    def _create_artist_tabs(self):
        ...
        print("DEBUG: Début _create_artist_tabs")
        ...
        print("DEBUG: Nettoyage youtube_results_frame")
        # Nettoyer tout le contenu de youtube_results_frame
        for widget in self.youtube_results_frame.winfo_children():
            try:
                widget.destroy()
            except Exception as e:
                print(f"DEBUG: Erreur destruction widget: {e}")
        
        print("DEBUG: Vérification youtube_canvas")
        # Cacher le canvas et la scrollbar des résultats
        if hasattr(self, 'youtube_canvas'):
            print(f"DEBUG: youtube_canvas existe, valeur: {self.youtube_canvas}")
            if self.youtube_canvas is not None:
                try:
                    if self.youtube_canvas.winfo_exists():
                        self.youtube_canvas.pack_forget()
                        print("DEBUG: youtube_canvas masqué")
                except Exception as e:
                    print(f"DEBUG: Erreur masquage youtube_canvas: {e}")
        
        print("DEBUG: Vérification scrollbar")
        if hasattr(self, 'scrollbar'):
            print(f"DEBUG: scrollbar existe, valeur: {self.scrollbar}")
            if self.scrollbar is not None:
                try:
                    if self.scrollbar.winfo_exists():
                        self.scrollbar.pack_forget()
                        print("DEBUG: scrollbar masqué")
                except Exception as e:
                    print(f"DEBUG: Erreur masquage scrollbar: {e}")
        
        print("DEBUG: Vérification thumbnail_frame")
        # Cacher aussi la frame thumbnail si elle est visible
        if hasattr(self, 'thumbnail_frame'):
            print(f"DEBUG: thumbnail_frame existe, valeur: {self.thumbnail_frame}")
            if self.thumbnail_frame is not None:
                try:
                    if self.thumbnail_frame.winfo_exists():
                        self.thumbnail_frame.pack_forget()
                        print("DEBUG: thumbnail_frame masqué")
                except Exception as e:
    ...
    def _search_artist_content_async(self):
        ...
        def find_channel_id_bg():
            try:
                ...
                if channel_id:
                    self.root.after(0, lambda: self._on_channel_id_found(channel_id))
                else:
                    self.root.after(0, lambda: self._on_channel_id_error())
                    
            except Exception as e:
                self.root.after(0, lambda: self._on_channel_id_error(str(e)))
    ...
    def _on_channel_id_found(self, channel_id):
        ...
        self.safe_after(100, lambda: self._start_parallel_searches())
    ...
    def _on_channel_id_error(self, error_msg="Impossible de trouver l'ID de la chaîne"):
        ...
        self._show_error_in_tabs(error_msg)
    ...
    def _display_results_in_batches(self, items, container, item_type, batch_size=3):
        ...
        def display_batch(batch_index):
            ...
            for i, item in enumerate(batch):
                ...
                try:
                    if item_type == "videos":
                        self._add_artist_result(item, index, container)
                    elif item_type == "releases":
