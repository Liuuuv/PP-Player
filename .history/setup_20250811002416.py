...
def load_icons(self):
    ...
    for key, filename in icon_names.items():
        try:
            ...
            # Créer une version plus petite pour l'icône clear
            if key == "clear":
                image_small = Image.open(path).resize((20, 20), Image.Resampling.LANCZOS)
                self.icons[key + "_small"] = ImageTk.PhotoImage(image_small)
            
            # Créer une version plus petite pour l'icône find
            if key == "find":
                image_small = Image.open(path).resize((20, 20), Image.Resampling.LANCZOS)
                self.icons[key + "_small"] = ImageTk.PhotoImage(image_small)
        except Exception as e:
...
def setup_controls(self):
        ...
        self.selection_label.pack(pady=(0, 2))
        ...
        buttons_volume_frame.pack(fill=tk.X, pady=10)
...
def setup_search_tab(self):
    ...
    # Bouton find pour scroller vers la musique en cours
    ...
    find_current_btn.pack(side=tk.RIGHT, padx=(0, 5))
    ...
    clear_playlist_btn.pack(side=tk.RIGHT)
