...
class MusicPlayer:
    ...
    def on_root_click(self, event):
        """Gère les clics sur la fenêtre principale pour retirer le focus des champs de saisie et boutons"""
        # Obtenir le widget qui a été cliqué
        clicked_widget = event.widget
        
        # Si on clique sur un champ de saisie ou un bouton, ne rien faire (laisser le focus)
        if isinstance(clicked_widget, (tk.Entry, tk.Text, tk.Button, ttk.Button)):
            return
        
        # Vérifier si on clique sur un parent d'un champ de saisie ou bouton
        parent = clicked_widget
        while parent:
            if isinstance(parent, (tk.Entry, tk.Text, tk.Button, ttk.Button)):
                return
            try:
        ...
        # Si on arrive ici, on n'a pas cliqué sur un champ de saisie ou bouton
        # Retirer le focus de tous les champs de saisie et boutons
        focused_widget = self.root.focus_get()
        if focused_widget and isinstance(focused_widget, (tk.Entry, tk.Text, tk.Button, ttk.Button)):
            self.root.focus_set()  # Donner le focus à la fenêtre principale
    
    def setup_focus_bindings(self):
        """Configure les bindings pour retirer le focus des champs de saisie et boutons"""
        def remove_focus(event):
            """Retire le focus des champs de saisie et boutons"""
            focused_widget = self.root.focus_get()
            if focused_widget and isinstance(focused_widget, (tk.Entry, tk.Text, tk.Button, ttk.Button)):
                self.root.focus_set()
