...
class ArtistTabManager:
    ...
    def __init__(self, music_player):
        ...
        # Démarrer le préchargement des recherches populaires
        self.preload_popular_searches()
    ...
    def preload_popular_searches(self):
        """Précharge les recherches populaires en arrière-plan"""
        # Liste des recherches populaires à précharger
        popular_searches = [
            "music", "song", "official", "remix", "cover", "live", "acoustic",
            "piano", "guitar", "jazz", "rock", "pop", "electronic", "classical"
        ]
        
        def preload_search(query):
            """Précharge une recherche spécifique"""
            try:
                if hasattr(self.player, 'youtube_entry'):
                    # Sauvegarder la recherche actuelle
                    current_query = self.player.youtube_entry.get()
                    
                    # Vérifier si déjà en cache
                    cache_key = f"search_{query}"
                    if not self.cache_manager.get_search_results(cache_key):
                        print(f"DEBUG: Préchargement de '{query}'...")
                        # Simuler une recherche rapide en arrière-plan
                ...
                print(f"Erreur préchargement '{query}': {e}")
        
        # Précharger quelques recherches populaires après 30 secondes
        def start_preloading():
            for i, query in enumerate(popular_searches[:5]):  # Limiter à 5 pour ne pas surcharger
                # Étaler les préchargements sur plusieurs minutes
                delay = 30000 + (i * 60000)  # 30s, 1m30, 2m30, etc.
                self.player.safe_after(delay, lambda q=query: preload_search(q))
        
        # Démarrer le préchargement après 30 secondes
        self.player.safe_after(30000, start_preloading)
