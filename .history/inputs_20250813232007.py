...
def on_global_volume_up(self, event):
    ...
    self.status_bar.config(text=f"Volume augmenté: {int(new_volume)}%")
    # Effacer le message après 2 secondes
    self.root.after(2000, lambda: self.status_bar.config(text=""))
    return "break"
...
def on_global_volume_down(self, event):
    ...
    self.volume_slider.set(new_volume)
    self.status_bar.config(text=f"Volume diminué: {int(new_volume)}%")
    # Effacer le message après 2 secondes
    self.root.after(2000, lambda: self.status_bar.config(text=""))
    return "break"
