...
class MusicPlayer:
    ...
    def create_ui(self):
        ...
        style = ttk.Style()
        style.configure('Downloading.TFrame', background='#ff4444')  # Rouge pour indiquer le téléchargement
        style.map('Downloading.TFrame',
                background=[('active', '#ff6666')])  # Légère variation au survol
        ...
        ttk.Button(
            button_frame, image=self.icons["hey"]
    ...
    def _add_playlist_item(self, filepath, thumbnail_path=None, is_downloading=False):
        """Ajoute un élément à la playlist avec gestion de l'état de téléchargement"""
        try:
            # Variables locales
            ...
            item_frame = ttk.Frame(
                ...
                style='Downloading.TFrame' if is_downloading else 'TFrame',
                padding=(5, 2)
            )
            item_frame.pack(fill='x', pady=1)
            
            # Variables pour les composants enfants
            thumbnail_label = tk.Label(
                item_frame,
                bg='#ff4444' if is_downloading else '#3d3d3d',
                width=80,
                height=45
            )
            ...
            if not is_downloading:
                # Bindings normaux si pas en téléchargement
                ...
                def select_item(f):
                    ...
                    self._play_playlist_item(f)
                
                item_frame.bind("<Double-1>", lambda e, f=filepath: select_item(f))
                thumbnail_label.bind("<Double-1>", lambda e, f=filepath: select_item(f))
                title_label.bind("<Double-1>", lambda e, f=filepath: select_item(f))
            else:
                # Désactiver les interactions si en téléchargement
                delete_btn.config(state=tk.DISABLED)
                item_frame.bind("<Button-1>", lambda e: "break")
                thumbnail_label.bind("<Button-1>", lambda e: "break")
                title_label.bind("<Button-1>", lambda e: "break")

            # Stocker les références
            item_frame.filepath = filepath
            item_frame.downloading = is_downloading
    ...
    def _download_youtube_thread(self):
        try:
            ...
            ydl_opts = {
                ...
                'quiet': True,
                'no_warnings': True,
                'progress_hooks': [self._download_progress_hook],  # Ajout du hook de progression
                'nopart': True,  # Don't use .part files
                'continue_dl': False,  # Don't try to continue
            }
            ...
            self.root.after(0, lambda: self.status_bar.config(text=f"Erreur: {str(e)}"))
    ...
    def _add_downloaded_file(self, filepath, thumbnail_path, title):
        """Ajoute le fichier téléchargé et met à jour son apparence"""
        # Variables locales
        self.playlist.append(filepath)
        
        # D'abord ajouter en rouge (état téléchargement)
        self._add_playlist_item(filepath, thumbnail_path, is_downloading=True)
        
        # Après un court délai, mettre à jour en version normale
        def update_item_state():
            for child in self.playlist_container.winfo_children():
                if hasattr(child, 'filepath') and child.filepath == filepath:
                    ...
                    for widget in [child.thumbnail_label, child.title_label]:
                        widget.config(bg='#3d3d3d')
                    
                    # Réactiver les interactions
                    child.delete_btn.config(state=tk.NORMAL)
                    child.unbind("<Button-1>")
                    child.thumbnail_label.unbind("<Button-1>")
                    child.title_label.unbind("<Button-1>")
                    
                    # Recréer les bindings normaux
                    child.bind("<Double-1>", lambda e, f=filepath: self._play_playlist_item(f))
                    child.thumbnail_label.bind("<Double-1>", lambda e, f=filepath: self._play_playlist_item(f))
                    child.title_label.bind("<Double-1>", lambda e, f=filepath: self._play_playlist_item(f))
                    
                    break
        
        self.root.after(100, update_item_state)
        self.status_bar.config(text=f"{title} ajouté à la playlist")
