            ...
            try:
                ...
                if searches_to_preload:
                    ...
                    for query in searches_to_preload:
                        print(f"  - Préchargement: '{query}' (basé sur votre historique)")
                        # Programmer le préchargement avec délai pour éviter la surcharge
                        delay = searches_to_preload.index(query) * 30000  # 30s entre chaque
                        self.player.safe_after(delay, lambda q=query: self._preload_personal_search(q))
                
                # Reprogrammer la vérification dans 2 heures
                self.player.safe_after(7200000, check_and_preload)  # 2 heures
