...
def show_selection_menu(self, event):
    ...
    try:
        if not (hasattr(self, 'root') and self.root.winfo_exists()):
            ...
            return
        # Vérifier qu'au moins un des containers existe (downloads ou playlist)
        has_downloads_container = hasattr(self, 'downloads_container') and self.downloads_container.winfo_exists()
        has_playlist_container = hasattr(self, 'playlist_content_container') and self.playlist_content_container.winfo_exists()
        
        if not (has_downloads_container or has_playlist_container):
            print("DEBUG: Aucun container valide trouvé (downloads_container ou playlist_content_container)")
            return
