...
class MusicPlayer:
    ...
    def setup_media_keys(self):
        """Configure les touches multimédia globales"""
            ...
            try:
                ...
                if key in [keyboard.Key.media_play_pause, keyboard.Key.media_next, keyboard.Key.media_previous]:
                    ...
                    if key == keyboard.Key.media_play_pause:
                        ...
                        print("Touche Play/Pause interceptée par le lecteur")
                    elif key == keyboard.Key.media_next:
                        self.root.after(0, self.next_track)
                        print("Touche Next interceptée par le lecteur")
                    elif key == keyboard.Key.media_previous:
                        self.root.after(0, self.prev_track)
                        print("Touche Previous interceptée par le lecteur")
        ...
        def selective_suppress(key):
            """Fonction pour supprimer sélectivement les touches multimédia"""
            # Supprimer seulement les touches multimédia, laisser passer le reste
            if key in [keyboard.Key.media_play_pause, keyboard.Key.media_next, keyboard.Key.media_previous]:
                return on_media_key_press(key)
            return True  # Laisser passer toutes les autres touches
        
        # Démarrer le listener des touches multimédia
        try:
            # Première tentative : avec suppression sélective des événements multimédia
            self.media_key_listener = keyboard.Listener(
                on_press=selective_suppress,
                suppress=True  # Supprimer seulement les touches que nous interceptons
            )
            self.media_key_listener.daemon = True  # Thread daemon pour qu'il se ferme avec l'application
            self.media_key_listener.start()
            print("Touches multimédia activées avec suppression sélective (priorité maximale)")
        except Exception as e:
            print(f"Impossible d'activer les touches multimédia avec suppression: {e}")
            # Essayer une version de fallback sans suppression
            try:
                self.media_key_listener = keyboard.Listener(
                ...
                self.media_key_listener.start()
                print("Touches multimédia activées en mode fallback (sans suppression)")
            except Exception as e2:
                print(f"Impossible d'activer les touches multimédia même en fallback: {e2}")
    ...
    def on_closing(self):
        ...
        # Arrêter le listener des touches multimédia
        if self.media_key_listener:
            try:
                self.media_key_listener.stop()
            except Exception as e:
                print(f"Erreur lors de l'arrêt du listener multimédia: {e}")
    ...
    def toggle_random_mode(self):
        ...
        if self.random_mode:
            self.random_button.config(image=self.icons["random_on"])
        else:
            self.random_button.config(image=self.icons["random_off"])

    def toggle_loop_mode(self):
        self.loop_mode = (self.loop_mode + 1) % 3
        if self.loop_mode == 0:
            self.loop_button.config(image=self.icons["loop_off"])
        elif self.loop_mode == 1:
            self.loop_button.config(image=self.icons["loop_on"])
        else:  # self.loop_mode == 2
            self.loop_button.config(image=self.icons["loop_one"])
