...
from winsdk.windows.media import SystemMediaTransportControls, MediaPlaybackType, SystemMediaTransportControlsButton
...
class MusicPlayer:
    def __init__(self, root):
        ...
        # Listener pour les touches multimédia
        self.media_key_listener = None
        
        # Contrôles multimédia Windows
        self.smtc = None
    ...
    def setup_keyboard_bindings(self):
        ...
        # Configuration des touches multimédia Windows
        self.setup_windows_media_controls()
    ...
    def setup_windows_media_controls(self):
        """Configure les contrôles multimédia Windows (SMTC)"""
        try:
            # Obtenir les contrôles multimédia système
            self.smtc = SystemMediaTransportControls.get_for_current_view()
            
            # Activer les boutons que nous voulons utiliser
            ...
            self.smtc.add_button_pressed(self.on_media_button_pressed)
            
            # Définir le type de média
            self.smtc.playback_status = SystemMediaTransportControls.PlaybackStatus.STOPPED
            
            print("Contrôles multimédia Windows activés (SMTC)")
            self.media_session_active = True
            
        except Exception as e:
            print(f"Impossible d'activer les contrôles multimédia Windows: {e}")
            # Fallback vers l'ancienne méthode
            self.setup_media_keys_fallback()
    
    def on_media_button_pressed(self, sender, args):
        """Gestionnaire pour les boutons multimédia Windows"""
        try:
            button = args.button
            
            if button == SystemMediaTransportControlsButton.PLAY:
                self.root.after(0, self.play_pause)
                print("Bouton Play Windows intercepté")
            elif button == SystemMediaTransportControlsButton.PAUSE:
                self.root.after(0, self.play_pause)
                print("Bouton Pause Windows intercepté")
            elif button == SystemMediaTransportControlsButton.NEXT:
                self.root.after(0, self.next_track)
                print("Bouton Next Windows intercepté")
            elif button == SystemMediaTransportControlsButton.PREVIOUS:
                self.root.after(0, self.prev_track)
                print("Bouton Previous Windows intercepté")
                
        except Exception as e:
            print(f"Erreur lors du traitement du bouton multimédia Windows: {e}")
    
    def setup_media_keys_fallback(self):
        """Méthode de fallback avec pynput si SMTC ne fonctionne pas"""
        def on_media_key_press(key):
            """Gère l'appui sur les touches multimédia"""
            try:
                # Vérifier si c'est une touche multimédia que nous voulons intercepter
                if key in [keyboard.Key.media_play_pause, keyboard.Key.media_next, keyboard.Key.media_previous]:
                    # Traiter la touche selon son type
                    if key == keyboard.Key.media_play_pause:
                        # Utiliser after pour exécuter dans le thread principal de tkinter
                        self.root.after(0, self.play_pause)
                        print("Touche Play/Pause interceptée par le lecteur (fallback)")
                    elif key == keyboard.Key.media_next:
                        self.root.after(0, self.next_track)
                        print("Touche Next interceptée par le lecteur (fallback)")
                    elif key == keyboard.Key.media_previous:
                        self.root.after(0, self.prev_track)
                        print("Touche Previous interceptée par le lecteur (fallback)")
                    
                    # Supprimer seulement les touches multimédia
                    return False
                    
            except Exception as e:
                print(f"Erreur lors du traitement de la touche multimédia: {e}")
            
            # Pour toutes les autres touches, laisser passer l'événement
            return True
        
        # Démarrer le listener des touches multimédia
        try:
            self.media_key_listener = keyboard.Listener(
                on_press=on_media_key_press,
                suppress=False
            )
            self.media_key_listener.daemon = True
            self.media_key_listener.start()
            print("Touches multimédia activées en mode fallback (pynput)")
        except Exception as e:
            print(f"Impossible d'activer les touches multimédia même en fallback: {e}")
    
    def update_media_session(self, title="", artist="", status="stopped"):
        """Met à jour la session multimédia Windows"""
        if not self.media_session_active or not self.smtc:
            return
            
        try:
            # Mettre à jour le statut de lecture
            if status == "playing":
                self.smtc.playback_status = SystemMediaTransportControls.PlaybackStatus.PLAYING
            elif status == "paused":
                self.smtc.playback_status = SystemMediaTransportControls.PlaybackStatus.PAUSED
            else:
                self.smtc.playback_status = SystemMediaTransportControls.PlaybackStatus.STOPPED
            ...
            print(f"Erreur lors de la mise à jour de la session multimédia: {e}")
    ...
    def play_track(self):
        try:
            ...
            # Mettre à jour la session multimédia Windows avec les informations de la chanson
            song_name = os.path.splitext(os.path.basename(song))[0]
            self.update_media_session(title=song_name, artist="", status="playing")
    ...
    def play_pause(self):
        ...
        if pygame.mixer.music.get_busy() and not self.paused:
            ...
            # Mettre à jour la session multimédia Windows
            self.update_media_session(status="paused")
        elif self.paused:
            ...
            self.play_button.config(text="Pause")
            # Mettre à jour la session multimédia Windows
            self.update_media_session(status="playing")
