...
class DragDropHandler:
    ...
    def _on_drag_release(self, event, frame):
        ...
        if hasattr(frame, 'is_dragging') and frame.is_dragging:
            ...
            if dx > 100:
                # Ajouter à la queue (file d'attente) selon le type d'élément
                self._add_dragged_item_to_queue(frame)
            # Si on a dragué suffisamment vers la gauche (moins de -100 pixels)
            elif dx < -100:
                # Lire après la chanson actuelle selon le type d'élément
                self._play_after_current_dragged_item(frame)
    ...
    def _update_drag_visual(self, frame, dx, dy):
        ...
        if dx > 100:  # Drag vers la droite - ajouter à la queue
            self._set_frame_colors(frame, '#4a6a4a')  # Vert pour indiquer l'ajout à la queue
        elif dx < -100:  # Drag vers la gauche - lire après la chanson actuelle
            self._set_frame_colors(frame, '#6a4a6a')  # Violet pour indiquer "play after current"
        else:
    ...
    def _add_dragged_item_to_queue(self, frame):
        """Ajoute l'élément dragué à la queue selon son type"""
        try:
            if frame.drag_item_type == "file":
                # Fichier local
                self._add_file_to_queue(frame.drag_file_path)
            elif frame.drag_item_type == "youtube":
                # Résultat YouTube
                self._add_youtube_to_queue(frame.drag_video_data, frame)
            elif frame.drag_item_type == "playlist_item":
                # Élément de playlist (copier vers la queue)
                self._add_playlist_item_to_queue(frame.drag_file_path)
    ...
    def _add_file_to_queue(self, file_path):
        """Ajoute un fichier local à la queue (sera joué après la chanson actuelle)"""
        if not file_path:
            return
            
        filename = file_path.split('/')[-1] if '/' in file_path else file_path.split('\\')[-1]
        
        # Vérifier si la musique est déjà la dernière de la queue
        if (hasattr(self.music_player, 'play_queue') and 
            self.music_player.play_queue and 
            self.music_player.play_queue[-1] == file_path):
            self.music_player.status_bar.config(text=f"Déjà en dernière position de la queue: {filename}")
            return
        
        # Ajouter à la main playlist si pas déjà présent (nécessaire pour que next_track() puisse la trouver)
        if file_path not in self.music_player.main_playlist:
            self.music_player.main_playlist.append(file_path)
            # Ajouter l'élément visuel à la playlist
            if hasattr(self.music_player, '_add_main_playlist_item'):
                self.music_player._add_main_playlist_item(file_path)
        
        # Ajouter à la queue
        if not hasattr(self.music_player, 'play_queue'):
            self.music_player.play_queue = []
        
        # Supprimer de la queue si déjà présent (pour le replacer à la fin)
        if file_path in self.music_player.play_queue:
            self.music_player.play_queue.remove(file_path)
        
        # Ajouter à la fin de la queue
        self.music_player.play_queue.append(file_path)
        
        # Message de statut
        queue_position = len(self.music_player.play_queue)
        self.music_player.status_bar.config(text=f"Ajouté à la queue (position {queue_position}): {filename}")
        
        # NE PAS désactiver main_playlist_from_playlist pour préserver les numéros!
    
    def _add_youtube_to_queue(self, video_data, frame):
        """Ajoute une vidéo YouTube à la queue (télécharge si nécessaire)"""
        if not video_data:
            return
        ...
        if existing_file:
            # Le fichier existe déjà, l'ajouter directement à la queue
            self._add_file_to_queue(existing_file)
            return
        
        # Le fichier n'existe pas, procéder au téléchargement
        # (Logique similaire aux autres fonctions de téléchargement)
        ...
        # Ajouter à la liste des téléchargements pour la queue
        ...
        # Lancer le téléchargement (à implémenter)
        self.music_player.status_bar.config(text=f"Téléchargement en cours pour la queue: {title}")
        
    def _add_playlist_item_to_queue(self, file_path):
    ...
    def _add_file_to_playlist(self, file_path):
        ...
        if file_path:
            ...
            # PRÉSERVATION DES NUMÉROS: Ne pas désactiver main_playlist_from_playlist
            # Les numéros des musiques d'origine restent affichés
    ...
    def _add_to_queue(self, file_path):
        ...
        if (len(self.music_player.main_playlist) > 0 and 
            ...
            song_index = self.music_player.main_playlist.index(file_path)
            ...
            while (target_position < len(self.music_player.main_playlist) and 
                   target_position < song_index and
            ...
            # Si la musique n'est pas déjà à la bonne position
            if song_index != target_position:
                # Retirer la musique de sa position actuelle
                self.music_player.main_playlist.pop(song_index)
