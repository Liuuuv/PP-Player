...
def load_icons(self):
    ...
    for key, filename in icon_names.items():
        try:
            ...
            if key in ["output", "stats", "import", "clear_cache", "select_downloads_folder", "errors", "rename", "delete"]:
                image_small = Image.open(path).resize((16, 16), Image.Resampling.LANCZOS)
                self.icons[key + "_small"] = ImageTk.PhotoImage(image_small)
        except Exception as e:
...
def load_playlists(self):
    ...
    try:
        ...
        if os.path.exists(playlists_file):
            ...
            print("Fichier playlists.json non trouvé, création des playlists par défaut")
...
def load_config(self):
    ...
    try:
        ...
        if os.path.exists(self.config_file):
            ...
            # Charger les paramètres de chargement virtuel
            if "virtual_loading" in config:
                virtual_config = config["virtual_loading"]
                self.virtual_initial_load_count = virtual_config.get('initial_load_count', 20)
                self.virtual_scroll_load_count = virtual_config.get('scroll_load_count', 10)
            else:
                # Valeurs par défaut si pas de configuration
                self.virtual_initial_load_count = 20
                self.virtual_scroll_load_count = 10
...
def setup_search_tab(self):
    ...
    self._bind_mousewheel(self.playlist_canvas, self.playlist_canvas, func=search_tab.main_playlist.forward_wheel)
    self._bind_mousewheel(self.playlist_container, self.playlist_canvas, func=search_tab.main_playlist.forward_wheel)
