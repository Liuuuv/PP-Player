...
class ArtistTabManager:
    ...
    def __init__(self, music_player):
        ...
        # Démarrer le préchargement intelligent basé sur l'historique personnel
        self.start_intelligent_preloading()
    ...
    def show_cache_stats(self):
        ...
        if hasattr(self, 'cache_manager'):
            ...
            print(f"Recherches en cache: {stats['searches']}")
            print(f"États d'interface: {stats['interface_states']}")
            print("=============================")
            return stats
        return None
    ...
    def _schedule_cache_cleanup(self):
        ...
        def cleanup():
            try:
                ...
                self.cache_manager.clear_expired_searches()
                # Sauvegarder le cache périodiquement
                self.cache_manager.save_cache_to_disk()
                # Reprogrammer le nettoyage dans 10 minutes
                self.player.safe_after(600000, cleanup)  # 10 minutes
            except:
                pass
        
        # Premier nettoyage dans 10 minutes
        self.player.safe_after(600000, cleanup)
    
    def start_intelligent_preloading(self):
        """Démarre le préchargement intelligent basé sur l'historique personnel de l'utilisateur"""
        def check_and_preload():
            try:
                # Vérifier si le préchargement est activé
                from search_tab.config import get_preload_config
                if not get_preload_config('enable_intelligent_preload'):
                    return
                
                # Obtenir les recherches personnelles à précharger
                searches_to_preload = self.cache_manager.get_searches_to_preload()
                
                if searches_to_preload:
                    print(f"DEBUG: Préchargement intelligent de {len(searches_to_preload)} recherches personnelles")
                    
                    # Utiliser la configuration pour les délais
                    delay_seconds = get_preload_config('preload_delay_seconds') or 30
                    
                    for query in searches_to_preload:
                        print(f"  - {get_preload_config('preload_message') or 'Préchargement'}: '{query}' (basé sur votre historique)")
                        # Programmer le préchargement avec délai configurable
                        delay = searches_to_preload.index(query) * delay_seconds * 1000  # Convertir en ms
                        self.player.safe_after(delay, lambda q=query: self._preload_personal_search(q))
                
                # Reprogrammer la vérification selon la configuration
                check_interval_hours = get_preload_config('preload_check_interval_hours') or 2
                self.player.safe_after(check_interval_hours * 3600000, check_and_preload)
                
            except Exception as e:
                print(f"Erreur préchargement intelligent: {e}")
        
        # Première vérification selon la configuration
        try:
            from search_tab.config import get_preload_config
            start_delay_minutes = get_preload_config('preload_start_delay_minutes') or 5
        except ImportError:
            start_delay_minutes = 5
        
        self.player.safe_after(start_delay_minutes * 60000, check_and_preload)
    
    def _preload_personal_search(self, search_query):
        """Précharge une recherche personnelle spécifique en arrière-plan"""
        try:
            # Vérifier que l'application est toujours active
            if hasattr(self.player, '_app_destroyed') and self.player._app_destroyed:
                return
            
            # Vérifier si pas déjà en cache
            cache_key = f"search_{search_query}"
            if self.cache_manager.get_search_results(cache_key):
                return  # Déjà en cache
            
            # Sauvegarder l'état actuel de l'interface
            current_query = ""
            if hasattr(self.player, 'youtube_entry') and self.player.youtube_entry.winfo_exists():
                current_query = self.player.youtube_entry.get()
            
            # Effectuer la recherche en arrière-plan (simulation)
            print(f"DEBUG: Préchargement en cours pour '{search_query}'...")
            
            # Ici on pourrait implémenter une vraie recherche en arrière-plan
            # Pour l'instant, on simule juste le processus
            
            # Restaurer l'état de l'interface
            if current_query and hasattr(self.player, 'youtube_entry'):
                try:
                    self.player.youtube_entry.delete(0, tk.END)
                    self.player.youtube_entry.insert(0, current_query)
                except:
                    pass
                    
        except Exception as e:
            print(f"Erreur préchargement '{search_query}': {e}")
    ...
    def cleanup_resources(self):
        ...
        try:
            ...
            if hasattr(self.player, 'artist_search_cancelled'):
                self.player.artist_search_cancelled = True
            
            # Fermer les pools de threads avec timeout
            if hasattr(self.player, '_thumbnail_executor'):
                try:
                    self.player._thumbnail_executor.shutdown(wait=True, timeout=1.0)
                except:
                    # Force shutdown si timeout
                    self.player._thumbnail_executor.shutdown(wait=False)
                    
            if hasattr(self.player, '_playlist_executor'):
                try:
                    self.player._playlist_executor.shutdown(wait=True, timeout=1.0)
                except:
                    # Force shutdown si timeout
                    self.player._playlist_executor.shutdown(wait=False)
            
            # Sauvegarder le cache avant de le vider
            if hasattr(self, 'cache_manager'):
                self.cache_manager.save_cache_to_disk()
