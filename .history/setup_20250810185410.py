...
def setup_controls(self):
        ...
        self.selection_label = ttk.Label(
            control_frame, text="", 
            font=('Helvetica', 10), foreground='#cccccc'
        )
        ...
        self.prev_button = ttk.Button(button_frame, image=self.icons["prev"], command=self.prev_track)
        ...
        self.next_button = ttk.Button(button_frame, image=self.icons["next"], command=self.next_track)
...
def setup_search_tab(self):
    ...
    # Frame gauche pour les plateformes
    platform_frame = ttk.Frame(checkbox_frame)
    platform_frame.pack(side=tk.LEFT)
    
    # Case à cocher YouTube
    youtube_checkbox = tk.Checkbutton(
        platform_frame,
        text="YouTube",
        variable=self.youtube_selected,
        command=self.on_youtube_checkbox_change,
        bg='#2d2d2d',
        fg='white',
        selectcolor='#3d3d3d',
    ...
    youtube_checkbox.pack(side=tk.LEFT, padx=(0, 20))
    
    # Case à cocher YouTube Music
    youtube_music_checkbox = tk.Checkbutton(
        platform_frame,
        text="YouTube Music",
        variable=self.youtube_music_selected,
        command=self.on_youtube_music_checkbox_change,
        bg='#2d2d2d',
        fg='white',
        selectcolor='#3d3d3d',
        activebackground='#2d2d2d',
    ...
    youtube_music_checkbox.pack(side=tk.LEFT)
    
    # Frame droite pour les filtres de contenu
    content_filter_frame = ttk.Frame(checkbox_frame)
    content_filter_frame.pack(side=tk.RIGHT)
    
    # Cases à cocher pour les types de contenu
    filter_configs = [
        ("Track", self.filter_track),
        ("Artist", self.filter_artist),
        ("Album", self.filter_album),
        ("Playlist", self.filter_playlist)
    ]
    
    for i, (text, variable) in enumerate(filter_configs):
        checkbox = tk.Checkbutton(
            content_filter_frame,
            text=text,
            variable=variable,
            command=self.on_content_filter_change,
            bg='#2d2d2d',
            fg='white',
            selectcolor='#3d3d3d',
            activebackground='#2d2d2d',
            ...
            relief='flat',
            bd=0,
            font=('Arial', 8)  # Police plus petite pour les filtres
        )
        checkbox.pack(side=tk.LEFT, padx=(10 if i == 0 else 5, 0))
