...
class MusicPlayer:
    ...
    def _enable_scroll_propagation(self, widget):
        """Permet aux événements de scroll de passer à travers ce widget sans être interceptés"""
        def propagate_scroll_event(event):
            # Chercher le canvas scrollable le plus proche dans la hiérarchie
            current_widget = widget
            while current_widget:
                parent = current_widget.master
                if parent and hasattr(parent, 'winfo_children'):
                    # Chercher un canvas dans les enfants du parent
                    for sibling in parent.winfo_children():
                        if hasattr(sibling, 'yview_scroll') and sibling != current_widget:
                            # Trouvé un canvas scrollable, lui envoyer l'événement
                            if event.delta:
                                sibling.yview_scroll(int(-1*(event.delta/120)), "units")
                            else:
                                # Pour Linux
                                if event.num == 4:
                                    sibling.yview_scroll(-1, "units")
                                elif event.num == 5:
                                    sibling.yview_scroll(1, "units")
                            return "break"
                current_widget = parent
            
            # Si aucun canvas trouvé, chercher dans la fenêtre principale
            if hasattr(self, 'notebook'):
                current_tab = self.notebook.select()
                if current_tab:
                    tab_widget = self.notebook.nametowidget(current_tab)
                    # Chercher récursivement un canvas dans l'onglet actuel
                    canvas = self._find_scrollable_canvas(tab_widget)
                    if canvas:
                        if event.delta:
        ...
        # Bind les événements de scroll sur le widget
        widget.bind("<MouseWheel>", propagate_scroll_event)
        widget.bind("<Button-4>", propagate_scroll_event)  # Linux
        widget.bind("<Button-5>", propagate_scroll_event)  # Linux
        
        # Appliquer récursivement à tous les enfants
        for child in widget.winfo_children():
            try:
                self._enable_scroll_propagation(child)
            except:
                pass  # Ignorer les erreurs pour les widgets qui ne supportent pas les bindings
    
    def _find_scrollable_canvas(self, widget):
        """Trouve récursivement un canvas scrollable dans un widget"""
        if hasattr(widget, 'yview_scroll'):
            return widget
        
        if hasattr(widget, 'winfo_children'):
            for child in widget.winfo_children():
                canvas = self._find_scrollable_canvas(child)
                if canvas:
                    return canvas
