...
def on_progress_press(self, event):
    ...
    # Show waveform only if the show_waveform button is activated
    if self.show_waveform_current and not self.waveform_canvas.winfo_height():
        self.waveform_canvas.config(height=80)
        self.waveform_canvas.pack(fill=tk.X, pady=(0, 10))
    ...
    elif self.show_waveform_current:
...
def on_progress_drag(self, event):
    ...
    if self.user_dragging:
        ...
        # Only draw waveform if the show_waveform button is activated
        if self.show_waveform_current:
            self.draw_waveform_around(pos)
...
def on_progress_release(self, event):
    ...
    try:
        ...
        # Update waveform only if the show_waveform button is activated
        if self.show_waveform_current:
            self.draw_waveform_around(self.current_time)
