...
def _on_mousewheel(self, event, canvas):
        ...
        # Vérifier le scroll infini pour la playlist SEULEMENT si activé
        if hasattr(self, 'playlist_canvas') and canvas == self.playlist_canvas:
            print(f"🖱️ DEBUG: Scroll détecté sur playlist_canvas")
            print(f"🖱️ DEBUG: Event delta: {getattr(event, 'delta', 'N/A')}, num: {getattr(event, 'num', 'N/A')}")
            
            # CORRECTION CRITIQUE: Vérifier si le scroll infini est activé
            try:
                from search_tab.config import get_main_playlist_config, USE_NEW_CONFIG
                scroll_infini_active = USE_NEW_CONFIG and get_main_playlist_config('enable_infinite_scroll')
                print(f"🔧 DEBUG: Scroll infini activé: {scroll_infini_active}")
            except:
                scroll_infini_active = False
                print("⚠️ DEBUG: Impossible de vérifier le statut du scroll infini")
            
            # SEULEMENT appeler nos fonctions si le scroll infini est activé
            if scroll_infini_active:
                print("✅ DEBUG: Scroll infini activé, appel des fonctions personnalisées")
                
                if hasattr(self, '_update_display_based_on_scroll_position'):
                    print("🔄 DEBUG: Appel de _update_display_based_on_scroll_position depuis _on_mousewheel")
                    ...
                    print("❌ DEBUG: _update_display_based_on_scroll_position non trouvée")
                
                if hasattr(self, '_check_infinite_scroll'):
                    print("🔄 DEBUG: Appel de _check_infinite_scroll depuis _on_mousewheel")
                    # Différer légèrement la vérification pour laisser le scroll se terminer
                    self.root.after(50, self._check_infinite_scroll)
                else:
                    print("❌ DEBUG: _check_infinite_scroll non trouvée")
            else:
                print("⏸️ DEBUG: Scroll infini désactivé, utilisation du scroll normal de Tkinter")
        else:
            if hasattr(self, 'playlist_canvas'):
                print(f"🖱️ DEBUG: Scroll sur autre canvas: {type(canvas)} (playlist_canvas: {type(self.playlist_canvas)})")
            else:
                print("🖱️ DEBUG: Pas de playlist_canvas défini")
