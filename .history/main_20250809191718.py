...
class MusicPlayer:
    ...
    def _on_library_search_change(self, event):
        """Appelée à chaque changement dans la barre de recherche"""
        ...
        if not search_term:
            ...
            # Normaliser les espaces dans le terme de recherche
            normalized_search_term = ' '.join(search_term.split())
            
            # Filtrer les fichiers selon le terme de recherche
            filtered_files = []
            for filepath in self.all_downloaded_files:
                filename = os.path.basename(filepath).lower()
                # Normaliser les espaces dans le nom de fichier aussi
                normalized_filename = ' '.join(filename.split())
    ...
    def _add_download_item(self, filepath):
        ...
        try:
            ...
            item_frame = tk.Frame(
                ...
                bg='#4a4a4a',  # Fond gris uniforme comme les résultats YouTube
                relief='flat',
                bd=1,
            ...
            thumbnail_label = tk.Label(
                item_frame,
                bg='#4a4a4a',  # Même fond que le frame parent
                width=10,
                height=3,
                anchor='center'
            )
