...
class MusicPlayer:
    ...
    def _add_main_playlist_item(self, filepath, thumbnail_path=None, song_index=None):
        ...
        return search_tab.main_playlist.add_item(self, filepath, thumbnail_path, song_index)
    ...
    def select_playlist_item(self, item_frame=None, index=None, auto_scroll=True):
        ...
        """
        return search_tab.main_playlist.select_item(self, item_frame, index, auto_scroll)
    ...
    def download_and_create_playlist_from_selection(self):
        ...
        tools.download_and_create_playlist_from_selection(self)
    ...
    def select_playlist_content_item(self, current_filepath):
        ...
        return tools.select_playlist_content_item(self, current_filepath)

    
    def _remove_main_playlist_item(self, filepath, frame, event=None, song_index=None):
        """Supprime un élément de la main playlist"""
        return search_tab.main_playlist.remove_item(self, filepath, frame, event=event, song_index=song_index)
    ...
    def _add_search_result(self, video, index):
        ...
        try:
            ...
            if not is_channel and (artist or album):  # Ajouter binding pour metadata_frame s'il existe
            ...
            if not is_channel and (artist or album):
    ...
    def _add_youtube_to_playlist(self, video, frame, playlist_name):
        ...
        tools._add_youtube_to_playlist(self, video, frame, playlist_name)
    ...
    def _create_new_playlist_dialog_youtube(self, video, frame):
        ...
        tools._create_new_playlist_dialog_youtube(self, video, frame)
    ...
    def _download_and_add_to_playlist_thread(self, video, frame, playlist_name):
        ...
        services.downloading._download_and_add_to_playlist_thread(self, video, frame, playlist_name)
    ...
    def _add_downloaded_to_playlist(self, filepath, thumbnail_path, title, playlist_name, url=None):
        ...
        tools._add_downloaded_file_to_playlist(filepath, thumbnail_path, title, playlist_name, url)
    ...
    def _download_and_add_after_current(self, video, frame):
        ...
        services.downloading._download_and_add_after_current(self, video, frame)
    ...
    def _download_youtube_thumbnail(self, video_info, filepath):
        ...
        services.downloading._download_youtube_thumbnail(self, video_info, filepath)
    ...
    def _show_artist_content(self, artist_name, video_data):
        """Affiche le contenu d'un artiste avec des sous-onglets Vidéos et Sorties"""
        try:
            ...
            if not self.current_artist_channel_url:
                # Fallback: construire l'URL de recherche
                self.current_artist_channel_url = f"https://www.youtube.com/@{artist_name}"
            
            # Sauvegarder le contenu original si pas déjà fait
            if self.original_search_content is None:
                self._save_original_search_content()
            
            # Créer l'interface artiste
            self._create_artist_interface()
    ...
    def _save_original_search_content(self):
        """Sauvegarde le contenu original de l'onglet recherche"""
        self.original_search_content = {
            'children': list(self.search_tab.winfo_children())
        }
        
        # Cacher tous les widgets actuels
        for child in self.search_tab.winfo_children():
            child.pack_forget()
    
    def _create_artist_interface(self):
        """Crée l'interface avec les sous-onglets pour l'artiste"""
        # Frame principale pour le mode artiste
        artist_main_frame = tk.Frame(self.search_tab, bg='#2d2d2d')
        artist_main_frame.pack(fill=tk.BOTH, expand=True)
        
        # Header avec le nom de l'artiste et bouton retour
        header_frame = tk.Frame(artist_main_frame, bg='#2d2d2d', height=40)
        ...
        back_btn.pack(side=tk.LEFT, padx=(10, 20))
        
        # Titre de l'artiste
        artist_title = tk.Label(
            header_frame,
            text=f"Contenu de : {self.current_artist_name}",
            bg='#2d2d2d',
            fg='white',
            font=('TkDefaultFont', 14, 'bold')
        )
        artist_title.pack(side=tk.LEFT, expand=True)
        
        # Créer le notebook pour les sous-onglets
        self.artist_notebook = ttk.Notebook(artist_main_frame, takefocus=0)
        self.artist_notebook.pack(fill=tk.BOTH, expand=True, padx=10, pady=(0, 10))
        
        # Onglet Vidéos
        self.videos_tab = ttk.Frame(self.artist_notebook)
        self.artist_notebook.add(self.videos_tab, text="Vidéos")
        
        # Onglet Sorties
        self.releases_tab = ttk.Frame(self.artist_notebook)
        self.artist_notebook.add(self.releases_tab, text="Sorties")
        
        # Créer le contenu des onglets
        self._setup_videos_tab()
        self._setup_releases_tab()
        
        # Stocker la référence pour pouvoir la détruire plus tard
        self.artist_main_frame = artist_main_frame
    
    def _setup_videos_tab(self):
        """Configure l'onglet Vidéos"""
        return setup._setup_videos_tab(self)
    
    def _setup_releases_tab(self):
        """Configure l'onglet Sorties"""
        # Canvas et scrollbar pour les sorties
        releases_canvas = tk.Canvas(self.releases_tab, bg='#2d2d2d')
        releases_scrollbar = ttk.Scrollbar(self.releases_tab, orient="vertical", command=releases_canvas.yview)
        releases_canvas.configure(yscrollcommand=releases_scrollbar.set)
        
        # Frame pour contenir les résultats
        self.releases_container = tk.Frame(releases_canvas, bg='#2d2d2d')
        releases_canvas_frame = releases_canvas.create_window((0, 0), window=self.releases_container, anchor="nw")
        
        # Empaqueter
        releases_scrollbar.pack(side="right", fill="y")
        releases_canvas.pack(side="left", fill="both", expand=True)
        
        # Configuration du scroll
        def configure_releases_scroll(event):
            releases_canvas.configure(scrollregion=releases_canvas.bbox("all"))
            canvas_width = releases_canvas.winfo_width()
            releases_canvas.itemconfig(releases_canvas_frame, width=canvas_width)
        
        self.releases_container.bind('<Configure>', configure_releases_scroll)
        
        # Message de chargement
        loading_label = tk.Label(
            self.releases_container,
        ...
        self.releases_canvas = releases_canvas
        self.releases_scrollbar = releases_scrollbar
    
    def _search_artist_videos(self):
        """Recherche toutes les vidéos de l'artiste"""
        def search_videos():
            try:
                ...
                search_opts = {
                    ...
                    'quiet': True,
                    'no_warnings': True,
                    'max_downloads': 100,  # Limiter à 100 vidéos
                    'ignoreerrors': True
                }
                ...
                with YoutubeDL(search_opts) as ydl:
                    ...
                    search_queries = [
                        f"ytsearch100:{self.current_artist_name}",
                        f"ytsearch100:{self.current_artist_name} official",
                        f"ytsearch100:{self.current_artist_name} music video"
                    ]
                    ...
                    for query in search_queries:
                        try:
                            ...
                            if results and 'entries' in results:
                                ...
                                videos = [v for v in videos if 'youtube.com/watch' in v.get('url', '')]
                    ...
                    def get_upload_date(video):
                        upload_date = video.get('upload_date', '19700101')
                        try:
                            return int(upload_date) if upload_date else 0
                        except:
                            return 0
    ...
    def _search_artist_releases(self):
        """Recherche les sorties officielles de l'artiste"""
        def search_releases():
            try:
                # Recherche des sorties/albums officiels
                search_opts = {
                    'extract_flat': True,
                    'quiet': True,
                    'no_warnings': True,
                ...
                with YoutubeDL(search_opts) as ydl:
                    # Recherches spécifiques pour les sorties
                    search_queries = [
                        f"ytsearch50:{self.current_artist_name} album",
                        f"ytsearch50:{self.current_artist_name} EP",
                        f"ytsearch50:{self.current_artist_name} single",
                        f"ytsearch50:{self.current_artist_name} official audio"
                    ]
                    ...
                    for query in search_queries:
                        try:
                            results = ydl.extract_info(query, download=False)
                            if results and 'entries' in results:
                                releases = self._filter_search_results(results['entries'])
                                # Filtrer pour les contenus qui semblent être des sorties officielles
                                releases = [r for r in releases if any(keyword in r.get('title', '').lower() 
                                           for keyword in ['album', 'ep', 'single', 'official', 'audio', 'full'])]
                    ...
                    for release in all_releases:
                        url = release.get('url', '')
                        if url not in unique_releases:
                            unique_releases[url] = release
                    
                    final_releases = list(unique_releases.values())
                    
                    # Trier par date de publication (plus récentes en premier)
                    def get_upload_date(release):
                        upload_date = release.get('upload_date', '19700101')
                        try:
                            return int(upload_date) if upload_date else 0
                        except:
                            return 0
                    
                    final_releases.sort(key=get_upload_date, reverse=True)
                    
                    # Limiter à 30 sorties max
                    final_releases = final_releases[:30]
                    
                    # Afficher les résultats dans l'interface
                    self.root.after(0, lambda: self._display_artist_releases(final_releases))
    ...
    def _display_artist_videos(self, videos):
        """Affiche les vidéos de l'artiste"""
        # Vider le container
        ...
        for i, video in enumerate(videos):
            # Utiliser la même fonction que pour les résultats de recherche normaux
            # mais avec un container différent
            self._add_artist_video_result(video, i, self.videos_container)
        
        self.status_bar.config(text=f"{len(videos)} vidéos trouvées pour {self.current_artist_name}")
    
    def _display_artist_releases(self, releases):
        """Affiche les sorties de l'artiste"""
        # Vider le container
        for widget in self.releases_container.winfo_children():
            widget.destroy()
        ...
        for i, release in enumerate(releases):
            self._add_artist_video_result(release, i, self.releases_container)
        
        self.status_bar.config(text=f"{len(releases)} sorties trouvées pour {self.current_artist_name}")
    
    def _add_artist_video_result(self, video, index, container):
        """Ajoute un résultat vidéo dans le contenu artiste (version simplifiée de _add_search_result)"""
        try:
            title = video.get('title', 'Sans titre')
            duration = video.get('duration', 0)
            url = video.get('url', '')
            ...
            title_label = tk.Label(
                ...
                anchor='w',
                justify='left',
                wraplength=400  # Ajuster selon la largeur
            )
            title_label.grid(row=0, column=0, sticky='ew', pady=(2, 0))
    ...
    def _display_videos_error(self, error_msg):
        """Affiche une erreur dans l'onglet vidéos"""
        for widget in self.videos_container.winfo_children():
            widget.destroy()
        
        error_label = tk.Label(
            self.videos_container,
            text=f"Erreur lors du chargement des vidéos:\n{error_msg}",
            bg='#2d2d2d',
            fg='#ff6666',
            font=('TkDefaultFont', 10),
            justify='center'
    ...
    def _return_to_search(self):
        ...
        try:
            ...
            # Détruire l'interface artiste
            if hasattr(self, 'artist_main_frame'):
                self.artist_main_frame.destroy()
                delattr(self, 'artist_main_frame')
            
            # Restaurer le contenu original
            if self.original_search_content is not None:
                for child in self.original_search_content['children']:
                    child.pack(fill=tk.BOTH, expand=True)
                self.original_search_content = None
