...
class MusicPlayer:
    ...
    def _should_load_more_results(self):
        ...
        if (self.is_loading_more or 
            ...
            self.current_search_batch >= 3):  # Maximum 3 lots (30 résultats)
            return False
    ...
    def search_youtube(self):
        ...
        self.search_results_count = 0
        self.is_loading_more = False
        self.current_search_batch = 1  # Numéro du lot actuel
        ...
        # Lancer la première recherche de 10 résultats
        threading.Thread(target=self._perform_batch_search, args=(query, 1), daemon=True).start()

    def _perform_batch_search(self, query, batch_number):
        """Effectue une recherche par lot de 10 résultats"""
        try:
            search_opts = {
                'extract_flat': True,
                'quiet': True,
                'no_warnings': True,
                'max_downloads': 50,  # Plus de résultats pour avoir de la variété
                'ignoreerrors': True
            }
            
            # Stratégie pour obtenir des résultats différents selon le lot
            search_queries = []
            if batch_number == 1:
                # Premier lot : recherche normale
                search_queries = [f"ytsearch50:{query}"]
            elif batch_number == 2:
                # Deuxième lot : variations de la requête
                search_queries = [
                    f"ytsearch50:{query} music",
                    f"ytsearch50:{query} song",
                    f"ytsearch50:{query} audio"
                ]
            else:  # batch_number == 3
                # Troisième lot : autres variations
                search_queries = [
                    f"ytsearch50:{query} cover",
                    f"ytsearch50:{query} remix",
                    f"ytsearch50:{query} live"
                ]
            
            all_videos = []
            seen_urls = set()
            
            # Ajouter les URLs déjà vues pour éviter les doublons
            for widget in self.results_container.winfo_children():
                if hasattr(widget, 'video_url'):
                    seen_urls.add(widget.video_url)
            ...
            with YoutubeDL(search_opts) as ydl:
                        ...
                        if results and 'entries' in results:
                            ...
                            video_results = [
                                ...
                                if (entry and 
                                    ...
                                    entry.get('duration', 0) <= 600.0 and  # Durée max de 10 minutes
                                    entry.get('url') not in seen_urls)  # Pas de doublons
                            ]
                            ...
                            for video in video_results:
                                if video.get('url') not in seen_urls:
                                    ...
                                    # Arrêter si on a assez de résultats pour ce lot
                                    if len(all_videos) >= 10:
                                        break
                        
                        # Si on a assez de résultats, arrêter les recherches
                        if len(all_videos) >= 10:
                            break
                            
                    except Exception as e:
                        print(f"Erreur recherche variante: {e}")
                        continue
                
                # Si c'est le premier lot, nettoyer le container
                if batch_number == 1:
                    self.root.after(0, self._clear_results)
                
                # Prendre seulement 10 résultats pour ce lot
                batch_results = all_videos[:10]
                
                # Afficher les résultats de ce lot
                for i, video in enumerate(batch_results):
                ...
                self.root.after(0, lambda: self.status_bar.config(
                    text=f"{self.search_results_count} résultats trouvés (lot {batch_number}) - {len(batch_results)} nouveaux"
                ))
                
        except Exception as e:
            self.root.after(0, lambda: self.status_bar.config(text=f"Erreur recherche lot {batch_number}: {e}"))
        finally:
            self.is_searching = False
            self.is_loading_more = False
    ...
    def _load_more_search_results(self):
        ...
        print(f"_load_more_search_results appelée - Lot actuel: {self.current_search_batch}, Résultats: {self.search_results_count}")
        
        if (self.is_loading_more or 
            self.is_searching or
            not self.current_search_query or 
            self.search_results_count >= self.max_search_results or
            self.current_search_batch >= 3):
        ...
        self.status_bar.config(text=f"Recherche du lot {self.current_search_batch}...")
        
        # Lancer la recherche du prochain lot
        threading.Thread(
            target=self._perform_batch_search, 
            args=(self.current_search_query, self.current_search_batch), 
            daemon=True
        ).start()
