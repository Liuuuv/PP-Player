...
class MusicPlayer:
    ...
    def _display_results_progressively(self, start_index, end_index, batch_number, current_index=None):
        ...
        if next_index < end_index and not self.search_cancelled:
            # Réduire le délai à 30ms pour un affichage plus fluide
            self.root.after(30, lambda: self._display_results_progressively(start_index, end_index, batch_number, next_index))
        ...
        try:
            ...
            if video.get('thumbnails'):
                ...
                if is_channel:
                    ...
                    self.root.after(200, lambda: self._queue_thumbnail_load(thumbnail_label, thumbnail_url, is_circular=True))
                else:
                    # Pour les vidéos, miniature normale
                    self.root.after(200, lambda: self._queue_thumbnail_load(thumbnail_label, thumbnail_url, is_circular=False))
