...
class MusicPlayer:
    def __init__(self, root):
        ...
        self.max_search_results = 20  # Réduire le nombre maximum de résultats
        ...
        # Charger les paramètres d'optimisation depuis config
        self.load_search_optimization_config()
        
        # Variables pour limiter les threads de miniatures
        self.thumbnail_threads = 0  # Compteur de threads de miniatures actifs
        self.thumbnail_queue = []  # Queue des miniatures en attente
        ...
        self.last_scroll_check = 0  # Timestamp de la dernière vérification de scroll
    ...
    def load_search_optimization_config(self):
        """Charge les paramètres d'optimisation depuis config.py"""
        from config import SEARCH_OPTIMIZATION, PERFORMANCE_MODES
        
        # Charger le mode actuel
        current_mode = SEARCH_OPTIMIZATION.get('performance_mode', 'balanced')
        
        if current_mode in PERFORMANCE_MODES:
            config = PERFORMANCE_MODES[current_mode]
        else:
            config = SEARCH_OPTIMIZATION
        
        # Appliquer les paramètres
        self.max_thumbnail_threads = config.get('max_thumbnail_threads', 2)
        self.fast_display_mode = config.get('fast_display_mode', True)
        self.progressive_delay_ms = config.get('progressive_delay_ms', 30)
        self.thumbnail_load_delay_ms = config.get('thumbnail_load_delay_ms', 200)
        self.max_search_results = config.get('max_search_results', 15)
        self.search_delay = config.get('search_delay_ms', 300)
        self.results_per_page = config.get('results_per_page', 10)
        self.scroll_check_interval_ms = config.get('scroll_check_interval_ms', 200)
        self.use_low_priority_threads = config.get('use_low_priority_threads', True)
        
        # Recalculer les lots
        self.max_search_batchs = self.max_search_results // self.results_per_page + 1
        
        print(f"Configuration d'optimisation chargée - Mode: {current_mode}")
    
    def apply_performance_mode(self, mode_name):
        """Applique un mode de performance prédéfini"""
        from config import PERFORMANCE_MODES
        
        if mode_name not in PERFORMANCE_MODES:
            print(f"Mode '{mode_name}' non reconnu. Modes disponibles: {list(PERFORMANCE_MODES.keys())}")
            return False
        
        config = PERFORMANCE_MODES[mode_name]
        
        # Appliquer les paramètres
        self.max_thumbnail_threads = config['max_thumbnail_threads']
        self.fast_display_mode = config['fast_display_mode']
        self.progressive_delay_ms = config['progressive_delay_ms']
        self.thumbnail_load_delay_ms = config['thumbnail_load_delay_ms']
        self.max_search_results = config['max_search_results']
        self.search_delay = config['search_delay_ms']
        self.results_per_page = config['results_per_page']
        self.scroll_check_interval_ms = config['scroll_check_interval_ms']
        self.use_low_priority_threads = config['use_low_priority_threads']
        
        # Recalculer les lots
        self.max_search_batchs = self.max_search_results // self.results_per_page + 1
        
        # Mettre à jour le fichier config
        self.save_performance_mode(mode_name)
        
        if hasattr(self, 'status_bar'):
            self.status_bar.config(text=f"Mode {mode_name} appliqué")
            self.root.after(2000, lambda: self.status_bar.config(text="Prêt"))
        
        print(f"Mode de performance '{mode_name}' appliqué")
        return True
    
    def save_performance_mode(self, mode_name):
        """Sauvegarde le mode de performance dans le fichier config"""
        try:
            ...
            # Remplacer la ligne du mode
            import re
            pattern = r"'performance_mode':\s*'[^']*'"
            replacement = f"'performance_mode': '{mode_name}'"
            ...
            if re.search(pattern, content):
                ...
                print(f"Mode '{mode_name}' sauvegardé dans config.py")
            else:
                print("Impossible de trouver la ligne performance_mode dans config.py")
                
        except Exception as e:
            print(f"Erreur lors de la sauvegarde du mode: {e}")
    ...
    def setup_keyboard_bindings(self):
        ...
        # Raccourcis pour les modes de performance
        self.root.bind('<Control-F1>', lambda e: self.apply_performance_mode('performance'))
        self.root.bind('<Control-F2>', lambda e: self.apply_performance_mode('balanced'))
        self.root.bind('<Control-F3>', lambda e: self.apply_performance_mode('quality'))
        self.root.bind('<Control-F4>', lambda e: self.show_performance_settings())
    ...
    def _check_scroll_position(self):
        """Vérifie la position du scroll et charge plus si nécessaire avec limitation de fréquence"""
        import time
        current_time = time.time()
        
        # Limiter les vérifications selon la configuration
        interval = getattr(self, 'scroll_check_interval_ms', 200) / 1000.0
        if current_time - self.last_scroll_check < interval:
            return
            
        self.last_scroll_check = current_time
    ...
    def _start_new_search(self):
        ...
        # Remplacer root.update() par root.update_idletasks() pour éviter les saccades
    ...
    def _perform_complete_search(self, query):
        ...
        try:
            ...
            search_opts = {
                ...
                'quiet': True,
                'no_warnings': True,
                'max_downloads': 15,  # Réduire à 15 résultats pour moins de charge
                'ignoreerrors': True,
            ...
            with YoutubeDL(search_opts) as ydl:
                ...
                # Recherche de 15 résultats pour réduire la charge
    ...
    def _display_batch_results(self, batch_number):
        ...
        # Afficher les résultats de ce lot - option pour affichage par blocs ou progressif
        if hasattr(self, 'fast_display_mode') and self.fast_display_mode:
            # Mode rapide : afficher tous les résultats du lot d'un coup
            self._display_results_batch(start_index, end_index, batch_number)
        else:
            # Mode progressif : afficher un par un avec délai
            self._display_results_progressively(start_index, end_index, batch_number)
    
    def _display_results_progressively(self, start_index, end_index, batch_number, current_index=None):
        """Affiche les résultats progressivement pour éviter de bloquer l'UI"""
        if current_index is None:
            current_index = start_index
        
        # Vérifier l'annulation
        if self.search_cancelled or current_index >= end_index:
            if not self.search_cancelled and current_index >= end_index:
                # Tous les résultats de ce lot ont été affichés
                self.root.after(0, lambda: self._safe_update_status(batch_number))
            return
        
        # Afficher le résultat actuel
        if current_index < len(self.all_search_results):
            video = self.all_search_results[current_index]
            if not self.search_cancelled:
                self._safe_add_search_result(video, current_index)
                self.search_results_count += 1
        
        # Programmer l'affichage du prochain résultat
        next_index = current_index + 1
        if next_index < end_index and not self.search_cancelled:
            # Utiliser le délai configuré
            delay = getattr(self, 'progressive_delay_ms', 30)
            self.root.after(delay, lambda: self._display_results_progressively(start_index, end_index, batch_number, next_index))
        elif not self.search_cancelled and current_index >= end_index - 1:
            # Mettre à jour le statut seulement à la fin du lot pour éviter les saccades
            self.root.after(0, lambda: self._safe_update_status(batch_number))
    
    def _display_results_batch(self, start_index, end_index, batch_number):
        """Affiche tous les résultats d'un lot d'un coup pour un affichage plus rapide"""
        if self.search_cancelled:
            return
            
        # Afficher tous les résultats du lot sans délai
        for i in range(start_index, min(end_index, len(self.all_search_results))):
            if self.search_cancelled:
                break
                
            video = self.all_search_results[i]
            self._safe_add_search_result(video, i)
            self.search_results_count += 1
        
        # Mettre à jour le statut une seule fois à la fin
        if not self.search_cancelled:
            self.root.after(0, lambda: self._safe_update_status(batch_number))
    
    def toggle_display_mode(self):
        """Bascule entre le mode d'affichage rapide et progressif"""
        self.fast_display_mode = not self.fast_display_mode
        mode_text = "rapide" if self.fast_display_mode else "progressif"
        if hasattr(self, 'status_bar'):
            self.status_bar.config(text=f"Mode d'affichage: {mode_text}")
            self.root.after(2000, lambda: self.status_bar.config(text="Prêt"))
    
    def optimize_search_performance(self):
        """Applique les optimisations de performance pour les recherches"""
        # Réduire le nombre de threads de miniatures
        self.max_thumbnail_threads = 1
        
        # Activer le mode d'affichage rapide
        self.fast_display_mode = True
        
        # Réduire le nombre maximum de résultats
        self.max_search_results = 10
        
        # Augmenter le délai de recherche pour éviter les recherches trop fréquentes
        self.search_delay = 500
        
        if hasattr(self, 'status_bar'):
            self.status_bar.config(text="Mode performance activé")
            self.root.after(2000, lambda: self.status_bar.config(text="Prêt"))
    
    def restore_default_performance(self):
        """Restaure les paramètres de performance par défaut"""
        ...
        if hasattr(self, 'status_bar'):
            self.status_bar.config(text="Paramètres par défaut restaurés")
            self.root.after(2000, lambda: self.status_bar.config(text="Prêt"))
    ...
    def _show_search_results(self):
        ...
        try:
            ...
            if hasattr(self, 'youtube_canvas') and self.youtube_canvas.winfo_exists():
                ...
                # Différer la mise à jour du canvas pour éviter les saccades
                self.root.after_idle(lambda: self.youtube_canvas.configure(scrollregion=self.youtube_canvas.bbox("all")))
        except Exception as e:
    ...
    def _add_search_result(self, video, index):
        ...
        try:
            ...
            # Charger la miniature en arrière-plan avec limitation des threads
            # Différer le chargement pour éviter les saccades lors de l'affichage initial
            if video.get('thumbnails'):
                thumbnail_url = video['thumbnails'][1]['url'] if len(video['thumbnails']) > 1 else video['thumbnails'][0]['url']
                # Utiliser le délai configuré pour le chargement des miniatures
                delay = getattr(self, 'thumbnail_load_delay_ms', 200)
                if is_channel:
                    # Pour les chaînes, utiliser une miniature circulaire
                    self.root.after(delay, lambda: self._queue_thumbnail_load(thumbnail_label, thumbnail_url, is_circular=True))
                else:
                    # Pour les vidéos, miniature normale
                    self.root.after(delay, lambda: self._queue_thumbnail_load(thumbnail_label, thumbnail_url, is_circular=False))
                
        except Exception as e:
            print(f"Erreur affichage résultat: {e}")
    
    def _queue_thumbnail_load(self, label, url, is_circular=False):
        """Ajoute une miniature à la queue de chargement avec limitation des threads"""
        if self.thumbnail_threads < self.max_thumbnail_threads:
            # Lancer immédiatement si on n'a pas atteint la limite
            self.thumbnail_threads += 1
            if is_circular:
                threading.Thread(
                    target=self._load_circular_thumbnail_with_callback,
                    args=(label, url),
                    daemon=True
                ).start()
            else:
                threading.Thread(
                    target=self._load_thumbnail_with_callback,
                    args=(label, url),
                    daemon=True
                ).start()
        else:
            # Ajouter à la queue si on a atteint la limite
            self.thumbnail_queue.append((label, url, is_circular))
    
    def _process_thumbnail_queue(self):
        """Traite le prochain élément de la queue des miniatures"""
        if self.thumbnail_queue and self.thumbnail_threads < self.max_thumbnail_threads:
            label, url, is_circular = self.thumbnail_queue.pop(0)
            self.thumbnail_threads += 1
            if is_circular:
                threading.Thread(
                    target=self._load_circular_thumbnail_with_callback,
    ...
    def _load_thumbnail_with_callback(self, label, url):
        """Charge une miniature et traite la queue ensuite"""
        try:
            # Réduire la priorité du thread si configuré
            if getattr(self, 'use_low_priority_threads', True):
                import os
            ...
            self._load_thumbnail(label, url)
        finally:
            self.thumbnail_threads -= 1
            # Traiter le prochain élément de la queue
            self.root.after(0, self._process_thumbnail_queue)
    
    def _load_circular_thumbnail_with_callback(self, label, url):
        """Charge une miniature circulaire et traite la queue ensuite"""
        try:
            # Réduire la priorité du thread si configuré
            if getattr(self, 'use_low_priority_threads', True):
                import os
            ...
            self._load_circular_thumbnail(label, url)
        finally:
            self.thumbnail_threads -= 1
            # Traiter le prochain élément de la queue
            self.root.after(0, self._process_thumbnail_queue)
    ...
    def _load_thumbnail(self, label, url):
        ...
        try:
            ...
            # Timeout plus court et headers pour réduire la charge
            headers = {
            ...
            # Ne pas imprimer les erreurs pour éviter le spam
            pass
