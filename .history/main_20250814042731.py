...
class MusicPlayer:
    ...
    def _enable_scroll_propagation(self, widget):
        """Active la propagation des événements de scroll sur un widget et ses enfants"""
        def propagate_scroll(event):
            # Trouver le canvas parent qui peut être scrollé
            parent = widget.winfo_parent()
            while parent:
                parent_widget = self.root.nametowidget(parent)
                if hasattr(parent_widget, 'yview_scroll'):
                    # C'est un canvas scrollable, propager l'événement
                    if event.delta:
                        parent_widget.yview_scroll(int(-1*(event.delta/120)), "units")
                    else:
                        # Pour Linux
                        if event.num == 4:
                            parent_widget.yview_scroll(-1, "units")
                        elif event.num == 5:
                            parent_widget.yview_scroll(1, "units")
                    return "break"  # Empêcher la propagation supplémentaire
                parent = parent_widget.winfo_parent()
        
        # Bind les événements de scroll sur le widget
        widget.bind("<MouseWheel>", propagate_scroll)
        widget.bind("<Button-4>", propagate_scroll)  # Linux
        widget.bind("<Button-5>", propagate_scroll)  # Linux
