...
class MusicPlayer:
    ...
    def _load_more_search_results(self):
        ...
        # Calculer combien de résultats devraient être affichés jusqu'à maintenant
        expected_results = self.current_search_batch * self.results_per_page
        print(f"_load_more_search_results appelée - Lot actuel: {self.current_search_batch}, Résultats attendus: {expected_results}/{len(self.all_search_results)}")
        
        if (self.is_loading_more or 
            self.is_searching or
            self.search_cancelled or  # Vérifier si la recherche a été annulée
            not self.current_search_query or 
            expected_results >= len(self.all_search_results) or
            self.current_search_batch >= self.max_search_batchs):
            print(f"Arrêt du chargement: loading={self.is_loading_more}, searching={self.is_searching}, cancelled={self.search_cancelled}, expected={expected_results}, total={len(self.all_search_results)}, batch={self.current_search_batch}/{self.max_search_batchs}")
            return
