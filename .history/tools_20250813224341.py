...
def _add_youtube_to_playlist(self, video, frame, playlist_name):
    ...
    if existing_file:
        ...
        # Temporairement définir search_list et target_playlist pour que _download_youtube_thread fonctionne
        original_search_list = getattr(self, 'search_list', [])
        original_target_playlist = getattr(self, '_target_playlist_for_download', None)
        ...
        self._target_playlist_for_download = playlist_name
        
        # Lancer le téléchargement dans un thread comme download_selected_youtube
        threading.Thread(
            target=self._download_youtube_thread,
            args=(url, True),  # add_to_playlist=True mais on utilisera _target_playlist_for_download
            daemon=True
        ).start()
        
        # Restaurer les valeurs originales après un délai
        def restore_original_values():
            self.search_list = original_search_list
            self._target_playlist_for_download = original_target_playlist
        
        # Restaurer après 100ms pour laisser le temps au thread de démarrer
        threading.Timer(0.1, restore_original_values).start()
