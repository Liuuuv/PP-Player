...
class MusicPlayer:
    ...
    def _create_artist_tabs(self):
        ...
        # Créer le notebook qui remplace les résultats directement dans youtube_results_frame
        self.artist_notebook = ttk.Notebook(self.youtube_results_frame, takefocus=0)
        self.artist_notebook.pack(fill=tk.BOTH, expand=True, padx=5, pady=5)
    ...
    def _search_artist_releases(self):
                    ...
                    if hasattr(self, 'current_artist_channel_url') and self.current_artist_channel_url:
                        try:
                            ...
                            if channel_info and 'entries' in channel_info:
                                ...
                                # Garder seulement les vraies playlists
                                valid_playlists = []
                                for p in playlists:
                                    if p and p.get('id'):
                                        ...
                                        if (p.get('_type') == 'playlist' or 
                                            ...
                                            print(f"Playlist valide: {p.get('title', 'Sans titre')} - {p.get('playlist_count', 0)} vidéos")
                                
                                all_playlists.extend(valid_playlists[:15])  # Prendre les 15 premières
                        except Exception as e:
                            print(f"Erreur extraction playlists chaîne: {e}")
                    
                    # Si pas de playlists trouvées, essayer une recherche spécifique aux playlists
                    if not all_playlists:
                        print("Aucune playlist trouvée sur la chaîne, recherche alternative...")
        ...
        # Lancer en arrière-plan et enregistrer le thread
        threading.Thread(target=search_releases, daemon=True).start()
    ...
    def _search_artist_playlists(self):
                ...
                with YoutubeDL(search_opts) as ydl:
                    ...
                    if hasattr(self, 'current_artist_channel_url') and self.current_artist_channel_url:
                        try:
                            ...
                            if '/channel/' in self.current_artist_channel_url:
                                playlists_url = self.current_artist_channel_url + '/playlists'
                            elif '/@' in self.current_artist_channel_url:
                                playlists_url = self.current_artist_channel_url + '/playlists'
                            else:
                                playlists_url = self.current_artist_channel_url
                        ...
                        for query in search_queries:
                            try:
                                ...
                                if results and 'entries' in results:
                                    ...
                                    if len(all_playlists) >= 20:  # Plus de playlists que pour les sorties
                                        break
                            except Exception as e:
                                print(f"Erreur recherche {query}: {e}")
                                continue
        ...
        threading.Thread(target=search_playlists, daemon=True).start()
