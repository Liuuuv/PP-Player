...
def _extract_and_save_metadata(self, info, filepath):
    ...
    try:
        ...
        # print(title, "|", uploader, "|", info.get('album', ''), "Debug: _extract_and_save_metadata")
        
        # Essayer d'extraire l'artiste et l'album depuis le titre et les métadonnées YouTube
        artist = None
        album = info.get('album', '')  # Utiliser l'album depuis YouTube si disponible
        
        # Méthode 1: Chercher des patterns comme "Artist - Title" ou "Artist: Title"
        if ' - ' in title:
            parts = title.split(' - ', 1)
            if len(parts) == 2:
                potential_artist = parts[0].strip()
                # Vérifier que ce n'est pas juste un numéro ou quelque chose de générique
        ...
        # Méthode 2: Utiliser le nom de la chaîne comme artiste si on n'en a pas trouvé
        if not artist:
            if channel and channel != uploader:
                artist = channel
            elif uploader:
                # Nettoyer le nom de l'uploader (enlever "Official", "VEVO", etc.)
                clean_uploader = uploader.replace(' - Topic', '').replace('VEVO', '').replace('Official', '').strip()
                if clean_uploader and len(clean_uploader) > 1:
                    artist = clean_uploader
        
        # Méthode 3: Si pas d'album depuis YouTube, chercher dans la description et le titre
        if not album:
            description = info.get('description', '') or ''
            
            # Patterns courants pour les albums
            album_patterns = [
                r'from the album[:\s]+"([^"]+)"',
                r'album[:\s]+"([^"]+)"',
                r'from[:\s]+"([^"]+)"',
            ...
            # Si on n'a pas trouvé d'album dans la description, chercher dans le titre
            if not album:
                # Patterns dans le titre
                title_album_patterns = [
                    r'\(from "([^"]+)"\)',
                    r'\[from "([^"]+)"\]',
                    r'\(([^)]+) EP\)',
                    r'\[([^]]+) EP\]',
                ...
                for pattern in title_album_patterns:
