...
class CustomProgressSlider(tk.Frame):
        ...
        try:
            ...
            if not force_redraw:
                if self.dragging:
                    ...
                    print(f"[DEBUG] draw_slider - DRAGGING CHECK")
                    print(f"  value_changed: {value_changed} ({self.last_drawn_value} -> {self.current_value})")
                    print(f"  drag_position_changed: {drag_position_changed} ({self.last_drawn_drag_x} -> {current_drag_x})")
                    
                    if not (value_changed or drag_position_changed):
                        print(f"  -> SKIPPING REDRAW (no changes)")
                        return
                    else:
                        print(f"  -> PROCEEDING WITH REDRAW")
                else:
    ...
    def set_value(self, value):
        ...
        # DEBUG: Messages de debug pendant le drag
        if self.dragging:
            print(f"[DEBUG] set_value called during drag: {value} -> {new_value}")
        ...
        if abs(new_value - self.current_value) < 0.01:  # Seuil de changement minimal
            if self.dragging:
                print(f"  -> SKIPPING (change too small: {abs(new_value - self.current_value)})")
            return
        
        # Toujours mettre à jour la valeur (même pendant le drag)
        # Cela permet à la barre bleue de continuer de progresser
