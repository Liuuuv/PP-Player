...
def load_icons(self):
    ...
    for key, filename in icon_names.items():
        try:
            ...
            # Créer une version plus petite pour les icônes clear, find et auto_scroll
            if key in ["clear", "find", "auto_scroll"]:
                image_small = Image.open(path).resize((20, 20), Image.Resampling.LANCZOS)
                self.icons[key + "_small"] = ImageTk.PhotoImage(image_small)
            
            # Créer une version plus petite pour les icônes output, stats et import
            if key in ["output", "stats", "import"]:
                image_small = Image.open(path).resize((16, 16), Image.Resampling.LANCZOS)
                self.icons[key + "_small"] = ImageTk.PhotoImage(image_small)
        except Exception as e:
...
def setup_controls(self):
        ...
        # Permettre le scroll sur cette frame
        self._enable_scroll_propagation(control_frame)
        ...
        # Permettre le scroll sur cette frame
        self._enable_scroll_propagation(song_info_frame)
        ...
        # Permettre le scroll sur cette frame
        self._enable_scroll_propagation(buttons_volume_frame)
        ...
        # Permettre le scroll sur cette frame
        self._enable_scroll_propagation(volume_offset_frame)
        ...
        # Permettre le scroll sur cette frame
        self._enable_scroll_propagation(button_frame)
        ...
        self.prev_button = ttk.Button(button_frame, image=self.icons["prev"], command=self.prev_track_manual, takefocus=0)
        ...
        self.next_button = ttk.Button(button_frame, image=self.icons["next"], command=self.next_track_manual, takefocus=0)
        ...
        # Permettre le scroll sur cette frame
        self._enable_scroll_propagation(volume_frame)
        ...
        # Permettre le scroll sur cette frame
        self._enable_scroll_propagation(status_frame)
...
def setup_keyboard_bindings(self):
        ...
        def test_binding(event):
            print("DEBUG: Test binding Ctrl+Alt+0 (pavé numérique) détecté !")
