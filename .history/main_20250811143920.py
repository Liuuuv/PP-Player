...
class MusicPlayer:
    ...
    def _check_scroll_position(self):
        ...
        print("Vérification de la position du scroll...")
    ...
    def _bind_mousewheel(self, widget, canvas):
        """Lie la molette de souris directement sur le widget et son canvas"""
        # Créer une fonction de callback spécifique pour ce canvas
        def on_mousewheel(event):
            return self._on_mousewheel(event, canvas)
        
        # Lier directement sur le widget et le canvas (pas bind_all)
        ...
        # Stocker les références pour pouvoir les supprimer si nécessaire
        if not hasattr(self, '_scroll_bindings'):
            self._scroll_bindings = {}
        self._scroll_bindings[canvas] = {
            'widget': widget,
            'callback': on_mousewheel
        }

    def _bind_scroll(self, canvas):
        """Active le défilement pour un canvas spécifique (méthode dépréciée, gardée pour compatibilité)"""
        # Cette méthode n'est plus utilisée avec la nouvelle approche
        pass

    def _unbind_scroll(self, canvas):
        """Désactive le défilement pour un canvas spécifique (méthode dépréciée, gardée pour compatibilité)"""
        # Cette méthode n'est plus utilisée avec la nouvelle approche
        pass
    ...
    def _on_mousewheel(self, event, canvas):
        ...
        try:
            # Vérifier que le canvas existe et est scrollable
            if not canvas.winfo_exists():
                return
            
            # Vérifier qu'il y a quelque chose à scroller
            ...
            if event.delta:
                ...
                # Linux qui utilise event.num au lieu de event.delta
                if event.num == 4:
                    canvas.yview_scroll(-1, "units")
                elif event.num == 5:
                    canvas.yview_scroll(1, "units")
        except Exception as e:
            # En cas d'erreur, ne pas faire planter l'application
