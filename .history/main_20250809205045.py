...
class MusicPlayer:
    ...
    def _show_current_song_thumbnail(self):
        """Affiche la miniature de la chanson en cours dans la zone des résultats"""
        # Vérifier que results_container existe
        ...
        if len(self.playlist) > 0 and self.current_index < len(self.playlist):
            ...
            # Frame pour la miniature - centré et plus grand
            ...
            # Label pour la miniature - beaucoup plus grand
            thumbnail_label = tk.Label(
                thumbnail_frame,
                bg='#3d3d3d',
                width=40,
            ...
            no_song_frame = tk.Frame(
                self.results_container,
                bg='#3d3d3d',
                relief='flat'
            )
            no_song_frame.pack(fill='both', expand=True, padx=20, pady=20)
    ...
    def _add_download_item(self, filepath):
        ...
        try:
            ...
            # Double-clic pour ajouter en fin de file d'attente
            def on_item_double_click():
                self._add_to_queue_end(filepath)
    ...
    def _add_to_queue_end(self, filepath):
        """Ajoute une musique en fin de file d'attente"""
        try:
            # Ajouter à la playlist si pas déjà présent
            if filepath not in self.playlist:
                self.playlist.append(filepath)
                self.status_bar.config(text=f"Ajouté en file d'attente: {os.path.basename(filepath)}")
            else:
                # Si déjà présent, le déplacer à la fin
                self.playlist.remove(filepath)
                self.playlist.append(filepath)
                self.status_bar.config(text=f"Déplacé en fin de file: {os.path.basename(filepath)}")
            
            # Mettre à jour l'affichage de la playlist
            self._refresh_playlist_display()
                
        except Exception as e:
            print(f"Erreur _add_to_queue_end: {e}")
            self.status_bar.config(text=f"Erreur lors de l'ajout en file d'attente")
    ...
    def _play_after_current(self, filepath):
        ...
        try:
            ...
            if len(self.playlist) > 0 and self.current_index < len(self.playlist):
                ...
                song_index = self.playlist.index(filepath)
                ...
                if song_index != target_position:
                    ...
                    self.playlist.insert(target_position, filepath)
