...
class DragDropHandler:
    ...
    def _update_drag_visual(self, frame, dx, dy):
        ...
        if 100 > dx > 50:
            self._set_frame_colors(frame, COLORDRAG_RIGHT_HALF, frame.is_in_queue)  # Vert clair pour indiquer l'activation partielle
        elif dx >= 100:  # Drag vers la droite - ajouter à la playlist
            self._set_frame_colors(frame, COLORDRAG_RIGHT, frame.is_in_queue)  # Vert pour indiquer l'activation
        elif -100 < dx < -50:
            self._set_frame_colors(frame, COLORDRAG_LEFT_HALF, frame.is_in_queue)  # Orange clair pour indiquer l'activation partielle
        elif dx <= -100:  # Drag vers la gauche - placer en premier dans la queue
            self._set_frame_colors(frame, COLORDRAG_LEFT, frame.is_in_queue)  # Jaune-orange pour indiquer "premier dans la queue"
        # else:
    ...
    def _end_visual_drag(self, frame):
        ...
        if hasattr(frame, 'selected') and frame.selected:
            target_color = COLOR_SELECTED  # Couleur bleue de sélection
        # elif hasattr(frame, 'filepath') and hasattr(self.music_player, 'main_playlist'):
            ...
            #     self.music_player.current_index < len(self.music_player.main_playlist) and 
            #     self.music_player.main_playlist[self.music_player.current_index] == frame.filepath):
            # target_color = COLOR_SELECTED  # Couleur bleue pour la chanson en cours
        
        self._set_frame_colors(frame, target_color, frame.is_in_queue)
    
    def _set_frame_colors(self, frame, color, exclude_queue_indicator=False):
        ...
        if hasattr(self.music_player, '_set_item_colors'):
            self.music_player._set_item_colors(frame, color, exclude_queue_indicator=exclude_queue_indicator)
        else:
            # Fallback
            try:
    ...
    def _add_to_queue(self, file_path):
        ...
        # Ajouter à l'onglet téléchargements comme import
        if hasattr(self.music_player, 'add_file_import_to_tab'):
            self.music_player.add_file_import_to_tab(file_path)
    ...
    def _add_youtube_to_playlist(self, video_data, frame):
        ...
        self.music_player._reset_frame_appearance(frame, COLOR_DOWNLOAD)
        ...
        try:
            ...
            try:
                self.music_player._reset_frame_appearance(frame, COLOR_ERROR)
            except:
                pass
