...
def play_track(self):
    try:
        ...
        print(f"🎯 DEBUG PLAY_TRACK: Appel de select_playlist_item(index={self.current_index})")
        try:
            self.select_playlist_item(index=self.current_index, auto_scroll=False)
            print(f"✅ DEBUG PLAY_TRACK: select_playlist_item terminé avec succès")
        except tk.TclError as e:
            # Playlist container non accessible, ignorer
            print(f"❌ DEBUG PLAY_TRACK: Erreur dans select_playlist_item: {e}")
            pass
        except Exception as e:
            print(f"❌ DEBUG PLAY_TRACK: Erreur inattendue dans select_playlist_item: {e}")
            import traceback
            traceback.print_exc()
        
        # Mettre en surbrillance la piste courante dans la bibliothèque aussi
        print(f"📚 DEBUG PLAY_TRACK: Appel de select_library_item_from_filepath")
        try:
            self.select_library_item_from_filepath(song)
            print(f"✅ DEBUG PLAY_TRACK: select_library_item_from_filepath terminé")
        except Exception as e:
            print(f"❌ DEBUG PLAY_TRACK: Erreur dans select_library_item_from_filepath: {e}")
            import traceback
            traceback.print_exc()
        
        # Mettre en surbrillance la piste courante dans l'affichage du contenu de playlist si on y est
        print(f"📋 DEBUG PLAY_TRACK: Appel de select_playlist_content_item_from_filepath")
        try:
            self.select_playlist_content_item_from_filepath(song)
            print(f"✅ DEBUG PLAY_TRACK: select_playlist_content_item_from_filepath terminé")
        except Exception as e:
            print(f"❌ DEBUG PLAY_TRACK: Erreur dans select_playlist_content_item_from_filepath: {e}")
            import traceback
            traceback.print_exc()
        
        # Mettre à jour la miniature dans la zone de recherche si elle est vide
        print(f"🖼️ DEBUG PLAY_TRACK: Vérification de la miniature")
        try:
            if (not self.current_search_query and 
                hasattr(self, 'results_container') and self.results_container.winfo_exists()):
                
                print(f"🖼️ DEBUG PLAY_TRACK: Conditions miniature OK, vérification enfants")
                # Vérifier le nombre d'enfants de manière sécurisée
                try:
                    children = self.results_container.winfo_children()
                    children_count = len(children)
                    print(f"🖼️ DEBUG PLAY_TRACK: {children_count} enfants dans results_container")
                except tk.TclError as e:
                    # Erreur lors de l'accès aux enfants, ignorer
                    print(f"❌ DEBUG PLAY_TRACK: Erreur accès enfants results_container: {e}")
                    children = []
                    children_count = 0
                
                if children_count <= 1:
                    print(f"🖼️ DEBUG PLAY_TRACK: Nettoyage et affichage miniature")
                    ...
                    self._show_current_song_thumbnail()
                    print(f"✅ DEBUG PLAY_TRACK: Miniature affichée")
                else:
                    print(f"🖼️ DEBUG PLAY_TRACK: Trop d'enfants ({children_count}), pas de miniature")
            else:
                print(f"🖼️ DEBUG PLAY_TRACK: Conditions miniature non remplies")
        except tk.TclError as e:
            # Results container non accessible, ignorer
            print(f"❌ DEBUG PLAY_TRACK: Erreur miniature: {e}")
            pass
        except Exception as e:
            print(f"❌ DEBUG PLAY_TRACK: Erreur inattendue miniature: {e}")
            import traceback
            traceback.print_exc()
...
def add_selection_to_queue_last(self):
    ...
    print("DEBUG: add_selection_to_queue_last appelé")
    if not self.selected_items:
        return
...
def _add_song_item(self, filepath, container, playlist_name=None, song_index=None):
    ...
    try:
        ...
        def on_item_right_click(event):
            print(f"DEBUG: on_item_right_click appelé pour {os.path.basename(filepath)}")
            print(f"DEBUG: Éléments sélectionnés: {len(self.selected_items) if self.selected_items else 0}")
        ...
        def debug_right_click_wrapper(event):
            print(f"DEBUG: Clic droit détecté sur widget {event.widget} pour fichier {os.path.basename(filepath)}")
...
def clear_selection(self):
    ...
    print("DEBUG: clear_selection appelé")
    try:
