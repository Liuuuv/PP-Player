...
class MusicPlayer:
    ...
    def _load_playlist_thumbnail(self, filepath, label):
        ...
        library_tab.playlists._load_playlist_thumbnail(self, filepath, label)

    
    def save_playlists(self):
        """Sauvegarde les playlists dans un fichier JSON"""
        library_tab.playlists.save_playlists(self)
    ...
    def _remove_from_playlist_view(self, filepath, playlist_name, event=None):
        ...
        tools._remove_from_playlist_view(self, filepath, playlist_name, event)
    ...
    def play_track(self):
        try:
            ...
            print(f"About to generate waveform for: {song}")
            self.generate_waveform_preview(song)
            print(f"After waveform generation. waveform_data_raw is None: {self.waveform_data_raw is None}")
    ...
    def _display_thumbnail(self, label, photo):
        ...
        tools._display_thumbnail(label, photo)
    ...
    def generate_waveform_preview(self, filepath):
        ...
        search_tab.control.generate_waveform_preview(self, filepath)

    def get_adaptive_waveform_data(self, canvas_width=None):
        """Génère des données waveform adaptées à la durée de la musique"""
        return search_tab.control.get_adaptive_waveform_data(self, canvas_width)
    
    def play_pause(self):
        return search_tab.control.play_pause(self)

    def play_selected(self, event):
        search_tab.control.play_selected(self, event)
    
    def prev_track(self):
        search_tab.control.prev_track(self)

    def next_track(self):
        search_tab.control.next_track(self)
    
    def show_waveform_on_clicked(self):
        search_tab.control.show_waveform_on_clicked(self)

    def draw_waveform_around(self, time_sec, window_sec=5):
        search_tab.control.draw_waveform_around(self, time_sec, window_sec)

    def set_volume(self, val):
        tools.set_volume(self, val)
    
    def set_volume_offset(self, val):
        tools.set_volume_offset(self, val)
    
    def _apply_volume(self):
        tools._apply_volume(self)
    
    def _reset_volume_offset(self, event):
        tools._reset_volume_offset(self, event)

    def on_progress_press(self, event):
        search_tab.control.on_progress_press(self, event)
    
    def on_progress_drag(self, event):
        search_tab.control.on_progress_drag(self, event)

    def on_progress_release(self, event):
        search_tab.control.on_progress_release(self, event)
    
    def on_waveform_canvas_resize(self, event):
        search_tab.control.on_waveform_canvas_resize(self, event)
    ...
    def set_position(self, val):
        search_tab.control.set_position(self, val)
