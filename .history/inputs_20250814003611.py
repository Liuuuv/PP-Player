...
def on_global_seek_forward(self, event):
    ...
    try:
        if pygame.mixer.music.get_busy():
            ...
            new_position = self.current_time + 5
            if new_position < self.song_length:
                pygame.mixer.music.set_pos(new_position)
                self.current_time = new_position
                print(f"DEBUG: Avance de 5s - Position: {new_position}s")
                self.status_bar.config(text=f"Avancé de 5s - {new_position:.0f}s")
            else:
                # Si on dépasse la fin, aller à la chanson suivante
                self.next_track()
                print("DEBUG: Fin de chanson atteinte, passage à la suivante")
        else:
