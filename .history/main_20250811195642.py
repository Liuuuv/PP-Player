...
class MusicPlayer:
    ...
    def add_selection_to_main_playlist(self):
        ...
        added_count = 0
        selected_list = list(self.selected_items)  # Convertir le set en liste pour maintenir l'ordre
    ...
    def add_selection_to_queue_first(self):
        ...
        selected_list = list(self.selected_items)  # Convertir le set en liste
        added_count = 0
        ...
        if len(self.main_playlist) == 0:
            ...
            # Pour maintenir l'ordre correct, on insère dans l'ordre inverse à la même position
            # Cela permet d'avoir D E F dans le bon ordre au lieu de F D E
            for filepath in reversed(selected_list):
                if not filepath.startswith("https://"):
                    # Ajouter à la main playlist s'il n'y est pas déjà
                    if filepath not in self.main_playlist:
                        # Insérer à la même position (les éléments précédents sont poussés vers la droite)
                        ...
                        for queue_index in self.queue_items:
                            if queue_index >= insert_position:
                        ...
                        self.queue_items.add(insert_position)
                        added_count += 1
    ...
    def add_selection_to_queue_last(self):
        ...
        for i, filepath in enumerate(selected_list):
            if not filepath.startswith("https://"):
                # Ajouter à la main playlist s'il n'y est pas déjà
                if filepath not in self.main_playlist:
