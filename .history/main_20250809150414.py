...
class MusicPlayer:
    def __init__(self, root):
        ...
        # Playlist Frame (left side)
        playlist_frame = ttk.Frame(middle_frame, width=300)
        playlist_frame.pack(side=tk.LEFT, fill=tk.BOTH, padx=(0, 10), expand=True)
        ...
        # Youtube Results Frame (right side)
        youtube_results_frame = ttk.Frame(middle_frame, width=300)
        youtube_results_frame.pack(side=tk.RIGHT, fill=tk.BOTH, expand=True)

        # Remplacer le Listbox par un Canvas avec Scrollbar
        self.youtube_results_container = ttk.Frame(youtube_results_frame)
        self.youtube_results_container.pack(fill=tk.BOTH, expand=True)

        self.youtube_canvas = tk.Canvas(
            self.youtube_results_container,
            bg='#3d3d3d',
            highlightthickness=0
        )
