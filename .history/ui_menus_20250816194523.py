...
def _download_youtube_video(self, frame, add_to_main_playlist=False):
    ...
    url = ''
    if hasattr(frame, 'video_data') and frame.video_data is not None and hasattr(frame.video_data, 'webpage_url') and frame.video_data.webpage_url is not None:
        url = frame.video_data.get('webpage_url', '')
    else:
        if hasattr(frame, 'video_url') and frame.video_url is not None:
            url = frame.video_url
