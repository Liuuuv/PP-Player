...
class MusicPlayer:
    ...
    def _perform_initial_search(self, query):
        ...
        try:
            ...
            with YoutubeDL(search_opts) as ydl:
                ...
                print(f"DEBUG: Résultats bruts trouvés: {len(results['entries'])}")
                print(f"DEBUG: Résultats après filtrage: {len(self.all_search_results)}")
                ...
                # Debug: avant l'affichage
                print(f"DEBUG: Tentative d'affichage de {len(self.all_search_results)} résultats")
            ...
            print(f"DEBUG: Exception dans _perform_initial_search: {e}")
            import traceback
            traceback.print_exc()
            if not self.search_cancelled:
    ...
    def _filter_search_results(self, entries):
        ...
        print(f"DEBUG: Filtrage - show_artists_val: {show_artists_val}, show_tracks_val: {show_tracks_val}")
        ...
        for entry in entries:
            ...
            print(f"DEBUG: Entry '{title}' - is_video: {is_video}, is_channel: {is_channel}, duration: {duration}")
            
            # Filtrer selon les préférences
            if is_video and show_tracks_val and duration <= 600.0:  # Vidéos (tracks) max 10 minutes
                filtered_results.append(entry)
                print(f"DEBUG: Ajouté comme vidéo: {title}")
            elif is_channel and show_artists_val:  # Chaînes (artists)
                filtered_results.append(entry)
                print(f"DEBUG: Ajouté comme chaîne: {title}")
            else:
                print(f"DEBUG: Filtré: {title} (is_video: {is_video}, show_tracks: {show_tracks_val}, duration: {duration}, is_channel: {is_channel}, show_artists: {show_artists_val})")
    ...
    def _update_search_results_ui(self):
        ...
        return search_tab._update_search_results_ui(self)
