...
class MusicPlayer:
    ...
    def _display_playlists(self):
        ...
        # Organiser les playlists en lignes de 3
        playlist_items = list(self.playlists.items())
        
        for row in range(0, len(playlist_items), 3):
            ...
            # Ajouter jusqu'à 3 playlists dans cette ligne
            for col in range(3):
    ...
    def _add_playlist_display_item(self, playlist_name, songs):
        """Ajoute un élément d'affichage pour une playlist"""
        try:
            # Frame principal pour la playlist
            ...
            playlist_frame.columnconfigure(0, weight=1)  # Nom
            playlist_frame.columnconfigure(1, minsize=80, weight=0)  # Nombre de chansons
            playlist_frame.columnconfigure(2, minsize=80, weight=0)  # Bouton renommer
            playlist_frame.columnconfigure(3, minsize=80, weight=0)  # Bouton supprimer
            playlist_frame.rowconfigure(0, minsize=50, weight=0)
            
            # 1. Nom de la playlist (colonne 0)
            name_label = tk.Label(
                playlist_frame,
                text=playlist_name,
                bg='#4a4a4a',
            ...
            name_label.grid(row=0, column=0, sticky='nsew', padx=(15, 10), pady=10)
            
            # 2. Nombre de chansons (colonne 1)
            count_label = tk.Label(
                playlist_frame,
                text=f"{len(songs)} chansons",
                bg='#4a4a4a',
                fg='#cccccc',
                font=('TkDefaultFont', 9),
                anchor='center'
            )
            count_label.grid(row=0, column=1, sticky='ns', padx=5, pady=10)
            
            # 3. Bouton renommer (colonne 2) - sauf pour la playlist principale
            if playlist_name != "Principale":
                ...
                rename_btn.grid(row=0, column=2, sticky='ns', padx=5, pady=10)
            
            # 4. Bouton supprimer (colonne 3) - sauf pour la playlist principale
            if playlist_name != "Principale":
                delete_btn = tk.Button(
                    playlist_frame,
                    text="Supprimer",
                    command=lambda name=playlist_name: self._delete_playlist_dialog(name),
                    bg="#ff4444",
                    fg="white",
                ...
                delete_btn.grid(row=0, column=3, sticky='ns', padx=5, pady=10)
            
            # Double-clic pour voir le contenu de la playlist
            def on_playlist_double_click():
                self._show_playlist_content_dialog(playlist_name)
            
            playlist_frame.bind("<Double-1>", lambda e: on_playlist_double_click())
            name_label.bind("<Double-1>", lambda e: on_playlist_double_click())
    ...
    def _show_playlist_content_dialog(self, playlist_name):
        """Affiche le contenu d'une playlist dans une fenêtre"""
        dialog = tk.Toplevel(self.root)
        dialog.title(f"Contenu de '{playlist_name}'")
        ...
        title_label = tk.Label(dialog, text=f"Playlist: {playlist_name}", 
                              bg='#2d2d2d', fg='white', font=('TkDefaultFont', 12, 'bold'))
        ...
        listbox = tk.Listbox(list_frame, yscrollcommand=scrollbar.set,
                            bg='#3d3d3d', fg='white', selectbackground='#4a8fe7',
                            font=('TkDefaultFont', 9))
