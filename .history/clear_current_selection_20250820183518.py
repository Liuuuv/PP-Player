...
def clear_current_song_selection(self):
    ...
    try:
        ...
        if hasattr(self, 'main_playlist_container'):
            try:
                # Vérifier que le container existe encore
                if self.main_playlist_container.winfo_exists():
                    try:
        ...
        if hasattr(self, 'playlist_content_container'):
            try:
                ...
                if self.playlist_content_container.winfo_exists():
                    ...
                    try:
                        children = self.playlist_content_container.winfo_children()
                    except tk.TclError:
                        # Erreur lors de l'accès aux enfants, ignorer
                        children = []
                    ...
                    for child in children:
                        try:
                            ...
                            if child.winfo_exists() and hasattr(child, 'selected'):
                                ...
                                # Vérifier spécifiquement si ce frame contient delete.png
                                has_delete_png = False
                                if hasattr(child, 'winfo_children'):
                                    try:
                                        subchildren = child.winfo_children()
                                    except tk.TclError:
                                        # Erreur lors de l'accès aux sous-enfants, ignorer
                                        subchildren = []
...
def clear_all_current_song_selections(self):
    ...
    2. Dans _remove_from_main_playlist() quand on supprime la chanson en cours
...
2. Dans search_tab/main_playlist.py, fonction _remove_from_main_playlist() :
