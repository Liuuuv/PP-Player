...
def _start_text_animation(self, full_title, frame):
    ...
    truncated_title = tools._truncate_text_for_display(self, full_title, max_width_pixels=frame.max_width, font_family='Helvetica', font_size=12)
...
def _animate_title_step(self, frame):
    ...
    max_width_pixels = frame.max_width
    ...
    visible_text = self._get_scrolled_title_text(frame.full_text, frame.scroll_position, max_width_pixels)
    ...
    if frame.scroll_position >= len(frame.full_text) + 8:  # +8 pour les espaces ajoutés
        ...
        print("BLABLOUU ", frame.cget('font'), type(frame.cget('font')))
        truncated_title = tools._truncate_text_for_display(self, frame.full_text, max_width_pixels=max_width_pixels, font_family='Helvetica', font_size=frame.cget('font')[1])
        frame.config(text=truncated_title)
...
def _reset_text_animation(self, frame):
    ...
    self._stop_text_animation(frame)
    frame.scroll_position = 0
    frame.pause_counter = 0
    frame.animation_id = None
    frame.config(text=frame.full_text)
