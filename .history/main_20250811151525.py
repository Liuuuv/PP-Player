...
class MusicPlayer:
    ...
    def play_track(self):
        try:
            ...
            # Tronquer le titre s'il est trop long en utilisant la fonction existante
            truncated_title = tools._truncate_text_for_display(self, song_name, max_width_pixels=400, font_family='Helvetica', font_size=12)
            ...
            if hasattr(self, 'results_counter_label') and self.results_counter_label.winfo_exists():
                ...
                if total_count > 0:
                    ...
                    if total_count >= self.max_search_results:
                        ...
                        counter_text = f"Résultats {displayed_count}/{total_count}"
                else:
    ...
    def _add_search_result(self, video, index):
        ...
        try:
            ...
            # Binding pour la fin du drag
            if not is_channel:
                def on_result_release(event, frame=result_frame):
                    self._on_drag_release(event, frame)
                
                result_frame.bind("<ButtonRelease-1>", on_result_release)
                duration_label.bind("<ButtonRelease-1>", on_result_release)
                title_label.bind("<ButtonRelease-1>", on_result_release)
                thumbnail_label.bind("<ButtonRelease-1>", on_result_release)
            ...
            # Événements de drag-and-drop pour ajouter à la playlist
            if not is_channel:  # Seulement pour les vidéos, pas les chaînes
                # Variables pour le drag
                result_frame.drag_start_x = 0
                result_frame.drag_start_y = 0
                result_frame.is_dragging = False
                result_frame.original_bg = '#4a4a4a'
                result_frame.add_icon_label = None
                
                # Bindings pour le drag (utiliser des événements qui n'interfèrent pas)
                result_frame.bind("<B1-Motion>", lambda e, f=result_frame: self._on_drag_motion(e, f))
                
                # Bindings pour les éléments enfants aussi
                for widget in [title_label, duration_label, thumbnail_label]:
                    widget.bind("<B1-Motion>", lambda e, f=result_frame: self._on_drag_motion(e, f))
    ...
    def _on_drag_start(self, event, frame):
        """Démarre le drag d'un résultat YouTube"""
        frame.drag_start_x = event.x_root
        frame.drag_start_y = event.y_root
        frame.is_dragging = False
    ...
    def _on_drag_motion(self, event, frame):
        ...
        if frame.is_dragging:
            # Déplacer visuellement le frame
    ...
    def _on_drag_release(self, event, frame):
        ...
        if frame.is_dragging:
            ...
            if dx > 100:
                ...
                self._add_dragged_video_to_playlist(frame)
            
            # Remettre l'apparence normale
            self._end_visual_drag(frame)
            frame.is_dragging = False
        else:
            # Si pas de drag, traiter comme un clic normal
            self._handle_normal_click(event, frame)
    ...
    def _start_visual_drag(self, frame):
        ...
        if frame.add_icon_label is None:
    ...
    def _add_dragged_video_to_playlist(self, frame):
        """Ajoute la vidéo draggée à la playlist"""
        if hasattr(frame, 'video_data'):
            video = frame.video_data
            ...
            frame.video_data['search_frame'] = frame
            try:
                # Télécharger ET ajouter à la playlist (comportement par défaut)
                self.download_selected_youtube(None, add_to_playlist=True)
                self.status_bar.config(text=f"Ajout à la playlist: {video.get('title', 'Titre inconnu')[:30]}...")
            except Exception as e:
                # En cas d'erreur, remettre l'apparence normale
                frame.config(bg='#ffcc00')  # Jaune pour erreur
    ...
    def _handle_normal_click(self, event, frame):
        """Gère un clic normal (sans drag)"""
        # Vérifier si Shift est enfoncé pour la sélection multiple
        if event.state & 0x1:  # Shift est enfoncé
            self.shift_selection_active = True
            # Utiliser l'URL comme identifiant unique pour les résultats YouTube
            if hasattr(frame, 'video_data'):
                video_url = frame.video_data.get('webpage_url') or f"https://www.youtube.com/watch?v={frame.video_data.get('id')}"
                self.toggle_item_selection(video_url, frame)
        # Pour les clics normaux sans Shift, ne rien faire (éviter les actions non désirées)
    ...
    def _add_downloaded_file(self, filepath, thumbnail_path, title, url=None, add_to_playlist=True):
        ...
        # Vérifier si on doit ajouter à la playlist et si le fichier n'y est pas déjà
        if add_to_playlist and filepath not in self.main_playlist:
            self.main_playlist.append(filepath)
            self._add_main_playlist_item(filepath, thumbnail_path)
