...
def load_icons(self):
    ...
    for key, filename in icon_names.items():
        try:
            ...
            # Créer une version plus petite pour les icônes output, stats et download
            if key in ["output", "stats", "download"]:
                image_small = Image.open(path).resize((16, 16), Image.Resampling.LANCZOS)
                self.icons[key + "_small"] = ImageTk.PhotoImage(image_small)
        except Exception as e:
...
def create_ui(self):
    ...
    # Bouton Download plus petit et plus haut que les onglets, à gauche du bouton stats
    self.download_button = tk.Button(
        self.main_frame, 
        image=self.icons["download_small"],
        bg="#3d3d3d",
        fg="white",
    ...
    self.download_button.place(in_=self.notebook, relx=1.0, rely=0.0, anchor="ne", x=-60, y=-5)
    create_tooltip(self.download_button, "Télécharger des musiques\nTélécharge une musique ou une playlist YouTube")
...
def setup_keyboard_bindings(self):
        ...
        self.root.bind('<Control-Alt-p>', self.on_global_play_pause)
        self.root.bind('<Control-Alt-P>', self.on_global_play_pause)
