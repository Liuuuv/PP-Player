...
class MusicPlayer:
    ...
    def _should_load_more_results(self):
        ...
        if (self.is_loading_more or 
            ...
            self.search_results_count >= self.max_search_results):
            return False
        
        # Vérifier si on est proche du bas
        try:
            canvas_height = self.youtube_canvas.winfo_height()
            scroll_region = self.youtube_canvas.bbox("all")
            if not scroll_region:
                return False
            
            total_height = scroll_region[3] - scroll_region[1]
            current_view = self.youtube_canvas.canvasy(canvas_height)
            
            # Charger plus si on est à 80% du bas
            return current_view >= total_height * 0.8
        except:
            return False
    ...
    def _add_download_item(self, filepath):
        ...
        try:
            ...
            thumbnail_label.grid(row=0, column=0, sticky='nsew', padx=(0, 5))
    ...
    def search_youtube(self):
        ...
        threading.Thread(target=self._perform_initial_search, args=(query,), daemon=True).start()

    def _perform_initial_search(self, query):
        """Effectue la recherche initiale et récupère jusqu'à 30 résultats"""
        try:
            search_opts = {
                'extract_flat': True,
                'quiet': True,
                'no_warnings': True,
                'max_downloads': 50,  # Chercher plus pour avoir 30 après filtrage
                'ignoreerrors': True
            }
            
            with YoutubeDL(search_opts) as ydl:
                # Chercher plus de résultats pour avoir le choix après filtrage
                ...
                video_results = [
                    ...
                    if entry and "https://www.youtube.com/watch?v=" in entry.get('url', '')
                    and entry.get('duration', 0) <= 600.0  # Durée max de 10 minutes
                ]
                
                # Limiter à 30 résultats maximum
                self.all_search_results = video_results[:self.max_search_results]
                
                # Afficher les premiers résultats (10)
                self._display_search_results(0, self.results_per_page)
    ...
    def _display_search_results(self, start_index, count):
        """Affiche une portion des résultats de recherche"""
        end_index = min(start_index + count, len(self.all_search_results))
        ...
        self.root.after(0, lambda: self.status_bar.config(
            text=f"{self.search_results_count}/{len(self.all_search_results)} résultats affichés"
        ))

    def _perform_search(self, query, start_index=0):
        try:
            # Calculer combien de résultats chercher
            results_to_fetch = min(self.results_per_page, self.max_search_results - self.search_results_count)
            
            search_opts = {
                'extract_flat': True,
                'quiet': True,
                'no_warnings': True,
                'max_downloads': results_to_fetch + 5,  # Un peu plus pour filtrer
                'ignoreerrors': True
            }
            
            with YoutubeDL(search_opts) as ydl:
                # Utiliser un nombre plus élevé pour avoir plus de choix après filtrage
                search_count = min(results_to_fetch + 10, 50)
                ...
                # Si c'est une nouvelle recherche, nettoyer le container
                if start_index == 0:
                    self.root.after(0, self._clear_results)
                ...
                video_results = [
                    entry for entry in results['entries']
                    if entry and "https://www.youtube.com/watch?v=" in entry.get('url', '')  # Seulement les vidéos
                    and entry.get('duration', 0) <= 600.0  # Durée max de 10 minutes
                ]
                
                # Prendre seulement les résultats demandés après le start_index
                video_results = video_results[start_index:start_index + results_to_fetch]
                
                # Afficher les résultats
                for i, video in enumerate(video_results):
                    if self.search_results_count < self.max_search_results:
                        self.root.after(0, lambda v=video, idx=self.search_results_count: self._add_search_result(v, idx))
                        self.search_results_count += 1
                    
                # Mettre à jour le statut
                if start_index == 0:
                    self.root.after(0, lambda: self.status_bar.config(
                        text=f"{self.search_results_count} résultats trouvés"
                    ))
                else:
                    self.root.after(0, lambda: self.status_bar.config(
                        text=f"{self.search_results_count}/{self.max_search_results} résultats chargés"
                    ))
    ...
    def _load_more_search_results(self):
        ...
        if (self.is_loading_more or 
            not self.current_search_query or 
            self.search_results_count >= len(self.all_search_results)):
            return
        
        self.is_loading_more = True
        self.status_bar.config(text="Chargement de plus de résultats...")
        
        # Afficher les prochains résultats
        start_index = self.search_results_count
        count = min(self.results_per_page, len(self.all_search_results) - start_index)
        
        if count > 0:
            self._display_search_results(start_index, count)
