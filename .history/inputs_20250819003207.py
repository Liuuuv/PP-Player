...
def _on_mousewheel(self, event, canvas):
        ...
        # Marquer que nous sommes en train de traiter un événement de scroll
        ...
        try:
            ...
            # Vérifier le scroll infini pour la playlist, mais avec un délai plus long
            # pour éviter les recalculs trop fréquents
            if hasattr(self, 'playlist_canvas') and canvas == self.playlist_canvas:
                if hasattr(self, '_check_infinite_scroll'):
                    # Annuler toute vérification précédente en attente
                    if hasattr(self, '_scroll_check_after_id') and self._scroll_check_after_id:
                        self.root.after_cancel(self._scroll_check_after_id)
                    
                    # Programmer une nouvelle vérification avec un délai plus long
                    self._scroll_check_after_id = self.root.after(100, self._check_infinite_scroll)
        finally:
