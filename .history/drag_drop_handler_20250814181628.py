...
class DragDropHandler:
    ...
    def _add_to_queue(self, file_path):
        ...
        # Utiliser la fonction de base pour ajouter à la queue
        self.music_player.add_file_to_queue(file_path, position='last')
    ...
    def _add_youtube_to_playlist(self, video_data, frame):
        ...
        def on_download_complete(result):
            if result['success']:
                # Ajouter à la queue
                self.music_player.add_file_to_queue(result['filepath'], position='last')
                self.music_player.status_bar.config(text=f"Téléchargé et ajouté à la queue: {os.path.basename(result['filepath'])}")
                # Remettre l'apparence normale
                self.music_player._reset_frame_appearance(frame, '#4a4a4a')
            else:
                # En cas d'erreur, apparence jaune
                self.music_player._reset_frame_appearance(frame, '#ffcc00')
                self.music_player.status_bar.config(text=f"Erreur téléchargement: {result.get('error', 'Erreur inconnue')}")
        
        # Télécharger de manière asynchrone
        self.music_player.download_youtube_video_async(url, title, on_download_complete)
    ...
    def _add_to_queue_first_file(self, file_path):
        ...
        if file_path:
            ...
            self.music_player.add_file_to_queue(file_path, position='first')
    ...
    def _add_to_queue_first_youtube(self, video_data, frame):
        ...
        def on_download_complete(result):
            if result['success']:
                # Ajouter en premier dans la queue
                self.music_player.add_file_to_queue(result['filepath'], position='first')
                self.music_player.status_bar.config(text=f"Téléchargé et placé en premier dans la queue: {os.path.basename(result['filepath'])}")
                # Remettre l'apparence normale
                self.music_player._reset_frame_appearance(frame, '#4a4a4a')
            else:
                # En cas d'erreur, apparence jaune
    ...
    def _add_to_queue_first(self, file_path):
        ...
        # Utiliser la fonction de base pour ajouter en premier dans la queue
        self.music_player.add_file_to_queue(file_path, position='first')
