...
def update_upload_date_from_links(self):
    ...
    for file in self.all_downloaded_files:
        update_upload_date_from_links(self, file)

def update_duration(self, file):
    try:
        import json
        
        # Utiliser le fichier durations.json dans le dossier .cache
        infos_file = os.path.join(self.downloads_folder, "youtube_urls.json")
        
        # Charger les infos existantes (url, upload_date)
        infos = {}
        if os.path.exists(infos_file):
            try:
                ...
                print(f"Erreur lecture cache durées: {e}")
                infos = {}
        
        # Obtenir la durée réelle du fichier local
        try:
            import mutagen
            ...
            if audio_file and hasattr(audio_file.info, 'length'):
                ...
                # Mettre à jour la durée dans le cache
                infos_file[file][url] = actual_duration
                
                # Sauvegarder le cache des durées mis à jour
                with open(durations_file, 'w', encoding='utf-8') as f:
                    json.dump(durations, f, ensure_ascii=False, indent=2)
                
                print(f"Durée mise à jour pour {os.path.basename(file)}: {actual_duration:.1f}s")
            else:
                print(f"Impossible d'obtenir la durée pour {os.path.basename(file)}")
        except Exception as e:
            print(f"Erreur lecture durée fichier {os.path.basename(file)}: {e}")
            
    except Exception as e:
        print(f"Erreur mise à jour durées: {e}")