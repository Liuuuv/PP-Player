...
class DragDropHandler:
    ...
    def _add_playlist_item_to_main(self, file_path):
        ...
        if file_path:
            ...
            # Vérifier si c'est déjà la dernière musique de la playlist
            if (self.music_player.main_playlist and 
                self.music_player.main_playlist[-1] == file_path):
                self.music_player.status_bar.config(text=f"Déjà en dernière position: {filename}")
                return
            
            # Toujours ajouter à la fin (même si c'est un doublon)
            self.music_player.add_to_main_playlist(file_path, allow_duplicates=True, show_status=False)
