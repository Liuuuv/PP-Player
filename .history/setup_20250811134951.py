...
def load_icons(self):
    ...
    for key, filename in icon_names.items():
        try:
            ...
            # Créer une version plus petite pour l'icône output
            if key == "output":
                image_small = Image.open(path).resize((16, 16), Image.Resampling.LANCZOS)
                self.icons[key + "_small"] = ImageTk.PhotoImage(image_small)
        except Exception as e:
...
def setup_controls(self):
        ...
        # Song Info Frame (conteneur horizontal pour titre + compteur)
        song_info_frame = ttk.Frame(control_frame)
        song_info_frame.pack(fill=tk.X, pady=(5, 2))
        
        # Song Info (titre centré avec troncature)
        self.song_label = ttk.Label(
            song_info_frame, text="No track selected", 
            font=('Helvetica', 12, 'bold'),
            anchor='center',
            width=50  # Largeur fixe pour permettre la troncature
        )
        self.song_label.pack(side=tk.LEFT, expand=True, fill=tk.X)
        
        # Label pour afficher le compteur de résultats (à droite du titre)
        ...
        self.results_counter_label.pack(side=tk.RIGHT)
        ...
        # Frame pour centrer verticalement le volume offset
        offset_center_frame = ttk.Frame(volume_offset_frame)
        offset_center_frame.pack(expand=True, fill=tk.BOTH)
        
        ttk.Label(offset_center_frame, text="Volume Offset").pack(pady=(10, 2))
        self.volume_offset_slider = ttk.Scale(
            offset_center_frame, from_=-75, to=75, 
        ...
        self.song_length_label.pack(anchor="e")
        
        # Frame pour centrer verticalement le volume principal
        volume_center_frame = ttk.Frame(volume_frame)
        volume_center_frame.pack(expand=True, fill=tk.BOTH)
        
        ttk.Label(volume_center_frame, text="Volume").pack(pady=(10, 2))
        self.volume_slider = ttk.Scale(
            volume_center_frame, from_=0, to=100, 
