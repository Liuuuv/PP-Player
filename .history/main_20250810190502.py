...
class MusicPlayer:
    ...
    def _is_artist_content(self, title, uploader):
        """Détermine si le contenu correspond à un artiste (chaîne officielle)"""
        uploader_lower = uploader.lower()
        
        # Indicateurs de chaînes d'artistes officielles
        artist_channel_indicators = [
            'vevo',           # Chaînes Vevo (artistes officiels)
            'official',       # Chaînes officielles
            'records',        # Labels de musique
            'music'           # Chaînes de musique
        ]
        
        # Vérifier si l'uploader est une chaîne d'artiste
        return any(indicator in uploader_lower for indicator in artist_channel_indicators)
