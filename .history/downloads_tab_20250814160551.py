...
def setup_downloads_tab(self):
    ...
    clean_btn.pack(side=tk.RIGHT, padx=(0, 5))
...
def create_download_item_widget(self, download_item):
    ...
    # 2. Miniature de la vidéo (même style que library_tab)
    ...
    if hasattr(download_item, 'video_data') and download_item.video_data:
        ...
        if thumbnail_url:
            self._load_download_thumbnail(thumbnail_label, thumbnail_url)
    ...
    # Créer l'overlay de progression (barre verte semi-transparente en arrière-plan)
    progress_overlay = tk.Frame(item_frame, bg='#2d4a2d')  # Vert très sombre et subtil
...
def update_progress_overlay(self, download_item):
    ...
    try:
        # Attendre que la frame soit affichée
        self.root.after(50, lambda: self._update_progress_overlay_delayed(download_item))
...
def _update_progress_overlay_delayed(self, download_item):
    ...
    try:
        ...
        frame_width = widgets['frame'].winfo_width()
        if frame_width <= 1:  # Si la frame n'est pas encore affichée, utiliser une largeur par défaut
            frame_width = 400
        ...
        if download_item.completed:
            progress_width = frame_width  # 100% si terminé
            bg_color = '#2d4a2d'  # Vert sombre pour terminé
        elif download_item.error:
            progress_width = 0  # 0% si erreur
            bg_color = '#4a2d2d'  # Rouge sombre pour erreur
        else:
            progress_width = int((progress_value / 100.0) * frame_width)
            bg_color = '#2d4a2d'  # Vert sombre pour en cours
        
        # Mettre à jour la largeur et la couleur de l'overlay
        progress_overlay.place(width=progress_width)
...
def _load_download_thumbnail(self, label, url):
