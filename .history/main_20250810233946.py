...
class MusicPlayer:
    ...
    def _download_and_add_to_playlist_thread(self, video, frame, playlist_name):
        ...
        try:
            ...
            with YoutubeDL(ydl_opts) as ydl:
                ...
                self.root.after(0, lambda: self._add_downloaded_to_playlist(final_path, thumbnail_path, safe_title, playlist_name))
                
                # Remettre l'apparence normale
                frame.config(bg='#4a4a4a')
                frame.title_label.config(bg='#4a4a4a', fg='white')
                frame.duration_label.config(bg='#4a4a4a', fg='#cccccc')
                frame.thumbnail_label.config(bg='#4a4a4a')
        
        except Exception as e:
            self.root.after(0, lambda e=e: self.status_bar.config(text=f"Erreur: {str(e)}"))
            if 'search_frame' in video:
                # Apparence d'erreur (jaune)
                frame.config(bg='#ffcc00')
                frame.title_label.config(bg='#ffcc00', fg='#333333')
                frame.duration_label.config(bg='#ffcc00', fg='#666666')
                frame.thumbnail_label.config(bg='#ffcc00')
        finally:
    ...
    def _add_downloaded_to_playlist(self, filepath, thumbnail_path, title, playlist_name):
