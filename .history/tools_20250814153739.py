...
def _download_progress_hook(self, d):
    ...
    if d['status'] == 'downloading':
        ...
        if hasattr(self, 'current_download_url'):
            status_text = f"Téléchargement... ({percent} - {speed})"
...
def _download_youtube_thread(self, url, add_to_playlist=True):
    ...
    try:
        ...
        if not existing_file:
            self.root.after(0, lambda: self.add_download_to_tab(url, title))
        if existing_file:
            ...
            self.root.after(0, lambda: self.update_download_progress(url, 100, "Fichier existant"))
