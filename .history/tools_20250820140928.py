...
def update_duration(self, file):
    try:
        ...
        # Mettre à jour les métadonnées si le fichier existe dans les métadonnées
        if filename in metadata:
            ...
            try:
                ...
                if audio_file and hasattr(audio_file.info, 'length'):
                    ...
                    if isinstance(metadata[filename], dict):
                        metadata[filename]['duration'] = actual_duration
                    else:
                        # Convertir l'ancien format vers le nouveau format avec la durée
                        old_url = metadata[filename] if isinstance(metadata[filename], str) else None
                        metadata[filename] = {
                            'url': old_url,
                            'upload_date': None,
                    ...
                    print(f"Durée mise à jour pour {filename}: {actual_duration:.1f}s")
                else:
                    print(f"Impossible d'obtenir la durée pour {filename}")
            except Exception as e:
                print(f"Erreur lecture durée fichier {filename}: {e}")
            
    except Exception as e:
        print(f"Erreur mise à jour métadonnées YouTube: {e}")