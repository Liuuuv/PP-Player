...
def _add_downloaded_file(self, filepath, thumbnail_path, title, url=None, add_to_playlist=True):
    ...
    # Vérifier s'il faut placer en dernier dans la queue
    if url and hasattr(self, 'pending_queue_last_additions') and url in self.pending_queue_last_additions:
        # Placer en dernier dans la queue via le drag drop handler
        if hasattr(self, 'drag_drop_handler'):
            self.drag_drop_handler._add_to_queue_last(filepath)
            self.status_bar.config(text=f"{title} placé en dernier dans la queue")
        # Nettoyer la liste d'attente pour cette URL
        del self.pending_queue_last_additions[url]
...
def add_selection_to_queue_last(self):
    ...
    # Si la playlist est vide, ajouter normalement sans queue
    if len(self.main_playlist) == 0:
        ...
        if added_count > 0:
            ...
            self.status_bar.config(text="Aucun élément n'a été ajouté")
