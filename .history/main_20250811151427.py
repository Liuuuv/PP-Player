    ...
    def _add_search_result(self, video, index):
        ...
        try:
            ...
            if not is_channel:  # Seulement pour les vidéos, pas les chaînes
                ...
                result_frame.bind("<ButtonPress-1>", lambda e, f=result_frame: self._on_drag_start(e, f))
                result_frame.bind("<B1-Motion>", lambda e, f=result_frame: self._on_drag_motion(e, f))
                result_frame.bind("<ButtonRelease-1>", lambda e, f=result_frame: self._on_drag_release(e, f))
                
                # Bindings pour les éléments enfants aussi
                for widget in [title_label, duration_label, thumbnail_label]:
                    widget.bind("<ButtonPress-1>", lambda e, f=result_frame: self._on_drag_start(e, f))
                    widget.bind("<B1-Motion>", lambda e, f=result_frame: self._on_drag_motion(e, f))
                    widget.bind("<ButtonRelease-1>", lambda e, f=result_frame: self._on_drag_release(e, f))
