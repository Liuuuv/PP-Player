...
def test_clear_current_song_selection():
    ...
    # Vérifier qu'au moins le widget sélectionné a été traité
    widget_colors = [(w, c) for w, c in music_player.set_item_colors_calls]
    assert (music_player.selected_widget1, '#4a4a4a') in widget_colors
...
def test_fallback_without_set_item_colors():
    ...
    # Créer un music player sans _set_item_colors
    class MockMusicPlayerNoSetColors:
        def __init__(self):
            self.selected_widget1 = MockWidget(selected=True, bg_color='#5a9fd8')
            self.normal_widget = MockWidget(selected=False, bg_color='#4a4a4a')
    ...
    music_player = MockMusicPlayerNoSetColors()
