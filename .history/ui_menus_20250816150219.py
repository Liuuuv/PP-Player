...
def select_downloads_folder(self):
    ...
    try:
        ...
        new_downloads_folder = os.path.join(new_folder, "downloads")
        ...
        # Message de succès
        self.status_bar.config(text=f"Dossier de téléchargements changé vers : {new_downloads_folder}")
    ...
    for item in os.listdir(source_folder):
        ...
        try:
            if os.path.isdir(source_path):
                # Si c'est un dossier, le déplacer récursivement
...
def _update_downloads_path_config(self, new_downloads_folder):
    ...
    try:
        ...
        self.ydl_opts['outtmpl'] = os.path.join(new_downloads_folder, '%(title)s.%(ext)s')
