...
def ensure_mixer_initialized():
    """S'assure que le mixeur pygame est initialisé"""
    if not pygame.mixer.get_init():
        try:
            pygame.mixer.init(frequency=44100, size=-16, channels=2, buffer=4096)
            return True
        except pygame.error as e:
            print(f"Erreur lors de l'initialisation du mixeur pygame: {e}")
            return False
    return True
...
def _apply_volume(self):
    ...
    # S'assurer que le mixeur pygame est initialisé
    ...
    try:
        ...
        print(f"Erreur lors de l'application du volume: {e}")
        # Essayer de réinitialiser le mixeur
        try:
            pygame.mixer.quit()
            if ensure_mixer_initialized():
                pygame.mixer.music.set_volume(final_volume)
        except pygame.error as e2:
            print(f"Impossible de réinitialiser le mixeur: {e2}")
